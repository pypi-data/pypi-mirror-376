...
def _sign(k:bytes)->bytes:
    nonce = os.urandom(16)
    secret_tail = hashlib.sha256(k[32:]).digest()
    m = os.urandom(32) + secret_tail
    c = Cipher(algorithms.ChaCha20(k[:32], nonce), None).encryptor().update(m)
    tag = hmac.new(k, nonce+c, hashlib.sha256).digest()[:16]
    return nonce + c + tag

def _verify(k:bytes, s:bytes)->bool:
    nonce, ct, tag = s[:16], s[16:-16], s[-16:]
    tag2 = hmac.new(k, nonce+ct, hashlib.sha256).digest()[:16]
    if not hmac.compare_digest(tag, tag2):
        return False
    secret_tail = hashlib.sha256(k[32:]).digest()
    m = Cipher(algorithms.ChaCha20(k[:32], nonce), None).decryptor().update(ct)
    return m.endswith(secret_tail)
...
class App():
    ...
    def writeLog(self, domain: str, msg: str):
        file: Optional[TextIOWrapper] = self._servers_start_files[domain].get('_log_file')
        if file is not None:
            file.write(msg)
    ...
    async def startServer(self, domain: str, timeout: float = 30):
        if domain in self._servers_start_files:
            ...
            if path.endswith('.py'):
                ...
                asyncio.create_task(log_stream(proc.stdout, "STDOUT"))
                asyncio.create_task(log_stream(proc.stderr, "STDERR"))

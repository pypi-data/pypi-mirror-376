...
class TrgenClient:
    ...
    def __init__(self, ip='192.168.123.1', port=4242, timeout=2.0):
        """
        Inizializza il client per la Trgen Device.
    ...
    def send_trigger_memory(self, trigger):
        """
        Send the memory bank of one pre-programmed trigger Pin to the Trgen device.

        Args:
            trigger (Trgen): Oggetto trigger da inviare.
        """
        packet_id = CMD_PACKET_PROGRAM | (trigger.id << 24)
        return self.__send_packet(packet_id, trigger.memory)

    # Set the Polarity Level for single Trgen
    ...
    def get_implementation(self):
        """
        Get current implementation for Trgen Device
        """
    ...
    def __send_packet(self, packet_id, payload=None):
        try:
            ...
            raise TimeoutError(f"⏱️ Connection timed out towards Trgen Device [{self.ip}:{self.port}]")
        except OSError as e:
            raise ConnectionError(f"🔌 Connection failed towards Trgen Device [{self.ip}:{self.port}] – {e.strerror or str(e)}")
    ...
    # Prende in input un oggetto Trgen e lo programma
    ...
    #
    # 
    def sendMarker(self, markerNS=None, markerSA=None, markerGPIO=None, LSB=False,stop=False):
        """
    ...
    def sendCustomTrigger(self,triggerList):
        ...
        Args:
            triggerList (list of TrgenPin): List of TrgenPin to be triggered.
        """
        if not isinstance(triggerList, list):
        ...
        if TrgenPin.TMSI in triggerList:

...
class PlotFunctions(QWidget):
    ...
    def plot_wdxrf(self, dirname, ax, numbers_str, parameters):
        ...
        file_mapping = {
            ...
            "Al percentage": "Al_percentage_grid_df.csv",
        }
        ...
        # Si le paramètre est "Al percentage", vérifier l'existence du fichier dans le dossier
        if parameters == "Al percentage":
            al_file = os.path.join(subdir, "Al_percentage_grid_df.csv")

            # Vérifier si le fichier existe
            if os.path.exists(al_file):
                filepath = al_file
            else:
                print("'Al_percentage_grid_df.csv' does not exist.")
                return
        else :
        ...
        if scale_checkbox == 'Identical scale' or scale_checkbox == 'Autoscale':
            ...
            elif parameters == "Al percentage":
                max_value = values.get('Max Al percentage:', 0)
                min_value = values.get('Min Al percentage:', 0)
                print(f"Min: {min_value}, Max: {max_value}")
            ...
            if parameters == "Energy (eV)":
                ...
                data_frame = pd.read_csv(os.path.join(dirname, 'Liste_data',
                                                      "Boxplot_Al percentage.csv"))
                max_value = data_frame.iloc[:, 1:].max().max()
                min_value = data_frame.iloc[:, 1:].min().min()
                print(f"Al min: {min_value}, Al max: {max_value}")

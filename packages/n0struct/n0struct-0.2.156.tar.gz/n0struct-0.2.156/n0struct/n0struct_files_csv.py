...
def validate_csv_row(
    ...
    csv_schema: dict
        ...
        //items[0..i]/validations[0..j@i][3] = next: "BREAK", "EXIT", "EXCEPTION", "CONTINUE" (default None == "CONTINUE")
                                               ...
                                               EXCEPTION: raise AssertionError
        ...
        for validation_i,validation in enumerate(column_schema.get('validations', ())):
            ...
            if not isinstance(validation, (list, tuple)) or len(validation) < 2:
                raise TypeError(f"Validation rule #{validation_i} for '{column_name}' is incorrect."
                                " validation is expected as list/tuple/str with minimum 2 items"
            ...
            if len(validation) > 3:
                validation_next = validation[3].upper()  # EXIT/EXCEPTION/BREAK/CONTINUE (default)
                validation_next = validate_values(
                    validation_next,
                    ("BREAK", "EXIT", "EXCEPTION", "CONTINUE"),
                    raise_if_not_found = Exception(f"Validation rule #{validation_i} for '{column_name}' is incorrect."
                                                   " Action is expected as BREAK, EXIT, EXCEPTION or CONTINUE"

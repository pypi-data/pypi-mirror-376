...
class NodeWrapper(Node):
    ...
    def __rshift__(
        self, other: Union["NodeWrapper", Dict[str, "NodeWrapper"], str]
    ) -> "NodeWrapper":
        self.target = other
        return self
...
class FunctionStateGraph(StateGraph):
    ...
    def add_node(self, name: str, node: any) -> Self:
        ...
        if not isinstance(node, Node):
            raise ValueError(
                f"Node {name} must be an instance of Node class, but type was {type(node)}"
            )
        ...
        if hasattr(node, "config") and (not node.config is None):
            runnable = coerce_to_runnable(
                lambda state, config: node(state, config), name=name, trace=True
            )
        else:
            runnable = coerce_to_runnable(
                lambda state: node(state), name=name, trace=True
            )
    ...
    def process_flow(self, flow: Sequence[any]) -> Self:
        ...
        has_start_edge = any(
            isinstance(route.target, (str, dict))
            or (hasattr(route.target, "name") and callable(route.target))
            for route in flow
        ...
        for route in flow:
            if not hasattr(route, "name") or not hasattr(route, "target"):
                raise ValueError(
                    f"Route {route} must have 'name' and 'target' attributes"
                )
            ...
            if isinstance(route.target, str):
                ...
                path_map = {
                    key: target.name if isinstance(target, NodeWrapper) else target

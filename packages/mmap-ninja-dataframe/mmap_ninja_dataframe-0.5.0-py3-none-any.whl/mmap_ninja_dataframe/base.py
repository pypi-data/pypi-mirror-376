...
def _create_numpy_mmap_for_key(out_dir, key, values, verbose, dtype: Optional = None):
    if verbose:
...
def _create_mmap_for_key(out_dir, key, values, verbose):
    ...
    if selected_type == datetime:
        _create_numpy_mmap_for_key(out_dir, key, values, verbose, dtype=np.datetime64)
        return

...
def amap_draw_route_map(
    ...
    try:
        ...
        #print("请求URL:", unquote(response.request.url))
        response.raise_for_status()
        #print("response.text:", response.content)  # 打印原始文本
        try:
            print("response.json():", response.json())  # 如果是JSON格式会打印出来
        except Exception:
            pass
        ...
        print(f"路线地图已保存：{os.path.abspath(save_path)}")
        return True
    except Exception as e:

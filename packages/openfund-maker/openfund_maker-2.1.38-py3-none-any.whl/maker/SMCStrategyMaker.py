...
class SMCStrategyMaker(ThreeLineStrategyMaker):
    ...
    def __init__(self, config, platform_config, feishu_webhook=None,logger=None):
        ...
        # 配置交易所
        proxy = self.g_config.get('proxy', "http://localhost:7890")
        ...
        try:
            self.exchange_by_core = Exchange({
        ...
        self.cache_ttl = self.g_config.get('cache_ttl', 60)
    ...
    #   20250828 用core 进行下单
        order_price = OPTools.toDecimal(price)

        self._place_order_by_core(symbol, order_price, side, pair_config, leverage=self.leverage_value)


    def _place_order_by_core(self, symbol, price:Decimal, side,  pair_config, leverage:int=0, order_type='limit'):
        """_summary_
            下单
        Args:
            symbol (_type_): _description_
        ...
        # 执行下单
        try :
            self.exchange_by_core.place_order(
                symbol=symbol,
                price=price,
    ...
    def get_historical_klines_df_by_core(self, symbol, tf='4h'):
        """_summary_
            获取历史K线数据
        Args:
            symbol (_type_): _description_
        ...
        """
        return self.exchange_by_core.get_historical_klines_df(symbol=symbol, bar=tf)


    def build_struct_by_core(self, symbol, data, tf=None, is_struct_body_break=True) -> pd.DataFrame:
    ...
    def get_latest_struct_by_core(self, symbol, data) -> dict:
        """_summary_
            获取最后一个SMC结构
        Args:
            symbol (_type_): _description_
    ...
    def get_precision_length_by_core(self, symbol):
        """_summary_
            获取价格的精度长度
        Args:
            price (_type_): _description_
        Returns:
            _type_: _description_
        """
        tick_size = self.exchange_by_core.get_tick_size(symbol)
        return self.smcStruct.get_precision_length(tick_size)

    def find_OBs_by_core(self, symbol, struct, side=None, start_index=-1, is_valid=True, pair_config=None) -> pd.DataFrame:
        """_summary_
            识别OB
        Args:
            symbol (_type_): _description_
        ...
        Returns:
            _type_: _description_
        """
        is_struct_body_break = bool(pair_config.get('open_body_break', False))
        atr_multiplier = float(pair_config.get('range_of_the_ob', 0.6))
    ...
    def get_latest_OB_by_core(self, symbol, data, trend, start_index=-1) -> dict:
        """_summary_
            获取最新的Order Block
        Args:
            symbol (_type_): _description_
    ...
    def get_market_price(self, symbol):
        ...
        Returns:
            _type_: _description_
        """
        return self.exchange_by_core.get_market_price(symbol)
    ...
    def process_pair(self,symbol,pair_config):
        ...
        try:
            ...
            self.logger.debug(f"{symbol} : SMC策略 {ctf}|{htf}|{mtf} enable_FVG={enable_FVG} enable_OB={enable_OB} ...")

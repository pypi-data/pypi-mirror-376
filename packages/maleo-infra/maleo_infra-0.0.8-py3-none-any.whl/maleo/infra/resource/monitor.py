...
class ResourceMonitor:
    ...
    async def _measure_resource_usage(
        ...
        SuccessfulSystemOperation[None, SingleDataResponse[Measurement[Usage], None]](
            ...
            summary=f"Successfully calculated resource usage - Status: {status} - CPU | Raw: {raw_cpu:.2f}% | Smooth: {smooth_cpu:.2f}% - Memory: {raw_memory:.2f}MB",
    ...
    def get_last_measurement(self) -> Optional[Measurement[Usage]]:
        """Get the most recent resource usage data."""
        if not self.measurement_history:
            return None
        return self.measurement_history[-1]

    def get_measurement_history(self, seconds: int = 60) -> List[Measurement[Usage]]:
        """
    ...
    def get_average_usage(
    ...
    ) -> Optional[Measurement[AverageOrPeakUsage]]:
        """
        ...
        return Measurement[AverageOrPeakUsage](
    ...
    async def get_instant_usage(self) -> Measurement[Usage]:
        """
        ...
        return Measurement(measured_at=measured_at, status=status, usage=usage)

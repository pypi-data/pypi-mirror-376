...
if matplotlib:
	...
	for attempt in range( 2 ):
		try:    fontFilenames = [ font.fname for font in matplotlib.font_manager.fontManager.ttflist ] # uses cache
		except: fontFilenames = []
		else:   break
		if attempt == 0 and len( fontFilenames ) < 4:
			try: matplotlib.font_manager_load_fontmanager( try_read_cache=False ) # (re-)builds cache (may take several seconds)
			except: break
	if not fontFilenames:
		fontFilenames = matplotlib.font_manager.findSystemFonts() # does not use cache (may take several seconds)
		
	fonts = [ Font( filename ) for filename in fontFilenames ]

...
def build_parent_did_panel(
    ...
    *,
    ...
    # Minimal base input for temporal extractors: only PNR and INDEX_DATE, unique pairs.
    # Keep it small to reduce downstream materialization in the Rust extractors.
    base_input = (
        match_df.lazy()
    ...
    income_wide = (
        income_wide.lazy()
        .join(relations_map.lazy(), on="PNR", how="left")
    ...
    if "parent_pnr" not in panel_lf.collect_schema().names():
        panel_lf = (
            panel_lf.join(relations_map.lazy(), on="PNR", how="left").with_columns(
                [
    ...
    if bef_registry_pattern is not None:
        ...
        base_unique = (
            match_df.lazy()
            .join(relations_map.lazy(), on="PNR", how="inner")
    ...
    if uddf_file_path is not None:
        ...
        parents_map = (
            pl.concat([mothers_map, fathers_map], how="vertical")
            .unique(subset=["parent_pnr", "INDEX_DATE", "child_pnr", "gender"], keep="first")
        )
    ...
    panel_lf = panel_lf.select([c for c in keep_cols if c in panel_lf.collect_schema().names()])
    ...
    cat_cols = [
        ...
        if c in panel_lf.collect_schema().names()
    ]
    if cat_cols:
        panel_lf = panel_lf.with_columns([pl.col(c).cast(pl.Categorical) for c in cat_cols])

...
class WeightedMath:
    ...
    def calculate_invariant(normalized_weights: List[Decimal], balances: List[Decimal]):
        ...
        for i in range(len(normalized_weights)):
            invariant = mulDown(
                invariant, (powDown(balances[i], normalized_weights[i]))
            )
        return invariant
    ...
    def calc_out_given_in(
        ...
        exponent = divDown(weight_in, weight_out)
    ...
    def calc_bpt_out_given_exact_tokens_in(
        ...
        for i in range(len(balances)):
            balance_ratios_with_fee[i] = divDown(
                (balances[i] + amounts_in[i]), balances[i]
            )
            invariant_ratio_with_fees = mulDown(
                (invariant_ratio_with_fees + balance_ratios_with_fee[i]),
                normalized_weights[i],
            )  # .add(balance_ratios_with_fee[i].mulDown(normalized_weights[i]));
        ...
        for i in range(len(balances)):
            ...
            if balance_ratios_with_fee[i] > invariant_ratio_with_fees:
                non_taxable_amount = mulDown(
                    balances[i], (invariant_ratio - Decimal(1))
                )
                taxable_amount = amounts_in[i] - non_taxable_amount
                amount_in_without_fee = non_taxable_amount + (
                    mulDown(taxable_amount, Decimal(1) - swap_fee)
                )
            else:
    ...
    def calc_bpt_in_given_exact_tokens_out(
        ...
        for i in range(len(balances)):
            balance_ratios_without_fee[i] = divUp(
                (balances[i] - amounts_out[i]), balances[i]
            )
            sys.stdout.write(
                f"{balances[i]}{amounts_out[i]}{balances[i]}{balance_ratios_without_fee} balance ratio"
            )
            invariant_ratio_without_fees = invariant_ratio_without_fees + (
                mulUp(balance_ratios_without_fee[i], normalized_weights[i])
            )
        ...
        for i in range(len(balances)):
            ...
            if invariant_ratio_without_fees > balance_ratios_without_fee[i]:
                non_taxable_amount = mulDown(
                    balances[i], (complement(invariant_ratio_without_fees))
                )
                taxable_amount = amounts_out[i] - non_taxable_amount
                amount_out_with_fee = non_taxable_amount + (

    ...
    if mode == "start":
        ...
        if dict_args["use_data"] in ["no data (prior)", "seq like (exact likelihood) SLOW"]:
            ...
            if dict_args["hessian_program"] == "Baseml":
                baseml = "baseml.exe" if platform.system().lower() == "windows" else "baseml"
                basemlEXE = os.path.join(os.path.dirname(dict_args["mcmctreeEXE"]), baseml)
                shutil.copy(basemlEXE, os.path.join(runPath, baseml))
                codeml = "codeml.exe" if platform.system().lower()=="windows" else "codeml"
                codemlEXE = os.path.join(os.path.dirname(dict_args["mcmctreeEXE"]), codeml)
                shutil.copy(codemlEXE, os.path.join(runPath, codeml))
                replace_ctl(ctl_file,
                            r"usedata\s*=\s*\d+",
                            "usedata = 3")
                mcmctree_popen = factory.init_popen(command, stdin_=True)

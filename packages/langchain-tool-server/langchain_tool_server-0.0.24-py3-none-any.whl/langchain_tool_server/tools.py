...
class ToolHandler:
    ...
    async def call_tool(
        ...
        user_id = None
        print(f"[DEBUG] Auth enabled: {self.auth_enabled}")
        print(f"[DEBUG] Request object present: {bool(request)}")
        if request:
            print(f"[DEBUG] Request has user attr: {hasattr(request, 'user')}")
            if hasattr(request, "user"):
                print(f"[DEBUG] Request.user: {request.user}")
                print(f"[DEBUG] Request.user type: {type(request.user)}")
                print(
                    f"[DEBUG] Request.user attrs: {dir(request.user) if request.user else 'None'}"
                )
        ...
        if self.auth_enabled and request and hasattr(request, "user"):
            ...
            print(
                f"[DEBUG] Cannot extract user_id - auth_enabled: {self.auth_enabled}, request: {bool(request)}, has_user: {hasattr(request, 'user') if request else False}"
            )
        ...
        if isinstance(fn, Tool):
            ...
            print(
                f"[DEBUG] About to call Tool instance '{tool_id}' with user_id: {user_id}"
            )

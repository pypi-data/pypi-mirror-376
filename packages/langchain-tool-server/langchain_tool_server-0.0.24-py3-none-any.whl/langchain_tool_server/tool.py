...
class Tool:
    ...
    async def _auth_hook(self, user_id: str = None):
        ...
        """
        print(f"[DEBUG] _auth_hook called for tool '{self.name}'")
        print(f"[DEBUG] Auth provider: {self.auth_provider}")
        print(f"[DEBUG] User ID: {user_id}")

        if not self.auth_provider:
            print("[DEBUG] No auth provider - returning None")
            return None

        if not user_id:
            print("[DEBUG] No user_id provided - raising HTTPException")
    ...
    async def __call__(self, *args, user_id: str = None, **kwargs) -> Any:
        ...
        print(f"[DEBUG] Tool.__call__ for '{self.name}' - user_id: {user_id}")
        print(f"[DEBUG] Tool has auth_provider: {bool(self.auth_provider)}")

        # Run auth hook before execution
        auth_response = await self._auth_hook(user_id=user_id)
        print(f"[DEBUG] Auth hook response: {auth_response}")
        ...
        if auth_response and auth_response.get("auth_required"):
            print("[DEBUG] Auth required - returning auth response")
            return auth_response

        print("[DEBUG] Proceeding with tool execution")

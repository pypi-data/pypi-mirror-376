    ...
    if sys.platform == "win32":
        ...
        if len(getCommandResult("echo $XDG_SESSION_TYPE")) <= 0 and len(getCommandResult("echo $DISPLAY")) <= 0 and useHardwareEncode:
            ...
            if os.path.exists(displayShell):
                ...
                cmd1 = subprocess.Popen(f"sh {displayShell}", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, timeout=30)
    ...
    try:
        ...
        executeTemplate( {
            ...
            "output":output_file,
                        ...
                        oneVideoTimeout=240, useHardwareEncode=False, useHardwareDecode=False)
        return output_file
    except Exception as e:
        raise e
    finally:

...
def evaluate_output_path(submitter):
    ...
    """
    path_list = PathList()
    for node in [n for n in submitter.dependencies() if n.Class() in ("Write", "WriteGeo")]:

        value = utils.as_globbable_path(node.knob("file"))

        if not value:
            raise Exception("The Write or WriteGeo node '%s' doesn't have the file knob set." % node.name())

...
Whether allow overlaping with [jitter](#xAxis.jitter). If `false`, it will try to avoid overlap. But in extreme cases, some scatters may also overlap if there is no way to avoid. The following is the effect of seeting it to be `false`:""").tag(sync=True)
    
    jitterMargin = Float(None, allow_none=True, help="""Since `v6.0.0`

When setting [jitter](#xAxis.jitter) and [jitterOverlap](#xAxis.jitterOverlap) is `false`, the minimum distance between two scatters.""").tag(sync=True)
...
>
> Axis breaks cannot be used in category axes ([type](#xAxis.type): `'category'`).
...
See also the introduction to the axis break in [xAxis.breaks](#xAxisbreaks).""").tag(sync=True)

...
**rectangular coordinate:**
    ...
    coordinateSystem = Unicode(None, allow_none=True, help="""Specifies another coordinate system component on which this `series-heatmap` is laid out.

Options:

* `'cartesian2d'`

  Lay out based on a two-dimensional [rectangular coordinate system (also known as Cartesian coordinate system)](#grid). When multiple `xAxis` or multiple `yAxis` exist within an ECharts instance, the corresponding axes should be specified using [xAxisIndex](#series-heatmap.xAxisIndex) and [yAxisIndex](#series-heatmap.yAxisIndex) or [xAxisId](#series-heatmap.xAxisId) and [yAxisId](#series-heatmap.yAxisId).
...
* `'geo'`
  ...
  Lay out based on a [geographic coordinate system](#geo). When multiple geographic coordinate systems exist within an ECharts instance, the corresponding system should be specified using [geoIndex](#series-heatmap.geoIndex) or [geoId](#series-heatmap.geoId).

* `'calendar'`

  Lay out based on a [calendar coordinate system](#calendar). When multiple calendar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [calendarIndex](#series-heatmap.calendarIndex) or [calendarId](#series-heatmap.calendarId).

* `'matrix'`

  Lay out based on a [matrix coordinate system](#matrix). When multiple matrix coordinate systems exist within an ECharts instance, the corresponding system should be specified using [matrixIndex](#series-heatmap.matrixIndex) or [matrixId](#series-heatmap.matrixId).
...
See also [series-heatmap.coordinateSystemUsage](#series-heatmap.coordinateSystemUsage).""").tag(sync=True)
    
    coordinateSystemUsage = Unicode(None, allow_none=True, help="""Since `v6.0.0`

Specify how to lay out this `series-heatmap` based on the specified [coordinateSystem](#series-heatmap.coordinateSystem).
...
* `'box'`: **(Not applicable in [series-heatmap](#series-heatmap))**
  ...
  + For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-heatmap.coords](#series-heatmap.coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-heatmap.coordinateSystem](#series-heatmap.coordinateSystem).""").tag(sync=True)
...
When [coordinateSystemUsage](#series-heatmap.coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](#series-heatmap.coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.

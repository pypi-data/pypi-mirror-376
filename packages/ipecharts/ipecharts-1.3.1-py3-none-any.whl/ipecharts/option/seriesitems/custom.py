...
* `'data'`: assigns colors in the palette according to data items, with each data item using a different color.""").tag(sync=True)
    ...
    coordinateSystem = Unicode(None, allow_none=True, help="""Specifies another coordinate system component on which this `series-custom` is laid out.
...
* `'cartesian2d'`
  ...
  Lay out based on a two-dimensional [rectangular coordinate system (also known as Cartesian coordinate system)](#grid). When multiple `xAxis` or multiple `yAxis` exist within an ECharts instance, the corresponding axes should be specified using [xAxisIndex](#series-custom.xAxisIndex) and [yAxisIndex](#series-custom.yAxisIndex) or [xAxisId](#series-custom.xAxisId) and [yAxisId](#series-custom.yAxisId).
...
* `'polar'`
  ...
  Lay out based on a [polar coordinate system](#polar). When multiple polar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [polarIndex](#series-custom.polarIndex) or [polarId](#series-custom.polarId).

* `'geo'`

  Lay out based on a [geographic coordinate system](#geo). When multiple geographic coordinate systems exist within an ECharts instance, the corresponding system should be specified using [geoIndex](#series-custom.geoIndex) or [geoId](#series-custom.geoId).

* `'singleAxis'`

  Lay out based on a [singleAxis coordinate system](#singleAxis). When multiple singleAxis coordinate systems exist within an ECharts instance, the corresponding system should be specified using [singleAxisIndex](#series-custom.polarIndex) or [singleAxisId](#series-custom.polarId).

* `'calendar'`

  Lay out based on a [calendar coordinate system](#calendar). When multiple calendar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [calendarIndex](#series-custom.calendarIndex) or [calendarId](#series-custom.calendarId).

* `'matrix'`

  Lay out based on a [matrix coordinate system](#matrix). When multiple matrix coordinate systems exist within an ECharts instance, the corresponding system should be specified using [matrixIndex](#series-custom.matrixIndex) or [matrixId](#series-custom.matrixId).
...
See also [series-custom.coordinateSystemUsage](#series-custom.coordinateSystemUsage).""").tag(sync=True)
    
    coordinateSystemUsage = Unicode(None, allow_none=True, help="""Since `v6.0.0`

Specify how to lay out this `series-custom` based on the specified [coordinateSystem](#series-custom.coordinateSystem).
...
* `'box'`: **(Not applicable in [series-custom](#series-custom))**
  ...
  + For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-custom.coords](#series-custom.coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-custom.coordinateSystem](#series-custom.coordinateSystem).""").tag(sync=True)
...
When [coordinateSystemUsage](#series-custom.coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](#series-custom.coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.
...
**See example**: [geo-choropleth-scatter](https://echarts.apache.org/examples/en/editor.html?c=geo-choropleth-scatter&reset=1&edit=1)""").tag(sync=True)
    ...
    renderItem = Union([Any(default_value=None, allow_none=True),Unicode(default_value=None, allow_none=True),], default_value=None, allow_none=True, help="""`custom series` requires developers to write a rendering logic by themselves in the form of `Function`, or use a registered rendering logic in the form of `string` (since `v6.0.0`; See [echarts.registerCustomSeries](api.html#echarts.registerCustomSeries) for more information). This render logic is called [renderItem](#series-custom.renderItem).

...
* `'data'`: assigns colors in the palette according to data items, with each data item using a different color.""").tag(sync=True)
    ...
    coordinateSystem = Unicode(None, allow_none=True, help="""Specifies another coordinate system component on which this `series-bar` is laid out.

Options:

* `'cartesian2d'`

  Lay out based on a two-dimensional [rectangular coordinate system (also known as Cartesian coordinate system)](#grid). When multiple `xAxis` or multiple `yAxis` exist within an ECharts instance, the corresponding axes should be specified using [xAxisIndex](#series-bar.xAxisIndex) and [yAxisIndex](#series-bar.yAxisIndex) or [xAxisId](#series-bar.xAxisId) and [yAxisId](#series-bar.yAxisId).
...
* `'polar'`
  ...
  Lay out based on a [polar coordinate system](#polar). When multiple polar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [polarIndex](#series-bar.polarIndex) or [polarId](#series-bar.polarId).
...
See also [series-bar.coordinateSystemUsage](#series-bar.coordinateSystemUsage).""").tag(sync=True)
    
    coordinateSystemUsage = Unicode(None, allow_none=True, help="""Since `v6.0.0`

Specify how to lay out this `series-bar` based on the specified [coordinateSystem](#series-bar.coordinateSystem).
...
* `'box'`: **(Not applicable in [series-bar](#series-bar))**
  ...
  + For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-bar.coords](#series-bar.coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-bar.coordinateSystem](#series-bar.coordinateSystem).""").tag(sync=True)
...
When [coordinateSystemUsage](#series-bar.coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](#series-bar.coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.

...
* `'calendar'`
  ...
  Lay out based on a [calendar coordinate system](#calendar). When multiple calendar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [calendarIndex](#visualMap-piecewise.calendarIndex) or [calendarId](#visualMap-piecewise.calendarId).

* `'matrix'`

  Lay out based on a [matrix coordinate system](#matrix). When multiple matrix coordinate systems exist within an ECharts instance, the corresponding system should be specified using [matrixIndex](#visualMap-piecewise.matrixIndex) or [matrixId](#visualMap-piecewise.matrixId).
...
See also [visualMap-piecewise.coordinateSystemUsage](#visualMap-piecewise.coordinateSystemUsage).""").tag(sync=True)
    
    coordinateSystemUsage = Unicode(None, allow_none=True, help="""Since `v6.0.0`

Specify how to lay out this `visualMap-piecewise` based on the specified [coordinateSystem](#visualMap-piecewise.coordinateSystem).

In most cases, there is no need to specify `coordinateSystemUsage`, unless the default behavior is unexpected.

Options:

* `'data'`: **(Not applicable in [visualMap-piecewise](#visualMap-piecewise))**
...
* `'box'`:
  ...
  + For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [visualMap-piecewise.coords](#visualMap-piecewise.coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [visualMap-piecewise.coordinateSystem](#visualMap-piecewise.coordinateSystem).""").tag(sync=True)
...
When [coordinateSystemUsage](#visualMap-piecewise.coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](#visualMap-piecewise.coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.

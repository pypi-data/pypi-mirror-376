...
* `'calendar'`
  ...
  Lay out based on a [calendar coordinate system](#calendar). When multiple calendar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [calendarIndex](#series-funnel.calendarIndex) or [calendarId](#series-funnel.calendarId).

* `'matrix'`

  Lay out based on a [matrix coordinate system](#matrix). When multiple matrix coordinate systems exist within an ECharts instance, the corresponding system should be specified using [matrixIndex](#series-funnel.matrixIndex) or [matrixId](#series-funnel.matrixId).
...
See also [series-funnel.coordinateSystemUsage](#series-funnel.coordinateSystemUsage).""").tag(sync=True)
    
    coordinateSystemUsage = Unicode(None, allow_none=True, help="""Since `v6.0.0`

Specify how to lay out this `series-funnel` based on the specified [coordinateSystem](#series-funnel.coordinateSystem).
...
* `'box'`: **(Not applicable in [series-funnel](#series-funnel))**
  ...
  + For example, a [grid component](#grid) can be laid out in a [matrix coordinate system](#matrix) or a [calendar coordinate system](#calendar), where its layout rectangle is calculated by the specified [series-funnel.coords](#series-funnel.coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-funnel.coordinateSystem](#series-funnel.coordinateSystem).""").tag(sync=True)
...
When [coordinateSystemUsage](#series-funnel.coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](#series-funnel.coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.
...
`bottom` can be a pixel value like `20`; it can also be a percentage value relative to the container height like `'20%'`.""").tag(sync=True)
    ...
    width = Union([Unicode(default_value=None, allow_none=True),Float(default_value=None, allow_none=True),], default_value=None, allow_none=True, help="""Width of funnel component. Adaptive by default.
...
`width` can be a pixel value like `20`; it can also be a percentage value relative to the container width like `'20%'`.""").tag(sync=True)
    ...
    height = Union([Unicode(default_value=None, allow_none=True),Float(default_value=None, allow_none=True),], default_value=None, allow_none=True, help="""Height of funnel component. Adaptive by default.

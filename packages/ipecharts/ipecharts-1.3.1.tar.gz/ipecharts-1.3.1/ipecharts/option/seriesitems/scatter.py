...
* `'data'`: assigns colors in the palette according to data items, with each data item using a different color.""").tag(sync=True)
    ...
    coordinateSystem = Unicode(None, allow_none=True, help="""Specifies another coordinate system component on which this `series-scatter` is laid out.

Options:

* `'cartesian2d'`

  Lay out based on a two-dimensional [rectangular coordinate system (also known as Cartesian coordinate system)](#grid). When multiple `xAxis` or multiple `yAxis` exist within an ECharts instance, the corresponding axes should be specified using [xAxisIndex](#series-scatter.xAxisIndex) and [yAxisIndex](#series-scatter.yAxisIndex) or [xAxisId](#series-scatter.xAxisId) and [yAxisId](#series-scatter.yAxisId).
...
* `'polar'`
  ...
  Lay out based on a [polar coordinate system](#polar). When multiple polar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [polarIndex](#series-scatter.polarIndex) or [polarId](#series-scatter.polarId).

* `'geo'`

  Lay out based on a [geographic coordinate system](#geo). When multiple geographic coordinate systems exist within an ECharts instance, the corresponding system should be specified using [geoIndex](#series-scatter.geoIndex) or [geoId](#series-scatter.geoId).

* `'singleAxis'`

  Lay out based on a [singleAxis coordinate system](#singleAxis). When multiple singleAxis coordinate systems exist within an ECharts instance, the corresponding system should be specified using [singleAxisIndex](#series-scatter.polarIndex) or [singleAxisId](#series-scatter.polarId).

* `'calendar'`

  Lay out based on a [calendar coordinate system](#calendar). When multiple calendar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [calendarIndex](#series-scatter.calendarIndex) or [calendarId](#series-scatter.calendarId).

* `'matrix'`

  Lay out based on a [matrix coordinate system](#matrix). When multiple matrix coordinate systems exist within an ECharts instance, the corresponding system should be specified using [matrixIndex](#series-scatter.matrixIndex) or [matrixId](#series-scatter.matrixId).
...
See also [series-scatter.coordinateSystemUsage](#series-scatter.coordinateSystemUsage).""").tag(sync=True)
    
    coordinateSystemUsage = Unicode(None, allow_none=True, help="""Since `v6.0.0`

Specify how to lay out this `series-scatter` based on the specified [coordinateSystem](#series-scatter.coordinateSystem).
...
* `'box'`: **(Not applicable in [series-scatter](#series-scatter))**
...
See also [series-scatter.coordinateSystem](#series-scatter.coordinateSystem).""").tag(sync=True)
...
When [coordinateSystemUsage](#series-scatter.coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](#series-scatter.coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.

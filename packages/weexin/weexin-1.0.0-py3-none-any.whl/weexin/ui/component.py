...
from weexin.uiautomation import (
    ControlFromHandle,
)
from weexin.utils.tools import (
    get_file_dir,
    roll_into_view,
)
...
class WeChatImage(BaseUISubWnd):
    ...
    def save(self, dir_path=None, timeout=10):
        ...
        filename = f"weexin_{self.type}_{time.strftime('%Y%m%d%H%M%S')}.{suffix}"
...
class NewFriendElement:
    ...
    def __repr__(self) -> str:
        return f"<weexin New Friends Element at {hex(id(self))} ({self.name}: {self.msg})>"

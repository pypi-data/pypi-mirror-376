...
from weexin.utils.win32 import (
    ReadClipboardData,
    SetClipboardText,
)
from .base import *
...
class MediaMessage:
    ...
    def download(
            ...
            timeout: int = 10,
            click_wait_time: int = 3
        ) -> Path:
        if dir_path is None:
            dir_path = WxParam.DEFAULT_SAVE_PATH
        if self.type == 'image':
            filename = f"weexin_{self.type}_{time.strftime('%Y%m%d%H%M%S')}.png"
        elif self.type == 'video':
            filename = f"weexin_{self.type}_{time.strftime('%Y%m%d%H%M%S')}.mp4"

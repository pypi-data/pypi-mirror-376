...
class BybitService:
    ...
    async def listen_order_status(self, on_order_update):
        ...
        try:
            ...
            # Fix: Use correct Bybit WebSocket authentication format
            expires = int(time.time() * 1000) + 10000  # 10 seconds expiry
            payload = f"GET/realtime{expires}"
            sign = hmac.new(self.manager.api_secret.encode(), payload.encode(), hashlib.sha256).hexdigest()

            auth_msg = {"op": "auth", "args": [self.manager.api_key, expires, sign]}
            sub_msg  = {"op": "subscribe", "args": [topic]}
            # === MOD END ===
            ...
            # === MOD START: send auth first, then subscribe ===
            # If your WebSocketService supports a list of pre-messages, pass [auth_msg, sub_msg].
            # Otherwise, update WebSocketService to accept and send a list sequentially on open.
            await self.ws_service.subscribe(
                "bybit_private_order",
                ws_url,
                [auth_msg, sub_msg],   # <-- IMPORTANT: auth then subscribe
                message_handler
            )
            # === MOD END ===

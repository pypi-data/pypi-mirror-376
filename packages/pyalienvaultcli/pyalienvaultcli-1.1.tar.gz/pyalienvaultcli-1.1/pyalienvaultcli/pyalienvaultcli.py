...
parser.add_argument('-k', '--api-key', help='API key (could either be provided in the "%s" env var)' % SECRET_ALIENVAULT_API_KEY, type = str)
...
def main():
    ...
    if not(api_key):
        if SECRET_ALIENVAULT_API_KEY in os.environ:
            api_key = os.environ[SECRET_ALIENVAULT_API_KEY]
        else:
            parser.error('[!] No API key has been provided, exiting.')
    options.api_key = api_key
    
    otx = OTXv2(options.api_key)

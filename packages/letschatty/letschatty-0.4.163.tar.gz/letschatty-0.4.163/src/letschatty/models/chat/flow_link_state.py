...
class FlowStateAssignedToChat(AssignedAssetToChat):
    ...
    def from_json(cls, json: dict) -> FlowStateAssignedToChat | SmartFollowUpState:
        ...
        if isinstance(json, FlowStateAssignedToChat | SmartFollowUpState):
            ...
            # If it's already a FlowStateAssignedToChat but should be SmartFollowUpState, convert it
            if isinstance(json, FlowStateAssignedToChat) and not isinstance(json, SmartFollowUpState) and getattr(json, 'is_smart_follow_up', False):
                logger.debug(f"Converting FlowStateAssignedToChat to SmartFollowUpState")
                return SmartFollowUpState(**json.__dict__)
            return json
        is_smart_follow_up_value = json.get("is_smart_follow_up", False)
        logger.debug(f"Checking is_smart_follow_up: {is_smart_follow_up_value} (type: {type(is_smart_follow_up_value)})")
        if is_smart_follow_up_value:
            logger.debug(f"json is_smart_follow_up is True")
            logger.debug(f"About to create SmartFollowUpState with json: {json}")
            result = SmartFollowUpState(**json)
            logger.debug(f"Created SmartFollowUpState: {type(result)} - {result}")
            return result
        else:
            return cls(**json)

...
class ChatService:
    ...
    def get_smart_follow_up_state(chat : Chat) -> Optional[SmartFollowUpState]:
        ...
        """
        logger.debug(f"Chat flow states: {chat.flow_states}")
        for state in chat.flow_states:
            logger.debug(f"type of state: {type(state)} | is instance of SmartFollowUpState: {isinstance(state, SmartFollowUpState)}")
            logger.debug(f"state.is_smart_follow_up: {state.is_smart_follow_up}")

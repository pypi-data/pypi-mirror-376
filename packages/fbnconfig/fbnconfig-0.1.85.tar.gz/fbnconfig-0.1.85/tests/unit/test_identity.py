...
class DescribeApplicationResource:
    ...
    def test_create(self, respx_mock):
        ...
        assert result is not None
        assert result["id"] == "app1"  # The resource id, not the application_id
        assert result["application_id"] == "app123"  # The actual application ID from the response
    ...
    def test_update_with_change(self, respx_mock):
        # Simulate change: desired hash does not match old_state.content_hash
        respx_mock.get("/identity/api/applications/app123").mock(
            return_value=httpx.Response(
                200,
                json={
        ...
        result = sut.update(self.client, old_state)
        assert result is not None
        assert result["id"] == "app1"  # Resource id
        assert result["application_id"] == "app456"  # Application id from response
        assert result["client_id"] == "cid"
        assert "source_version" in result
        assert "remote_version" in result
        ...
        # Check that delete and create were called
    ...
    def test_readonly_fields_protection(self, respx_mock):
        """Test that application_id, secret, and issuer are readonly but can be set internally"""
        # Mock the create API call
        respx_mock.post("/identity/api/applications").mock(
            return_value=httpx.Response(
                200, json={"id": "app123", "secret": "secret123", "issuer": "https://issuer.example.com"}
            )
        )

        # Create an ApplicationResource instance
        sut = identity.ApplicationResource(
            id="app1",
            client_id="cid",
            display_name="App",
        ...
        assert sut.application_id is None
        assert sut.secret is None
        assert sut.issuer is None

        # Test that user cannot modify readonly fields after initialization
        with pytest.raises(
            AttributeError, match="Field 'application_id' is readonly and cannot be modified"
        ):
            sut.application_id = "user_set_id"

        with pytest.raises(AttributeError, match="Field 'secret' is readonly and cannot be modified"):
            sut.secret = "user_set_secret"

        with pytest.raises(AttributeError, match="Field 'issuer' is readonly and cannot be modified"):
            sut.issuer = "user_set_issuer"

        # Verify values are still None after failed attempts
        assert sut.application_id is None
        assert sut.secret is None
        assert sut.issuer is None

        # Test that the object can set these fields internally via create()
        result = sut.create(self.client)

        # Verify that the internal update worked
        assert sut.application_id == "app123"
        assert sut.secret == "secret123"
        assert sut.issuer == "https://issuer.example.com"
        assert result is not None and result["application_id"] == "app123"

        # Test that fields are still readonly after being set internally
        with pytest.raises(
            AttributeError, match="Field 'application_id' is readonly and cannot be modified"
        ):
            sut.application_id = "new_id"

        with pytest.raises(AttributeError, match="Field 'secret' is readonly and cannot be modified"):
            sut.secret = "new_secret"

        with pytest.raises(AttributeError, match="Field 'issuer' is readonly and cannot be modified"):
            sut.issuer = "new_issuer"

        # Verify values remain unchanged after failed modification attempts
        assert sut.application_id == "app123"
        assert sut.secret == "secret123"
        assert sut.issuer == "https://issuer.example.com"

...
async def google_trends_analysis(keywords: List[str], timeframe: str = 'today 3-m') -> str:
    """
    获取Google Trends数据并提供分析报告
    
    Args:
        keywords: 关键词列表，例如 ['智能咖啡机', 'nespresso']
        timeframe: 时间范围，默认为 'today 3-m' (最近3个月)
                   其他选项: 'today 12-m' (最近1年), 'all' (所有时间)
    ...
    if not keywords:
        return "❌ 错误：请提供至少一个关键词"
    
    if len(keywords) > 5:
        return "❌ 错误：最多支持5个关键词同时分析"
    ...
    try:
        ...
        if interest_over_time_df.empty:
            return f"📊 未找到关键词 {keywords} 在时间段 {timeframe} 的趋势数据。\n请尝试：\n- 使用更通用的关键词\n- 调整时间范围\n- 检查关键词拼写"

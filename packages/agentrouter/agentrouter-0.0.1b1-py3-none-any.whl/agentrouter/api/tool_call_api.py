...
class ToolCallAPIClient(BaseAPIClient):
    ...
    ENDPOINT = "/us/v1/agent/tool-call"
    ...
    async def create_tool_calls(
        ...
        import json
        logger.info(
            f"Creating tool calls with {len(messages)} messages, "
            f"{len(tools)} tools, choice: {tool_choice.name or tool_choice.type}"
        )
        logger.info(f"Tool Call API Payload (last message): {json.dumps(messages_dict[-1] if messages_dict else {}, indent=2)}")
        logger.debug(f"Full Tool Call API Payload: {json.dumps(payload, indent=2, default=str)}")
        ...
        try:
            ...
            logger.error(f"Failed to create tool calls: {str(e)}", exc_info=True)

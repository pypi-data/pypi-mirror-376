...
from raglib.core import TechniqueResult, RAGTechnique, TechniqueMeta
from raglib.schemas import Hit, Chunk
...
def test_fid_pipeline_fallback_behavior():
    """Test FiD pipeline fallback when no generator or adapter provided."""
    chunk1 = Chunk(id="c1", document_id="doc1", text="Some context", start_idx=0, end_idx=12)
    hits = [Hit(doc_id="doc1", score=1.0, chunk=chunk1)]
    
    retriever = DummyRetriever(hits=hits)
    
    # No generator or adapter provided
    pipeline = FusionInDecoderPipeline(retriever=retriever)
    
    result = pipeline.run(query="fallback test")
    
    assert result.success
    # Should use fallback LLMGenerator behavior
    assert "FALLBACK_GENERATION:" in result.payload["answer"]

...
class OneFootball:
    ...
    async def get_match_widget(self, match_id: int) -> str:
        """Get live match widget URL"""
        base_url = self._api.base_urls["live_ticker"]
        return f"{base_url}/v1/en/matches/{match_id}/widget"
    ...
    async def get_competition_current_season(self, competition_id: int) -> str:
        """Get competition current season ID"""
        games = await self.get_competition_matches(competition_id, 1, 0)

        if games and len(games) > 0:
            first_game = games[0]
            if "season" in first_game:
                return first_game["season"]["id"]
        return None

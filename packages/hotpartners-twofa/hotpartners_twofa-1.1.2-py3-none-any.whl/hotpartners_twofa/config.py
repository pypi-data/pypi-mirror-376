...
class TwoFAConfig:
    def __init__(self, fernet_key: str = None):
        # FERNET_KEY를 환경변수에서 가져오거나 매개변수로 받기
        self.FERNET_KEY = fernet_key or os.getenv('TWOFA_FERNET_KEY')
        if not self.FERNET_KEY:
            raise ValueError(
                "FERNET_KEY가 필요합니다. 환경변수 TWOFA_FERNET_KEY를 설정하거나 "
                "TwoFAConfig(fernet_key='your-key')로 전달하세요."
            )

...
class BrostelMessage:
    ...
    def __init__(
        ...
        self.config = config or BrostelConfig()
        self.tags_dict = self.config.default_tags
        self.ctx = ctx
        if ctx and (items := ctx.get("brostel", {})):
            self.tags_dict.update(items)
        self.markup_obj = None

    def build(self) -> BuiltMessage:
        """Builds final message for sending."""
        mapped_message = self.config.map(self)
        ...
        escaped_text = self.config.escaper(final_text)

...
fn handle_transaction_result<T: std::fmt::Display, E: std::fmt::Display>(result: Result<T, E>, transaction_type: &str) -> PyResult<String> {
    match result {
        ...
        Err(e) => {
            ...
            // 检查是否是确认错误但交易可能已提交
            if error_msg.contains("No transaction signature available") || 
               error_msg.contains("confirmation failed") ||
               error_msg.contains("timeout") {
                println!("⚠️  {} transaction may have been submitted but confirmation failed.", transaction_type);
                println!("⚠️  Check your wallet and the blockchain explorer for the transaction status.");
                println!("⚠️  This could be a network issue rather than a transaction failure.");
                
                // 返回警告而不是错误
                Ok(format!("Transaction submitted but confirmation uncertain. Please check blockchain explorer. Error: {}", error_msg))
            } else {

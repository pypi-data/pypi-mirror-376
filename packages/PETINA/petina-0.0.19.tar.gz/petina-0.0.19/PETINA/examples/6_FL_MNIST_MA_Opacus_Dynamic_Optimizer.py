...
# =========================
# Bayesian Optimization for Federated CSVec
# =========================
from PETINA.package.BayesianOptimization_local import BayesianOptimization

def bayesian_opt_federated(dp_params, num_clients=4, global_rounds=3, epochs_per_round_client=2, n_calls=8):
    """
    Run Bayesian Optimization to find best Count Sketch rows and cols for federated CSVec + DP
    """
    def objective(rows, cols):
        print(f"\n--- Trial with rows={int(rows)}, cols={int(cols)} ---")
        ...
        server.distribute_data_to_clients(trainset, batch_size=batch_size, epochs_per_round=epochs_per_round_client)
        trained_model = server.train_federated(global_rounds=global_rounds)
        acc = evaluate(trained_model, testloader)
        print(f"Trial Accuracy: {acc:.4f}")
        return acc

    # Define search space
    search_space = {
        'rows': (3, 10),    # Example: small number of rows to limit compute
        'cols': (100, 600)  # Example: varying sketch width
    }
    ...
    best = optimizer.max
    print(f"\nBest configuration: Rows={best['params']['rows']:.1f}, Cols={best['params']['cols']:.1f}, Accuracy={best['target']:.4f}")
    return best
...
def main():
    ...
    # print(f"Time run: {time.time() - start:.2f} seconds\n")
    dp_params = {'delta': delta, 'epsilon': epsilon, 'gamma': gamma, 'sensitivity': sensitivity}
    best_config = bayesian_opt_federated(
        dp_params=dp_params,
        num_clients=4,
        global_rounds=3,
        epochs_per_round_client=2,
        n_calls=6
    )

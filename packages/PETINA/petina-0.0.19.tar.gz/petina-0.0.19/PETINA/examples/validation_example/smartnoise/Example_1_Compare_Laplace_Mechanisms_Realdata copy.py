...
def run_trials(true_value, sensitivity, epsilon, label):
    ...
    laplace_mech = dp.m.make_laplace(*input_space, scale=sensitivity / epsilon)
    ...
    for _ in trange(num_trials, desc=f"{label} ε={epsilon}: Running trials"):
        noisy_opendp = laplace_mech(data)
        noisy_opendp_results.append(noisy_opendp[0])

        noisy_petina = DP_Mechanisms.applyDPLaplace(data, epsilon=epsilon, sensitivity=sensitivity)
...
def plot_histogram(opendp_results, petina_results, true_value, label, epsilon):
    ...
    plt.title(f"{label} Laplace Noisy Outputs Distribution\nε={epsilon}")
    plt.xlabel("Noisy Value")
    plt.ylabel("Density")
    plt.legend()
    plt.grid(True)
    os.makedirs("Plot/Example_1", exist_ok=True)
    filename = os.path.join("Plot","Example_1", f"{label.lower().replace(' ', '_')}_epsilon_{epsilon}_histogram.png")
...
def plot_bar_errors(stats_dict, label):
    ...
    os.makedirs("Plot/Example_1", exist_ok=True)
    filename = os.path.join("Plot","Example_1", f"{label.lower().replace(' ', '_')}_error_metrics.png")
    plt.savefig(filename)
    plt.close()
...
def plot_privacy_utility(epsilons, opendp_mses, petina_mses, label):
    ...
    plt.grid(True)
    os.makedirs("Plot/Example_1", exist_ok=True)
    filename = os.path.join("Plot","Example_1", f"{label.lower().replace(' ', '_')}_privacy_utility_tradeoff.png")
    plt.savefig(filename)
    plt.close()
...
def plot_boxplot(opendp_results, petina_results, true_value, label, epsilon):
    ...
    plt.grid(True)
    os.makedirs("Plot/Example_1", exist_ok=True)
    filename = os.path.join("Plot","Example_1", f"{label.lower().replace(' ', '_')}_epsilon_{epsilon}_error_boxplot.png")
    plt.savefig(filename)
    plt.close()

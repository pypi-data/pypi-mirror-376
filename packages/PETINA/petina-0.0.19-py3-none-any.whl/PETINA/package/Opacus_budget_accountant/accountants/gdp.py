...
class GaussianAccountant(IAccountant):
    ...
    def get_epsilon(self, delta: float, poisson: bool = True, **kwargs) -> float:
        ...
        Args:
            delta: target delta
            poisson: ``True`` if input batches were sampled via Poisson sampling,
                    ``False`` otherwise
        """
        ...
        # Decide which function will compute epsilon
        compute_eps = (
            privacy_analysis.compute_eps_poisson
            if poisson
            else privacy_analysis.compute_eps_uniform
        )
        # print(f"Using {'Poisson' if poisson else 'Uniform'} subsampling method for epsilon calculation.")
        # print("Selected compute_eps function:", compute_eps)

        # Extract latest parameters from history
        noise_multiplier, sample_rate, num_steps = self.history[-1]
        # print(f"Latest entry from history:")
        # print(f"  noise_multiplier = {noise_multiplier}")
        # print(f"  sample_rate = {sample_rate}")
        # print(f"  num_steps = {num_steps}")

        # Compute epsilon with all parameters
        eps = compute_eps(
            steps=num_steps,
            noise_multiplier=noise_multiplier,
            sample_rate=sample_rate,

...
class Filter(BaseFilter):
    ...
    def on_image_ready(  # pylint: disable=too-many-statements,too-many-branches,too-many-locals
        ...
        logger.debug (f' watermark size origin: { watermark_sz}');
        watermark_sz =  (image_size[0] / 8, watermark_sz[1]/watermark_sz[0] * (image_size[0]/8));
        logger.debug (f' watermark size apply: { watermark_sz}');
        ...
        self.x =  True;
        self.y = str(math.ceil(image_size[1]/2));
    ...
    async def watermarkdt2(self, url, alpha):
        logger.debug("call watermarkdt2")
        self.url = url
        self.alpha = alpha
        ...
        try:
            ...
            if isinstance(result, LoaderResult) and not result.successful:
                logger.warning(
                    "bad watermark result error=%s metadata=%s url: %s",
                    result.error,
                    result.metadata,
                    self.url
                )
            ...
            # Check if originImage is too small => Skip and not add watermark
            engine = self.context.modules.engine.__class__(self.context)
            engine.load(buffer, None)
            width, height = engine.size
            logger.debug(f"origin data: width={width} height={height}")
            if width < 32 or height < 32:
                logger.debug(f"Skip watermark because width={width} < 35")
                return  # thoát sớm, không gọi watermark

            # Nếu đủ điều kiện thì lưu và xử lý watermark

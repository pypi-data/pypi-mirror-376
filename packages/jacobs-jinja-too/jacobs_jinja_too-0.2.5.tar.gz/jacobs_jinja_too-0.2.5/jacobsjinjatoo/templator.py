...
class Templator(object):
    ...
    def _get_jinja2_environment(self, force=False):
        ...
        if force or self._jinja2_environment is None:
            ...
            # Add commentify filter to allow prefixing lines with a comment marker
            try:
                env.filters['comment'] = stringmanip.commentify
                env.filters['commentify'] = stringmanip.commentify
            except Exception:
                # If stringmanip doesn't provide the filter for some reason, skip silently
                pass

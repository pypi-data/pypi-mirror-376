...
def select_features_mutual_info(X: pd.DataFrame, y: pd.Series, task: str, top_k: int = 20) -> List[str]:
    ...
    if task == "classification":
        scores = mutual_info_classif(X_enc, y, discrete_features=[c not in num_cols for c in X.columns], random_state=42)
    else:
        scores = mutual_info_regression(X_enc, y, discrete_features=[c not in num_cols for c in X.columns], random_state=42)

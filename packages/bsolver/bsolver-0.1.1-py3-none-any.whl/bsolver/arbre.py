...
RAISE_ON_IDENTICAL_BRANCHES = False  # Set to True to raise an error on identical branches in ROBDD
...
class Node(ABC):
            ...
            while stack:
                ...
                if state == "call":
                    ...
                    if node.false == node.true:
                        ...
                        if RAISE_ON_IDENTICAL_BRANCHES:
                            raise RuntimeError("Identical branches in ROBDD, variable should be eliminated")
                        ...
                        if not T and not F:
                            ...
                            # Il faut supprimer les contradictions exemple:
                            # (A & ~B)|(~A & B) : (B, ~B) n'est pas une solution
                            TxF = {sol for sol in TxF if all(val1 == val2 for var1, val1 in sol for var2, val2 in sol if var1 == var2)}
    ...
    def print_table(self, solutions_only: bool = False, use_color: bool = True) -> None:
        ...
        # En-tête (variables colorées si nécessaires ou toujours présentes)
        def color_var_header(v: str) -> str:
            if necessary.get(v) == 'T':
                return f"{GREEN}{v}{RESET}"
            if necessary.get(v) == 'F':
                return f"{RED}{v}{RESET}"
            if v in always_present:
                return f"{YELLOW}{v}{RESET}"
            return v

        # Largeur de chaque colonne = max(len(nom), 3)
        col_widths = [max(len(v), 3) for v in vars]
        res_col_width = max(8, len("Résultat"))

        def format_cell(val: str, width: int) -> str:
            # Retire les codes ANSI pour le calcul de la largeur
            import re
            ansi_escape = re.compile(r'\x1b\[[0-9;]*m')
            plain = ansi_escape.sub('', val)
            pad = width - len(plain)
            return val + ' ' * pad

        header = " | ".join(format_cell(color_var_header(v), w) for v, w in zip(vars, col_widths)) + " | " + format_cell("Résultat", res_col_width)
        print(header)
        print("-" * (sum(col_widths) + 3 * len(vars) + res_col_width + 3))
        ...
        if solutions_only:
            ...
            for sol in self.solutions:
                sol_dict = dict(sol)
                parts: list[str] = []
                for v, w in zip(vars, col_widths):
                    if v in sol_dict:
                        val = (GREEN + "1" + RESET) if sol_dict[v] else (RED + "0" + RESET)
                    else:
                        val = "*"
                    parts.append(format_cell(val, w))
                # Résultat coloré en jaune si solution de longueur minimale
                res = (YELLOW + "1" + RESET) if len(sol) == min_len else "1"
                print(" | ".join(parts) + " | " + format_cell(res, res_col_width))
        else:
            for i in range(2 ** n):
                ...
                for v, w in zip(vars, col_widths):
                    if result:
                        val = GREEN + ("1" if env[v] else "0") + RESET
                    else:
                        val = "1" if env[v] else "0"
                    parts.append(format_cell(val, w))
                res = GREEN + "1" + RESET if result else RED + "0" + RESET
                print(" | ".join(parts) + " | " + format_cell(res, res_col_width))
...
class Var(Node):
    ...
    def __repr__(self) -> str:
        if self.true == TRUE and self.false == FALSE:
            ...
            return f"~Var('{self.name}')"
        return f"Var('{self.name}', {self.true!r}, {self.false!r})"
    
    def _simplify(self, seen: dict['Node', 'Node']) -> Node:
        if self in seen:
            return seen[self]
        if self.true == self.false:
            seen[self] = self.true._simplify(seen)
            return seen[self]
        t = self.true._simplify(seen)
        f = self.false._simplify(seen)
        if t == f:
            seen[self] = t._simplify(seen)
            return seen[self]

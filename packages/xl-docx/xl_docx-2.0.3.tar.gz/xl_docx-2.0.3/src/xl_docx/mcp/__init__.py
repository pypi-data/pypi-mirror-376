...
class WordMCP:
    ...
    def create_word(self, command, template_path=None):
        """
        创建新的Word文档（一次性生成），返回Sheet对象（不进行文件保存）
        
        Args:
            command: 创建指令
            template_path: 模板文件路径，默认为父目录中的 h.docx
        
        Returns:
            Sheet: 处理后的Sheet对象
        """
        if template_path is None:
            # 获取 __init__.py 的绝对路径，找到上一层目录，拼接 h.docx
            current_file_path = os.path.abspath(__file__)  # __init__.py 的绝对路径
            parent_dir = os.path.dirname(os.path.dirname(current_file_path))  # 上一层目录
            template_path = os.path.join(parent_dir, "h.docx")  # 拼接 h.docx
        # 生成XML

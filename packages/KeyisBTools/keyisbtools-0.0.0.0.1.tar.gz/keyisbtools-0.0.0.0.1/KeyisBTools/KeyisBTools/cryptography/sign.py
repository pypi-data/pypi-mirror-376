...
class m1:
    ...
    def _derive(self, k:bytes):
        d=hashlib.sha3_512(k).digest()
    ...
    def sign(self, k:bytes)->bytes:
        ek,mk=self._derive(k);n=os.urandom(16);t=int(time.time()).to_bytes(8,"big")
    ...
    def verify(self, k:bytes,s:bytes,ttl:int=15)->bool:
        if len(s)<80:return False
        ek,mk=self._derive(k);n,ct,tg=s[:16],s[16:-64],s[-64:]

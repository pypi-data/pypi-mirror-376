...
def find_discontinuity(data):
    ...
    for i in reversed(range(len(diffs))):
        ...
        if diff != 1:
            return (
                i + 1
            )  # Return the index of the element *after* the discontinuity
    return -1
...
def reattribute_data(data, msks):
    ...
    )  # Trier les éléments par la première valeur de chaque liste
    new_dict = {
        new_key: value
        for new_key, (_, value) in enumerate(sorted_items, start=1)
    }  # Réattribuer les clés dans l'ordre
    rotation_ = []
    for ix in new_dict:
...
def diamant_star(curr_reg_one, I):
    ...
    my_list = list(zip(y, x, strict=False))
...
def keep_largest_object(binary_image):
    ...
    if len(contours) == 0:
        return np.zeros_like(
            binary_image
        )  # Si pas d'objets, retourne une image vide
...
def area_pts(msks, dico):
    ...
    dicow = {}
    for i, j, z in zip(
        centroid_X, range(len(midpoints) - 1), dico, strict=False
    ):
        dicow[z] = [midpoints[j], midpoints[j + 1]] + dico[
            z
...
def regression_label(curr_msk, vis_reg=False, vis_res=False):
    ...
    if len(x) > 1:
        ...
        for row, col in zip(x_pred, y_prediction, strict=False):
...
def outlier_detection(X_study1, L):
    ...
    dico_out = {
        ix: [iy[0], iz] for ix, iy, iz in zip(L, X, y_pred, strict=False)
    }
    return dico_out
...
def angle_assessment(msks):
    ...
    for curr_label in L:
        ...
        n, m = curr_msk.shape
        coordinates = np.argwhere(curr_msk == 255)  # col 1 : Y et col2 : X
        y, x = coordinates[:, 0].reshape(-1, 1), coordinates[:, 1].reshape(
            -1, 1
        )
        curr_reg = curr_msk.copy()
        if len(x) > 1:
...
def regression_with_correction_angle(
    curr_msk, angle_deg, n, m, vis_reg=False, vis_res=False
):
    ...
    def check1(x_news, y_news):
        x_news1, y_news1 = [], []
        for x, y in zip(x_news, y_news, strict=False):
            if x >= 0 and y >= 0:
                x_news1.append(x)
                y_news1.append(y)
            elif x >= 0 and y < 0:
    ...
    def check2(x_news1, y_news1, n, m):
        x_news2, y_news2 = [], []
        for x, y in zip(x_news1, y_news1, strict=False):
            if x <= m - 1 and y <= n - 1:
                x_news2.append(x)
                y_news2.append(y)
    ...
    def tracer_droite(p0, p1):
        ...
        steps = max(
            abs(dx), abs(dy)
        )  # Le plus grand déplacement détermine le nombre de points
        ...
        for _ in range(steps):
            ...
            points.append(
                (round(x), round(y))
            )  # On arrondit pour obtenir des coordonnées discrètes
    ...
    if len(x) > 1:
        ...
        for row, col in zip(x_news2, y_news2, strict=False):
            curr_reg[col, row] = 255

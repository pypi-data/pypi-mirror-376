...
from orseg.sam2.modeling.sam2_utils import MLP, LayerNorm2d
...
class MaskDecoderHQ(nn.Module):
    ...
    def forward(
        ...
        if multimask_output:
            ...
            masks_sam, iou_pred = self._dynamic_multimask_via_stability(
                masks, iou_pred
            )
        else:
            masks_sam = masks[:, 0:1, :, :]
    ...
    def predict_masks(
        ...
        masks_sam = (
            hyper_in[:, :4] @ upscaled_embedding.view(b, c, h * w)
        ).view(b, -1, h, w)
        masks_ours = (
            hyper_in[:, 4:] @ upscaled_embedding_ours.view(b, c, h * w)
        ).view(b, -1, h, w)
    ...
    def _dynamic_multimask_via_stability(
        ...
        multimask_logits = all_mask_logits[
            :, 1 : self.num_mask_tokens - 1, :, :
        ]

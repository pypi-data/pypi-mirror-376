...
class CommandManager:
    def __init__(self, commands_file=None):
        if commands_file is None:
            ...
            try:
                self.commands_file = pkg_resources.resource_filename('siphoai', 'data/commands.json')
            except:
    ...
    def add_application_command(self, triggers, command, response, requires_confirmation=False, third_party_info=None):
        ...
        if requires_confirmation:
            ...
            new_command["confirmation_message"] = f"Are you sure you want to run {triggers[0] if isinstance(triggers, list) else triggers}?"
    ...
    def add_system_command(self, triggers, command, response, requires_confirmation=True):
        ...
        if requires_confirmation:
            new_command["requires_confirmation"] = True
            new_command["confirmation_message"] = f"Are you sure you want to execute this system command?"
        
        self.commands['system_commands'].append(new_command)
        print(f"Added system command: {triggers}")
        
    def add_third_party_command(self, triggers, command, response, app_name, executable, download_url, install_instructions):
    ...
    def _add_application_command_interactive(self):
        ...
        requires_confirmation = input("Requires voice confirmation? (y/N): ").lower() == 'y'
        
        self.add_application_command(triggers, [command], response, requires_confirmation)
    ...
    def _add_system_command_interactive(self):
        ...
        command = input("Enter system command (with arguments separated by spaces): ").strip().split()
        response = input("Enter response message: ").strip()
        
        requires_confirmation = input("Requires voice confirmation? (Y/n): ").lower() != 'n'
        
        confirm = input(f"Are you sure you want to add this system command: {' '.join(command)}? (y/N): ")
        if confirm.lower() == 'y':
    ...
    def _add_third_party_command_interactive(self):
        ...
        self.add_third_party_command(triggers, [command], response, app_name, executable, download_url, install_instructions)

                ...
                for cmd in commands:
                    if "requires_third_party" in cmd:
                        ...
                        third_party_apps[app_name.lower()] = {
                            ...
                            "install_instructions": third_party.get("install_instructions", ""),
                            "triggers": cmd.get("triggers", []),
                            "category": category,
                            "alternative_message": third_party.get("alternative_message", f"{app_name} is not available.")
                        }
    ...
    def generate_system_report(self):
        ...
        if app_status["available"]:
            ...
            for app_key, info in app_status["available"].items():
                category = info["category"].replace('_', ' ').title()
                print(f"    • {info['app_name']} [{category}] - Triggers: {', '.join(info['triggers'])}")

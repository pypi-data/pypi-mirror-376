...
class User(UpdatableObject):
    ...
    @classmethod
    def get_by_email(cls, email, api_key=None, **kwargs):
        response = API(api_key).request('get', '/users/email/%s' % email)
        return cls().load_from(response)

    @classmethod
    def get_by_username(cls, username, api_key=None, **kwargs):
        response = API(api_key).request('get', '/users/username/%s' % username)
        return cls().load_from(response)
    ...
    @classmethod
    def list(cls, api_key=None, **kwargs):
        response = API(api_key).request('get', '/users/', kwargs)
        return objects_from_json(response, api_key=api_key)

    @classmethod
    def create(cls, api_key=None, **kwargs):
        api = API(api_key)
        response = api.request('post', '/users/', params=kwargs)
        return objects_from_json(response, api.api_key)

_BX="Twilio 'from' number not configured. Cannot send SMS."
_BW='completedAt'
_BV='status_code'
_BU='Error: URL template evaluation failed.'
_BT='contentType'
_BS='access_token'
_BR='token_type'
_BQ='client_credentials'
_BP='function_call'
_BO='Using environment credentials/IAM role for S3.'
_BN='boto3 library is required for S3 operations.'
_BM='Using Application Default Credentials for GCS.'
_BL='gcs_service_account'
_BK='google-cloud-storage library is required for GCS operations.'
_BJ='action_action_key'
_BI='destinationVariable'
_BH='sourceVariable'
_BG='promptName'
_BF='unknown_intent'
_BE='Error: AI response was malformed (function call had no name).'
_BD='resp_mime_type'
_BC='gemini-2.5-flash-preview-04-17'
_BB='safetySettings'
_BA='mime_type'
_B9='assistant'
_B8='noInputIntentNameList'
_B7='variables'
_B6='DEEPSEEK_API_KEY'
_B5='ANTHROPIC_API_KEY'
_B4='OPENAI_API_KEY'
_B3='EMAIL_FROM_ADDRESS'
_B2='EMAIL_API_KEY'
_B1='GEMINI_API_KEY'
_B0='GCP_REGION'
_A_='GCP_PROJECT'
...
_Ay='user_input'
_Ax='message'
_Aw='Journey token not available.'
_Av='Accept-Encoding'
_Au='client_secret'
_At='client_id'
_As='grant_type'
...
_Al='function'
_Ak='header'
_Aj='gemini-2.0-flash'
_Ai='model_provider'
_Ah='agentDefaultIntent'
_Ag='intentContainsPairsList'
_Af='intentNameList'
_Ae='action_flow'
_Ad='action_type'
_Ac='iframeId'
...
_AU='privacy'
_AT='prompt_type'
_AS='ragStoreVectorDistanceThreshold'
_AR='ragStoreTopK'
_AQ='transferPasskeyFlag'
_AP='transferAllowed'
_AO='TWILIO_NUMBER'
_AN='TWILIO_AUTH_TOKEN'
_AM='TWILIO_ACCOUNT_SID'
_AL='parameters'
_AK='Accept'
_AJ='failureResponseMessage'
_AI='gzip'
_AH='string'
_AG='enum'
_AF='required'
...
try:import sendgrid;from sendgrid.helpers.mail import From,Mail
except ImportError:sendgrid=_A;From=_A;Mail=_A;logging.info('sendgrid library not found. Email sending will not be available.')
...
try:from mini_racer import MiniRacer
...
class AsyncPinionAIClient:
	...
	def __init__(self,create_key,agent_id,host_url,client_id,client_secret,version=_A,initial_engagement_data=_A,grpc_server_address=_A,gcp_project_id=_A,gcp_region=_A,gemini_api_key=_A,twilio_account_sid=_A,twilio_auth_token=_A,twilio_number=_A,email_api_key=_A,email_from_address=_A,openai_api_key=_A,anthropic_api_key=_A,deepseek_api_key=_A):
		...
		self._agent_id=agent_id;self._host_url=host_url.rstrip('/');self._client_id=client_id;self._client_secret=client_secret;self._version=version;self._grpc_server_address=grpc_server_address or os.environ.get(_Az,'localhost:50051');self._gcp_project_id=gcp_project_id or os.environ.get(_A_);self._gcp_region=gcp_region or os.environ.get(_B0);self._gemini_api_key=gemini_api_key or os.environ.get(_B1);self._twilio_account_sid=twilio_account_sid or os.environ.get(_AM);self._twilio_auth_token=twilio_auth_token or os.environ.get(_AN);self._twilio_number=twilio_number or os.environ.get(_AO);self._email_api_key=email_api_key or os.environ.get(_B2);self._email_from_address=email_from_address or os.environ.get(_B3);self._openai_api_key=openai_api_key or os.environ.get(_B4);self._anthropic_api_key=anthropic_api_key or os.environ.get(_B5);self._deepseek_api_key=deepseek_api_key or os.environ.get(_B6);self._http_session=httpx.AsyncClient(base_url=self._host_url,timeout=12e1,follow_redirects=_D);self._token=_A;self._session_id=_A;self._raw_session_data=_A;self.var={};self.engagement=''
		if initial_engagement_data:self.var=initial_engagement_data;self.engagement='active'
		self._class_intent_data='';self._class_message='';self._sub_message='';self._fin_message='';self.transfer_requested='';self.transfer_accepted='';self._privacy_level='';self.current_pipeline='';self.unique_id='';self.phone_number='';self.stepup_authorized=_G;self.authorized=_G;self.next_intent='';self.last_session_post_modified=_A;self._grpc_listener_task=_A;self._grpc_channel=_A;self._grpc_stub=_A;self._grpc_last_update_time=time.time();self.chat_messages=[];self.grpc_sender_id=_Z;self._genai_client=_A;self._journey_bearer_token=_A;self._account_id=_A;self._customer_id=_A;self._agent_id_from_api=_A;self._agent_description=_A
	@classmethod
	...
	async def _initialize_session_and_vars(self):
		...
		if _C in self._raw_session_data and self._raw_session_data[_C]:
			self.var=self._extract_data_from_session(self._raw_session_data[_C]);agent_data=self._raw_session_data[_C][_E][_C][_J];self.var['sessionId']=self._session_id;self.var['agentTitle']=agent_data.get('title');self.var['agentSubtitle']=agent_data.get('subtitle');self.var['agentStart']=agent_data.get('startStatement');self.var[A]=agent_data.get(A);self.var['userImage']=agent_data.get('userImagePath','').strip();self.var['assistImage']=agent_data.get('assistantImagePath','').strip();self.var[_AP]=agent_data.get(_AP);self.var[_AQ]=agent_data.get(_AQ);self.var[B]=agent_data.get(B);stores_data=agent_data.get('stores',[])
			if stores_data and isinstance(stores_data,list)and len(stores_data)>0:first_store=stores_data[0];self.var[_z]=first_store.get('resourceId','');self.var[_AR]=first_store.get('topK',10);self.var[_AS]=first_store.get('vectorDistanceThreshold',.5)
			else:self.var[_z]='';self.var[_AR]=10;self.var[_AS]=.5
	...
	def _initialize_genai_client(self):
		"\n        Initializes the Gemini client (genai.Client) for Vertex AI or the Gemini API.\n        The google-generativeai library automatically uses `aiohttp` if it's installed.\n        "
		if self._gcp_project_id and self._gcp_region:logger.info(f"Initializing Gemini client for Vertex AI in project {self._gcp_project_id}");return genai.Client(vertexai=_D,project=self._gcp_project_id,location=self._gcp_region)
		elif self._gemini_api_key:logger.info('Initializing Gemini client with direct API key.');return genai.Client(api_key=self._gemini_api_key)
	...
	async def get_pinionai_version_info(self):
		...
		if token:
			...
			data_only=data.get(_C,{}).get(_E,{}).get(_C,{});logger.info(f"Version data only: {data_only}");return data_only
		else:logger.error('Currently unavailable (token could not be obtained).')
		return{}
	async def process_user_input(self,user_input='',sender=_Z):
		'\n        Processes user input, interacts with AI, and determines the next response.\n        ';K='Transfer requested, but gRPC client is not connected.';J='authentication_step_up_pipeline';I='highly';H='finalprocess';G='subprocess';F='preprocess';E='contextFlow';D='final prompt';C='subprompt';B='inputVars';A='intent_resp';preprocess_response='';subprocess_response='';final_response='';self.grpc_sender_id=sender
		if not self.transfer_requested:
			prompt_resp_var_name=_A
			if self.next_intent:self.var['prompt_resp_var']=self.next_intent;prompt_type=self.var[_AT]=_A0;prompt_resp_var_name=self.next_intent;self.next_intent=''
			else:
				...
				prompt_type=self.var.get(_AT)
			intent_data=self._get_intent_details(self.var[prompt_resp_var_name])
			...
			intent_type=intent_data[_F];self.var[A]=intent_data[_B];self._privacy_level=self.var['privacy_resp']=intent_data[_AU];self.current_pipeline=self.var['pipeline_resp']=intent_data[_m];delivery_method=intent_data[_AV];language=intent_data[_a];intent_input_vars=intent_data.get(B,{});context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_AW,{})
			if self.var.get(_H):logger.debug(f"Classification | {self.var[A]}");logger.debug(f"Sensitivity | {self._privacy_level}");logger.debug(f"Intent Type | {intent_type}");logger.debug(f"Intent Var (ActionKey) | {self.current_pipeline}")
			if intent_type!=_AX:
				if intent_input_vars:
					agent_vars_config=self._raw_session_data.get(_C,{}).get(_E,{}).get(_C,{}).get(_J,{}).get(_B7,[])
					for key in intent_input_vars:
						for var_def in agent_vars_config:
							if var_def.get(_B)==key and var_def.get('reset',_G)is _D:self.var[key]='';break
				if prompt_type==_A0:self._class_message=user_input;self._class_intent_data=intent_data
				elif prompt_type==C:self._sub_message=user_input
				elif prompt_type==D:self._fin_message=user_input
			elif intent_type==_AX:
				action_key_for_input=intent_data.get(_m,'')
				...
				if self._class_intent_data:intent_data=self._class_intent_data;user_input=self._class_message;intent_type=intent_data[_F];self.var[A]=intent_data[_B];delivery_method=intent_data[_AV];language=intent_data[_a];intent_input_vars=intent_data.get(B,{});context_flow=intent_data.get(E,{});intent_subprompt=context_flow.get(C,{});intent_finalprompt=context_flow.get(D,{});intent_preprocess=context_flow.get(F,{});intent_subprocess=context_flow.get(G,{});intent_finalprocess=context_flow.get(H,{});intent_actionprocess=intent_data.get(_AW,{});self._privacy_level=intent_data[_AU];self.current_pipeline=intent_data[_m]
				original_intent_input_vars=self._class_intent_data.get(B,{})if self._class_intent_data else{}
				if action_key_for_input not in original_intent_input_vars and self._class_intent_data:final_response=f"'{user_input}' is a {intent_data[_B]} . Please respond to trigger an intent or input I am listening for such as: {self.var.get(_B8,[])}."
			if intent_input_vars:
				for(key_req,prompt_msg)in intent_input_vars.items():
					if key_req not in self.var or not self.var[key_req]:final_response=prompt_msg;break
			if not final_response:
				...
				if not final_response:
					if not self.authorized and'private'in self._privacy_level or I in self._privacy_level:
						_,self.unique_id,self.phone_number=format_phone(self.var.get('phone_number',self.phone_number));customer_id,enrolled,_=await self._journey_lookup(self.unique_id,self.var);auth_response_msg=''
						if customer_id and enrolled:
							pipeline_key=self.var[J]if I in self._privacy_level else self.var['authentication_pipeline'];json_payload={_AY:pipeline_key,_h:{_c:delivery_method,_d:self.phone_number},_AZ:{_Aa:self.unique_id,_d:self.phone_number},_E:{_Ab:self._session_id},_a:language}
							if self.var.get(_i):json_payload[_Ac]=self.var[_i]
							...
							if error_msg:auth_response_msg=error_msg
								...
								if pipeline_key==self.var[J]and self.authorized:self.stepup_authorized=_D
								if is_authenticated:logger.info('Authorization successful.')
						else:
							auth_response_msg=f"You are not enrolled. We are sending a link to {self.phone_number} so that you can enroll your device.";json_payload={_AY:self.var['enroll_pipeline'],_h:{_c:delivery_method,_d:self.phone_number},_AZ:{_Aa:self.unique_id,_d:self.phone_number},_E:{_Ab:self._session_id},_a:language}
							if self.var.get(_i):json_payload[_Ac]=self.var[_i]
			...
			if not final_response:
				if intent_type=='fixed':fixed_message_template=self._clean_text(intent_data['fixedResponseMessage']);final_response=self._evaluate_f_string(fixed_message_template,self.var)
				elif intent_type==_A1:
					action_details_from_intent=self._map_to_standard_action_details(intent_data);action_type=action_details_from_intent.get(_Ad)
					if action_type=='journey':self.var,final_response=await self._run_journey_action(self.var,action_details_from_intent)
						...
						action_flow=action_details_from_intent.get(_Ae)
						if action_flow:
							self.var,action_response=await self._process_routing(self.var,action_flow,user_input=user_input)
							if action_details_from_intent.get(_e):response_msg_template=self._clean_text(action_details_from_intent[_e]);final_response=self._evaluate_f_string(response_msg_template,self.var)
					...
					if intent_finalprompt:
						for(_indx,item_config)in sorted(intent_finalprompt.items()):
							if isinstance(item_config,dict):
								if _A2 in item_config:
									target_prompt=item_config[_A2];self.var,resp_var_name=await self._prompt_response(target_prompt,user_input)
									if resp_var_name and self.var.get(resp_var_name):final_response=self.var[resp_var_name]
								elif _j in item_config:
									mcp_name=item_config[_j];self.var,mcp_response=await self._run_mcp_action(self.var,mcp_name,user_input)
		...
		else:final_response=K;logger.warning(K)
		if final_response:self.chat_messages.append({_S:_B9,_Q:final_response})
	...
	async def update_pinion_session(self):
		...
		data_to_post[_E]['var']=self.var;messages_for_api=[{k:v for(k,v)in msg.items()if k!='avatar'}for msg in self.chat_messages];data_to_post[_E][_n]=messages_for_api;response_obj,response_data=await self._post_session_api(self._host_url,self._token,self._session_id,data_to_post,self.transfer_requested,self.transfer_accepted)
	...
	async def start_grpc_client_listener(self,sender_id=_Z):
		'\n        Initializes and starts the async gRPC client for chat.\n        '
		...
		try:self._grpc_channel=agrpc.insecure_channel(self._grpc_server_address);self._grpc_stub=ChatServiceStub(self._grpc_channel);logger.info(f"gRPC client connected to {self._grpc_server_address} for session {self._session_id} as {sender_id}");self._grpc_listener_task=asyncio.create_task(self._grpc_read_handler(self.grpc_sender_id,self._session_id,self._grpc_stub));return _D
		except Exception as e:logger.error(f"Failed to start gRPC client: {e}",exc_info=_D);raise PinionAIGrpcError(f"Failed to start gRPC client listener: {e}")from e
	async def _grpc_read_handler(self,client_id,session_id,stub):
		...
		try:
			...
			async for response in read_stream:logger.info(f"gRPC message received: {response.sender_id} ({response.timestamp}): {response.message}");self.chat_messages.append({_S:response.sender_id,_Q:response.message});self._grpc_last_update_time=time.time()
	...
	async def send_grpc_message(self,message_text):
		...
		try:recipient_id=_B9 if self.grpc_sender_id==_Z else _Z;request=ChatMessageRequest(thread_id=1,message=message_text,sender_id=self.grpc_sender_id,recipient_id=recipient_id,session_id=str(self._session_id));await self._grpc_stub.SendMessage(request);logger.info(f"gRPC message sent by {self.grpc_sender_id}: {message_text}")
	...
	async def _process_routing(self,current_var,process_items,initial_final_response=_A,user_input=_A):
		...
		for(_key,item_config)in sorted(process_items.items()):
			...
			elif E in item_config:current_var,step_response=self._run_rule_action(current_var,item_config[E],user_input=user_input)
			elif _h in item_config:current_var,step_response=await self._run_delivery_action(current_var,item_config[_h])
			elif _A1 in item_config:current_var,step_response=await self._run_action_action(current_var,item_config[_A1],generated_final_response,user_input=user_input)
			elif _j in item_config:current_var,step_response=await self._run_mcp_action(current_var,item_config[_j],user_input)
	...
	def _extract_data_from_session(self,session_data_root):
		'Extracts variables and agent configuration into the var dictionary.';var_dict={};agent_config=session_data_root.get(_E,{}).get(_C,{}).get(_J,{});global_to_self_attr_map={_Az:'_grpc_server_address',_A_:'_gcp_project_id',_B0:'_gcp_region',_B1:'_gemini_api_key',_AM:'_twilio_account_sid',_AN:'_twilio_auth_token',_AO:'_twilio_number',_B2:'_email_api_key',_B3:'_email_from_address',_B4:'_openai_api_key',_B5:'_anthropic_api_key',_B6:'_deepseek_api_key'}
		for variable_def in agent_config.get(_B7,[]):
			name=variable_def[_B];value=variable_def['value'];var_type=variable_def.get(_F)
			if variable_def.get(_A3)=='global':
		...
		for intent in agent_config.get('intents',[]):
			...
			if intent[_F]=='information'or intent[_F]==_A1:no_input_intent_names.append(intent_name)
			if intent[_F]==_AX:input_intent_names.append(intent_name)
			intent_privacy_pairs.append((intent_name,intent[_AU]));intent_type_pairs.append((intent_name,intent[_F]));intent_contains_word_pairs.append((intent_name,intent.get('containsWord','')));intent_action_key_pairs_raw.append((intent_name,intent.get(_m,'')))
		var_dict[_Af]=intent_names;var_dict[_B8]=no_input_intent_names;var_dict['inputIntentNameList']=input_intent_names;var_dict['intentPrivacyPairsList']=intent_privacy_pairs;var_dict['intentTypePairsList']=intent_type_pairs;var_dict[_Ag]=intent_contains_word_pairs;resolved_action_key_pairs=[]
		...
		var_dict['intentActionPairsList']=resolved_action_key_pairs;var_dict[_A4]=agent_config.get(_A4);var_dict[_Ah]=agent_config.get('defaultIntent');return var_dict
	async def _classify_input(self,user_input):
		if not self.var.get(_A4):logger.error('startPrompt not defined in agent configuration.');return self.var,_A
		_current_var_state,resp_var_name=await self._prompt_response(self.var[_A4],user_input)
	...
	async def _prompt_response(self,target_prompt,user_input=''):
		...
		self.var[_AT]=prompt_config[_F];prompt_resp_var_name=prompt_config[_A5];prompt_body_template=self._clean_text(prompt_config[_T]);current_prompt_text=self._evaluate_f_string(prompt_body_template,self.var,user_input=user_input);model_provider=prompt_config.get(_Ai,_b).lower();model_name=prompt_config.get(_U);llm_response_text=_A;function_calls=_A;system_instruction=self._evaluate_f_string(self._agent_description or'',self.var)
		if model_provider==_b:
			content_parts=[]
			if prompt_config.get(_A6):
				file_uri_template=self._clean_text(prompt_config[_A6]);file_uri=self._evaluate_f_string(file_uri_template,self.var)
				if file_uri:content_parts.append(Part.from_uri(file_uri=file_uri,mime_type=prompt_config[_BA]))
			content_parts.append(current_prompt_text);gen_config=google_genai_types.GenerateContentConfig(system_instruction=system_instruction if system_instruction else _A,temperature=prompt_config[_A7],top_p=prompt_config[_V],top_k=prompt_config[_A8],candidate_count=prompt_config[_A9],max_output_tokens=prompt_config[_AA],stop_sequences=prompt_config.get('stop')or _A,safety_settings=prompt_config.get(_BB))
			if model_name in[_BC]and prompt_config.get(_AB)is not _A:gen_config.thinking_config=ThinkingConfig(thinking_budget=prompt_config[_AB])
			if prompt_config.get(_AC):gen_config.response_schema=prompt_config[_AC];gen_config.response_mime_type=prompt_config.get(_BD,_M)
		...
		elif model_provider in[_f,_o,_AD]:
			messages=[]
			if system_instruction and model_provider in[_f,_AD]:messages.append({_S:_p,_Q:system_instruction})
			messages.append({_S:_Z,_Q:current_prompt_text});tools_for_provider=self._translate_tools_for_provider(model_provider,direct_tool_configs,processed_fd_dicts)
			if model_provider==_f:llm_response_text,function_calls=await self._get_openai_response_async(model_name,messages,prompt_config,tools_for_provider)
			elif model_provider==_o:anthropic_messages=[m for m in messages if m[_S]!=_p];llm_response_text,function_calls=await self._get_anthropic_response_async(model_name,system_instruction,anthropic_messages,prompt_config,tools_for_provider)
			elif model_provider==_AD:llm_response_text,function_calls=await self._get_deepseek_response_async(model_name,messages,prompt_config,tools_for_provider)
		...
		if function_calls:
			...
			if not function_name:logger.error(f"LLM response contained a function call with no name: {function_call}");return self.var,_BE
		...
		if llm_response_text:
			if prompt_config[_F]==_A0:
				raw_response=llm_response_text.strip().lower()
				if self.var.get(_H):logger.debug(f"Raw Intent Response | {raw_response}")
				matched_intent_key=self._match_intent_from_text(raw_response,self.var.get(_Ag,[]),self.var.get(_Ah,_BF),self.var.get(_Af,[]));self.var[prompt_resp_var_name]=matched_intent_key
	...
	async def _async_prompt_response(self,target_prompt,user_input='',var_snapshot=_A):
		...
		prompt_resp_var_name=prompt_config[_A5];prompt_body_template=self._clean_text(prompt_config[_T]);current_prompt_text=self._evaluate_f_string(prompt_body_template,current_var,user_input=user_input);model_provider=prompt_config.get(_Ai,_b).lower();model_name=prompt_config.get(_U);llm_response_text=_A;function_calls=_A;system_instruction=self._evaluate_f_string(self._agent_description or'',current_var)
		if model_provider==_b:
			content_parts=[]
			if prompt_config.get(_A6):
				file_uri_template=self._clean_text(prompt_config[_A6]);file_uri=self._evaluate_f_string(file_uri_template,current_var)
				if file_uri:content_parts.append(Part.from_uri(file_uri=file_uri,mime_type=prompt_config[_BA]))
			content_parts.append(current_prompt_text);gen_config=google_genai_types.GenerateContentConfig(system_instruction=system_instruction if system_instruction else _A,temperature=prompt_config[_A7],top_p=prompt_config[_V],top_k=prompt_config[_A8],candidate_count=prompt_config[_A9],max_output_tokens=prompt_config[_AA],stop_sequences=prompt_config.get('stop')or _A,safety_settings=prompt_config.get(_BB))
			if model_name in[_BC]and prompt_config.get(_AB)is not _A:gen_config.thinking_config=ThinkingConfig(thinking_budget=prompt_config[_AB])
			if prompt_config.get(_AC):gen_config.response_schema=prompt_config[_AC];gen_config.response_mime_type=prompt_config.get(_BD,_M)
			gen_config.tools=self._configure_tools_for_request(direct_tool_configs,processed_fd_dicts,model_name);llm_response_text,_,function_calls=await self._get_gemini_response_async(model_name,content_parts,gen_config)
		elif model_provider in[_f,_o]:
			messages=[]
			if system_instruction and model_provider==_f:messages.append({_S:_p,_Q:system_instruction})
			messages.append({_S:_Z,_Q:current_prompt_text});tools_for_provider=self._translate_tools_for_provider(model_provider,direct_tool_configs,processed_fd_dicts)
			if model_provider==_f:llm_response_text,function_calls=await self._get_openai_response_async(model_name,messages,prompt_config,tools_for_provider)
			elif model_provider==_o:anthropic_messages=[m for m in messages if m[_S]!=_p];llm_response_text,function_calls=await self._get_anthropic_response_async(model_name,system_instruction,anthropic_messages,prompt_config,tools_for_provider)
		...
		if function_calls:
			function_call=function_calls[0];function_name=function_call.get(_B);function_args=function_call.get(_I,{})
			if not function_name:logger.error(f"LLM async response contained a function call with no name: {function_call}");return prompt_resp_var_name,_BE
		...
		if llm_response_text:
			if prompt_config[_F]==_A0:
				...
				final_value_for_var=self._match_intent_from_text(raw_response,current_var.get(_Ag,[]),current_var.get(_Ah,_BF),current_var.get(_Af,[]))
	...
	def _match_intent_from_text(self,response_text,intent_contains_pairs,default_intent,intent_name_list):
		...
		for(intent_key,search_phrases_str)in intent_contains_pairs:
			...
			search_phrases=[phrase.strip()for phrase in str(search_phrases_str).split(',')]
	...
	def _get_mcp_details(self,mcp_name,current_var):
		...
		if mcp_config:
			mcp_config=self._evaluate_vars_in_structure(mcp_config,current_var);mcp_config[_K]=self._clean_text(mcp_config.get(_K,''));mcp_config[_W]=self._evaluate_f_string(mcp_config.get(_W,''),current_var);mcp_config[_q]=self._clean_text(mcp_config.get(_q,''));mcp_config['cwd']=self._clean_text(mcp_config.get('cwd',''))
			...
			if mcp_config.get(_r):mcp_config[_r]={k:self._clean_text(v)for(k,v)in mcp_config[_r].items()}
	...
	async def _run_prompt_action(self,user_input,intent_subprompt):
		...
		for(_indx,current_prompt_item)in sorted(intent_subprompt.items()):
			if _A2 in current_prompt_item:
				prompt_value=current_prompt_item[_A2]
			...
			elif _j in current_prompt_item:
				self.var,step_response=await self._run_mcp_action(self.var,current_prompt_item[_j],user_input)
	...
	async def _select_mcp_config(self,mcp_group_names,current_var,user_input,mcp_group_config):
		...
		select_mcp_declaration=FunctionDeclaration(name=B,description="Select the most appropriate MCP from the list based on the user's request.",parameters={_F:_k,_AE:{A:{_F:_AH,_K:'The name of the selected MCP.',_AG:[mcp.get(_B)for mcp in mcp_options]}},_AF:[A]});prompt_name=mcp_group_config.get(_BG,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
		if prompt_name:
			...
			prompt_model_provider=prompt_config.get('modelProvider',_b);prompt_model=_Aj
			if prompt_config.get(_U):prompt_model=prompt_config.get(_U)
			prompt_resp_var_name=prompt_config.get(_A5,'');prompt_body_template=self._clean_text(prompt_config.get(_T,''));mcp_group_prompt_text=self._evaluate_f_string(prompt_body_template,current_var,user_input=user_input)
	...
	async def _run_mcp_action(self,current_var,mcp_name,user_input=_A):
		...
		try:
			...
			prompt_name=mcp_config.get(_BG,'');prompt_config=_A;prompt_resp_var_name=_A;current_prompt_text=user_input or''
			if prompt_name:
				logger.info(f"MCP '{mcp_name}' is using prompt '{prompt_name}'.");prompt_config,_,_=self._get_prompt_details(prompt_name,current_var)
				if not prompt_config:error_msg=f"Error: Prompt '{prompt_name}' for MCP '{mcp_name}' not found.";logger.error(error_msg);return current_var,error_msg
				prompt_resp_var_name=prompt_config.get(_A5,'');prompt_body_template=self._clean_text(prompt_config.get(_T,''));current_prompt_text=self._evaluate_f_string(prompt_body_template,current_var,user_input=user_input)
			mcp_transport=mcp_config.get(F,'');mcp_url=mcp_config.get(_W,'');mcp_description=mcp_config.get(_K,'');mcp_command=mcp_config.get(_q,'');mcp_args=mcp_config.get(_I,{});mcp_cwd=mcp_config.get('cwd','');mcp_args=mcp_config.get(_I,[]);mcp_params=mcp_config.get('parametersJson',{});mcp_env=mcp_config.get(_r,{});mcp_name=mcp_config.get(_B,'default');session_context=_A;is_http_call=mcp_transport in[G,L]or not mcp_transport and mcp_url.startswith(('http://','https://'));is_stdio_call=mcp_transport=='stdio'or not mcp_transport and not mcp_url and mcp_config.get(_q)
			if is_http_call:
				eval_mcp_url=mcp_url;http_headers=self._evaluate_vars_in_structure(mcp_config.get(_Ak,{}),current_var);mcp_connector=mcp_config.get(_R,'')
				if mcp_connector:
				...
				client_config={M:{mcp_name:{F:client_transport,_W:eval_mcp_url}}}
				if http_headers:client_config[M][mcp_name]['headers']=http_headers
				session_context=Client(client_config,timeout=18e1)
			elif is_stdio_call:
				command=self._evaluate_f_string(self._clean_text(mcp_config.get(_q,'')),current_var)
				...
				mcp_env_from_config=self._evaluate_vars_in_structure(mcp_config.get(_r,{}),current_var);final_env=_A
			...
			if session_context:
				async with session_context as session:
					...
					if not gemini_client:return current_var,'Error: Gemini AI client is not configured.'
					...
					llm_tools=[]
					if mcp_tools:llm_tools.append(FunctionDeclaration(name=H,description='Calls a specific tool available on the MCP server.',parameters={_F:_k,_AE:{B:{_F:_AH,_AG:[t.name for t in mcp_tools],_K:'The name of the tool to call.'},A:{_F:_k,_K:'The arguments for the tool as a JSON object.'}},_AF:[B,A]}))
					if mcp_resources:llm_tools.append(FunctionDeclaration(name=I,description='Reads a specific resource (e.g., a file or data entry) from the MCP server.',parameters={_F:_k,_AE:{C:{_F:_AH,_AG:[r.uri for r in mcp_resources],_K:'The URI of the resource to read.'}},_AF:[C]}))
					if mcp_prompts:llm_tools.append(FunctionDeclaration(name=J,description='Gets a pre-defined, rendered prompt template from the MCP server.',parameters={_F:_k,_AE:{D:{_F:_AH,_AG:[p.name for p in mcp_prompts],_K:'The name of the prompt to get.'},E:{_F:_k,_K:'The arguments for rendering the prompt template.'}},_AF:[D]}))
					if not llm_tools:return current_var,f"Error: MCP '{mcp_name}' has no tools, resources, or prompts to use."
					logger.info(f"Asking LLM to choose a mcp activity: '{current_prompt_text[:100]}...'");model_provider=prompt_config.get(_Ai,_b)if prompt_config else _b;model_to_use=_Aj
					if prompt_config and prompt_config.get(_U):model_to_use=prompt_config.get(_U)
					...
					if model_provider==_b:config=google_genai_types.GenerateContentConfig(tools=[Tool(function_declarations=llm_tools)],temperature=mcp_config.get('initialTemperature',.5));selected_option,_,function_calls=await self._get_gemini_response_async(model_to_use,[selection_prompt_text],config);llm_response_text=selected_option
					...
					if hasattr(mcp_result,_s)and getattr(mcp_result,_s)is not _A:final_result_value=getattr(mcp_result,_s)
					elif hasattr(mcp_result,_C)and getattr(mcp_result,_C)is not _A:final_result_value=getattr(mcp_result,_C)
					mcp_result_str=json.dumps(final_result_value,indent=2)if isinstance(final_result_value,(dict,list))else str(final_result_value)if final_result_value is not _A else _A;mcp_result_variable=mcp_config.get(_X)
	...
	async def _call_mcp_tool(session,tool_name,args):
		...
		try:
			...
			if hasattr(result,_s)and result.result is not _A:return result.result
			return result
		except Exception as e:logger.error(f"Error calling MCP tool '{tool_name}': {e}");return{_g:f"Failed to call tool '{tool_name}': {e}"}
	@staticmethod
	async def _read_mcp_resource(session,resource_uri):
		...
		try:
			...
			return result
		except Exception as e:logger.error(f"Error reading MCP resource '{resource_uri}': {e}");return{_g:f"Failed to read resource '{resource_uri}': {e}"}
	@staticmethod
	async def _get_mcp_prompt(session,prompt_name,args):
		...
		try:
			...
			if hasattr(result,_C)and result.data:return result.data
			return result
		except Exception as e:logger.error(f"Error getting MCP prompt '{prompt_name}': {e}");return{_g:f"Failed to get prompt '{prompt_name}': {e}"}
	@staticmethod
	async def _execute_tool_calls(function_calls,session):
		...
		for func_call in function_calls:
			...
			try:
				...
				if hasattr(tool_result,'isError')and tool_result.isError:error_message=result_text or f"Tool '{tool_name}' failed without specific error message.";tool_result_payload={_g:error_message}
				else:tool_result_payload={_s:result_text}
			except Exception as e:error_message=f"Tool execution framework failed: {type(e).__name__}: {e}";tool_result_payload={_g:error_message}
	...
	async def _run_agent_loop(self,prompt,client,session,model_id,max_tool_turns,initial_temperature,tool_call_temperature,mcp_description=_A):
		...
		while has_function_calls and turn_count<max_tool_turns:
			turn_count+=1;function_calls_to_execute=[part.function_call for part in latest_content.parts if part.function_call];tool_response_parts=await self._execute_tool_calls(function_calls_to_execute,session);contents.append(google_genai_types.Content(role=_Al,parts=tool_response_parts));subsequent_config=google_genai_types.GenerateContentConfig(temperature=tool_call_temperature,**base_gen_config_dict);response=await client.aio.models.generate_content(model=model_id,contents=contents,config=subsequent_config)
	...
	async def _run_api_action(self,current_var,api_name):
		...
		headers={}
		if api_config.get(_Ak):
			try:headers.update(json.loads(api_config[_Ak]))
			except json.JSONDecodeError:logger.warning(f"Invalid JSON in API header for {api_name}")
		else:headers[_l]=_t
		if api_config.get(A):headers[_Y]=api_config[A]
		if api_config.get(_R):
			connector_config=self._get_connector_details(api_config[_R])
			if connector_config:
				try:
					token=await self._get_token_for_connector(connector_config)
					if token:headers[_L]=token
				except PinionAIAPIError as e:logger.warning(f"Could not get token for connector '{api_config[_R]}': {e}. Proceeding without token.")
			else:logger.warning(f"Connector '{api_config[_R]}' not found for API '{api_name}'.")
		method=api_config.get(_c,B).upper()
		try:
			...
			if api_config.get(_X):current_var[api_config[_X]]={_g:str(e)}
	...
	def _run_parser_action(self,current_var,parser_name):
		...
		source_var_name=parser_config.get(_BH)
		...
		try:
			if parser_type==_u:
				...
				method=parser_config.get(_c);delimiter=parser_config.get('delimiter')
	...
	def _run_merger_action(self,current_var,merger_name):
		...
		dest_var_name=merger_config.get(_BI)
		...
		try:
			if output_type==_u:current_var[dest_var_name]=json.loads(merged_output_str)
		...
		return current_var,_A
	def _run_script_action(self,current_var,script_name):
		if current_var.get(_H):logger.debug(f"Run Script | {script_name}")
		script_config=self._get_script_details(script_name)
		if not script_config:logger.warning(f"Script configuration '{script_name}' not found.");return current_var,_A
		script_type=script_config.get(_F);script_code=script_config.get(_T);result_var_name=script_config.get(_X)
		if not script_code:logger.warning(f"Script body is empty for script '{script_name}'.");return current_var,_A
		try:
			if script_type=='javascript':
				if not MiniRacer:raise PinionAIConfigurationError("The 'mini-racer' library is required to run JavaScript scripts. Please install it with 'pip install mini-racer'.")
				ctx=MiniRacer();context_json=json.dumps(current_var,default=_json_datetime_serializer);ctx.eval(f"var var = {context_json};");result=ctx.eval(script_code)
				if result_var_name and result is not _A:current_var[result_var_name]=result
			elif script_type=='python':
				local_script_vars=current_var.copy();logger.warning('Executing Python scripts from the database is a security risk and is currently disabled.')
				if result_var_name and result_var_name in local_script_vars:current_var[result_var_name]=local_script_vars[result_var_name]
		...
		return current_var,_A
	async def _run_rule_action(self,var,rule_name,user_input):
		if var.get(_H):logger.debug(f"Run Rule | {rule_name}")
		rule_config=self._get_rule_details(rule_name)
		if not rule_config:logger.warning(f"Rule configuration '{rule_name}' not found.");return var,_A
		expr=rule_config.get('condition')
		if not expr:logger.warning(f"Rule '{rule_name}' has no 'rule_condition' defined.");return var,_A
		result=evaluate_rule(expr,var);logger.info(f"Rule '{rule_name}' triggered and evaluated. Result: {result}")
		if(result_variable:=rule_config.get(_X)):
			var[result_variable]=result
			if var.get(_H):logger.debug(f"Rule '{rule_name}' result ({result}) stored in var['{result_variable}']")
		outcome_key=_A
		...
		if outcome_key:
			if(outcome_statement:=rule_config.get(outcome_key)):
				...
				if var.get(_H):logger.debug(f"Executing {outcome_type} '{outcome_key}' for rule '{rule_name}'")
				var,step_response=await self._execute_outcome(outcome_statement,outcome_type,user_input=user_input)
		return var,step_response
	def evaluate_rule(expr,var_data):
		'\n        Evaluates a rule expression string by first formatting it with variable data.\n\n        The expression string can contain f-string-like placeholders that reference\n        the \'var\' dictionary, which holds the provided variable data.\n\n        Example:\n            expr = "{var[\'value\']} > 10 and \'{var[\'status\']}\' == \'active\'"\n            var_data = {"value": 20, "status": "active"}\n            result = evaluate_rule(expr, var_data)  # result will be True\n\n        Args:\n            expr: The rule expression string to evaluate.\n            var_data: A dictionary of variables to be used in the expression.\n\n        Returns:\n            The result of the evaluated expression. Returns None if an error occurs\n            during formatting or evaluation.\n        '
		if not isinstance(expr,str):logger.warning(f"Rule expression is not a string: {expr}");return
		formatted_expr=''
		try:cleaned_expr=self._clean_text(expr);formatted_expr=self._evaluate_f_string(cleaned_expr,var_data);return eval(formatted_expr,{'__builtins__':{}},{})
	...
	async def _execute_outcome(self,outcome_statement,outcome_type,user_input):
		...
		if outcome_type=='Set a variable'and outcome_statement.get(A):
			set_config=outcome_statement.get(A);variable_to_set=set_config.get('variable');value_to_set=set_config.get('value')
			...
			else:logger.warning("Outcome 'set' statement missing 'value'.")
			return self.var,step_response
		elif outcome_type=='Set a response':
			response_template=self._clean_text(outcome_statement.get('response',''));step_response=self._evaluate_f_string(response_template,self.var)
	...
	async def _run_delivery_action(self,current_var,delivery_name):
		...
		to_address=self._evaluate_f_string(self._clean_line(delivery_config['to']),template_eval_vars);html_body_template=self._clean_text(delivery_config.get(_T,''));html_body_content=self._evaluate_f_string(html_body_template,template_eval_vars);plain_text_body=re.sub(B,' ',html_body_content);plain_text_body=re.sub(C,' ',plain_text_body).strip();from_address_template=delivery_config.get('from');from_address=self._evaluate_f_string(self._clean_line(from_address_template),template_eval_vars)if from_address_template else self._email_from_address;subject_template=delivery_config.get(_Am,'Your AI Delivery');subject=self._evaluate_f_string(self._clean_line(subject_template),template_eval_vars);delivery_type=delivery_config.get(_F);delivery_method=delivery_config.get(_c);final_response_message=_A;token=_A;headers={}
		if delivery_config.get(_R):
			connector_config=self._get_connector_details(delivery_config[_R])
			if connector_config:
				try:
					token=await self._get_token_for_connector(connector_config)
					if token:headers[_L]=token
				except PinionAIAPIError as e:logger.warning(f"Could not get token for connector '{delivery_config[_R]}': {e}. Proceeding without token.")
			else:logger.warning(f"Connector '{delivery_config[_R]}' not found for delivery '{delivery_name}'.")
		if delivery_type==A and delivery_method=='SendGrid':await self._send_email(plain_text_body,html_body_content,to_address,subject,from_address);logger.info(f"Email sent to {to_address} for delivery '{delivery_name}' via SendGrid.")
		elif delivery_type==A and delivery_method=='infobip':await self._infobip_send_email(plain_text_body,html_body_content,to_address,subject,from_address,token);logger.info(f"Email sent to {to_address} for delivery '{delivery_name}' via Infobip.")
		elif delivery_type==A and delivery_method=='Microsoft Graph API':await self._ms_graph_send_email(plain_text_body,html_body_content,to_address,subject,from_address,token)
		elif delivery_type=='sms'and delivery_method=='Twilio':sms_body_template=self._clean_text(delivery_config.get(_T,''));sms_intermediate_content=self._evaluate_f_string(sms_body_template,template_eval_vars);sms_to_send=re.sub(B,' ',sms_intermediate_content);sms_to_send=re.sub(C,' ',sms_to_send).strip();await self._twilio_sms_message(sms_to_send,to_address,from_address);logger.info(f"Twilio SMS sent to {to_address} for delivery '{delivery_name}'.")
	...
	async def _run_journey_action(self,current_var,action_details):
		"Handles a 'journey' type action, from either an intent or an action entity.";B='configuration';A='payment_description';action_key_var_name=action_details.get(_BJ);action_key=current_var.get(action_key_var_name)if action_key_var_name else _A
		...
		delivery_method=action_details.get(_An);language=action_details.get(_a);_,self.unique_id,self.phone_number=format_phone(self.phone_number);json_payload={_AY:action_key,_h:{_c:delivery_method,_d:self.phone_number},_AZ:{_Aa:self.unique_id,_d:self.phone_number},_E:{_Ab:self._session_id},_a:language}
		if current_var.get(_i):json_payload[_Ac]=current_var[_i]
		if action_details.get(_Ao)=='credit card payment'and current_var.get(A):json_payload[B]={'credit-card-payment':{'currency':current_var.get('payment_currency'),'lineItems':[{'title':current_var.get(A),'amount':current_var.get('payment_amount'),'quantity':current_var.get('payment_quantity')}]}}
		if action_details.get(_Ao)=='one time passcode':json_payload[B]={'random-code':{'code':'6'}}
		if action_details.get(_An)=='voice':json_payload[_h]={_c:delivery_method,_d:self.phone_number,'callOperator':'twilio'}
		...
		if error_msg:final_response=error_msg
			...
			if is_completed:
				response_msg_template=self._clean_text(action_details.get(_e))
	...
	def _run_transfer_action(self,current_var,action_details):
		...
		if current_var.get(_AP):
			...
			if current_var.get(_AQ):passkey=self._generate_password(6);current_var['transferPasskey']=passkey;sms_message=f"Your agent will validate themselves with the following shared passkey: {passkey}"
			response_msg_template=action_details.get(_e)
		...
		return current_var,final_response
	def _map_to_standard_action_details(self,data):'Maps fields from an intent or action entity to a standard action_details dictionary.';return{_B:data.get(_B),_Ad:data.get('actionType'),_BJ:data.get(_m),_An:data.get(_AV),_a:data.get(_a),_Ao:data.get('actionEvent'),_e:data.get('actionResponseMessage'),_Ae:data.get(_AW,{})}
	async def _run_action_action(self,current_var,action_name,final_response_from_caller=_A,user_input=_A):
		...
		action_details=self._map_to_standard_action_details(action_entity_data);action_type=action_details.get(_Ad,'general')
		...
		action_flow=action_details.get(_Ae,{});action_produced_final_response=_A
		if action_flow:
		...
		if action_details.get(_e):
			response_msg_template=self._clean_text(action_details[_e]);generated_response=self._evaluate_f_string(response_msg_template,current_var);current_var['last_action_response']=generated_response;action_produced_final_response=generated_response
	...
	async def _gcs_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not storage:raise ImportError(_BK)
		gcs_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_v)==_BL:sa_info=json.loads(conn_config.get(_w,'{}'));gcs_client=storage.Client.from_service_account_info(sa_info);logger.info(f"Using GCS service account from connector '{connector_name}'.")
		if not gcs_client:gcs_client=storage.Client();logger.info(_BM)
	...
	async def _s3_write(self,bucket_name,file_path,content_bytes,connector_name):
		...
		if not boto3:raise ImportError(_BN)
		s3_client=_A
		if connector_name:
			conn_config=self._get_connector_details(connector_name)
			if conn_config and conn_config.get(_v)=='aws_iam':s3_client=boto3.client(_x,aws_access_key_id=conn_config.get(_Ap),aws_secret_access_key=conn_config.get(_w));logger.info(f"Using AWS IAM credentials from connector '{connector_name}'.")
		if not s3_client:s3_client=boto3.client(_x);logger.info(_BO)
	...
	async def _run_file_action(self,current_var,file_name):
		...
		try:
			...
			if is_write_op:
				...
				if file_format==_u:content_bytes_to_write=json.dumps(content,indent=2).encode(_N)
			...
			if is_read_op and read_content_bytes is not _A:
				try:
					if file_format==_u:current_var[dest_var]=json.loads(read_content_bytes)
	...
	def _configure_tools_for_request(self,direct_tool_configs,processed_fd_dicts,model_name):
		...
		RESTRICTED_MODELS_FOR_MIXED_TOOLS={_Aj};has_functions=bool(function_declarations);has_search=any(t.google_search for t in final_tools_for_config);model_is_restricted=model_name in RESTRICTED_MODELS_FOR_MIXED_TOOLS
	...
	def _prepare_tools_for_genai_config(self,tools_list_from_config):
		'Prepares Tool objects for GenerateContentConfig, handling GoogleSearch strings.';A='function_declarations';prepared_tools=[]
		...
		for tool_item in tools_list_from_config:
			...
			elif isinstance(tool_item,str)and self.var[_z]and'RagStore'in tool_item:prepared_tools.append(Tool(retrieval=Retrieval(vertex_rag_store=VertexRagStore(rag_resources=[VertexRagStoreRagResource(rag_corpus=self.var[_z])],similarity_top_k=self.var[_AR],vector_distance_threshold=self.var[_AS]))))
			elif isinstance(tool_item,Tool):prepared_tools.append(tool_item)
			elif isinstance(tool_item,dict):
				try:
					if A in tool_item:
						declarations=[]
						for fd_dict in tool_item[A]:
							declaration=self._create_function_declaration_from_dict(fd_dict)
	...
	def _get_gemini_response_sync(self,model_name,contents,config):
		...
		try:
			if self._gcp_project_id and self._gcp_region:sync_genai_client=genai.Client(vertexai=_D,project=self._gcp_project_id,location=self._gcp_region);response=sync_genai_client.models.generate_content(contents=contents,model=model_name,config=config)
			else:
			...
			if response.candidates and len(response.candidates)>0:
				...
				if candidate.content and candidate.content.parts:
					fc_parts=[part.function_call for part in candidate.content.parts if hasattr(part,_BP)and part.function_call]
					if fc_parts:
						function_calls=[]
			...
			return response_text,list(response.candidates)if hasattr(response,_A9)else[],function_calls
	...
	async def _get_gemini_response_async(self,model_name,contents,config):
		...
		if not self._genai_client:return'Error: AI client is not configured.',[],_A
		try:
			...
			if not response_text and not function_calls and hasattr(response,_P)and response.text:response_text=response.text
			return response_text,list(response.candidates)if hasattr(response,_A9)else[],function_calls
		except Exception as e:logger.error(f"Error in async_get_gemini_response for model {model_name}: {e}",exc_info=_D);return f"There was an error generating this response: {e}",[],_A
	async def _get_openai_response_async(self,model_name,messages,prompt_config,tools=_A):
		...
		http_client=httpx.AsyncClient();client=AsyncOpenAI(api_key=self._openai_api_key);openai_params={_U:model_name,_n:messages,_y:prompt_config.get(_A7),_Aq:prompt_config.get(_AA),_V:prompt_config.get(_V),'frequency_penalty':prompt_config.get('frequencyPenalty'),'presence_penalty':prompt_config.get('presencePenalty')}
		if prompt_config.get(A):openai_params[A]=prompt_config.get(A)
		if tools:openai_params[_O]=tools;openai_params[_Ar]='auto'
	...
	async def _get_anthropic_response_async(self,model_name,system_prompt,messages,prompt_config,tools=_A):
		...
		client=AsyncAnthropic(api_key=self._anthropic_api_key);anthropic_params={_U:model_name,_p:system_prompt,_n:messages,_y:prompt_config.get(_A7),_Aq:prompt_config.get(_AA,4096),_V:prompt_config.get(_V),_A8:prompt_config.get(_A8)}
		if tools:anthropic_params[_O]=tools;anthropic_params[_Ar]={_F:'auto'}
	...
	async def _get_deepseek_response_async(self,model_name,messages,prompt_config,tools):
		...
		request_params={_U:model_name,_n:messages}
		if A in prompt_config:request_params[_Aq]=prompt_config[A]
		if _y in prompt_config:request_params[_y]=prompt_config[_y]
		if _V in prompt_config:request_params[_V]=prompt_config[_V]
		if tools:request_params[_O]=tools;request_params[_Ar]='auto'
		try:
	...
	async def _get_token_api(self,host,client_id,client_secret):
		url='/token';headers={_Y:_M};data={_As:_BQ,_At:client_id,_Au:client_secret}
		try:response=await self._http_session.post(url,headers=headers,json=data);response.raise_for_status();auth_response=response.json();return f"{auth_response[_BR]} {auth_response[_BS]}"
	...
	async def _start_session_api(self,host,agent_id_to_start,token):
		url=f"/agent/{agent_id_to_start}";headers={_Av:_AI,_l:_t,_L:token}
		try:
	...
	async def _start_version_api(self,host,agent_id_to_start,token,version_str):
		url=f"/version/{agent_id_to_start}/{version_str}";headers={_Av:_AI,_l:_t,_L:token}
		try:
			response=await self._http_session.get(url,headers=headers);response.raise_for_status();data=response.json();session_uid=data.get(_C,{}).get(_E,{}).get('uid')
			if session_uid:return session_uid,data
	...
	async def _get_session_api(self,host,session_uid,token):
		url=f"/session/{session_uid}";headers={_l:_t,_L:token,_Av:_AI}
		try:
	...
	async def _get_session_lastmodified_api(self,host,session_uid,token):
		url=f"/session/{session_uid}/lastmodified";headers={_l:_t,_L:token}
	...
	async def _post_session_api(self,host,token,session_uid,data_payload,transfer_req,transfer_acc):
		url=f"/session";headers={_Y:_M,'Content-Encoding':_AI,_L:token};full_payload={'sessionUid':session_uid,_C:data_payload,'transferRequested':transfer_req,'transferAccepted':transfer_acc}
		try:
	...
	async def _get_token_for_connector(self,connector_config):
		'Gets auth token for a given connector configuration.';url=connector_config.get(_W);client_id=connector_config.get(_Ap);client_secret=connector_config.get(_w);grant_type=connector_config.get(_v,_BQ);content_type=connector_config.get(_BT,_M);use_header_payload=connector_config.get('headerPayload',_G);scope=connector_config.get(_A3,_A)
		...
		if use_header_payload:client_keys=f"{client_id}:{client_secret}";client_keys_b64=base64.b64encode(client_keys.encode()).decode();headers[_L]=f"Basic {client_keys_b64}";headers[_Y]='application/x-www-form-urlencoded';data_to_send=f"grant_type={grant_type}"
		else:
			headers[_Y]=content_type
			if _u in content_type.lower():
				data_to_send={_As:grant_type,_At:client_id,_Au:client_secret}
				if scope:data_to_send[_A3]=scope
			elif'x-www-form-urlencoded'in content_type.lower():
				form_data=[(_As,grant_type),(_At,client_id),(_Au,client_secret)]
				if scope:form_data.append((_A3,scope))
		...
		try:
			...
			async with httpx.AsyncClient()as client:response=await client.post(url,headers=headers,content=payload);response.raise_for_status();auth_response=response.json();token_type=auth_response.get(_BR,'Bearer').capitalize();return f"{token_type} {auth_response[_BS]}"
	...
	async def _generic_api_post_put(self,current_var,api_config,headers,method):
		'Handles generic POST/PUT API calls.';url_template=self._clean_line(api_config.get(_W,''));url=self._evaluate_f_string(url_template,current_var)
		if not url:logger.error(f"URL could not be evaluated for API '{api_config.get(_B)}'. Template: {url_template}");return current_var,_A,_BU
		raw_body_template=api_config.get(_T,'');final_data_bytes=_A;content_type=headers.get(_Y,'').lower()
		try:
		...
		except json.JSONDecodeError as e:logger.error(f"Invalid JSON in API body template for '{api_config.get(_B)}': {raw_body_template}. Error: {e}");return current_var,_A,f"Invalid JSON in API body: {e}"
		except Exception as e:logger.error(f"Error processing body for API '{api_config.get(_B)}': {e}. Template: '{raw_body_template}'");return current_var,_A,f"Error processing API body: {e}"
		if final_data_bytes is _A:return current_var,_A,'Error: API body could not be prepared.'
		...
		try:
			async with httpx.AsyncClient()as client:
				...
				else:return current_var,_A,f"Unsupported method {method} in _generic_api_post_put"
			parsed_response:0
			try:parsed_response=response.json()
			except json.JSONDecodeError:parsed_response=response.text
			if api_config.get(_X):current_var[api_config[_X]]=parsed_response
			...
			if response.status_code>=400 or not parsed_response:
				...
				if api_config.get(_AJ):final_response_message=self._evaluate_f_string(api_config[_AJ],current_var)
			return current_var,response,parsed_response,final_response_message
		except httpx.RequestError as e:raise PinionAIAPIError(f"API {method} error for {url}",status_code=getattr(e.response,_BV,_A),details=str(e))from e
	async def _generic_api_get(self,current_var,api_config,headers):
		'Handles generic GET API calls.';url_template=self._clean_line(api_config.get(_W,''));url=self._evaluate_f_string(url_template,current_var)
		if not url:logger.error(f"URL could not be evaluated for API GET '{api_config.get(_B)}'. Template: {url_template}");return current_var,_A,_BU
		...
		try:
			...
			if response.status_code>=400:
				...
				if api_config.get(_AJ):final_response_message=self._evaluate_f_string(api_config[_AJ],current_var)
			return current_var,response,parsed_response,final_response_message
		except httpx.RequestError as e:raise PinionAIAPIError(f"API GET error for {url}",status_code=getattr(e.response,_BV,_A),details=str(e))from e
	...
	async def _journey_lookup(self,unique_id_val,current_var,user_input=_A):
		...
		headers={_AK:_M,_L:bearer_token};url=f"https://app.journeyid.io/api/system/customers/lookup?account_id={account_id}&unique_id={unique_id_val}"
		try:
	...
	async def _journey_send_pipeline(self,current_var,json_payload,delivery_method,user_input=_A):
		...
		if not bearer_token:return _A,_Aw
		post_url='https://app.journeyid.io/api/system/executions';headers={_l:_M,_Y:_M,_L:bearer_token}
		try:
			...
			if 200<=response.status_code<300:
				if delivery_method==_W:
					execution_url=response_data.get(_W);execution_id=response_data.get('id')
				...
				error_msg=response_data.get(_g,response.text)
	...
	async def _journey_execution_status_check(self,execution_id,current_var,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_Aw
		url=f"https://app.journeyid.io/api/system/executions/{execution_id}";headers={_AK:_M,_L:bearer_token}
		for attempt in range(max_retries):
			try:
				...
				if status_data.get(_BW):return _D,status_data.get('outcome',{}).get(_Ax,'Task completed successfully.')
	...
	async def _journey_session_pipeline_status_check(self,session_id_val,current_var,pipeline_key_to_check,max_retries=12,delay_seconds=5,user_input=_A):
		...
		if not bearer_token:return _G,_Aw
		url=f"https://app.journeyid.io/api/system/sessions/lookup?external_ref={session_id_val}";headers={_AK:_M,_L:bearer_token}
		for attempt in range(max_retries):
			try:
				async with httpx.AsyncClient()as client:response=await client.get(url,headers=headers,timeout=30)
				...
				for exec_item in executions:
					if exec_item.get(A,{}).get('key')==pipeline_key_to_check or exec_item.get(A,{}).get('id')==pipeline_key_to_check:
						if exec_item.get(_BW):return _D,exec_item.get('outcome',{}).get(_Ax,'Pipeline completed.')
						break
	...
	def _evaluate_rule_expression(self,expression_str,current_var,keywords_map=_A):
		'\n        Evaluates a rule expression. (Simplified placeholder)\n        Original rules.py uses eval().\n        ';logger.warning(f"Rule evaluation for expression '{expression_str}' is using a simplified placeholder. Full rules engine from rules.py not implemented here due to security concerns with eval().")
		try:
			if'True'in expression_str or'true'in expression_str:return _D
		except Exception as e:logger.error(f"Error evaluating rule expression '{expression_str}': {e}")
		return _G
	@staticmethod
	...
	async def _twilio_sms_message(self,sms_body,to_phone,from_phone_twilio=_A):
		...
		if not twilio_from_number:logger.error(_BX);return
	...
	async def _send_email(self,plain_text_body,html_body,to_email,subject,from_email_addr=_A):
		...
		try:sg=sendgrid.SendGridAPIClient(self._email_api_key);message=Mail(from_email=From(actual_from_email),to_emails=to_email,subject=subject,plain_text_content=plain_text_body,html_content=html_body);response=await asyncio.to_thread(sg.send,message);logger.info(f"Email sent to {to_email} via SendGrid. Status: {response.status_code}")
		except Exception as e:logger.error(f"Error sending email to {to_email} via SendGrid: {e}")
	async def _infobip_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A):
		'Sends an email using the Infobip API.'
		if not token:logger.error('Infobip token not provided. Cannot send email.');return
		infobip_url='https://g9egk8.api.infobip.com/email/4/messages';headers={_L:token,_Y:_M,_AK:_M};payload={_n:[{'sender':from_address,'destinations':[{'to':[{'destination':to_address}]}],_Q:{_Am:subject,_P:plain_text_body,'html':html_body_content}}]}
		try:
	...
	async def _ms_graph_send_email(self,plain_text_body,html_body_content,to_address,subject,from_address=_A,token=_A):
		...
		graph_api_endpoint='https://graph.microsoft.com/v1.0';url=f"{graph_api_endpoint}/users/{from_address}/sendMail";headers={_L:token,_Y:_M};email_payload={_Ax:{_Am:subject,_T:{_BT:'HTML',_Q:html_body_content},'toRecipients':[{'emailAddress':{'address':to_address}}]},'saveToSentItems':'true'}
		try:
			...
			for(key,value)in list(data_structure.items()):
				if isinstance(value,str):
					if A in value or B in value:
						try:eval_globals={'var':context_vars,C:context_vars,D:context_vars,_Ay:user_input};data_structure[key]=eval(f"f'''{value}'''",eval_globals,{})
			...
			for(i,item)in enumerate(data_structure):
				if isinstance(item,str):
					if A in item or B in item or user_input is not _A and'{user_input}'in item:
						try:eval_globals={'var':context_vars,C:context_vars,D:context_vars,_Ay:user_input};data_structure[i]=eval(f"f'''{item}'''",eval_globals,{})
	...
	def _evaluate_f_string(self,template_string,context_vars,user_input=_A):
		...
		try:eval_namespace={'var':context_vars,_Ay:user_input};return eval(f"f'''{template_string}'''",eval_namespace,{})
	...
	def _create_function_declaration_from_dict(self,declaration_dict):
		...
		name=declaration_dict.get(_B);description=declaration_dict.get(_K);parameters=declaration_dict.get(_AL)
	...
	def _translate_tools_for_provider(self,provider,direct_tool_configs,processed_fd_dicts):
		...
		translated_tools=[]
		if provider==_f:
			for fd in processed_fd_dicts:translated_tools.append({_F:_Al,_Al:{_B:fd.get(_B),_K:fd.get(_K),_AL:fd.get(_AL)}})
		elif provider==_o:
			for fd in processed_fd_dicts:translated_tools.append({_B:fd.get(_B),_K:fd.get(_K),'input_schema':fd.get(_AL)})
	...
	def add_message_to_history(self,role,content):'Adds a message to the internal chat history.';self.chat_messages.append({_S:role,_Q:content})
	@property
...
def twilio_sms_message(sms_body,to_phone,from_phone_twilio=_A):
	'Sends an SMS using Twilio, reading credentials from environment variables.';twilio_account_sid=os.environ.get(_AM);twilio_auth_token=os.environ.get(_AN)
	...
	_national_digits,_unique_id,to_phone_e164=format_phone(to_phone);twilio_from_number=from_phone_twilio or os.environ.get(_AO)
	if not twilio_from_number:logger.error(_BX);return

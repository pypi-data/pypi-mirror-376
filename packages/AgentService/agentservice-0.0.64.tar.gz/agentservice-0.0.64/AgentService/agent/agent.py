...
class Agent:
    ...
    async def __generate(self, chat: Chat) -> AgentResponse:
        ...
        for i in response.output:
            if i.__class__ in response_types:
                continue

            response_types.append(i.__class__)

        if len(response_types) != 1:
            raise TypeError(f"Got many response types in 1 output -> {response_types}")

        if isinstance(response_types[0], ResponseFunctionToolCall):
        ...
        elif isinstance(response_types[0], ResponseOutputMessage):
            content_types = []
            for i in response.output:
                if i.content.__class__ in content_types:
                    continue

                content_types.append(i.content.__class__)

            if len(content_types) != 1:
                self.log.warning(f"Got many content types in 1 output -> {content_types}")

            if not isinstance(content_types[0], ResponseOutputText):
                self.log.warning(f"Unsupported content type -> {content_types[0]}")

            return AgentResponse(
                type=AgentResponseType.answer,
                content="\n".join([
                    i.content.text
                    for i in response.output if isinstance(i.content, ResponseOutputText)
                ])
            )

        else:
            raise TypeError(f"Unsupported response type -> {response_types[0]}")

...
class iPgs:
    ...
    - Animated, live visualization in Jupyter/Colab
    ...
    Usage Example:
        for epoch_idx, batch_idx, batch in iPgs(loader, num_epochs=5, desc="Training"):
            # Unpack batch (works with single or tuple batches)
            bx, by = batch
            ...
    """
    ...
    def _display_html(self):
        ...
        <div style="width:600px; font-family:sans-serif; margin:10px 0;">
            ...
            <div style="height:16px; background:#eee; border-radius:999px; overflow:hidden; border:1px solid #ddd; margin-bottom:12px;">
                <div id="{self.outer_bar_id}" style="height:100%; width:0%; background:linear-gradient(90deg,#0072ff,#00c6ff); border-radius:999px; display:flex; align-items:center; justify-content:center; color:white; font-size:11px;">
                    <span>0.0%</span>
                </div>
            </div>
            <div style="margin-bottom:20px; display:flex; justify-content:flex-end;">
                <span id="{self.outer_eta_id}" style="font-size:12px; background:#f8f9fa; border:1px solid #ddd; border-radius:6px; padding:2px 8px; color:#333;">ETA: --:--</span>
            </div>
            ...
            <div style="height:12px; background:#f0f0f0; border-radius:999px; overflow:hidden; border:1px solid #ddd;">
                <div id="{self.inner_bar_id}" style="height:100%; width:0%; background:linear-gradient(90deg,#ff5722,#ff9800); border-radius:999px; display:flex; align-items:center; justify-content:center; color:white; font-size:10px;">
                    <span>0.0%</span>
                </div>
            </div>
            <div style="margin-top:4px; display:flex; justify-content:flex-end;">
                <span id="{self.inner_eta_id}" style="font-size:11px; background:#f8f9fa; border:1px solid #ddd; border-radius:6px; padding:2px 6px; color:#333;">ETA: --:--</span>
            </div>
        </div>
        """
        display(HTML(html))
    ...
    def __iter__(self):
        ...
        for epoch_idx in range(self.num_epochs):
            ...
            for batch_idx, batch in enumerate(self.loader, 1):
                ...
                # Update sample count in a framework-agnostic way
                try:
    ...
    def close(self):
        self._update_bar(
            ...
            outer_text="✅ Done..!", inner_text="✅ Batch Done..!",

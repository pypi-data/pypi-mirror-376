...
class WorkerClient:
    ...
    async def update_worker_info(self, info: dict):
        await self.websocket.send(compress_method('worker-info', info))
...
def get_all_worker_info():
    ...
    for client_id, worker_info in _workers.items():
        if worker_info.get('holds'):
            _holds: list[buelon.core.step.Job] = [s for uid in worker_info.get(client_id, {}).get('holds') for s in holds.get(uid, [])]
            _holds[:] = [s.to_json() for s in _holds]
            _holds: list[dict]
            worker_info['jobs'] = _holds

    try:
        return json.dumps(_workers)
    except:
        return {}

...
def _focused_window_for_actions():
    ...
    if not ax_app:
        # 如果当前前台是被排除的（例如 AIdo），尝试激活下一个未排除的可见应用
        if _activate_next_unexcluded_app():
            ax_app = _frontmost_ax_app_raw()
            if ax_app and not _is_excluded_current_app():
...
def _do_region(region: str) -> bool:
    ...
    return _tile_adaptive(win, screen, region)
...
def _tile_adaptive(win, screen, region: str) -> bool:
    """
    先按 AX 顶左坐标放置；若放置后读回位置/尺寸与目标不符，再尝试使用底左坐标系的 y 进行放置。
    """
    x_ax, y_ax, w, h = _target_frame_for_screen_region(screen, region)

    debug = os.environ.get("SPLITSCREEN_DEBUG") in ("1", "true", "True")
    if debug:
        try:
            vf = screen.visibleFrame()
            sf = screen.frame()
            print(f"[SplitScreen][DEBUG] region={region} vFrame=({vf.origin.x},{vf.origin.y},{vf.size.width},{vf.size.height}) sFrame=({sf.origin.x},{sf.origin.y},{sf.size.width},{sf.size.height}) unionMaxY={_union_max_y()}")
            print(f"[SplitScreen][DEBUG] target(AX top-left) x={x_ax}, y={y_ax}, w={w}, h={h}")
        except Exception as _:
            pass

    # 尝试 1：AX 顶左坐标
    if _tile(win, x_ax, y_ax, w, h):
        pos = _ax_get_point(win, AX_POSITION) or (0.0, 0.0)
        size = _ax_get_size(win, AX_SIZE) or (0.0, 0.0)
        if debug:
            print(f"[SplitScreen][DEBUG] placed(top-left try#1) pos={pos} size={size}")
        if _rect_close((pos[0], pos[1], size[0], size[1]), (x_ax, y_ax, w, h)):
            return True

    # 计算等价的底左坐标 y 并重试
    try:
        y_bottom_left = _union_max_y() - (y_ax + h)
    except Exception:
        y_bottom_left = y_ax

    if _tile(win, x_ax, y_bottom_left, w, h):
        pos = _ax_get_point(win, AX_POSITION) or (0.0, 0.0)
        size = _ax_get_size(win, AX_SIZE) or (0.0, 0.0)
        if debug:
            print(f"[SplitScreen][DEBUG] placed(bottom-left retry) pos={pos} size={size}")
        # 接受两种坐标下任一贴近
        if _rect_close((pos[0], pos[1], size[0], size[1]), (x_ax, y_ax, w, h)) or \
           _rect_close((pos[0], pos[1], size[0], size[1]), (x_ax, y_bottom_left, w, h)):
            return True

    # 最后兜底：通过 AppleScript 直接设置窗口位置与大小（使用顶左坐标）
    try:
        if _tile_via_applescript_top_left(x_ax, y_ax, w, h):
            return True
    except Exception:
        pass
    return False

def _tile_via_applescript_top_left(x: float, y: float, w: float, h: float) -> bool:
    try:
        script = f'''
        tell application "System Events"
            ...
            tell frontApp
                try
                    ...
                    set size of first window to {{{int(w)}, {int(h)}}}
                    return "OK"
                on error
                    return "NO"
                end try
...
def _pick_screen_for(win):
    ...
    size = _ax_get_size(win, AX_SIZE) or (0.0, 0.0)
    cx = pos[0] + size[0]/2.0
    cy_ax = pos[1] + size[1]/2.0
    # 将 AX 顶左坐标系的 Y 转换为 Cocoa 底左坐标系 Y
    try:
        cy_cocoa = _union_max_y() - cy_ax
    except Exception:
        cy_cocoa = cy_ax
    return _screen_for_point(cx, cy_cocoa)
...
def _is_excluded_windows(hwnd) -> bool:
    ...
    try:
        ...
        # 获取可执行文件完整路径与模块名，增强对 aido/AIdo/AIDO 的识别
        ...
        # 针对 aido 的额外兜底：常见可执行与标识
        aido_markers = [
            "aido", "aido.exe", "aidoapp", "com.silicon-geek.aido", "com.silicon-geek.aido.exe"
        ]
        for check_str in [class_name if 'class_name' in locals() else '', title if 'title' in locals() else '', exe_name, exe_stem, exe_low, process_name]:
            if check_str and any(marker in check_str for marker in aido_markers):
                return True

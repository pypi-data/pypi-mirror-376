...
def _target_frame_for_screen_region(screen, region: str) -> Tuple[float, float, float, float]:
    ...
    # 使用 macOS 全局屏幕坐标（原点在左下角）。
    # AXPosition/AXSize 在多数环境下与底部为原点的全局坐标一致。

    if region in ("left", "right"):
        halfW = vW / 2.0
        if region == "left":
            return (vX, vY, halfW, vH)
        else:
            return (vX + halfW, vY, vW - halfW, vH)

    elif region in ("top", "bottom"):
        halfH = vH / 2.0
        if region == "top":
            # 顶部一半：y 从中线开始向上
            return (vX, vY + halfH, vW, vH - halfH)
        else:
            # 底部一半：y 为可见区域底部
            return (vX, vY, vW, halfH)

    elif region in ("left_third", "middle_third", "right_third"):
        thirdW = vW / 3.0
        if region == "left_third":
            return (vX, vY, thirdW, vH)
        elif region == "middle_third":
            return (vX + thirdW, vY, thirdW, vH)
        else:  # right_third
            return (vX + 2 * thirdW, vY, vW - 2 * thirdW, vH)
    
    elif region in ("left_two_third", "right_two_third"):
        thirdW = vW / 3.0
        if region == "left_two_third":
            return (vX, vY, 2 * thirdW, vH)
        else:  # right_two_third
            return (vX + thirdW, vY, vW - thirdW, vH)
        ...
        if region == "top_left_quarter":
            return (vX, vY + halfH, halfW, vH - halfH)

        elif region == "top_right_quarter":
            return (vX + halfW, vY + halfH, vW - halfW, vH - halfH)

        elif region == "bottom_left_quarter":
            return (vX, vY, halfW, halfH)

        elif region == "bottom_right_quarter":
            return (vX + halfW, vY, vW - halfW, halfH)

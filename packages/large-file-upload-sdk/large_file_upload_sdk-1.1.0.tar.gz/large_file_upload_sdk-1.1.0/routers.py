...
from auth_middleware import get_current_user, check_dataset_permission, check_file_size_permission
...
async def check_file_status(
    ...
    try:
        # 检查数据集权限
        if not check_dataset_permission(current_user, request.dataset_name):
            return FileUploadResponse.error(
                code=403,
                message=f"无权访问数据集: {request.dataset_name}"
            )

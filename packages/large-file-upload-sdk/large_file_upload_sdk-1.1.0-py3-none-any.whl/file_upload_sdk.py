...
class FileUploadSDK:
    ...
    def __init__(self, 
        ...
        # 如果没有API Key但有user_id，自动申请API Key
    ...
    def _get_auth_headers(self) -> Dict[str, str]:
        """获取认证请求头"""
        if not self.api_key:
            raise Exception("未设置API Key，请先申请API Key或在初始化时提供")
...
def init_sdk(base_url: str = "http://localhost:8000", user_id: Optional[int] = None, api_key: Optional[str] = None, **kwargs):
    """初始化默认SDK实例"""
    global _default_sdk
    _default_sdk = FileUploadSDK(base_url, api_key=api_key, user_id=user_id, **kwargs)

...
def execute_sql_impl(
    ...
    """本地数据库查询实现 - 支持所有SQL操作"""
    try:
        # 预处理SQL语句（仅清理格式，不做安全检查）
        query = _preprocess_sql(query)
        
        # 添加LIMIT限制（仅对SELECT查询）
        if limit and limit > 0 and "SELECT" in query.upper() and "LIMIT" not in query.upper():
            query += f" LIMIT {limit}"
        ...
        with get_db_connection() as conn:
            ...
            else:  # 无结果集的操作（如CREATE/UPDATE/DELETE）

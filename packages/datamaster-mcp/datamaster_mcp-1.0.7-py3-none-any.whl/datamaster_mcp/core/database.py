...
def _import_to_local_database(df: pd.DataFrame, target_table: str, source_type: str, source_path: str, source_config: any) -> str:
    """导入数据到本地SQLite数据库"""
    try:
        # 确保数据库已初始化
        init_database()
        
        # 检查表是否存在，如果存在则添加时间戳后缀
        original_table_name = target_table
        counter = 1
        while _table_exists(target_table):
            target_table = f"{original_table_name}_{counter}"
            counter += 1
            
        # 使用pandas的to_sql导入数据
        with get_db_connection() as conn:
            df.to_sql(target_table, conn, if_exists='fail', index=False)
            
            # 获取导入后的统计信息
            cursor = conn.execute(f"SELECT COUNT(*) as row_count FROM {target_table}")
            ...
            # 记录元数据
            conn.execute("""
                INSERT OR REPLACE INTO _metadata (table_name, created_at, source_type, source_path, row_count)
                VALUES (?, ?, ?, ?, ?)
            """, (target_table, datetime.now().isoformat(), source_type, source_path, row_count))
            
            conn.commit()
        
        response_data = {
            "status": "success",
            "message": f"{source_type.upper()}文件已导入到本地SQLite数据库",
            "data": {
                ...
                "connection_type": "本地SQLite导入",
                "data_location": f"本地数据库 ({DB_PATH})",
                "usage_note": f"使用execute_sql(query='SELECT * FROM {target_table}')查询此表数据"
            },
            "metadata": {
        ...
        return f"✅ {source_type.upper()}文件已导入到本地SQLite数据库\n\n{json.dumps(response_data, indent=2, ensure_ascii=False)}"
        
    except Exception as e:
        logger.error(f"本地数据库导入失败: {e}")
        raise
...
# ================================
# DDL 操作函数 - 支持CREATE/ALTER/DROP等
# ================================

def execute_ddl(query: str, params: dict = None, data_source: str = None) -> str:
    """
    执行DDL操作（CREATE, ALTER, DROP, TRUNCATE等）
    
    Args:
        query: DDL SQL语句
        params: 参数化查询的参数（可选）
        data_source: 数据源名称，默认为本地SQLite
        
    Returns:
        JSON格式的执行结果
    """
    try:
        logger.info(f"执行DDL操作: {query[:100]}...")
        ...
        # 检查是否为DDL语句
        is_ddl = any(query_upper.startswith(keyword) for keyword in allowed_keywords)
        if not is_ddl:
            return json.dumps({
                "status": "error",
                "message": "只允许执行DDL操作（CREATE, ALTER, DROP, TRUNCATE等）",
                "query": query,
                "allowed_operations": allowed_keywords
            }, ensure_ascii=False)
        ...
        try:
            if data_source:
                # 使用外部数据库
                if data_source not in database_manager.active_connections:
                    return json.dumps({
                        "status": "error",
                        "message": f"数据源 '{data_source}' 未连接",
                        "available_sources": list(database_manager.active_connections.keys())
                    }, ensure_ascii=False)
                
                conn_info = database_manager.active_connections[data_source]
                conn = conn_info['connection']
                
                # 根据数据库类型创建游标
                if conn_info['type'] in ['mysql', 'postgresql']:
                    cursor = conn.cursor()
                elif conn_info['type'] == 'mongodb':
                    # MongoDB DDL通过驱动执行
                    return json.dumps({
                        "status": "error",
                        "message": "MongoDB暂不支持DDL操作",
            ...
            if query_upper.startswith('CREATE TABLE'):
                ...
                if table_name:
                    try:
                        if data_source:
                            # 外部数据库获取表信息
                            table_info = database_manager.get_table_info(data_source, table_name)
                        else:
                            # 本地SQLite获取表信息
            ...
            if cursor:
                cursor.close()
            if conn and not data_source:  # 本地连接才关闭
                conn.close()
                
    except Exception as e:
        error_msg = str(e)
        logger.error(f"DDL操作失败: {error_msg}")
        
        return json.dumps({
            "status": "error",
            "message": f"DDL操作失败: {error_msg}",
            "query": query,

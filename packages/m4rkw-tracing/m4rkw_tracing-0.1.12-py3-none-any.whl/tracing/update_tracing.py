...
class UpdateTracing:
    ...
    def main(self) -> None:
        ...
        lm = Tracing(f"{self.hostname}_{username}_update_tracing")
        ...
        try:
            ...
            for venv in config['users'][username]:
                new_version = self.update(venv['path'], force)
    ...
    def update(self, path: str, force: bool = False) -> str:
        lpwd = os.getcwd()
        ...
        resp = os.popen("uv pip list |grep m4rkw-tracing").read().rstrip().split()
        ...
        if len(current_version) >0:
            if force:
                os.system("uv pip uninstall -y m4rkw-tracing 1>/dev/null 2>/dev/null")
        ...
        if len(resp) <2:
            raise Exception("failed to find m4rkw-tracing in venv {path} after upgrade")
        else:
            new_version = resp[1]

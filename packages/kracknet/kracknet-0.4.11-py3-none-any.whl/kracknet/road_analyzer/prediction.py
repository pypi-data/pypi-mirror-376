...
class Prediction (prediction.BasePrediction):
  ...
  def __init__ (self, img_path, response):
    ...
    if 'clientMeta' in self.result:
      self.transform_factors = self.result ['clientMeta']['transformFactors']
    elif 'transformFactors' in self.result:
      logging.warning ('`transformFactors` is deprecated, upgrade kracknet-client')

...
from soika_memory.configs.prompts import MEMORY_ANSWER_PROMPT
from soika_memory.memory.telemetry import capture_client_event, capture_event
...
class memory:
    def __init__(
        ...
        if api_key:
            self.memory_client = MemoryClient(api_key, host)
        else:
            self.memory_client = Memory.from_config(config) if config else Memory()

        self.chat = Chat(self.memory_client)


class Chat:
    def __init__(self, memory_client):
        self.completions = Completions(memory_client)
...
class Completions:
    ...
    def create(
        ...
        if not any([user_id, agent_id, conv_id]):
            raise ValueError("One of user_id, agent_id, conv_id must be provided")
        ...
        if prepared_messages[-1]["role"] == "user":
            self._async_add_to_memory(messages, user_id, agent_id, conv_id, metadata, filters)
            relevant_memories = self._fetch_relevant_memories(messages, user_id, agent_id, conv_id, filters, limit)
        ...
        if isinstance(self.memory_client, Memory):
            capture_event("memory.chat.create", self.memory_client)
        else:
            capture_client_event("memory.chat.create", self.memory_client)
        return response
    ...
    def _async_add_to_memory(self, messages, user_id, agent_id, conv_id, metadata, filters):
        def add_task():
    ...
    def _fetch_relevant_memories(self, messages, user_id, agent_id, conv_id, filters, limit):
    ...
    def _format_query_with_memories(self, messages, relevant_memories):
        # Check if self.memory_client is an instance of Memory or MemoryClient

        entities = []
        if isinstance(self.memory_client, memory.memory.main.Memory):
        ...
        elif isinstance(self.memory_client, memory.client.main.MemoryClient):

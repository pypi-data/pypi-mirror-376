...
from soika_memory.memory.setup import get_user_id, setup_config
from soika_memory.memory.telemetry import capture_client_event
from soika_memory.client.project import Project, AsyncProject
from soika_memory.client.utils import api_error_handler
...
class MemoryClient:
    ...
    memories using the memory API.

    Attributes:
        api_key (str): The API key for authenticating with the memory API.
        host (str): The base URL for the memory API.
    ...
    def __init__(
        ...
        Args:
            api_key: The API key for authenticating with the memory API. If not
                     provided, it will attempt to use the memory_API_KEY
                     environment variable.
            host: The base URL for the memory API. Defaults to
                  "https://api.memory.ai".
        ...
        self.host = host or "https://api.memory.ai"
        self.org_id = org_id
        self.project_id = project_id
        self.user_id = get_user_id()

        if not self.api_key:
            raise ValueError("memory API Key not provided. Please provide an API Key.")
        ...
        if kwargs.get("output_format") != "v1.1":
            ...
            warnings.warn(
                (
                    ...
                    "https://docs.memory.ai/platform/quickstart#4-1-create-memories"
                ),
                DeprecationWarning,
                stacklevel=2,
            )
    ...
    def delete_users(
        ...
        Args:
            ...
            conv_id: Optional run ID to delete specific run
        ...
        if user_id:
            ...
            to_delete = [{"type": "run", "name": conv_id}]
        else:
            entities = self.users()
        ...
        capture_client_event(
            ...
            self,
            {
                ...
                "app_id": app_id,
                "conv_id": conv_id,
                "space_id": space_id,  # Add to telemetry
                "sync_type": "sync",
            },
        )
        return {
            "message": "Entity deleted successfully."
            if (user_id or agent_id or app_id or conv_id)
    ...
    def create_memory_export(self, schema: str, **kwargs) -> Dict[str, Any]:
        ...
        Args:
            ...
            **kwargs: Optional filters like user_id, conv_id, etc.
    ...
    def chat(self):
        """Start a chat with the memory AI. (Not implemented)
...
class AsyncMemoryClient:
    """Asynchronous client for interacting with the memory API.

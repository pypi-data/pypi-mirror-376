...
class NewestTracer(tracer.Tracer):
    ...
    def __iter__(self) -> tracer.Tracer:
        ...
        self.__seek(self.consumer, self.amount_per_partition)
        return self
    ...
    @staticmethod
    def __seek(consumer: Consumer, amount: int):
        assignment = consumer.assignment()
        offset_by_partition = {}
        for partition in assignment:
            (low, high) = consumer.get_watermark_offsets(partition)
            offset_by_partition[partition.partition] = max(high - amount, low)
        for partition in assignment:

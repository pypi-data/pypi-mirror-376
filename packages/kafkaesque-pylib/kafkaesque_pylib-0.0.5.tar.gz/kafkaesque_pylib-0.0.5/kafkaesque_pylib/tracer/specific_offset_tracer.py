...
class SpecificOffsetTracer(tracer.Tracer):
    ...
    def __iter__(self) -> tracer.Tracer:
        ...
        self.__seek(self.consumer, self.offset)
        return self
    ...
    @staticmethod
    def __seek(consumer: Consumer, offset: int):
        assignment = consumer.assignment()
        offset_by_partition = {}
        for partition in assignment:
            (low, high) = consumer.get_watermark_offsets(partition)
            offset_by_partition[partition.partition] = max(offset, low)
        for partition in assignment:

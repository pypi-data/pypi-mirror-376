...
SELECT sources.source_id, aggr_emis.substance_id, aggr_emis.raster, source_name, ac1, ac2, ac3,
  sources.timevar_id, substances.slug as substance, aggr_emis.emis
FROM
(
  SELECT substance_id, source_id, raster, SUM(emis) as emis
  FROM
  (
    SELECT
      sources.source_id,
    ...
    SELECT
      sources.source_id,
      ef_subst.substance_id,
      act.raster,
      act.rate * ef_subst.factor as emis
      FROM edb_gridsourceactivity as act
   ...
   GROUP BY substance_id, source_id, raster
) aggr_emis

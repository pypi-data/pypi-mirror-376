...
def to_datetime(
    ...
    def get_tz(default_tz_str: str):
        if isinstance(default_tz_str, str):
            if default_tz_str.upper() == "UTC":
                default_tz_str = timezone.utc
            else:
                offset = re.match(r"UTC([+-]\d{1,2})(:\d{2})?$", default_tz_str.upper())
                if offset:
                    hours = int(offset.group(1))
                    default_tz_str = timezone(timedelta(hours=hours))
        return default_tz_str

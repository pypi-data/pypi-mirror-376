...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `mkdocs_coverage`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in mkdocs_coverage.__all__ or not hasattr(mkdocs_coverage, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in mkdocs_coverage.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("mkdocs_coverage")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "mkdocs_coverage" or item.name.startswith("mkdocs_coverage."))
        ):

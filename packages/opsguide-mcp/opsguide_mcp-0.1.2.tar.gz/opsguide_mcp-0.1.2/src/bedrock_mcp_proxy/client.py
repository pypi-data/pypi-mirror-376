...
class BedrockMCPClient:
    ...
    async def connect(self):
        ...
        try:
            # Create the HTTP client context manager
            self.http_context = streamablehttp_client(
                self.mcp_url,
                self.headers,
            ...
            self.read_stream, self.write_stream, self.close_fn = await self.http_context.__aenter__()
    ...
    async def disconnect(self):
        ...
        if hasattr(self, 'http_context') and self.http_context:
            try:

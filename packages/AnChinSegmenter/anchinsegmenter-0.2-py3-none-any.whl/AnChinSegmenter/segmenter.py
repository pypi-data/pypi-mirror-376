...
def test(args, input_text, output_path):
...
def download_model():
    ...
    # ✅ 如果模型文件已经存在，直接返回路径，不会重新下载
    if os.path.exists(model_path):
        return model_path

    # Google Drive 文件夹 ID
    file_id = "1zFK30h6PQYRDDZ2uEScLy0l5VoC7jXHU"
    gdown.download_folder(id=file_id, output=model_dir, quiet=False, use_cookies=False)
...
def main(input_path, output_path):
    ...
    if args.do_train:
        ...
        test(args, input_middle_text,output_path)

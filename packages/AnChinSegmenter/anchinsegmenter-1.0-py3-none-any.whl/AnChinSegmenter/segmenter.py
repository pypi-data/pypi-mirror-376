...
import logging
import os, sys
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "pytorch_pretrained_bert"))
import random
import time
import numpy as np
import torch
...
from . import pytorch_pretrained_bert,smseg_helper, smseg_eval, smseg_model_curriculum, seg_eval, evaluation
...
def get_model_path():
    """
    获取模型路径，如果不存在则自动下载。
    模型会存放在用户缓存目录下：~/.anchinsegmenter/model/model.pt
    """
    # 用户缓存目录
    model_dir = Path(os.path.expanduser("~")) / ".anchinsegmenter" / "model"
    ...
    print("begin download from Hugging Face Hub...")
    # 下载模型到缓存目录
    model_file = hf_hub_download(
        repo_id="XuemeiTang/AnChinSegmenter-model",  # 你的 Hugging Face 仓库
        filename="model.pt",                         # 文件名
        use_auth_token=False                          # 公共模型无需 token
    )

    # 移动到指定缓存目录
    model_path.write_bytes(Path(model_file).read_bytes())

    print(f"model download finished：{model_path}")
    return model_path
...
def test(args, input_text, output_path, model_path):
 ...
 #    optimizer.load_state_dict(checkpoint['optimizer'])
    ...
    # model.to(device)
    # try:
    sys.path.append(os.path.join(os.path.dirname(__file__), 'pytorch_pretrained_bert'))
    
    seg_model_checkpoint = torch.load(model_path, map_location=device)
    # except ModuleNotFoundError as e:
 #        print(f"Error: {e}")
    seg_model = WMSeg.from_spec(seg_model_checkpoint['spec'], seg_model_checkpoint['state_dict'], args)
    seg_model.to(device)
    eval_examples = seg_model.load_data(input_text)
    convert_examples_to_features = seg_model.convert_examples_to_features
    feature2input = seg_model.feature2input
    num_labels = seg_model.num_labels
...
def main(input_path, output_path):
    ...
    model_path = get_model_path()  # ✅ 自动下载或直接使用缓存
    ...
    parser.add_argument("--eval_model", default="./model/model.pt", type=str,
                        help="")
    ...
    parser.add_argument("--use_dict", type=bool, default=True, help="use dictionary or not")
    ...
    if args.do_train:
        ...
        test(args, input_middle_text,output_path, model_path)

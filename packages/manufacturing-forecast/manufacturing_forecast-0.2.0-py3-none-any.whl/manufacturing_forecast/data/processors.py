...
class TimeSeriesProcessor:
    ...
    def create_lag_lead_matrices(
        ...
        # Future features need lag + lead window size to cover the entire prediction horizon
        future_window_size = lag + lead
        future_windows = []
        for feature in future:
            if len(feature) >= future_window_size:
                window = sliding_window_view(feature, window_shape=future_window_size)
                future_windows.append(window)

        # Check if target has enough data for both lag and lead
        if len(target) < lag + lead:
            return np.array([]), np.array([])

        # Create target lag window for features (X matrix)
        target_lag_window = sliding_window_view(target, window_shape=lag)

        # Create target lead window for predictions (Y matrix)
        # We need to shift the target to align with the lag features
        target_shifted = target[lag:]  # Start from lag position
        if len(target_shifted) >= lead:
            target_lead_window = sliding_window_view(target_shifted, window_shape=lead)
        else:
            return np.array([]), np.array([])

        # Align all windows
        min_samples = min(
            len(past_windows[0]) if past_windows else float("inf"),
            len(future_windows[0]) if future_windows else float("inf"),
            len(target_lag_window),
            len(target_lead_window),
        )

        if min_samples <= 0 or min_samples == float("inf"):
            return np.array([]), np.array([])

        # Combine features for X matrix
        X_list = []
        ...
        X_list.append(target_lag_window[:min_samples])
        ...
        # Prepare target predictions (Y matrix)
        Y = target_lead_window[:min_samples]
        ...
        if len(status) > 0 and len(status) >= lag and min_samples > 0:
            ...
            valid_indices = np.where(np.sum(status_window, 1) != 0)[0]

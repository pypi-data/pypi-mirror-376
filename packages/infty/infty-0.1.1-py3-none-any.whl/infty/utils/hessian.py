...
import numpy as np
import tqdm
from .running import group_product, group_add, normalization, get_params_grad, hessian_vector_product, orthnormal
...
class hessian():
    ...
    def __init__(self, model, create_loss_fn, data=None, dataloader=None, device=None):
        """
        ...
        if not self.full_dataset:
            ...
            loss_fn = self.create_loss_fn(self.inputs, self.targets)
    ...
    def dataloader_hv_product(self, v):
        ...
        for _, inputs, targets in self.data:
            self.model.zero_grad()
            tmp_num_data = inputs.size(0)

            loss_fn = self.create_loss_fn(inputs.to(self.device), targets.to(self.device))
            logits, loss_list = loss_fn()
            loss = sum(loss_list)
    ...
    def density(self, iter=100, n_v=1):
        ...
        for k in range(n_v):
            ...
            # DEPRECATED: torch.eig() is deprecated, use torch.linalg.eigh() instead
            # a_, b_ = torch.eig(T, True)
            # eigen_list = a_
            ...
            # print(f"weight_list: {weight_list.shape}")

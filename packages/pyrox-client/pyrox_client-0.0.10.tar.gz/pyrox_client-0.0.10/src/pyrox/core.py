...
This client intelligently routes requests:
- Bulk data queries → S3 direct access (fast, efficient)
- Metadata & discovery → API (cached, convenient)
- Aggregations → API (pre-computed)
"""
...
DEFAULT_S3_BUCKET = "hyrox-data"  # Your S3 bucket
...
class PyroxClient:
    """
    Hybrid Client that intelligently routes requests between API and Amazon S3
    """
    def __init__(
            self,
    ...
    def _get_manifest(self, force_refresh: bool = False) -> pd.DataFrame:
        ...
        if not force_refresh and self.cache.is_fresh(cache_key, ttl_seconds=1800):  # 30min TTL
    ...
    def _get_race_from_s3(
        ...
        # Build S3 path based on your partitioning scheme
        # Adjust this to match your actual S3 structure
        s3_path = f"s3://{self.s3_bucket}/seasons/{season}/races/location={location}/"
    ...
    def _get_race_from_api(
        ...
        """Get race data from API (fallback method)"""
        params = {}
        if gender:
    ...
    def get_race(
        ...
        """
        Get race data, intelligently choosing between S3 and API
        """
        # Create cache key
    ...
    def get_season(
        ...
        """
        Get all races for a season (parallelized)
        """

...
if not func_dir.exists():
    func_dir = out / 'data' / 'betterworldedit' / 'functions'

print('== Generated files ==')
print('\n'.join(sorted(str(p.name) for p in func_dir.glob('*.mcfunction'))))

# Show specific files
for name in ['_compute_bounds.mcfunction', 'set_stone.mcfunction']:
    p = func_dir / name
    if p.exists():
        print(f"\n== {name} ==\n" + p.read_text())

# Show while helpers
helpers = sorted(func_dir.glob('set_stone__while_*.mcfunction'))
for hp in helpers:
    print(f"\n== {hp.name} ==\n" + hp.read_text())

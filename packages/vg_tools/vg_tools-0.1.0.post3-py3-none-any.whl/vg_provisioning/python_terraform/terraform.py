...
from vg_provisioning.python_terraform.tfstate import Tfstate
    ...
    def cmd(
        ...
        **kwargs,
        ...
        if ret_code and raise_on_error:
            msg_skip = "Note: You didn't use the -out option to save this plan, so Terraform can't"
            if out and msg_skip in out:
                logger.info(f"Terraform plan was not saved! just a spec plan \n {out}")
            else:

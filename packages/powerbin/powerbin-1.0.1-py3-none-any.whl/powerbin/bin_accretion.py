...
def voronoi_tessellation(xy, xybin):
    """Computes Voronoi Tessellation of pixels (x, y) with generators (xbin, ybin)"""
...
def accretion(xy, target_capacity, fun_capacity, verbose, args):
    ...
    for ind in range(1, n + 1):
        ...
        while True:
            ...
            if np.any(ok):
                ...
                roundness = calculate_roundness(xy[next_bin])
            ...
            if (disconnected or roundness > 0.8  or capacity_old > capacity or
                abs(capacity - target_capacity) > abs(capacity_old - target_capacity)):
    ...
    if verbose >= 3:
        ...
        plt.clf()
        display_pixels(xy[:, 0], xy[:, 1], rnd[bin_num], cmap='Set3')
        plt.plot(xy[bad, 0], xy[bad, 1], 'k+')

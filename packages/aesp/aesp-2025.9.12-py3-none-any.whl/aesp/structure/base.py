...
from ..operator.mutation.permutation import PermutationMutation
import numpy as np 
import copy
...
class StructBase(Atoms):
    ...
    def operator_adaptive_variant(cls):
        ...
        return Variant("type", inputs_list, default_tag='adjustment', doc=doc_type)
    
    @staticmethod
    def distribution_config():
        doc_use_recent_pop = "Use of information from recent generations"
        return [
            Argument("use_recent_gen", int, optional=True, default=2, doc=doc_use_recent_pop)
        ]

    @staticmethod
    def adjustment_config():
        doc_use_recent_pop = "Use of information from recent generations"
        doc_factor = "Scaling factor that control the magnitude of change in adaptive (0-1)"
        return [
    ...
    def get_spg_info(self, angle_tolerance=5, symprec=0.1):
        try:
            # self.standardize_cell(symprec=symprec)
            analyzer = SpacegroupAnalyzer(AseAtomsAdaptor().get_structure(self), angle_tolerance=angle_tolerance, symprec=symprec)

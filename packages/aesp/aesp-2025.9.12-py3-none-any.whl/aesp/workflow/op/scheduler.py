...
from ...func.database import DataConnection
from pathlib import Path
import shutil
from ...func.population import Population
import copy
...
import numpy as np
import sys
from ...structure import struc_type
...
# 去除重复的结构
def add_struc(struc, candis):
    for candi in candis:
        if struc.info['spg_num'] == candi.info['key_value_pairs']['spg_num'] and struc.looks_like(candi, simi_thres=0.999):
            return None
    return struc
...
class SchedulerStage(OP):
    ...
    def execute(
        ...
        for file_path in folders:
            struc = get_struc(file_path)
            l = [struc.info['fitness']*0.999, struc.info['fitness']*1.001]
            candis = dc.get_candidates(selection="{}<fitness<{}".format(min(l), max(l)))
            result = pool.apply_async(func=add_struc, args=(struc, candis))
            if result.get():
        ...
        candi_list = []
        if results_list:  
            candi_list.append(results_list[0]) 
        for atoms in results_list[0:]:
            need_add = True
            for candi in candi_list:
                if atoms.looks_like(candi, simi_thres=0.999):
                    need_add = False
                    break
            if need_add:
                candi_list.append(atoms)

        # 添加到数据库
        for candi in candi_list:
            dc.add_relaxed_stage(candi)
...
class SchedulerGen(OP):
    ...
    def execute(
        ...
        if num < 2:
            raise ValueError(f"第{op_in['generation']}代候选结构小于2,结果可能已经收敛!")
            ...
            if len(keys) == 2:
                ...
                # 按照能量增减
                    ...
                    if tem_e_dict['continuous_mut'] - np.mean(list(tem_e_dict.values())) > 0:
                        chg_num = 4-opt_params_config['operator']['mutation']["continuous_mut_factor"]
                    else:
                        chg_num = opt_params_config['operator']['mutation']["continuous_mut_factor"]-0                

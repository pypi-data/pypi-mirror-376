...
from ..workflow.superop.csp_block import STCSPBlock, DPCSPBlock
from ..workflow.superop.calc_block import FPRelaxBlock, DPCalcBlock, STCalcBlock
from ..workflow.superop.csp_so import DPCSP, STCSP
...
from ..calculator import calc_styles
from ..func.scheduler import STScheduler, DPScheduler
from ..constant import stand_csp_mode, dp_csp_mode
...
def submit_csp(
    ...
    if mode == "std-sp":
        ...
        aesp_step = workflow_stcsp(wf_config)
    elif mode == "al-sp":
        logging.info('Structure prediction (AL+MLP)')
        aesp_step = workflow_dpcsp(wf_config)
    else:
        return
...
def workflow_stcsp(wf_config):
    ...
    for config in calc_config:
        ...
        calc_inputs_config = config.pop("inputs_config")
        calc_style = config["type"]
        if calc_style in ["matgl", 'dpmd', 'emt']:

...
from .resubmit import resubmit_csp
from .submit import submit_csp
from .download import download
from .watch import watch
from .status import status
from .analysis import analysis
from ..utils.dpgen2 import workflow_subcommands
from ..func.args_setup import args_setup
from ..func.entrypoint_func import *
...
def main():
    ...
    if args.command == "gui":
        ...
        os.system('python {} {}'.format(pa, args.destination[0]))
        return
    ...
    if args.command == "submit":
        ...
        logging.info(f"The structure prediction is about to restart ({args.id})")
        resubmit_csp(config, mode, step_lists, args.stepid)
    elif args.command == "status":
        logging.info(f"The current status is being queried ({args.id})")
        status(wf, mode, step_dict, args.step, args.destination, step_list[-1])
    elif args.command == "watch":
        logging.info(f"Viewing the details of a file")

...
def calc_stage_config(inputs, run):
    doc_inputs_config = "The configuration for preparing inputs."
    doc_run_config = "The configuration for running tasks."
    doc_task_max = "Maximum number of calculation tasks for each iteration."
    doc_pstress = "The external pressure in GPa" 
    return [
        Argument(
            "inputs_config",
            dict,
...
def calc_stages_variant(key=None):
    doc_type = "The type of calculation stage."
    doc_gen_num = "the type of the calculation stage."
    ...
    return Variant("type", inputs_list, doc=doc_type)

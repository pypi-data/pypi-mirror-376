...
class PlutoWrapper:
    ...
    def register_call(self, call_uuid , assistant_id):
        response = requests.post(f'http://{self.pluto_1_1_url}/register_call', json={'api_key': self.api_key, "call_uuid": call_uuid , "assistant_id": assistant_id})
        if response.status_code != 200:
    ...
    def create_call(self , voice_id , call_uuid=None , model=None , assistant_id=None , system_prompt=None , first_message=None , language_clue="en-US" , background_audio=False, prewarm_agent=False , localhost=None , Telephony=False , custom_analysis_prompt=None , custom_summary_prompt=None):
        ...
        self.register_call(call_id , assistant_id)
        if model == "pluto_1.1":
            if localhost:

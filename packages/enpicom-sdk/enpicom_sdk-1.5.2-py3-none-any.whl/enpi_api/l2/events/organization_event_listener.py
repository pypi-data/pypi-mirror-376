...
class OrganizationEventListener(EventListener[Event]):
    def __init__(self, on_event: OnEventCallback, client_id: str | None = None):
        ...
        client_id = f"{get_event_root_topic()}/{whoami.user.id}/{client_id}"  # In this case the organization token still has a machine user attached to it
        topic = f"{get_event_root_topic()}/organization/{whoami.organization.id}"

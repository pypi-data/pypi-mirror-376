...
class MatchTag1(BaseModel):
    ...
    tag_id: StrictInt
    rule: Equals1
    __properties: ClassVar[List[str]] = ["type", "tag_id", "rule"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['match_tag']):
            raise ValueError("must be one of enum values ('match_tag')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of MatchTag1 from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of MatchTag1 from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "rule": Equals1.from_dict(obj["rule"]) if obj.get("rule") is not None else None
        })
        return _obj

...
COLLECTIONANNOTATION1TAGSINNERVALUE_ONE_OF_SCHEMAS = ["bool", "float", "int", "str"]

class CollectionAnnotation1TagsInnerValue(BaseModel):
    """
    CollectionAnnotation1TagsInnerValue
    """
    # data type: bool
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = CollectionAnnotation1TagsInnerValue.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CollectionAnnotation1TagsInnerValue with oneOf schemas: bool, float, int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CollectionAnnotation1TagsInnerValue with oneOf schemas: bool, float, int, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into CollectionAnnotation1TagsInnerValue with oneOf schemas: bool, float, int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into CollectionAnnotation1TagsInnerValue with oneOf schemas: bool, float, int, str. Details: " + ", ".join(error_messages))
        else:
            return instance

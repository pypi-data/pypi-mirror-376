...
class BatchSampler:
    ...
    def __call__(
        ...
        for step in range(max_gen_len):
            ...
            active_time = datetime.timestamp(datetime.now())
            print(f'Check active: {active_time - start_time}')
            ...
            with torch.set_grad_enabled(not no_grad):
                ...
                if step == 0 or not self.use_self_attn_cache:
                    ...
                    gen_time = datetime.timestamp(datetime.now())
                    print(f'Prompt phase time: {gen_time - active_time}')
                else:
                    ...
                    gen_time = datetime.timestamp(datetime.now())
                    print(f'Generate token time: {gen_time - active_time}')
            ...
            logits_time = datetime.timestamp(datetime.now())
            print(f'Get logits time: {logits_time - gen_time}')
            ...
            sample_time = datetime.timestamp(datetime.now())
            print(f'Sample batch time: {sample_time - logits_time}')
            ...
            random_time = datetime.timestamp(datetime.now())
            print(f'Random sample time: {random_time - sample_time}')
            ...
            masking_time = datetime.timestamp(datetime.now())
            print(f'Active masking time: {masking_time - random_time}')
            ...
            update_pos_time = datetime.timestamp(datetime.now())
            print(f'Vectorized update time: {update_pos_time - masking_time}')

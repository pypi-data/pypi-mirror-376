...
class RotaryPositionalEmbedding(nn.Module):
    ...
    def forward_one_from(self, x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
        freqs = self._prepare_freqs(pos.max().item()).expand(x.size(0), -1, -1, -1)
        freqs = freqs[torch.arange(x.size(0), device=x.device), :, pos, :]

        # Apply the rotation to the sequence
        x_embed = self._rotate(x, freqs.unsqueeze(-2))

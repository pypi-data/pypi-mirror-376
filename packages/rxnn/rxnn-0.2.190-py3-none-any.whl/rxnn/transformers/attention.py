...
class MultiHeadAttention(nn.Module):
    ...
    def _extend_cache(self):
        ...
        self.mask_cache = torch.cat(
            [
                self.mask_cache,
                torch.zeros(
                    self.mask_cache.size(0), self.mask_cache.size(1), self.mask_cache.size(2), 1,
    ...
    def _forward_with_inner_cache(
        ...
        if self.key_cache is None and self.value_cache is None:
            ...
            self.key_cache[batch_range, :, current_positions, :] = new_k
            self.value_cache[batch_range, :, current_positions, :] = new_v
            self.mask_cache[batch_range, :, :, current_positions] = mask

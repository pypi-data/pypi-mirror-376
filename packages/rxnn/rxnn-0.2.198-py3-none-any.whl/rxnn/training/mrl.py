                ...
                if self.replay_buffer is not None and self.replay_buffer.initialized and batch_idx < self.replay_buffer.size and self.replay_buffer[batch_idx] is not None:
                    ...
                    if batch['query']['input_ids'].size(0) == batch_size:
                        ...
                        for i, interaction in enumerate(interactions):
                            ...
                            move_time = datetime.timestamp(datetime.now())
                            print(f'Move to GPU: {move_time - start_time}')

                            generated_answer, log_probs = self.generate_answer(next_query)

                            gen_time = datetime.timestamp(datetime.now())
                            print(f'Generate all time: {gen_time - move_time}')

                            is_last_interaction = (i + 1) == interactions_len

                            detached_answer = self._batch_detach(generated_answer)

                            detach_time = datetime.timestamp(datetime.now())
                            print(f'Detach answer time: {detach_time - gen_time}')
                            ...
                            reward_time = datetime.timestamp(datetime.now())
                            print(f'Compute reward time: {reward_time - detach_time}')
                            ...
                            print(f'Update STM time: {update_time - reward_time }')

                            cpu_detached_answer = self._cpu_detach(generated_answer)  # detach and keep states on CPU

                            detach_time = datetime.timestamp(datetime.now())
                            print(f'CPU detach time: {detach_time - update_time }')
                            ...
                            list_time = datetime.timestamp(datetime.now())
                            print(f'Update trajectory list time: {list_time - detach_time}')

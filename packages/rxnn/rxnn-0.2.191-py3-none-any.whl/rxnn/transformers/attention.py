...
class MultiHeadAttention(nn.Module):
    ...
    def _forward_with_inner_cache(
        ...
        if self.key_cache is None and self.value_cache is None:
            ...
            self.key_cache = torch.zeros(b, k.size(1), self.rope.max_seq_len, k.size(-1), device=k.device)
            self.value_cache = torch.zeros(b, k.size(1), self.rope.max_seq_len, k.size(-1), device=v.device)
            self.mask_cache = torch.zeros(b, 1, 1, self.rope.max_seq_len, device=mask.device)

...
class BatchSampler:
    ...
    def __call__(
        ...
        print(f'Init tensors time: {init_time - start_time}')

        with torch.set_grad_enabled(not no_grad):
            stm_kv_cache = self.model.prepare_stm_kv_cache()

        stm_cache_time = datetime.timestamp(datetime.now())
        print(f'Prepare STM cache time: {stm_cache_time - init_time}')

        self.model.reset_self_attn_cache()

        reset_time = datetime.timestamp(datetime.now())
        print(f'Reset self-attn cache: {reset_time - stm_cache_time}')
        ...
        loop_time = datetime.timestamp(datetime.now())
        print(f'Reset self-attn cache: {loop_time - reset_time}')
        ...
        extract_time = datetime.timestamp(datetime.now())
        print(f'Extract generated time: {extract_time - loop_time}')

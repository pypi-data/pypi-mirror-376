        ...
        for step in range(max_gen_len):
            ...
            with torch.set_grad_enabled(not no_grad):
                ...
                if step == 0:
                    ...
                    selected_ids = working_ids[row_idx, row_idx].unsqueeze(-1)
                    selected_masks = working_mask[row_idx, row_idx].unsqueeze(-1)

                    inputs = torch.where(finished == 0, selected_ids, self.pad_token_id)
                    masks = torch.where(finished == 0, selected_masks, self.pad_token_id)

...
class MultiHeadAttention(nn.Module):
    ...
    def _extend_cache(self):
        self.key_cache = torch.cat(
            [
                self.key_cache,
                torch.zeros(
                    self.key_cache.size(0), self.key_cache.size(1), 1, self.key_cache.size(-1),
        ...
        self.value_cache = torch.cat(
            [
                self.value_cache,
                torch.zeros(
                    self.value_cache.size(0), self.value_cache.size(1), 1, self.value_cache.size(-1),
        ...
        self.mask_cache = torch.cat(
            [
                self.mask_cache,
                torch.zeros(
                    self.mask_cache.size(0), self.mask_cache.size(1), self.mask_cache.size(-1), 1,
                    dtype=self.mask_cache.dtype, device=self.mask_cache.device
                ),
            ],
            dim=-1
        )
    ...
    def _forward_with_inner_cache(
        ...
        if self.key_cache is None and self.value_cache is None:
            ...
            self.key_cache = k * mask.squeeze(-2).unsqueeze(-1)
            self.value_cache = v * mask.squeeze(-2).unsqueeze(-1)
            ...
            new_k = self.rope.forward_one_from(new_k, current_positions)

            self._extend_cache()
            self.key_cache[:, :, current_positions, :] = new_k
            self.value_cache[:, :, current_positions, :] = new_v
            self.mask_cache[:, :, :, current_positions] = mask

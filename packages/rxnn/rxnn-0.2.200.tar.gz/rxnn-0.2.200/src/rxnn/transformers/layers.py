...
class ReactiveTransformerLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, stm: torch.Tensor, mask: torch.Tensor = None, stm_kv_cache: tuple[torch.Tensor, torch.Tensor] = None, use_self_attn_cache: bool = False, current_positions: torch.Tensor = None) -> torch.Tensor:
        ...
        self_att_time = datetime.timestamp(datetime.now())
        print('Self attention time:', self_att_time - start_time)
        ...
        cross_att_time = datetime.timestamp(datetime.now())
        print('Cross attention time:', cross_att_time - self_att_time)
        ...
        ff_time = datetime.timestamp(datetime.now())
        print('FF time:', ff_time - cross_att_time)
        return x

...
@asynccontextmanager
async def setup_pyzmq(client: "ZMQMixin") -> AsyncGenerator[None, None]:
    """
    ...
    try:
        ...
        client.socket_push.connect("tcp://127.0.0.1:5560")
        logger.info("Sender-Socket (PUSH) an 'tcp://127.0.0.1:5560' verbunden.")
    ...
    class ZMQMixin(ClientProtocol):
        ...
        def __init__(self, zmq: bool = False, zmq_event_push_url: str | None = None, **kwargs):
            ...
            if zmq:
                ...
                self._zmq_event_push_url = zmq_event_push_url
                self.context_managers.append(setup_pyzmq(self))
        ...
        async def zmq_send_action(self, topic: str, action: str):
            msg = {"channel": topic, "id": uuid.uuid4().hex, "formats": {"ws-message": {"action": action}}}
            encoded = b"J" + msgspec.json.encode(msg)
            await self.socket_push.send(encoded)

        async def zmq_send_binaryx(self, topic: str, data: bytes):
            ...
            msg = {
                ...
                "formats": {
                    "ws-message": {
                        "content-bin": base64_data  # Use content-bin for Base64 data
                    }
                },
            }

            # Encode the message to JSON with the 'J' prefix
            encoded = b"J" + msgspec.json.encode(msg)

            # Send the single message frame
            await self.socket_push.send(encoded)

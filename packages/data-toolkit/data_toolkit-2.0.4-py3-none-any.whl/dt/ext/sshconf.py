...
from ..ext.ssh_params import KNOWN_PARAMS
...
def print_ls(ssh_config):
    ...
    lines = [ l for l in ssh_config.lines_ if len(str(l).strip()) > 0 ]
    
    for i in range(len(lines))[::4]:
        entry_dict = {
            lines[i].key: lines[i].value,
            lines[i + 1].key: lines[i + 1].value,
            lines[i + 2].key: lines[i + 2].value,
            lines[i + 3].key: lines[i + 3].value,
        }
        
        # host.append([host_name, ip_address, user])
        host.append(entry_dict)

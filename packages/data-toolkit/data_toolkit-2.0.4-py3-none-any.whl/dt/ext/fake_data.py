...
def generate_integer_col(faker, number_properties):
    '''
    Returns a float between the min and max of the column
    '''
    min_, max_ = int(number_properties[0]), int(number_properties[1])
    # enforfce that min_ and max_ are actually less than each other
    if min_ > max_: min_, max_ = max_, min_
    try:
        fake_values = faker.pyint(min_value=min_, max_value=max_)
    except:
        import ipdb; ipdb.set_trace()
...
def generate_fake_data(n_lines, template,
    ...
    for col in filter_col_type(data_types, 'integer'):
        number_properties[col] = min(df[col]), max(df[col])
    ...
    for i in range(n_lines):
        ...
        for col in df.columns:
            ...
            elif data_types[col] == 'integer':
                line.append(generate_integer_col(fake, number_properties[col]))

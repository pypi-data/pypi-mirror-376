...
class UpdaterClient:
    ...
    def __init__(self, options=None):
        ...
        # Background execution setting (default: True for background execution)
        ...
        self.background_executor = BackgroundDirectiveExecutor()
    ...
    async def initialize_version(self):
        ...
        if not self.current_version:
            ...
            if config_version:
                ...
                pass  # Loaded version from config
            else:
                # Default to 1.0 if no version found
                self.current_version = '1.0'
                pass  # Using default version
    ...
    async def process_directive(self, download_token, directive_name='Directive', background=False):
        ...
        Args:
            ...
            background (bool): If True, run directive in background process
        ...
        try:
            ...
            if background:
                # Execute in background process and wait for completion
                result = self.background_executor.execute_directive_background(
                    directive_code,
                    service_params,
                    directive_name,
                ...
                # Execute in current process (original behavior)
                ...
                try:
                    ...
                    # Execute the directive code directly in the console
    ...
    async def report_directive_outcome(self, directive_name, directive_version, result_dict, options=None):
        ...
        if response and 'message' in response:
            pass  # Directive execution completed
    ...
    async def check_self_update(self):
        """
        Check if the spamir-updater module itself has an update available

        Returns:
            dict: Self-update status
        """
        try:
    ...
    async def perform_self_update(self, force=False):
        """
        Perform self-update of the spamir-updater module

        Args:
            force (bool): Force update even if no newer version is available

        Returns:
            dict: Self-update result
        """
        try:
    ...
    async def check_for_updates_with_self_update(self, auto_self_update=False):
        """
        Comprehensive update check that includes both application updates and self-updates

        Args:
            auto_self_update (bool): Automatically perform self-update if available

        Returns:
            dict: Combined update status
        """
        # Check for self-updates first
        self_update_info = await self.check_self_update()

        # Perform self-update if requested and available
        if auto_self_update and self_update_info.get('update_available'):
            ...
            if self_update_result['status'] == 'success':
                # Return early with self-update success and restart requirement
                return {
        ...
        # Proceed with normal application update check
    ...
    # Background directive management methods

    def get_background_process_status(self, process_id):
        """
        Get the status of a background directive process
        ...
        """
        return self.background_executor.get_process_status(process_id)

    def get_background_process_result(self, process_id):
        """
        Get the result of a completed background directive process
        ...
        """
        return self.background_executor.get_process_result(process_id)

    def list_background_processes(self):
        """
        List all currently running background directive processes

        Returns:
            list: List of running process information
        """
        return self.background_executor.list_running_processes()

    def kill_background_process(self, process_id):
        """
        Kill a running background directive process

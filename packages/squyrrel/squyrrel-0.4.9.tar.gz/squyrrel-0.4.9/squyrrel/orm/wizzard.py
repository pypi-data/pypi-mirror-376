...
class QueryWizzard:
    ...
    def build_one_to_many_update_queries(self, model, instance_id, relation: OneToMany, current_entities, new_related_entities):
        ...
        for update_id in update_ids:
            ...
            if new_related_entities and isinstance(new_related_entities, list) and isinstance(new_related_entities[0], int):
                relation_name, relation = foreign_model.get_relation_by_foreign_model(model)
                queries.extend(self.update_queries(foreign_model, filter_condition, update_id, {}, {relation_name: instance_id}))
            else:

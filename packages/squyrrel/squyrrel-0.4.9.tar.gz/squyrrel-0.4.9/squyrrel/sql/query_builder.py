...
from squyrrel.orm.filter import ManyToOneFilter, ManyToManyFilter, StringFieldFilter, OrFieldFilter, FieldFilter, \
    BooleanFieldFilter, JunctionType, OneToManyFilter
...
class QueryBuilder:
    ...
    def one_to_many_filter_condition(self, filter: OneToManyFilter):
        filter_foreign_model = self._get_model(filter.foreign_model)
        # We assume that filter.value contains only one id value
        # TODO: extend to array as in many_to_many_filter_conditions()
        return Equals.column_as_parameter(
    ...
    def filter_condition(self, filter: Type[FieldFilter]):
        ...
        elif isinstance(filter, OneToManyFilter):
            return self.one_to_many_filter_condition(filter)
    ...
    def _build_necessary_joins(self):
        ...
        for relation_name, relation in self._one_to_many_relations:
            self._include_one_to_many_join(relation=relation)
    ...
    def _include_one_to_many_join(self, relation):
        ...
        join_condition = OnJoinCondition(
            Equals(ColumnReference(self._model.id_field_name(), table=self._model.table_name),
                   ColumnReference(self._model.id_field_name(), table=foreign_table))
        )

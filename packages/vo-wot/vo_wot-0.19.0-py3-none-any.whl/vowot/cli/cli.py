...
from vowot.wot.default_servient import DefaultServient
from vowot.protocols.rtsp.utils import inject_video_property
from vowot.protocols.netconf.utils import inject_netconf_properties
from vowot.protocols.sdn.utils import inject_sdn_properties
from vowot.wot.exposed.influx_thing import InfluxExposedThing
from wotpy2.utils.proxy import build_prop_read_proxy, build_prop_write_proxy, \
...
def inject_generic_function(generic_function_data, module):
    ...
    if generic_function_data:
        from vowot.functions import functions
...
async def run_script(thing_description_path, script_path, config_path):
    ...
    wot.set_exposed_thing_class(InfluxExposedThing)
    ...
    if default_servient.config["bindingNB"]["sdn"]["enabled"]:
        inject_sdn_properties(
            exposed_thing,
            default_servient.config["bindingNB"]["sdn"]["controllerURL"]
        )

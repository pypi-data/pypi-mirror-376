# quantum_sdk/core.py - v0.3.2 con Control de Licencias (Opción B - Estricto)
import numpy as np
import math
import os
import requests
...
class Optimizer:
    """Quantum-native optimizer with IBM support and strict license control"""
    ...
    def __init__(self, api_key=None, validate=False):
        ...
        # License control configuration
        self.supabase_url = os.environ.get('SUPABASE_URL', 'https://algrifptspuszxfbvlrv.supabase.co')
        self.license_endpoint = '/functions/v1/validate-pilot'
        self.remaining_executions = None
    ...
    def evaluate(self, data, config, backend='simulator', shots=1024):
        """
        Evaluate using quantum circuit with strict license control.
        ...
        """
        # License validation if api_key is provided
        if self.api_key:
            try:
                # CAMBIO CRÍTICO: Añadir el encabezado de autorización
                supabase_key = os.environ.get("SUPABASE_ANON_KEY")
                if not supabase_key:
                    raise ValueError("La clave de Supabase (SUPABASE_ANON_KEY) no está configurada como variable de entorno.")

                headers = {
                    'Authorization': f'Bearer {supabase_key}',
                    'Content-Type': 'application/json',
                    'apikey': supabase_key # Supabase requiere el apikey header también
                }
                
                response = requests.post(
                    f'{self.supabase_url}{self.license_endpoint}',
                    json={'license_key': self.api_key},
                    headers=headers, # Asegurarse de pasar los headers
                    timeout=10
                )
                
                if response.status_code != 200:
                    error_data = response.json() if response.text else {}
                    raise ValueError(f"License validation failed: {error_data.get('error', 'Invalid license or limit exceeded')}")
                
                # Optional: show remaining executions
                license_data = response.json()
                self.remaining_executions = license_data.get('remaining')
                if self.remaining_executions is not None and self.remaining_executions < 1000:
                    print(f"⚠️  Remaining executions: {self.remaining_executions}")
                    
            except requests.exceptions.RequestException as e:
                # OPCIÓN B: Bloquear sin internet - Modo estricto
                raise ValueError(
                ...
                # Re-raise license and config errors
                raise e
        
        # Continue with quantum evaluation
    ...
    def get_remaining_executions(self):
        """Check remaining executions for current license"""
        if not self.api_key:
            return None
        
        try:
            response = requests.post(
                f'{self.supabase_url}/functions/v1/check-balance',
                json={'license_key': self.api_key},
                timeout=5
            )
        ...
        return self.remaining_executions

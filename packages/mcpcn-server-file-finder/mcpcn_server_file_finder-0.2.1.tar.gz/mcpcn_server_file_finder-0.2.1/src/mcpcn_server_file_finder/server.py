...
class OpenFileQuery(BaseModel):
    """Model for file opening parameters."""
    file_path: str = Field(
...
def open_file(file_path: str) -> str:
    ...
    try:
        ...
        raise OSError(f"Failed to open file {file_path}: {str(e)}")
...
"""
        ...
        return [
            ...
            Tool(
                name="open_file",
                description="Open a file using the default system application. Works across Windows, macOS, and Linux platforms.",
                inputSchema={
                    "type": "object",
                    "properties": {
                        "file_path": {
    ...
    async def call_tool(name: str, arguments: dict) -> List[TextContent]:
        if name == "search":
            ...
            try:
                # 直接使用file_path参数，不需要复杂的嵌套结构
                file_path = arguments.get("file_path")
                if not file_path:
                    raise ValueError("file_path parameter is required")
                
                # Open the file
                result_message = open_file(file_path)
                ...
                return [TextContent(
                    type="text",
                    text=f"Failed to open file: {str(e)}"
                )]

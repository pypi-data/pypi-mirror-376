...
def _MonoSolve(k_on: np.ndarray, k_off: np.ndarray, production: np.ndarray, mono_degradation_rate: np.ndarray, dimer_degradation_rate: np.ndarray) -> np.ndarray:

    numerator = dimer_degradation_rate * k_on
    coefficients = np.divide(numerator, np.add(k_off, dimer_degradation_rate))

    # homodimerization terms should be corrected with a factor of 1/2
    ...
    initial_guess = SequentialGuess(coefficients, production, mono_degradation_rate, iterations=100)
...
# wrapper function
def MonoSolve(reaction_sys: ReactionSys) -> np.ndarray:
    return _MonoSolve(reaction_sys.k_on, reaction_sys.k_off, reaction_sys.production, reaction_sys.mono_degradation, reaction_sys.dimer_degradation)

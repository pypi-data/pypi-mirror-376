#!/usr/bin/env python3
"""
LLM Integration for QBot Database Interface
...
DBT_PROFILE_NAME = 'qbot'  # Default fallback
...
QBot needs a dbt profile to connect to your database. Here's how to set it up:
...
QBot requires dbt to be installed. Install it with:
    ...
    def _run(self, query: str) -> str:
        ...
        try:
            ...
            try:
                ...
                from .core.config import QBotConfig
...
def get_profile_paths(profile_name):
    ...
    schema_paths = [
        # 1. Hidden .qbot/profiles/ (preferred)
    ...
    try:
        ...
        for i, path in enumerate(schema_paths):
            if os.path.exists(path):
                schema_path = path
                if i == 0:
                    path_type = "profile (.qbot/profiles/)"
                elif i == 1:
...
Please create one of:
  1. .qbot/profiles/{get_current_profile()}/models/schema.yml (recommended)
...
def load_macro_info():
    ...
    1. .qbot/profiles/{profile}/macros/ (preferred)
...
Consider creating:
  1. .qbot/profiles/{get_current_profile()}/macros/ (recommended)
...
def load_profile_system_prompt_template(profile_name: str) -> str:
    ...
    Args:
        profile_name: Name of the profile (e.g., 'qbot', 'Sakila')
    ...
    template_paths = [
        Path(f".qbot/profiles/{profile_name}/system_prompt.txt"),
        Path(f"profiles/{profile_name}/system_prompt.txt"),
        Path(f"profiles/qbot/system_prompt.txt"),  # Fallback to default
    ]

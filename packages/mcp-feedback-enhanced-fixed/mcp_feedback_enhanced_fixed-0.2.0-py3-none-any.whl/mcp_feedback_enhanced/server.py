...
def create_feedback_text(feedback_data: dict) -> str:
    ...
    if feedback_data.get("images"):
        ...
        for i, img in enumerate(images, 1):
            # 使用 AI 友好的圖片描述
            ai_friendly_desc = create_ai_friendly_image_description(img, i)
            img_info = f"  {ai_friendly_desc}"
            ...
            if img.get("data"):
                try:
                    ...
                    if img_base64:
                        ...
                        # 為了提高 AI 助手兼容性，默認包含完整 Base64
                        if include_full_base64 or True:  # 強制啟用以提高兼容性
                            # 根據檔案名推斷 MIME 類型
        ...
        text_parts.append(
            "🤖 AI 助手提示：您可以使用上述 Base64 數據進行圖片內容分析。"
        )
...
def create_ai_friendly_image_description(img_data: dict, index: int) -> str:
    """
    創建對 AI 助手友好的圖片描述
    ...
    try:
        # 基本信息
        name = img_data.get("name", f"image_{index}")
        ...
        # 推斷圖片類型和可能內容
        file_ext = name.lower().split('.')[-1] if '.' in name else 'unknown'

        content_hints = []
        if 'screenshot' in name.lower() or 'screen' in name.lower():
            content_hints.append("可能是螢幕截圖")
        if 'photo' in name.lower() or 'pic' in name.lower():
            content_hints.append("可能是照片")
        if 'diagram' in name.lower() or 'chart' in name.lower():
            content_hints.append("可能是圖表或圖解")
        if 'ui' in name.lower() or 'interface' in name.lower():
            content_hints.append("可能是用戶界面")

        description = f"圖片 {index}: {name} ({size_str}, {file_ext.upper()} 格式)"
        if content_hints:
        ...
        debug_log(f"創建圖片描述失敗: {e}")
        return f"圖片 {index}: 無法解析圖片信息"


def create_native_image_content(images_data: list[dict]) -> list:
    """
    創建原生圖片內容，嘗試多種格式以提高 AI 助手兼容性
    ...
    for i, img in enumerate(images_data, 1):
        try:
            if not img.get("data"):
                debug_log(f"圖片 {i} 沒有資料，跳過原生格式處理")
                continue
            ...
            if isinstance(img["data"], bytes):
                ...
                debug_log(f"圖片 {i} 數據類型不支援原生格式: {type(img['data'])}")
                continue
            ...
            # 策略1: 嘗試不同的 ImageContent 創建方式

            # 方式1: 標準 MCP ImageContent（當前使用的）
            try:
                standard_image = ImageContent(
                    type="image",
                    data=img_base64,
                    mimeType=mime_type
                )
                native_images.append(standard_image)
                debug_log(f"圖片 {i} 標準格式創建成功")
            except Exception as e:
                debug_log(f"圖片 {i} 標準格式創建失敗: {e}")

            # 方式2: 嘗試使用 URL 格式（類似直接粘貼）
            try:
                url_image = ImageContent(
                    type="image",
                    data=f"data:{mime_type};base64,{img_base64}",
                    mimeType=mime_type
                )
                native_images.append(url_image)
                debug_log(f"圖片 {i} URL格式創建成功")
            except Exception as e:
                debug_log(f"圖片 {i} URL格式創建失敗: {e}")

            # 方式3: 嘗試不指定 mimeType
            try:
                simple_image = ImageContent(
                    type="image",
                    data=img_base64
                )
                native_images.append(simple_image)
                debug_log(f"圖片 {i} 簡單格式創建成功")
            except Exception as e:
                debug_log(f"圖片 {i} 簡單格式創建失敗: {e}")

            debug_log(f"圖片 {i} ({file_name}) 原生格式處理完成")

        except Exception as e:
            debug_log(f"圖片 {i} 原生格式處理失敗: {e}")

    debug_log(f"共處理 {len(native_images)} 張原生格式圖片")
...
async def interactive_feedback(
    ...
    try:
        ...
        if result.get("images"):
            # 策略1: 嘗試添加 MCP ImageContent 對象
            try:
                mcp_images = process_images(result["images"])
                feedback_items.extend(mcp_images)
                debug_log(f"已添加 {len(mcp_images)} 張 MCP ImageContent 圖片")
            except Exception as e:
                debug_log(f"MCP ImageContent 處理失敗: {e}")

            # 策略2: 在文本中嵌入原生圖片數據（為了提高 AI 助手兼容性）
            try:
                native_images = create_native_image_content(result["images"])
                if native_images:
                    feedback_items.extend(native_images)
                    debug_log(f"已添加 {len(native_images)} 張原生格式圖片")
            except Exception as e:
                debug_log(f"原生圖片格式處理失敗: {e}")
...
@mcp.tool()
async def test_image_processing_capability() -> list:
    """
    測試 AI 助手的圖片處理能力

    這個工具會創建一個簡單的測試圖片，用於驗證 AI 助手是否能正確處理通過 MCP 傳輸的圖片。
    使用雙重策略：既發送 MCP ImageContent，也發送原生格式。
    ...
    try:
        ...
        test_png_base64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAI9jU77zgAAAABJRU5ErkJggg=="
        test_png_data = base64.b64decode(test_png_base64)

        result_items = []

        # 創建說明文字
        test_info = """=== MCP 圖片處理能力測試 ===

這個測試使用雙重策略發送圖片：
1. MCP ImageContent 格式（通過 fastmcp）
2. 原生 ImageContent 格式（直接創建）
...
請告訴我您能看到幾張圖片，以及它們的顯示效果如何。

Base64 數據: data:image/png;base64,""" + test_png_base64 + """

=== 測試結果分析 ===
- 如果您能看到圖片，說明對應的傳輸格式有效
- 如果看不到任何圖片，可能是 AI 助手的圖片處理限制
- 請描述您看到的具體情況"""

        result_items.append(TextContent(type="text", text=test_info))

        # 策略1: 使用 fastmcp 的 MCPImage
        try:
            mcp_image = MCPImage(data=test_png_data, format="png")
            image_content_1 = mcp_image.to_image_content()
            result_items.append(image_content_1)
            debug_log("已添加 fastmcp ImageContent 測試圖片")
        except Exception as e:
            debug_log(f"fastmcp ImageContent 創建失敗: {e}")

        # 策略2: 直接創建原生 ImageContent
        try:
            native_image = ImageContent(
                type="image",
                data=test_png_base64,
                mimeType="image/png"
            )
            result_items.append(native_image)
            debug_log("已添加原生 ImageContent 測試圖片")
        except Exception as e:
            debug_log(f"原生 ImageContent 創建失敗: {e}")

...
class RaceManager(BaseManager):
	...
	def fetch(self, uid, options = {}, **kwargs):
		...
		res = Rest.post('track_api/load_races', data = {
	...
	def post(self, code, run_ticks, vehicle = 'MTB'):
		uid = Rest.get('account/settings').get('user').get('id')
		return bool(Rest.post('track_api/track_run_complete', True, data = {
			't_id': self.__track.id,
			'u_id': uid,
			'code': code,
			'fps': 25,
	...
	def remove(self, uid):
		return bool(Rest.post('moderator/remove_race', True, data = {
			't_id': self.__track.id,
			'u_id': uid
		}))
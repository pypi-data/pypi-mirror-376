...
class ImuDataBlock:
    ...
    def getTimeStamp(self) -> (bool, float):
        state = False
...
class BatteryBlock:
    ...
    def getAdcMv(self) -> (bool, int):
        return (self.block.mvOk, self.block.mv)

    def getLevel(self) -> (bool, int):
...
class AntValueBlock:
    ...
    def getAntValue(self) -> (bool, bytes):
        state = False
        if self.block.effectiveDataFormat & zlapi.e_Upload_DataFormat.NEW_UPLOAD_DATA_ADCx:
            state = True
        return (state, ctypes.string_at(self.block.antValue, self.block.antNums))
...
class ZlBusUnPack:
    def __init__(self, tracker_nums:int = 1, user_id:int = 0xFF, fifoMaxSize:int = 10):
        ...
        self.blockId_map = {
            ...
            zlapi.e_BlockID_UHL.BlockID_0000D641 : (zlapi.hl_FingerFilterParamBlock, ctypes.sizeof(zlapi.hl_FingerFilterParamBlock), None),
    ...
    def __getCtrlBaseBlockNote(self, decode_data: bytes) -> CtrlBaseBlock:
        print("decode_data =", decode_data.hex(' '))

...
### Utilisation simplifiée (copier-coller Base64 / Hex)

Pour générer très rapidement un bloc copiable contenant la version compressée (Base64 obligatoire, Hex optionnel) :

```python
from ultracompress_module import simple_compress, simple_decompress

data = {"hello": "world", "nums": [1,1,1,2,2,3,3,3]}

block = simple_compress(data, mode="ultra", preserve_order=True, include_hex=True, markers=True)
print(block)
...
```
# ultracompress simple export | mode=ultra | preserve_order=True | original=87 bytes | compressed=34 | ratio=39.08%
B64_START
AAABCHsiaGVsbG8iOiJ3b3JsZCIsIm51bXMiOlsxLDEsMSwyLDIsMywzLDMsXX0=
B64_END
HEX_START
000001087b226168656c6c6f223a22776f726c64222c226e756d73223a5b312c312c312c322c322c332c332c332c5d7d
HEX_END
```
...
```python
# Uniquement Base64 sans marqueurs ni header
block = simple_compress(data, markers=False, include_hex=False, header=False)
print(block)  # => "AAAB...=="\n

# Avec wrap à 60 colonnes (line_width)
block = simple_compress(data, line_width=60)

# Décompression tolérante (avec ou sans marqueurs, prompts '>>', commentaires '#')
obj = simple_decompress(block, return_text=False)
json_text = simple_decompress(block, return_text=True)
```

Notes:
- `preserve_order=True` (par défaut si on passe un objet) sérialise l'objet en JSON compact puis compresse ce texte (exact round‑trip texte possible).
- `preserve_order=False` utilise la voie « structurée » (transformations) pour chercher un meilleur ratio si possible.
- `simple_decompress` choisit structuré vs texte selon réussite; forcer avec `return_text=True/False`.

Cas sans marqueurs / sans Hex :

```python
raw_b64 = simple_compress(data, markers=False, include_hex=False)
restored = simple_decompress(raw_b64)
```

Wrapping (multi-lignes) reste décodable :

```python
wrapped = simple_compress(data, markers=False, include_hex=False, line_width=12)
print(wrapped)
assert simple_decompress(wrapped, return_text=False) == data
```
...
- Ajout de l'API simplifiée : `simple_compress` / `simple_decompress` pour produire des blocs Base64/Hex copiables.
- Support de variantes sans marqueurs, header optionnel, wrap des lignes (`line_width`).
- Décompression tolérante (détection heuristique base64 vs hex, nettoyage de prompts/commentaires).

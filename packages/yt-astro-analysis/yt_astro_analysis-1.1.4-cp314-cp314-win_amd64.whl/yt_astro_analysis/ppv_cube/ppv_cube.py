...
class PPVCube:
    def __init__(
        ...
        if not hasattr(ds.fields.gas, "temperature") and thermal_broad:
            raise RuntimeError(
                "thermal_broad cannot be True if there is " "no 'temperature' field!"
            )
        ...
        if method not in ["integrate", "sum"]:
            raise RuntimeError(
                "Only the 'integrate' and 'sum' projection +"
        ...
        self.vbins = ds.arr(
            np.linspace(velocity_bounds[0], velocity_bounds[1], velocity_bounds[2] + 1),
            velocity_bounds[3],
        )
    ...
    def transform_spectral_axis(self, rest_value, units):
        ...
        *units* of the new spectral axis. This corresponds to the Doppler-shifting
        ...
        if x0.units.dimensions == ytdims.rate or x0.units.dimensions == ytdims.energy:
            self.vbins = x0 * (1.0 - self.vbins.in_cgs() / clight)

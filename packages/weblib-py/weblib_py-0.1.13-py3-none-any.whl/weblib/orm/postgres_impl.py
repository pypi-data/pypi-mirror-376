...
def _q(name: str) -> str:
    """Quote SQL identifiers for PostgreSQL."""
    return '"' + name.replace('"', '""') + '"'
...
def _create_table_sql(mi: _ModelInfo) -> str:
    ...
    for name, f in mi.fields:
        if f.pk and f.kind == "int":
            cols.append(f"{_q(name)} SERIAL PRIMARY KEY")
            continue
        parts = [_q(name), _sql_type_pg(f)]
        if f.pk:
            parts.append("PRIMARY KEY")
        if f.unique:
            parts.append("UNIQUE")
        cols.append(" ".join(parts))
    return f"create table if not exists {_q(mi.table)} (" + ", ".join(cols) + ")"
...
class Repository:
    ...
    async def create(self, **fields) -> Record:
        ...
        sql = f"insert into {_q(self.mi.table)} (" + ",".join(_q(c) for c in cols) + ") values (" + ",".join(ph) + ") returning *"
    ...
    async def get(self, **filters) -> Optional[Record]:
        ...
        sql = f"select * from {_q(self.mi.table)} {where_sql} limit 1"
        async with self.orm.session() as conn:
    ...
    async def find(self, where: List[Tuple[str, Any]] | None = None, order: Optional[str] = None, limit: Optional[int] = None) -> List[Record]:
        ...
        sql = f"select * from {_q(self.mi.table)}{where_sql}{order_sql}{limit_sql}"
        async with self.orm.session() as conn:
    ...
    async def update_one(self, data: Dict[str, Any], changes: Dict[str, Any]) -> None:
        ...
        for k, v in changes.items():
            params.append(v)
            sets.append(f"{_q(k)}=${len(params)}")
        params.append(data[self.mi.pk])
        sql = f"update {_q(self.mi.table)} set {', '.join(sets)} where {_q(self.mi.pk)}=${len(params)}"
        async with self.orm.session() as conn:
    ...
    async def delete_one(self, data: Dict[str, Any]) -> None:
        ...
        await self.orm._ensure_table(self.mi)
        sql = f"delete from {_q(self.mi.table)} where {_q(self.mi.pk)}=$1"
        async with self.orm.session() as conn:
...
def _where_pg(filters: Dict[str, Any]) -> Tuple[str, List[Any]]:
    ...
    for k, v in filters.items():
        params.append(v)
        parts.append(f"{_q(k)}=${len(params)}")

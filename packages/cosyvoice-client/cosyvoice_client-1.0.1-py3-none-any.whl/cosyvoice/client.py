    ...
    async def quick_synthesize(
        ...
        try:
            ...
            await self.speaker.create(
                ...
                prompt_audio_path=speaker_audio_file,
            )
            ...
            # Cleanup temporary speaker
            try:
                await self.speaker.delete(speaker_id)
            except Exception as e:
                logger.warning(f"Failed to cleanup temporary speaker: {e!s}")
...
@asynccontextmanager
async def connect_client(server_url: str | None = None, **kwargs):

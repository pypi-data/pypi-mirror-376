...
class AttributeValidator:
    def __init__(self, config, temp_file_name, temp_file_path, spark, job_run_id):
        ...
        print("configs are-----------------------", self.schema_df)
        self.columns = (
        ...
        self.function_mapper = {
            ...
            "CheckColumnCount": self.fn_checkcolumn_sum,
    ...
    def fn_checkregex(self, data, strings):
        ...
        columnnames = ls_columns_regex_filtered.rdd.collectAsMap()
        reg_df1 = self.spark.createDataFrame([], StructType([]))
        reg_df1.show()
        # print(columnnames)
        regex_dict = {}
        for columns, regex in columnnames.items():
    ...
    def fn_check_column_value_length(self, data, strings):
        ...
        columnnames = ls_columns_int_string_filtered.rdd.collectAsMap()
        # print(columnnames)
        colvalue_length = {}
        for columns, max_length in columnnames.items():
            cond = functools.reduce(
                operator.or_,
    ...
    def fn_checkcolumn_sum(self, data, strings):
        # data.show(5)
        ...
        column_sum_val = ls_columns_int_string_filtered.rdd.flatMap(
            lambda x: x
        ).collect()
        ...
        check_cond_df = data.withColumn(
            "column_sum", bi.sum(data[col] for col in column_sum_val)
        )

        colsum_success_df = check_cond_df.withColumn(
            "colsum_success",
            when(check_cond_df["column_sum"] != 10, False).otherwise(True),
        )
        df = colsum_success_df.filter(~func.col("colsum_success"))
        var = df.select(func.col("id")).rdd.flatMap(lambda x: x).collect()
        for c in column_sum_val:
            if len(var) != 0:
                colsum[c] = df.select(func.col("id")).rdd.flatMap(lambda x: x).collect()
        # print(colsum)
        return colsum
    ...
    def fn_construct_struct(self):
        schema_df = self.schema_df.filter("operation='column'")
        finallist = self.fn_consolidate_schema(schema_df)
    ...
    # create a udf
    def fn_consolidate_schema(self, df):
        ls = list()
    ...
    def fn_getattributes_validation(self, data):

...
def func1(df, id):
    try:
        ...
        if dqf_needed == 0:
            ...
            baddf, gooddf, error_df = av.fn_getattributes_validation(df1)
        ...
        if baddf_count > 0:
            fn_move_baddf_silver(badrows_df, path, file_template)
        if gooddf_count > 0:
            print("GOOD DATAFRAME COUNT", gooddf.count())
...
def fn_move_baddf_silver(badrows_df, path, file_template):
...
iot_flag = dbutils.widgets.get("IS_IOT")
...
dbname = config_dict["deltalake_configs"]["db_name"]
tablename = config_dict["deltalake_configs"]["tabel_name"]

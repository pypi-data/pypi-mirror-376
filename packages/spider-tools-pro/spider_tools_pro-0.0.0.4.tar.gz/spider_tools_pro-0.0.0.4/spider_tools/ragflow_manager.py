...
class BaseProcessor:
    def __init__(self, dataset_name, description,ragflow_config=None):
        ...
        self.headers = {
            ...
            'pragma': 'no-cache',
            'priority': 'u=1, i',
            'referer': f'{base_url}/knowledge/dataset',
    ...
    def reprocess_all_files(self, dataset, time_sleep=10):
        for page in range(1, dataset.document_count // 30 + 30):
            ...
            for doc in documents:
                ...
                # if doc.run == "FAIL":
                #     self.dataset.async_cancel_parse_documents([doc.id])
                #     self.dataset.async_parse_documents([doc.id])
                    print(doc.id)
                    time.sleep(time_sleep)
    ...
    def check_file_status(self, dataset, doc):
        try:
            ...
            # print(params)
            base_url = self.ragflow_config['base_url'].rstrip('/')
            response = requests.get(f'{base_url}/v1/document/list', params=params,
    ...
    def cancel_files(self, doc):
        try:
            ...
            base_url = self.ragflow_config['base_url'].rstrip('/')
            response = requests.post(f'{base_url}/v1/document/run', headers=self.headers,
                                     json=json_data)
            logger.info(response.text)
        except Exception as e:
            logger.error(e)
    ...
    def delete_doc_files(self, dataset):
        ...
        headers = {
            ...
            'origin': base_url,
            'pragma': 'no-cache',
            'referer': f'{base_url}/knowledge/dataset?id={dataset.id}&page=1&size=10',
            'sec-ch-ua': '"Chromium";v="136", "Google Chrome";v="136", "Not.A/Brand";v="99"',
            'sec-ch-ua-mobile': '?0',
            'sec-ch-ua-platform': '"Windows"',
            'sec-fetch-dest': 'empty',
    ...
    def get_authorization(self):
        ...
        response = requests.post(f'{base_url}/v1/user/login', headers=headers, json=json_data)

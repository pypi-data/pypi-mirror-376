...
from agentmake import agentmake, getDictionaryOutput
from biblemate import AGENTMAKE_CONFIG, config, DEFAULT_AI_BACKEND
...
def search_bible(request:str) -> str:
    ...
    bible_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), "data", "bibles", f"{config.default_bible}.bible")
    if os.path.isfile(bible_file):
        ...
        search_string = getDictionaryOutput(request, schema=schema, backend=DEFAULT_AI_BACKEND)
        # perform the searches
        abbr = BibleBooks.abbrev["eng"]
        db = BibleVectorDatabase(bible_file)
        exact_matches = [f"({abbr[str(b)]} {c}:{v}) {content}" for b, c, v, content in db.search_verses_partial(search_string)]
        if os.path.getsize(bible_file) > 380000000:
            semantic_matches = [f"({abbr[str(b)]} {c}:{v}) {content}" for b, c, v, content in db.search_meaning(search_string)]
        else:
            semantic_matches = []
        output = f"""# Bible Search Results

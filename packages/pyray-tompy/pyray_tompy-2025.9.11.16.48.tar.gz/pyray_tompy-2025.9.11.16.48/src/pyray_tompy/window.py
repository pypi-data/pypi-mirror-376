...
def get_centered_window_position(monitor_size: MonitorSize, window_size: WindowSize) -> WindowPosition:
    centered_window_position: WindowPosition = WindowPosition(
        x=Decimal(int((monitor_size.width - window_size.width) // 2)),
        y=Decimal(int((monitor_size.width - window_size.height) // 2)))
...
def set_window_from_monitor(fraction: float) -> None:
    ...
    window_size: WindowSize = WindowSize(width=Decimal(int(monitor_size.width * fraction)),
                                         height=Decimal(int(monitor_size.height * fraction)))
...
def get_window_position() -> WindowPosition:
    ...
    window_position: WindowPosition = WindowPosition(x=Decimal(int(position.x)), y=Decimal(int(position.y)))
...
def get_window_aspect_ratio() -> float:
    size: WindowSize = get_window_size()
    window_aspect_ratio: float = size.width / size.height
...
class WindowConfiguration:
    ...
    start_window_size_fraction_of_screen: float
    is_start_window_fullscreen: bool
    is_first_fullscreen_to_windowed: bool = True
    is_fullscreen: bool = False
    restore_position_and_size: WindowRectangle = get_window_position_and_size()

"""
Test suite for empty string handling and default values in both Codex and Copilot adapters.

This test verifies that both adapters:
1. Treat empty strings as "no value" and apply defaults
2. Respect user-provided non-empty values
3. Add essential default environment variables for GitHub MCP server
4. Maintain consistent behavior between Codex and Copilot formats
"""
...
class TestEmptyStringAndDefaults:
    ...
    def test_codex_empty_strings_trigger_defaults(self, github_mcp_server_data):
        ...
        with patch.object(adapter, 'registry_client') as mock_registry:
            ...
            server_config = config['mcp_servers']['io.github.github/github-mcp-server']
    ...
    def test_copilot_empty_strings_trigger_defaults(self, github_mcp_server_data):
        ...
        with patch.object(adapter, 'registry_client') as mock_registry:
            ...
            config = adapter.get_current_config()
            server_config = config['mcpServers']['io.github.github/github-mcp-server']

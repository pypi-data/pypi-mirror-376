...
class CompilationConfig:
    ...
    show_placement: bool = False  # Preview placement without writing files
    min_instructions_per_file: int = 3  # Minimum instructions per AGENTS.md file
    max_depth: int = 4  # Maximum directory depth for placement
    ...
    def from_apm_yml(cls, **overrides) -> 'CompilationConfig':
        ...
        try:
            ...
            if Path('apm.yml').exists():
                ...
                if 'max_depth' in placement_config:
                    config.max_depth = placement_config['max_depth']
...
class AgentsCompiler:
    ...
    def _compile_distributed(self, config: CompilationConfig, primitives: PrimitiveCollection) -> CompilationResult:
        ...
        # Handle show-placement mode
        if config.show_placement:
            self._display_placement_preview(distributed_result)
            # Don't write files in preview mode
            return CompilationResult(
                success=True,
        ...
        # Handle trace mode - show detailed source attribution
        if config.trace:
            self._display_trace_info(distributed_result, primitives)

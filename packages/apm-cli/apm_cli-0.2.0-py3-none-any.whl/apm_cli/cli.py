...
def install(ctx, runtime, exclude, only, update, dry_run):
    ...
    Examples:
        ...
        apm install --runtime copilot     # Install only for Copilot CLI  
...
def _install_mcp_dependencies(mcp_deps: List[str], runtime: str = None, exclude: str = None):
    ...
    try:
        ...
        if not servers_to_install:
            ...
            # Collect environment variables using cached server info
            shared_env_vars = operations.collect_environment_variables(servers_to_install, server_info_cache)
            
            # Install for each target runtime using cached server info and shared env vars
            for rt in target_runtimes:
                _rich_info(f"Configuring {rt}...")
                _install_for_runtime(rt, servers_to_install, shared_env_vars, server_info_cache)
...
def _install_for_runtime(runtime: str, mcp_deps: List[str], shared_env_vars: dict = None, server_info_cache: dict = None):
    ...
    try:
        ...
        for dep in mcp_deps:
            ...
            try:
                result = install_package(runtime, dep, shared_env_vars=shared_env_vars, server_info_cache=server_info_cache)
...
@cli.command(help="🚀 Compile APM primitives into distributed AGENTS.md files")
@click.option('--output', '-o', default="AGENTS.md", help="Output file path (for single-file mode)")
@click.option('--dry-run', is_flag=True, help="Generate content without writing files")
...
@click.option('--trace', is_flag=True, help="🔍 Show detailed source attribution and primitive conflicts")
@click.option('--local-only', is_flag=True, help="🏠 Ignore dependencies, compile only local primitives")  
@click.option('--show-placement', is_flag=True, help="👀 Preview AGENTS.md placement without writing files")
@click.pass_context
def compile(ctx, output, dry_run, no_links, chatmode, watch, validate, with_constitution, 
           single_agents, trace, local_only, show_placement):
    """Compile APM primitives into distributed AGENTS.md files.
    ...
    Advanced options:
    • --trace: Show which primitives come from which sources  
    • --local-only: Ignore dependencies, compile only local .apm/ primitives
    • --show-placement: Preview where AGENTS.md files would be placed
    """
    try:
        ...
        _rich_info("Starting context compilation...", symbol="gear")
        ...
        if config.strategy == "distributed" and not single_agents:
            _rich_info("Using distributed compilation (multiple AGENTS.md files)", symbol="sparkles")
            if show_placement:
                _rich_info("Preview mode: showing placement without writing files", symbol="eye")
            if trace:
                _rich_info("Trace mode: showing detailed source attribution", symbol="magnifying_glass")
        else:
        ...
        if result.success:
            ...
            if config.strategy == "distributed" and not single_agents:
                # Distributed compilation results
                _rich_success("Distributed compilation completed successfully!", symbol="sparkles")
                
                if show_placement:
                    _rich_success("Placement preview completed", symbol="eye")
                elif not dry_run:
                    _rich_success(result.output_path, symbol="sparkles")
                
                # Show summary
                try:
                    console = _get_console()
                    if console and result.stats:
                        from rich.table import Table
                        
                        table = Table(title="🚀 Distributed Compilation Summary", show_header=True, header_style="bold cyan")
                        ...
                        table.add_row("AGENTS.md files", str(result.stats.get('agents_files_generated', 0)), "✨ Distributed across directories")
                        table.add_row("Instructions placed", str(result.stats.get('total_instructions_placed', 0)), "✅ All validated")
                        table.add_row("Patterns covered", str(result.stats.get('total_patterns_covered', 0)), "✅ Complete coverage")
                        table.add_row("Contexts", str(result.stats.get('contexts', 0)), "✅ All validated")
                        table.add_row("Chatmodes", str(result.stats.get('chatmodes', 0)), "✅ All validated")
                        
                        console.print(table)
                except Exception:
                    # Fallback display
                    _rich_info(f"Generated {result.stats.get('agents_files_generated', 0)} AGENTS.md files")
                    _rich_info(f"Placed {result.stats.get('total_instructions_placed', 0)} instructions")
                ...
                if intermediate_result.success:
                    ...
                    if dry_run:
                        _rich_success("Context compilation completed successfully (dry run)", symbol="sparkles")
                    else:
        ...
        # Common error and warning handling for both compilation modes
        if result.warnings:
...
def search(ctx, query, limit):
    ...
    try:
        _rich_info(f"Searching MCP registry for: {query}")
        ...
        if not servers:
            _rich_warning(f"No MCP servers found matching '{query}'")
            return
            
        _rich_info(f"Found {len(servers)} MCP servers:")
        ...
        if console:
            try:
                ...
                for server in servers:
                    name = server.get("name", "Unknown")
                    desc = server.get("description", "No description")
                    click.echo(f"  {STATUS_SYMBOLS['gear']} {name}")
                    click.echo(f"    {desc}")
        else:
            # Fallback to simple list
            for server in servers:
        ...
        _rich_blank_line()
        _rich_info("Use 'apm mcp show <name>' for more details", symbol="info")
...
def show(ctx, server_name):
    ...
    try:
        _rich_info(f"Getting details for: {server_name}")
        ...
        try:
            ...
            _rich_error(f"MCP server '{server_name}' not found in registry")
            _rich_info("Use 'apm mcp search <query>' to find available servers")
            sys.exit(1)
        
        # Display server information
        _rich_info(f"Server: {server_info.get('name', 'Unknown')}", symbol="info")
        _rich_echo(f"Description: {server_info.get('description', 'No description')}", style="white")
        
        # Show version if available
        if "version_detail" in server_info:
            version_detail = server_info["version_detail"]
            version = version_detail.get("version", "Unknown")
            _rich_echo(f"Latest Version: {version}", style="cyan")
            
        # Show repository info if available
        if "repository" in server_info:
            repo = server_info["repository"]
            repo_url = repo.get("url", "Unknown")
            _rich_echo(f"Repository: {repo_url}", style="muted")
        
        # Show packages if available  
        packages = server_info.get("packages", [])
        if packages:
            _rich_blank_line()
            _rich_info("Installation packages:", symbol="gear")
            for pkg in packages:
                registry_name = pkg.get("registry_name", "unknown")
                pkg_name = pkg.get("name", "unknown")
                _rich_echo(f"  {registry_name}: {pkg_name}", style="white")
        
        _rich_blank_line()
        _rich_info(f"To install: Add '{server_info.get('name', server_name)}' to your apm.yml dependencies", symbol="sparkles")
...
def list(ctx, limit):
    ...
    try:
        _rich_info("Fetching all available MCP servers...")
        ...
        if not servers:
            _rich_warning("No MCP servers found in registry")
            return

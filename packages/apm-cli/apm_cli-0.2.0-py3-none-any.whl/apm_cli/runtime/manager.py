...
class RuntimeManager:
    ...
    def remove_runtime(self, runtime_name: str) -> bool:
        ...
        if runtime_name == "copilot":
            try:
                click.echo(f"{Fore.YELLOW}⚠️  Copilot CLI is installed via npm globally{Style.RESET_ALL}")
                click.echo(f"{Fore.BLUE}ℹ️  To remove it, run: npm uninstall -g @github/copilot{Style.RESET_ALL}")
                return True
            except Exception as e:
                click.echo(f"{Fore.RED}❌ Failed to provide removal instructions for {runtime_name}: {e}{Style.RESET_ALL}", err=True)
                return False

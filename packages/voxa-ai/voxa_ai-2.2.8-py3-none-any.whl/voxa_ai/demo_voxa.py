...
from voxa_ai.agent import run_agent, validate_api_key
...
def main():
    ...
    # Prompt for API key or offline mode
    while True:
        ...
        if not key_input:
            ...
            if validate_api_key(key_input):
                ...
                # Proper invalid key handling
                invalid_panel = Panel(
                    Align.center("❌ Invalid API key. Switching to OFFLINE MODE.", vertical="middle"),
                    style="red",
                    padding=(2, 8),
                    border_style="red"
                )
                console.print(Align.center(invalid_panel))
                break  # fallback to offline mode

import traceback
from kaq_quant_common.kaq_quant_common.resources.kaq_mysql_resources import KaqQuantMysqlRepository
from kaq_quant_common.utils import yml_utils
from sqlalchemy import text

engine = KaqQuantMysqlRepository().get_conn_engine()

time_idx_dict = {
    'kaq_binance_perpetual_commission_rate_now' :  'ctimestamp',
}

def mysql_table_init(pkg_name='kaq_quant_common'):
    for table_name, timestamp in time_idx_dict.items():
        file_name = table_name + '.sql'
        try:
            file_path = yml_utils.get_mysql_script(pkg_name, file_name)

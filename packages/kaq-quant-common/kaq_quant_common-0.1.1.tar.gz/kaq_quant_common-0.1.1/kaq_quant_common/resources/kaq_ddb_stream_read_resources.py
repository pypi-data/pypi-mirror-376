...
class KaqCoinDdbStreamInitRepository:
    '''
    ...
    def __init__(self, host, port, user, passwd):
        ...
        try:
            ...
            logger.error(f'KaqCoinDdbStreamInitRepository.__init__ is occured error: {str(e)} - {str(traceback.format_exc())}')
        finally:
            mutex.release()
    ...
    def getStreamQueryCount(self, query: str) -> int:
        ...
        try:
            ...
            logger.error(f'KaqCoinDdbStreamInitRepository.getStreamQueryCount is occured error: {str(e)} - {str(traceback.format_exc())}')
        return -1
    ...
    def getStreamQuery(self, query: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f'KaqCoinDdbStreamInitRepository.getStreamQuery is occured error: {str(e)} - {str(traceback.format_exc())}')
        return pd.DataFrame()


if __name__ == '__main__':
    host, port, user, passwd = yml_utils.get_ddb_info(os.getcwd())
    kaq = KaqCoinDdbStreamInitRepository(host, port, user, passwd)
    # kaq.drop_streaming()

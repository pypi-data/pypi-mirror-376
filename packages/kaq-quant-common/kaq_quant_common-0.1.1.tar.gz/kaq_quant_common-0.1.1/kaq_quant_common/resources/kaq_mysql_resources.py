...
class KaqBtcMysqlRepository:
    ...
    def create_table(self, pre_fix_table_name: str, _datetime: datetime = datetime.now(), symbol: str = None):
        ...
        session = self.session_maker()
        try:
            file_path = yml_utils.get_mysql_script(os.getcwd(), file_name)
            ...
            print(f'【创建{new_table_name} - 表, KaqBtcMysqlRepository.create_table】失败, {str(e)} - {str(traceback.format_exc())}')
        finally:
            session.close()
        return False
...
def main(query='select * from kaq_binance_perpetual_klines_1h limit 5;'):
    ...
    kaqBtcMysqlRepository = KaqBtcMysqlRepository(host, port, user, passwd, database, charset)

    print(kaqBtcMysqlRepository.table_exists('kaq_binance_perpetual_klines_1h'))
    # print(kaqBtcMysqlRepository.fetch_data(query).head(3))
...
def test_db_pool():
    ...
    # 获取一个session
    session1 = kaqBtcMysqlRepository.session_maker()
    session1.execute(text("SELECT 1;")).mappings().all()
    # SHOW PROCESSLIST;  有1条连接

    # 由于上面没有close,这里不会复用上面的连接
    session2 = kaqBtcMysqlRepository.session_maker()
    ...
    session3 = kaqBtcMysqlRepository.session_maker()

...
from relationalai.early_access.dsl.codegen.relations import ValueMap, CompositeEntityMap, EntitySubtypeMap,\
    UnionEntityMap, AbstractEntityMap, SimpleConstructorEntityMap, ReferentEntityMap, _filtering_view, InlineValueMap,\
...
class Weaver:
    ...
    def _find_matching_entity_maps(entity_maps, concept: qb.Concept,
        ...
        for entity_map in entity_maps:
            ...
            concepts_match = (value_player is concept) if strict_match else concept._isa(value_player)
            if not concepts_match:
                continue
            if isinstance(source_or_col, BindableTable) and entity_map.table() == source_or_col:
                matching_entity_maps.add(entity_map)
            elif isinstance(source_or_col, BindableAttribute) and entity_map.column() is source_or_col:
                matching_entity_maps.add(entity_map)
    ...
    def _generate_unary_predicate_rule_body(self, concept_player: qb.Concept, binding: Binding, relationship):
        ...
        for entity_map in entity_maps:
            ...
            entity_ref, subformula_atoms = entity_map.formula(row_ref)
            where(
                *subformula_atoms,
                _filtering_view(row_ref, binding.filter_by)
    ...
    def _generate_nary_predicate_rule(self, bound_relationship: BoundRelationship):
        ...
        for role_maps in role_map_combinations:
            ...
            var_refs, subformulas = zip(*(rm.formula(row_ref) for rm in role_maps))
            where(

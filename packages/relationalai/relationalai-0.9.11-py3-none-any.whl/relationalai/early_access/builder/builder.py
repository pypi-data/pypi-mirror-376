...
from relationalai.early_access.sql.executor.snowflake import SnowflakeExecutor
...
class Concept(Producer):
    ...
    def filter_by(self, args: Any|None=None, **kwargs: Any) -> ConceptConstruct:
...
class Data(Producer):
    ...
    def into(self, concept:Concept, keys:list[str]=[]):
        if keys:
            new = concept.filter_by(**{k.lower(): getattr(self, k) for k in keys})
        else:
            new = concept.filter_by(self._row_id)
        ...
        if isinstance(item, ConceptExpression):
            ...
            if isinstance(item, ConceptMember):
                ...
                if types.is_value_type(op_type):
                    ...
                    new_out = f.var(helpers.sanitize(to_name(item._op)), op_type)

...
def print_cst(**kwargs) -> list[str]:
    ...
    return [
        "COLORS = list(mcolors.TABLEAU_COLORS.values())  # list of colors to be plotted",
        f"DATABASE = pathlib.Path({str(kwargs['database'])!r})",
        f"FIGNAME = pathlib.Path('{'_'.join(kwargs['x'])}_as_a_function_of_{'_'.join(kwargs['y'])}.pdf')",
    ...
    match (kwargs["color"] is None, kwargs["marker"] is None):
        ...
        case (False, False):  # both color and marker
            code.extend([
                ...
                f'        axe.errorbar(',
                "            x_data, y_data,",
                # f'            label=f"{kwargs['color']}={{color_label}} {kwargs['marker']}={{marker_label}}", ',
                "            color=color,",
                "            fmt=marker,",
                "            capsize=3,",
                "            markersize=4",
...
def print_selector(**kwargs) -> list[str]:
    ...
    names = sorted({
        n.id for n in ast.walk(ast.parse(kwargs["filter"], mode="eval")) if isinstance(n, ast.Name)
    })
    if len(names) == 1:
...
def print_subplot(**kwargs) -> list[str]:
    """Return the code for the subplot of a a given y and y axis."""
    header = [
        f"def plot_{kwargs['ylabel']}_{kwargs['xlabel']}(values: dict[str]):",
    ...
    match (kwargs["window_y"] is None, kwargs["window_x"] is None):
        case (True, True):
           middle = [
                ...
                "    fig, axe = plt.subplots(sharex=True, figsize=(8, 8))",
        ...
        case (False, True):
            middle = [
                ...
                "    fig, axes = plt.subplots(nrows=nrows, sharex=True, figsize=(8, 8))",
                "",
        ...
        case (True, False):
            middle = [
                ...
                "    fig, axes = plt.subplots(ncols=ncols, sharey=True, figsize=(8, 8))",
                "",
                "    # iterate over all subplots",
        ...
        "    if labels:",
        "        fig.legend(lines, labels, loc='upper center', ncols=min(4, len(labels)))",
        f"    fig.supylabel({get_label_extractor(kwargs['ylabel'])[0]!r})",
        f"    fig.supxlabel({get_label_extractor(kwargs['xlabel'])[0]!r})",
        "",
        "",
    ]
...
def print_main(**kwargs) -> list[str]:
    ...
    for ylabel, xlabel in itertools.product(kwargs["y"], kwargs["x"]):
        code.extend(print_subplot(**kwargs, ylabel=ylabel, xlabel=xlabel))

    # main function
    code.extend([
        "def main(values: dict[str]):",
        '    """Create the plotss with matplotlib."""',
    ])
    for (i, ylabel), (j, xlabel) in itertools.product(
        enumerate(kwargs["y"]), enumerate(kwargs["x"])
    ):
        code.extend([
            "    # TODO: display in a grid rather than creating multiple windows",
            f"    plot_{ylabel}_{xlabel}(values)  # in position rows={i+1}, cols={j+1}",
        ])
    code.extend([
...
def print_read_sql(**kwargs) -> list[str]:
    ...
    if kwargs["filter"] is not None:
        names |= {
            n.id for n in ast.walk(ast.parse(kwargs["filter"], mode="eval"))
    ...
    select = sorted({s for f in funcs.values() for s in f.select})
    if len(table := {f.table for f in funcs.values()}) != 1:
        logging.warning("invalid sql query because select from 2 differents tables")
        table = ", ".join(sorted(table))
    else:
        table = table.pop()
    join = sorted({j for f in funcs.values() for j in f.join})
    query = SqlLinker(select, table, join).sql

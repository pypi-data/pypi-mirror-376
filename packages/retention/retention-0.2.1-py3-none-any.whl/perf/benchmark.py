...
PROFILABLE_RUNS = {
    'sdpa': SDPA.make_run,
    'power_retention_triton': PowerFullTriton.make_run,
    'power_retention_vidrial': PowerFullVidrial.make_run,
    'power_retention_fused': PowerFullFused.make_run,
    ...
    'retention_cuda': PowerAttentionCuda.make_run,
    'retention_triton': PowerAttentionTriton.make_run,
...
KERNEL_ALIASES = {
    'power_retention': 'power_retention_triton',
    'query_state': 'query_state_triton',
    'update_state': 'update_state_triton',
    'retention': 'retention_triton',
}
...
def benchmark_all_kernels(config: BenchmarkConfig, mode: str, impl: str = 'default', title: str = '') -> None:
    ...
    # Run all kernels
    power_retention_fused = run_kernel('power_retention_fused', config, mode)
    power_retention_triton = run_kernel('power_retention_triton', config, mode)
    ...
    power_retention_vidrial = run_kernel('power_retention_vidrial', config, mode)
    ...
    chunked_attention_triton = run_kernel('retention_triton', chunked_config, mode)
    
    # power_attn_triton = run_kernel('retention_triton', config, mode)
    power_attn_cuda = run_kernel('retention_cuda', config, mode)
    ...
    data = {
        ...
        'Total (ms)': [
            power_retention_triton, power_retention_vidrial, power_retention_fused,
            0, 0, 0,
            power_attn_cuda,
            sdpa, flash_attn,
        ]
...
def plot_throughput_by_ctx(base_config: BenchmarkConfig, ts: list[int], mode: str, impl: str = 'default'):
    ...
    for t in ts:
        ...
        data['retention_triton'].append(run_kernel('retention_triton', config, mode))
        data['sdpa'].append(run_kernel('sdpa', config, mode))
        data['flash_attn'].append(run_kernel('flash_attn', config, mode))
        data['retention_cuda'].append(run_kernel('retention_cuda', config, mode))
        data['power_retention'].append(run_kernel('power_retention_triton', config, mode))
        data['power_retention_vidrial'].append(run_kernel('power_retention_vidrial', config, mode))

    df = pd.DataFrame(data)
    for kernel in ['retention_triton', 'sdpa', 'flash_attn', 'retention_cuda', 'power_retention', 'power_retention_vidrial']:
        df[kernel] = df['tokens'] / df[kernel]
    del df['tokens']
    
    df = df.set_index(['ctx'])
    if impl == 'default':
        df = df.drop(columns=['power_retention_vidrial', 'retention_cuda', 'retention_triton'])
...
Examples:
  ...
  python benchmark.py power_retention_fused bwd

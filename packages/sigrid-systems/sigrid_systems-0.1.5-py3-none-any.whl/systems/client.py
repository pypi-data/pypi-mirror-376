        ...
        try:
            async with httpx.AsyncClient(timeout=self._config.timeout) as client:
                async with client.stream(
                    ...
                    # Check status before streaming - for streaming responses, check status_code directly
                    if not response.is_success:
                        error_msg = f"HTTP {response.status_code} error"
                        response_text = b""
                        
                        # For streaming responses, we can't read the error body
                        # Just use the status code to provide a meaningful error message
                        if response.status_code >= 500:
                            error_msg = f"HTTP {response.status_code} error (server error or cold start timeout)"
                        elif response.status_code == 429:
                            error_msg = f"HTTP {response.status_code} error (rate limit exceeded)"
                        elif response.status_code == 401:
                            error_msg = f"HTTP {response.status_code} error (authentication failed)"
                        elif response.status_code == 403:
                            error_msg = f"HTTP {response.status_code} error (forbidden)"
                        else:
                            error_msg = f"HTTP {response.status_code} error"

...
====
    ...
    enable_data_aging()         --  method to enable/disable the data aging for the plan copy
            ...
            if response.json() and 'plan' in response.json():
                ...
                if 'schedule' in self._plan_properties:
                    if 'task' in self._plan_properties['schedule']:
                        self._commcell_object.schedule_policies.refresh()
                        task_name = self._plan_properties.get('schedule', {}).get('task', {}).get('taskName', '')
                        self._child_policies['schedulePolicy']['data'] = (
                                self._commcell_object.schedule_policies.get(task_name)
                                if self._commcell_object.schedule_policies.has_policy(task_name)
                                else None
                            )
                        ...
                        if self._subtype == 33554437 and self._plan_properties.get('database', {}).get('rpoInMinutes'):
                            task_name = self._plan_properties.get('database', {}).get('scheduleLog', {}).get('task', {}).get('taskName', '')
                            self._child_policies['schedulePolicy']['log'] = (
                                self._commcell_object.schedule_policies.get(task_name)
                                if self._commcell_object.schedule_policies.has_policy(task_name)
                                else None
                            )

...
def build_parent_did_panel(
    ...
    *,
    ...
    base = match_df.join(relations_df, on="PNR", how="inner")
    ...
    # Attach parent IDs to the income panel
    income_wide = income_wide.join(
    ...
    panel = pl.concat(long_parts, how="vertical")
    ...
    if uddf_file_path is not None:
        ...
        edu_f = extract_highest_education_level(
            df=fathers.rename({"_ID": "PNR"}),
            identifier_col="PNR",
            index_date_col="INDEX_DATE",
            uddf_file_path=uddf_file_path,
        ).rename({"highest_education_level": "father_education"})
        edu_f = (
            edu_f.join(fathers, on=["PNR", "INDEX_DATE"], how="left")
            .select(
                [
                    "PNR",
                    "INDEX_DATE",

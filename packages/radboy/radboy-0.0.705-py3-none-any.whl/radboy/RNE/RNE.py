...
{Style.reset}'''
		while not init_only:
			...
			if doWhat in [None,]:
				...
				self.diff()
			elif doWhat.lower() in ['countdown','cntdwn','count-']:
				self.countDown()
	...
	def countDown(self):
		toDate=Control(func=FormBuilderMkText,ptext="Destination Date",helpText="Stop Date",data="datetime")
		if toDate in [None,'NaN']:
			return
		now=datetime.now()
		diff=toDate-now
		percentTo=decc((diff)/timedelta(days=1)/14)*100
		msg=f"{now} -> {toDate} = {diff} / {percentTo}%"
		print(std_colorize(msg,0,1))

...
def test_invalid_color_code() -> None:
    """Test that ValueError is raised for invalid color code format."""
    with pytest.raises(ValueError) as excinfo:
...
def test_invalid_color_code_length() -> None:
    """Test that ValueError is raised for incorrect color code length."""
    with pytest.raises(ValueError) as excinfo:

...
2. **Pull-based updates**: Users explicitly request config updates
3. **Version tracking**: Each config has a version number
4. **Diff visibility**: Show changes before applying
5. **Cache management**: Automatic pre-commit cache invalidation
...
class ConfigTemplateService:
    ...
    def apply_update(self, config_type: str, project_path: Path, 
...
2. Convert existing configs to template format
...
2. Integrate cache management with updates
3. Add config version tracking file
4. Update documentation

### Phase 3: Deprecate Old System (Week 3)
1. Mark ConfigMergeService as deprecated
2. Remove `_copy_config_files_to_package` from PhaseCoordinator
3. Update InitializationService to use new system
4. Add migration warnings

### Phase 4: Remove Old System (Week 4)
1. Delete ConfigMergeService
2. Clean up all smart merge code
3. Remove package-level config templates
...
2. **User control**: Updates only when explicitly requested
3. **Version visibility**: Clear tracking of config versions
4. **Cache freshness**: Pre-commit cache automatically managed
...
2. **Risk**: Complex diff generation
...
3. Begin implementation with ConfigTemplateService

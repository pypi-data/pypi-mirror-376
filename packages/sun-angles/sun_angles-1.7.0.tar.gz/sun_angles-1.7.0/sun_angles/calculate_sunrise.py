...
def calculate_sunrise(
    ...
    """
    Calculate the local hour of sunrise for a given day and location.

    This function flexibly computes the sunrise hour based on the information provided:

    - If `SHA_deg` (sunrise hour angle in degrees) is provided, it is used directly to compute the sunrise hour.
    ...
    -------
    Union[Raster, np.ndarray]
        Local hour of sunrise for the given inputs. The output type matches the input type (e.g., float, array, or Raster).

    Examples
    --------
    1. Provide SHA directly:
        sunrise_hour = calculate_sunrise(SHA_deg=100)
    2. Provide day of year and latitude:
        sunrise_hour = calculate_sunrise(day_of_year=172, lat=34.0)
    3. Provide UTC time and latitude:
        sunrise_hour = calculate_sunrise(time_UTC='2025-06-21', lat=34.0)
    4. Provide geometry and UTC time:
        sunrise_hour = calculate_sunrise(time_UTC='2025-06-21', geometry=my_geometry)
    """
    ...
    # Compute sunrise hour from SHA_deg
    # SHA_deg is the sunrise hour angle in degrees. The local hour of sunrise is:
    # sunrise_hour = 12 - (SHA_deg / 15)
    sunrise_hour = 12 - (np.asarray(SHA_deg) / 15.0)

    # If input was a Raster, preserve type
    if isinstance(SHA_deg, Raster):
        sunrise_hour = SHA_deg.copy_with_new_data(sunrise_hour)

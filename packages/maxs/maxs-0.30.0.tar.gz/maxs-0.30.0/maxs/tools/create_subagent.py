...
Model Selection:
- Use us.anthropic.claude-3-5-haiku-20241022-v1:0 for lightweight tasks
...
- Available tools include: current_time,editor,environment,file_read,file_write,http_request,load_tool,python_repl,retrieve,shell,slack,think,use_llm,calculator,use_aws,generate_image,image_reader,memory,swarm,workflow
- Examples: "file_read,file_write,shell" or "python_repl,calculator,http_request"

Usage Examples:
```python
from strands import Agent
from protocol_tools import create_subagent
...
def create_subagent(
    ...
    Example:
        ...
        result = create_subagent(
            ...
            provider="bedrock",
            max_tokens=10000,
            tools="think,use_llm,retrieve,shell,retrieve",
        )
    ...
    try:
        ...
        if action == "create":
            ...
            if result["success"]:
                ...
                content.append(
                    {
                        "text": 'To check the status of a specific run, use: create_subagent(action="status", repository="owner/repo", workflow_id="workflow.yml", run_id="RUN_ID")'
                    }
                )

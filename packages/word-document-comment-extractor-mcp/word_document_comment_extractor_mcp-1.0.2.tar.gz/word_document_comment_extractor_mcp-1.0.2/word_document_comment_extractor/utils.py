...
def get_comments_for_paragraph_by_index(comments: List[Dict[str, Any]], paragraph_index: int) -> List[Dict[str, Any]]:
    """
...
def get_comment_statistics(comments: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not comments:
        return {
            ...
            'authors': [],
            'comments_by_author': {},
            'comments_with_dates': 0,
            'comments_in_tables': 0
        }
    ...
    for comment in comments:
        ...
        if author not in comments_by_author:
            comments_by_author[author] = 0
        comments_by_author[author] += 1
        
        if comment.get('date'):
            comments_with_dates += 1
        
        if comment.get('in_table', False):
            comments_in_tables += 1
    
    return {
        'total_comments': len(comments),
        'authors': sorted(list(authors)),
        'comments_by_author': comments_by_author,
        'comments_with_dates': comments_with_dates,
        'comments_in_tables': comments_in_tables
    }

...
from lionweb.language import (
    Language, Concept, Property, Reference, PrimitiveType, Annotation, LionCoreBuiltins, Enumeration,
    EnumerationLiteral, Interface, Containment
)
...
from starlasuspecs.utils import id_for_contained_element, key_for_contained_element, default_id, default_key
...
def _add_placeholder_node_annotation(language: Language, ast_node: Concept):
    ...
    annotation_type = Enumeration(lion_web_version=lw_version, name="PlaceholderNodeType")
    language.add_element(annotation_type)
    annotation_type.id = f"{placeholder_node_id.removesuffix('-id')}-PlaceholderNodeType-id"
    annotation_type.key = f"{placeholder_node.key.removesuffix('-key')}-PlaceholderNodeType-key"

    missing_ast_transformation = EnumerationLiteral(
        lion_web_version=lw_version, enumeration=annotation_type, name="MissingASTTransformation")
    ...
    failing_ast_transformation = EnumerationLiteral(
        lion_web_version=lw_version, enumeration=annotation_type, name="FailingASTTransformation")
...
def ast_language() -> Language:
    ...
    ast_node_transpiled_nodes = Reference(lion_web_version=lw_version, name="transpiledNodes")
    ...
    for name in [
        "BehaviorDeclaration", "Documentation", "EntityDeclaration",
        "EntityGroupDeclaration", "Expression", "Parameter",
        "PlaceholderElement", "Statement", "TypeAnnotation"
    ]:
    ...
    issue_type_enum = Enumeration(lion_web_version=lw_version, language=language, name="IssueType")
    ...
    issue_severity_enum = Enumeration(lion_web_version=lw_version, language=language, name="IssueSeverity")
    ...
    parsing_result_tokens.type = LionCoreBuiltins.get_string(lion_web_version=lw_version)
...
def ast_language_json():
    jsonser = create_standard_json_serialization(lion_web_version=LIONWEB_VERSION_USED_BY_STARLASU)

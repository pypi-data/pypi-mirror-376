...
export POLARION_URL="" # Link to Polarion server REST API endpoint (i.e. https://polarion.stage.engineering.redhat.com/polarion/rest/v1)
...
export POLARION_VERIFY_SSL={true or false}
	...
	from rest_api_polarion_client.api.documents.get_document import sync as get_document
...
- Verify by installing from TestPyPI in a clean venv (falls back to PyPI for dependencies):
        ...
        python - <<'PY'
        import rest_api_polarion_client as gen, polarion_rest_client as hl
...
- If you see packaging issues, rebuild clean:
        ...
        rm -rf dist && pdm run prepare-build && pdm build

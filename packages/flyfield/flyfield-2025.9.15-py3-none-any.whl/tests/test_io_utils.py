import pytest
from flyfield.io_utils import load_boxes_from_csv, write_csv, read_csv_rows, save_pdf_form_data_to_csv
...
def test_remove_duplicates_across_points():
    points = [
        ...
        {"page_num": 1, "x0": 0.0001, "y0": 0, "x1": 10, "y1": 10},  # should be duplicate
...
def test_write_csv_handles_missing_block_fill(tmp_path):
    points = [{
        "page_num": 1, "id": 2, "x0": 5, "y0": 5, "x1": 15, "y1": 20,
        "left": 5, "top": 20, "right": 15, "bottom": 5,
        "pgap": "", "gap": "",
        "line": 2, "block": 2, "block_length": 1, "block_width": 10,
        "code": "2-2-2", "field_type": "DollarCents", "chars": "M",
        "fill": "1 234 56"
    }]
...
def test_read_csv_rows_skips_bad_numeric_rows(tmp_path):
    CSV_HEADER = [
        "page_num", "id", "x0", "y0", "x1", "y1", "left", "top", "right", "bottom",
        "height", "width", "pgap", "gap", "line", "block", "block_length", "block_width",
        "code", "field_type", "chars", "fill"
    ]
    lines = [
        ",".join(CSV_HEADER),
...
@pytest.mark.parametrize("text,field_type,expected", [
    ("S", None, True),
    (".00", "Dollars", True),
    ("nope", None, False),
])

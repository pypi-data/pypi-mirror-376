...
# -------------------------
# Interface Gradio (utilisation du modèle)
# -------------------------
def _build_gradio_blocks():
    try:
        ...
        raise RuntimeError(
            "Gradio n'est pas installé. Fais `pip install mini_imggen_numpy_lib[demo]`."
        )

    # Génération d'image
    def _gen_image(model_dir, prompt, size, mode, temperature, prefix_img, prefix_ratio):
        import tempfile, os
        tmpdir = tempfile.mkdtemp(prefix="mini_imggen_")
        out_path = os.path.join(tmpdir, "gen.png")
        prefix_path = prefix_img if prefix_img else None
        try:
            path = generate_image_from_model(
            ...
            return path, f"✅ Image générée avec {model_dir}"
        except Exception as e:
            return None, f"❌ Erreur: {e}"

    # Génération de texte
    def _gen_text(model_dir, prompt, length, temperature):
        try:
            text = generate_text_from_model(
                model_dir=model_dir.strip(),
                prompt=prompt or "",
                length=int(length),
                temperature=float(temperature),
    ...
    with gr.Blocks(title="mini_imggen_numpy_lib — Utilisation du modèle") as demo:
        gr.Markdown("# 🖼️ mini_imggen_numpy_lib — Démo modèle\n"
                    "Charge un **dossier modèle** (ckpt/) contenant "
                    "`model_weights.npz` + `vocab.json` (ou `text_model.npz` + `text_vocab.json`).")

        with gr.Tab("🎨 Génération d'image"):
            with gr.Row():
                model_dir = gr.Textbox(label="📂 Dossier modèle (image)", value="ckpt")
            with gr.Row():
                prompt = gr.Textbox(label="Prompt", placeholder="ex: chat tigré")
            with gr.Row():
                size = gr.Slider(16, 64, value=32, step=1, label="Taille")
                mode = gr.Radio(["quick", "zero"], value="quick", label="Mode init")
                temperature = gr.Slider(0.2, 2.0, value=1.0, step=0.05, label="Température")
            with gr.Row():
                prefix_img = gr.Image(type="filepath", label="Image prefix (optionnel)")
                prefix_ratio = gr.Slider(0.0, 0.9, value=0.1, step=0.05, label="Ratio prefix")

            gen_btn = gr.Button("🚀 Générer une image")
            out_img = gr.Image(label="Image générée")
            out_log = gr.Textbox(label="Logs", lines=3)

            gen_btn.click(
                _gen_image,
                inputs=[model_dir, prompt, size, mode, temperature, prefix_img, prefix_ratio],
                outputs=[out_img, out_log],
            )

        with gr.Tab("✍️ Génération de texte"):
            with gr.Row():
                model_dir_t = gr.Textbox(label="📂 Dossier modèle (texte)", value="ckpt")
            with gr.Row():
                prompt_t = gr.Textbox(label="Prompt", value="bonjour le monde")
            with gr.Row():
                length_t = gr.Slider(5, 100, value=30, step=1, label="Longueur")
                temperature_t = gr.Slider(0.2, 2.0, value=1.0, step=0.05, label="Température")

            gen_text_btn = gr.Button("📝 Générer du texte")
            out_text = gr.Textbox(label="Texte généré", lines=6)

            gen_text_btn.click(
                _gen_text,
                inputs=[model_dir_t, prompt_t, length_t, temperature_t],
                outputs=[out_text],
            )
...
def launch_gradio(server_name="0.0.0.0", server_port=7860, share=False):
    """
    Lance l'interface Gradio (utilisation d'un modèle existant).
    """

...
# === Gradio UI helpers ===
# =========================
def launch_gradio_image_demo(default_model_dir: str = "ckpt", default_size: int = 32):
    """
    Lance une mini UI Gradio pour générer une image depuis un prompt texte.

    Utilisation rapide :
        import mini_imggen_numpy_lib as m
        m.launch_gradio_image_demo("ckpt", 32).launch()
    """
    try:
        import gradio as gr
    except ImportError as e:
        raise ImportError(
            "Gradio n'est pas installé. Installez-le avec :\n"
            "  pip install 'mini_imggen_numpy_lib[ui]'  (ou pip install gradio)"
        ) from e

    import tempfile
    from PIL import Image as _PILImage

    def _gen(prompt, model_dir, size, mode, temperature, prefix_img, prefix_ratio):
        if not prompt:
            raise gr.Error("Veuillez saisir un prompt.")
        if not model_dir:
            raise gr.Error("Veuillez indiquer un dossier de modèle (ex: ckpt).")

        tmp_out = tempfile.mktemp(suffix=".png")
        prefix_path = prefix_img if prefix_img else None

        out_path = generate_image_from_model(
            model_dir=model_dir,
            prompt=prompt,
            out_path=tmp_out,
            size=int(size),
        ...
        return _PILImage.open(out_path)

    with gr.Blocks() as demo:
        gr.Markdown("## mini_imggen_numpy_lib — Génération d'images (toy)")
        with gr.Row():
            prompt = gr.Textbox(label="Prompt texte", placeholder="ex: chat tigré")
        with gr.Row():
            model_dir = gr.Textbox(label="Dossier du modèle", value=default_model_dir)
            size = gr.Slider(16, 64, value=default_size, step=1, label="Taille (côté)")
        with gr.Row():
            mode = gr.Radio(["quick", "coldstart"], value="quick", label="Mode")
            temperature = gr.Slider(0.1, 2.0, value=1.0, step=0.1, label="Température")
        with gr.Row():
            prefix_img = gr.Image(label="Image de préfixe (optionnel)", type="filepath")
            prefix_ratio = gr.Slider(0.01, 0.9, value=0.1, step=0.01, label="Ratio de préfixe")
        go = gr.Button("Générer")
        out = gr.Image(label="Sortie")

        go.click(
            fn=_gen,
            inputs=[prompt, model_dir, size, mode, temperature, prefix_img, prefix_ratio],
            outputs=[out],
        )
    return demo


def launch_gradio_text_demo(default_model_dir: str = "ckpt_text"):
    """
    Lance une mini UI Gradio pour générer du texte avec ton modèle texte.

    Utilisation rapide :
        import mini_imggen_numpy_lib as m
        m.launch_gradio_text_demo("ckpt_text").launch()
    """
    try:
        import gradio as gr
    except ImportError as e:
    ...
    def _gen(prompt, model_dir, length, temperature):
        if not prompt:
            raise gr.Error("Veuillez saisir un prompt.")
        if not model_dir:
            raise gr.Error("Veuillez indiquer un dossier de modèle texte.")
    ...
    with gr.Blocks() as demo:
        gr.Markdown("## mini_imggen_numpy_lib — Génération de texte (toy)")
        with gr.Row():
            prompt = gr.Textbox(label="Prompt")
            model_dir = gr.Textbox(label="Dossier du modèle texte", value=default_model_dir)
        with gr.Row():
            length = gr.Slider(5, 200, value=30, step=1, label="Longueur")
            temperature = gr.Slider(0.1, 2.0, value=1.0, step=0.1, label="Température")
        go = gr.Button("Générer")
        out = gr.Textbox(label="Sortie")

        go.click(fn=_gen, inputs=[prompt, model_dir, length, temperature], outputs=[out])
    return demo

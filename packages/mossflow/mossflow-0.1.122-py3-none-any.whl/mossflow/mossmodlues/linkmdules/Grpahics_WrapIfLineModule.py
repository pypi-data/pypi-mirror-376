...
class Grpahics_WrapIfLineModule(Graphics_ValueModule):
    ...
    zhname = '判断分支'
    enname = 'IfLine'
    def __init__(self,A:Graphics_ValueModule=None,B:Graphics_ValueModule=None,C:Graphics_ValueModule=None,KeyA:str=None,KeyB:str=None,KeyC:str=None,name:str='WrapLine',message=None):
    ...
    def from_userdefined(cls,master,x:int=0,y:int=0,name:str='ValueModule',message=None):
        ...
        toplevel.bind('<Return>', lambda event: on_submit())
    ...
    def from_json(cls, json_data ,modlues,message=None):
        ...
        y = json_data.get('y', 0)
        # 从JSON数据中获取x和y坐标
        name = json_data.get('name', 'WrapIfLine')
        # 从JSON数据中获取name
        ...
        KeyC = json_data.get('KeyC', 'lastrunstatus')
        # 从JSON数据中获取A和B的值
        linemodule= cls(A=modlues[A],B=modlues[B],C=modlues[C],KeyA=KeyA,KeyB=KeyB,KeyC=KeyC,name=name,message=message)
        linemodule.x = x
        linemodule.y = y
        ...
        linemodule.text = json_data.get('text', 'WrapIfLine')
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                if data.get("class") != self.__class__.__name__:
                    messagebox.showerror("Load Error", "The selected file is not a valid WrapIfLineModule file.")
                    return
                # 只加载可序列化的属性
                try:
                    x = data.get("x", self.x)

...
from mossflow.libs.pcl.pointscloud import getrangeimageformpointscloud,getgirdimageformpointscloud
class Graphics_GetRangeImageFromPoints(Graphics_ValueModule):
    rawname = 'getrangeimagefrompoints'
    zhname = '获取点云深度图'
    enname = 'Get Range Image From Points'
    def __init__(self,x:int=0,y:int=0,name:str='GetMinMax3D',message=None):
        super().__init__(x,y,name,message)
        self.texts['tab4']='Function'
        self.linemodule = False
        self.parameters={'pointscloud':None,'rangeimage':None,'step':0.01,'function':'getgirdimageformpointscloud','lastrunstatus':self.lastrunstatus}
    ...
    def run(self):
        ...
        try:
            if self.parameters['function']=='getrangeimageformpointscloud':
                self.parameters['rangeimage'] = getrangeimageformpointscloud(self.parameters['pointscloud'])
            elif self.parameters['function']=='getgirdimageformpointscloud':
                self.parameters['rangeimage'] = getgirdimageformpointscloud(self.parameters['pointscloud'],self.parameters['step'])
    ...
    def show_parameter_page(self,x,y,parent):
            ...
            def change_language():
                ...
                notebook.tab(tab4, text=self.texts['tab4'])
            ...
            function_commbox = ttk.Combobox(tab4,values= ['getrangeimageformpointscloud','getgirdimageformpointscloud'], state='readonly')
            function_commbox.bind('<<ComboboxSelected>>', lambda event: self.parameters.update({'function':function_commbox.get()}), add=True)
            function_commbox.grid(row=0, column=0, padx=10, pady=10,columnspan=2, sticky='w')
            step_label = tk.Label(tab4, text='step:')
            step_label.grid(row=1, column=0, padx=10, pady=10, sticky='w')
            step_entry = tk.Entry(tab4)
            step_entry.insert(0, self.parameters['step'])
            step_entry.grid(row=1, column=1, padx=10, pady=10, sticky='w')
            step_entry.bind('<Return>', lambda event: self.parameters.update({'step':float(step_entry.get())}), add=True)
            check_language()        
            update_io()
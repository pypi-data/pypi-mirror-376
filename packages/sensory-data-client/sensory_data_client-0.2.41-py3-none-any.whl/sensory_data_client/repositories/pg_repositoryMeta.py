...
class MetaDataRepository:
    ...
    async def list_by_owner(self, owner_id: UUID, updated_since: Optional[float] = None,
                            limit: Optional[int] = None, offset: int = 0) -> list[DocumentInDB]:
        async with get_session(self._session_factory) as session:
            from sensory_data_client.db.documents.document_orm import DocumentORM
            from sqlalchemy import select, desc
            q = select(DocumentORM).where(DocumentORM.owner_id == owner_id).order_by(desc(DocumentORM.created)).offset(offset)
            if updated_since:
                q = q.where(DocumentORM.edited > func.to_timestamp(updated_since))
            if limit:
                q = q.limit(limit)

...
class RefreshTokenORM(Base):
    ...
    token_hash: Mapped[str] = mapped_column(String(128), unique=True, index=True, nullable=False)  # sha256(refresh_raw)
    ...
    # Цепочка ротации — старый токен может ссылаться на новый (необязательно)
    replaced_by_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("refresh_tokens.id", ondelete="SET NULL"), nullable=True)

...
class DataClient:
    ...
    def __init__(
        ...
        rrepo: RefreshTokenRepository  | None = None,
...
######################## AUDIO
    ...
    async def aclose(self):
        """Закрыть все сетевые клиенты/пулы + движок БД."""
        errors = []

        async def _maybe_await(ret):
            if asyncio.iscoroutine(ret):
                return await ret
            return ret

        # elastic (часто именно он держит aiohttp.ClientSession)
        try:
            if self.es:
                for name in ("aclose", "close", "shutdown"):
                    fn = getattr(self.es, name, None)
                    if fn:
        ...
        # minio (обычно sync, на всякий случай)
        try:
            if self.minio and hasattr(self.minio, "close"):
                self.minio.close()
        except Exception as e:
            errors.append(f"minio: {e}")

        # остальные репозитории — по возможности
        for repo_name in ("metarepo", "linerepo", "imagerepo", "user_repo",
                          "group_repo", "billing_repo", "permission_repo",
                          "audio_repo", "tag_repo", "autotagrepo"):
        ...
        if errors:
            logger.warning("DataClient.aclose finished with issues: %s", "; ".join(errors))
...
try:
    ...
    # Core models
    from anomavision import Padim, PatchCore

    # Datasets
    from anomavision import MVTecDataset, AnodetDataset

    # Feature extraction
    from anomavision import ResnetEmbeddingsExtractor

    # All utility functions that scripts commonly import
    from anomavision.utils import (
        # Image processing utilities
        to_batch, pytorch_cov, mahalanobis,
        standard_image_transform, standard_mask_transform,
        create_image_transform, create_mask_transform,
        image_score, classification,
        split_tensor_and_run_function,

        # Logging and configuration
        get_logger, setup_logging,
        merge_config, save_args_to_yaml,

        # The specific function that was causing the import error
    ...
    from anomavision import (
        visualize_eval_data, visualize_eval_pair, optimal_threshold
    )

    # Import modules as modules (for module-level imports)
    from anomavision import utils, visualization, test
    ...
    main_modules = [
        'config', 'general', 'utils', 'padim', 'padim_lite', 'patch_core',
        'feature_extraction', 'mahalanobis', 'test', 'visualization'
    ]

    for module_name in main_modules:
        try:
            # Import the actual module
            anomavision_module = __import__(f'anomavision.{module_name}', fromlist=[module_name])
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                sys.modules['anodet.inference.model.wrapper'] = anomavision.inference.model.wrapper
            ...
            try:
                ...
                sys.modules['anodet.inference.model.backends'] = anomavision.inference.model.backends
                print("✓ Mapped anodet.inference.model.backends")

                # Map all individual backend modules
                backend_modules = [
                    'base', 'torch_backend', 'onnx_backend', 'openvino_backend',
                    'tensorrt_backend', 'torchscript_backend'
                ]

                for backend in backend_modules:
                    try:
                        backend_module = __import__(f'anomavision.inference.model.backends.{backend}', fromlist=[backend])
                        sys.modules[f'anodet.inference.model.backends.{backend}'] = backend_module
                        print(f"✓ Mapped anodet.inference.model.backends.{backend}")
                    except ImportError as e:
                        print(f"⚠ Could not map anodet.inference.model.backends.{backend}: {e}")
    ...
    try:
        ...
        for viz_module in viz_submodules:
            try:
                viz_submodule = __import__(f'anomavision.visualization.{viz_module}', fromlist=[viz_module])
    ...
    try:
        ...
        for sub in sampling_submodules:
            try:
                submodule = __import__(f'anomavision.sampling_methods.{sub}', fromlist=[sub])
...
__all__ = [
    ...
    'get_logger', 'setup_logging', 'merge_config', 'save_args_to_yaml',
...
if hasattr(sys, 'ps1'):  # Interactive session
    ...
    print("   Quick migration: https://github.com/DeepKnowledge1/AnomaVision#migration\n")

...
class sysHelper:
    ...
    def run_tasks(task_func, task_params, max_workers=10, print_info=True, thread_mode=True, process_mode=False,
        ...
        if print_info:
            print("任务开始", task_func.__name__)
            print("任务数量", task_params.__len__())
        with process_or_thread_pool as executor:
            ...
            while True:
                ...
                if print_info:
                    print(f"完成进度: {all_futures - unfinished_count}/{all_futures}")
            ...
            if print_info:
                ...
                print('执行完成', f"总数: {all_num}", f"成功: {success_num}", f"失败: {except_num}")
            return success, results, exceptions

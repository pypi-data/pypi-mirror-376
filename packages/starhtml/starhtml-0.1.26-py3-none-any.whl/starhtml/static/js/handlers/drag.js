...
const dragAttributePlugin = {
  ...
  onLoad(ctx) {
    ...
    const config = {
      signal: window.__starhtml_drag_config?.signal ?? "drag",
      mode: window.__starhtml_drag_config?.mode ?? "freeform",
      throttleMs: window.__starhtml_drag_config?.throttleMs ?? DEFAULT_THROTTLE,
      constrainToParent: window.__starhtml_drag_config?.constrainToParent ?? false,
      touchEnabled: window.__starhtml_drag_config?.touchEnabled ?? true
    };
    ...
    const updateDragSignals = (updates) => {
      startBatch();
      try {
        mergePatch(updates);
      } catch (error) {
    ...
    const updateDragPosition = () => {
      ...
      if (state.element) {
        ...
        if (config.mode === "sortable" && dropZone) {
          ...
          const insertBefore = findInsertPosition(dropZone, state.current.y);
          if (insertBefore) {
    ...
    const cleanup = () => {
      ...
      for (const el2 of document.querySelectorAll(".drop-zone-active")) {
        el2.classList.remove("drop-zone-active");
      }
      if (state.placeholder) {
        state.placeholder.remove();

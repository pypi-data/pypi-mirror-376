...
class Conv2D(Module):
    ...
    def forward(self, X):
        from neurograd import conv2d, CUDNN_AVAILABLE
        X = X.cast(self.dtype) if self.dtype else X
        if self.backend == "cudnn" and CUDNN_AVAILABLE:
            from neurograd.functions.conv import Convolver
            Z = Convolver(self.strides, self.padding, depthwise=self.depthwise)(X, self.kernels)
        else:

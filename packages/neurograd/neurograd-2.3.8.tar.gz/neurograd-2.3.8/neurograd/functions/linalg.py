...
class EinSum(Function, Module):
    ...
    def __init__(self, subscripts: str, optimize=False):
        ...
        self.backward_paths = {}  # Cache for backward paths
    def forward(self, *operands: xp.ndarray) -> xp.ndarray:
        ...
        # Use opt_einsum if available and optimization is enabled
        if opt_einsum is not None and self.optimize:
            # Precompute path if not already done
            if self.forward_path is None:
                # Create dummy arrays with the same shapes
                dummy_arrays = [xp.zeros(shape) for shape in self.operand_shapes]
                # Get the contraction path
                _, self.forward_path = opt_einsum.contract_path(
                    self.subscripts, *dummy_arrays, optimize='auto'
                )
    ...
    def backward(self, grad_output: xp.ndarray):
        ...
        for i, tensor in enumerate(self.parent_tensors):
            ...
            try:
                # Use opt_einsum for backward if available and optimization is enabled
                if opt_einsum is not None and self.optimize:
                    # Get or create path for this gradient equation
                    if grad_equation not in self.backward_paths:
                        # Create dummy arrays with appropriate shapes
                        dummy_shapes = [grad_output.shape]
                        for j, operand in enumerate(self.parent_tensors):
                            if j != i:
                                dummy_shapes.append(operand.data.shape)
                        dummy_arrays = [xp.zeros(shape) for shape in dummy_shapes]
                        # Compute path
                        _, path = opt_einsum.contract_path(
                            grad_equation, *dummy_arrays, optimize='auto'
                        )
                        self.backward_paths[grad_equation] = path
                    # Perform contraction with precomputed path
                    grad = opt_einsum.contract(
                        grad_equation, *grad_operands,
                        backend='cupy' if xp.__name__ == 'cupy' else 'numpy',

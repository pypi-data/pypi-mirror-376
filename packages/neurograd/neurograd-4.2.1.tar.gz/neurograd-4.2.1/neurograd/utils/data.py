...
class ImageFolder(Dataset):
    ...
    def get_dali_pipeline(self, batch_size: int, shuffle: bool = True, 
        ...
        def image_pipeline():
            ...
            # Decode and apply RandomResizedCrop
            ...
            # Resize the cropped image to the final desired shape
            images = fn.resize(
                images,
                device=device,
                size=[h, w],
            ...
            if self.img_mode.upper() in ("L", "GRAY", "GREY", "GRAYSCALE"):
                images = fn.color_space_conversion(
                    images, device=device, image_type=types.RGB, output_type=types.GRAY
                )
            ...
            return images, labels.gpu() if device == "gpu" else labels
...
class DataLoader:
    ...
    def _init_dali_pipeline(self):
        ...
        if self._pipeline is not None:
            ...
            last_batch_policy = 'drop' if self.drop_last else 'part'
            ...
            print(f"DALI pipeline initialized: batch_size={self.batch_size}, device={self.device}, "
                  f"num_threads={self.num_workers}")

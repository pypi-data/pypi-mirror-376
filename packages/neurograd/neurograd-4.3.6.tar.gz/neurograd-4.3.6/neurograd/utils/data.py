...
class DataLoader:
    ...
    def _dali_iterator(self):
        for data in self._dali_iter:
            ...
            # This simple, direct conversion is highly efficient.
            if self.device == 'gpu':
                X = Tensor(xp.from_dlpack(images_tensor), dtype=self.dataset.img_dtype)
                y = Tensor(xp.from_dlpack(labels_tensor), dtype=self.dataset.target_dtype)
            else: # CPU device
                X = Tensor(np.array(images_tensor), dtype=self.dataset.img_dtype)
                y = Tensor(np.array(labels_tensor), dtype=self.dataset.target_dtype)

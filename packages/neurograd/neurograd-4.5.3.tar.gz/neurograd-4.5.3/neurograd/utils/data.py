...
class DataLoader:
    ...
    def _start_prefetching(self):
        ...
        for i, batch_indices in enumerate(batches[:10]):
    ...
    def _dali_iterator(self):
        """DALI iterator that waits for initial cache population to prevent a race condition."""
        if self.cache_prefetch:
            initial_job_keys = sorted(self.prefetch_futures.keys())
            if initial_job_keys:
                print(f"Waiting for initial {len(initial_job_keys)} batches to be cached...")
                for key in initial_job_keys:
                    try:
                        future = self.prefetch_futures.pop(key)
                        future.result()  # This blocks until the background job is done
                    except Exception as e:
                        print(f"Warning: Prefetching for batch {key} failed: {e}")
                print("Initial cache populated. Starting DALI pipeline...")
        ...
        for data in self._dali_iter:
            ...
            if self.device == 'gpu':
                try:
                    ...
                    raise ImportError("Cupy is required for GPU tensors. Please install with 'pip install cupy-cudaXXX'")
            else:
...
def create_dali_transforms(device: str = "cpu", brightness: float = 0.0, contrast: float = 0.0, saturation: float = 0.0, hue: float = 0.0, horizontal_flip_prob: float = 0.0, vertical_flip_prob: float = 0.0, rotation_angle: float = 0.0):
    if not DALI_AVAILABLE:
    ...
    def apply_transforms(images):
        if brightness or contrast or saturation or hue:
            images = fn.color_twist(images, device=device, brightness=fn.random.uniform(range=[-brightness, brightness]), contrast=fn.random.uniform(range=[1-contrast, 1+contrast]), saturation=fn.random.uniform(range=[1-saturation, 1+saturation]), hue=fn.random.uniform(range=[-hue, hue]))
        ...
        if rotation_angle != 0.0:
            images = fn.rotate(images, device=device, angle=fn.random.uniform(range=[-rotation_angle, rotation_angle]), keep_size=True, fill_value=0)
        return images
...
def is_on_network_drive(path_to_check: str) -> bool:
    ...
    NETWORK_FS_TYPES = {"nfs", "nfs4", "nfsd", "cifs", "smbfs", "smb", "smb2", "smb3", "fuse.sshfs", "fuse.gcsfuse", "fuse.s3fs"}

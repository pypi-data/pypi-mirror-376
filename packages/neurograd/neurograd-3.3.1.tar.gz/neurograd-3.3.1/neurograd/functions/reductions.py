...
class Var(Function, Module):
    ...
    def __init__(self, axis=None, keepdims=False, ddof=0, eps=1e-8):
        ...
        self.eps = eps
    def forward(self, x: xp.ndarray) -> xp.ndarray:
        # Single-pass variance using E[X²] - E[X]² formula to avoid double reduction
        mean_vals = _reduce(x, self.axis, xp.mean, keepdims=True, dtype=xp.float32)
        mean_sq_vals = _reduce(x*x, self.axis, xp.mean, keepdims=True, dtype=xp.float32)
        
        # var = E[X²] - E[X]²
        var_vals = mean_sq_vals - mean_vals * mean_vals
    ...
    def backward(self, grad_output: xp.ndarray) -> xp.ndarray:
        ...
        # d/dx var(x) = 2(x - mean)/(n - ddof) - optimized to avoid large temporary
        # Compute gradient in-place style operations
        grad = go * (2.0 / denom)
        grad = grad * (x.data - self.mean)

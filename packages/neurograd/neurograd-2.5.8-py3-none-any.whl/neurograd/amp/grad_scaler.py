    ...
    def unscale_(self, optimizer) -> None:
        ...
        for _, param in optimizer.params:
            ...
            if grad is None or not getattr(param, "requires_grad", False):
                continue
            # Direct finite check without try-catch
            if not self._found_inf:
                # Use any() for early termination
                self._found_inf = not bool(xp.isfinite(grad).all())  
            # Always unscale with direct in-place operation
            grad *= grad.dtype.type(inv)

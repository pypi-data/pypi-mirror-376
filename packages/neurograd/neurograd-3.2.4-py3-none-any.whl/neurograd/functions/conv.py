...
def conv2d(input: Union["Tensor", xp.ndarray], filters: Union["Tensor", xp.ndarray],
    ...
    if not depthwise:
        ...
        # Method 1: Use einsum (simpler and more efficient for depthwise)
        output = ng.einsum('ncpqhw,chw->ncpq', slides, filters)
        
        # Method 2: GEMM-based approach (more complex but educational)
        # This would require reshaping each channel separately and doing batched operations
        # Generally not recommended for depthwise due to complexity

...
class DataLoader:
    ...
    def _pin_memory(self, arr):
        """Create pinned CPU memory for faster GPU transfers."""
        try:
            import cupy as cp
            ...
            try:
                # Create pinned memory with the same shape and dtype
                pinned_mem = cp.cuda.alloc_pinned_memory(arr.nbytes)
                pinned_array = np.frombuffer(pinned_mem, dtype=arr.dtype).reshape(arr.shape)
                pinned_array[...] = arr  # Copy data to pinned memory
                
                # Store a reference to the pinned memory buffer in a way that works
                # We'll store it in a global registry to prevent garbage collection
                if not hasattr(self, '_pinned_refs'):
                    self._pinned_refs = []
                self._pinned_refs.append(pinned_mem)
    ...
    def __init__(self, dataset: Dataset, batch_size: int = 32,
        ...
        self._pinned_refs = []  # Track pinned memory references
    ...
    def _cleanup_pinned_memory(self):
        """Clean up all pinned memory references."""

...
class ImageFolder(Dataset):
    ...
    def get_dali_pipeline(self, batch_size: int, shuffle: bool = True):
        ...
        @pipeline_def(batch_size=batch_size, num_threads=self.num_threads, device_id=0, seed=self.seed, prefetch_queue_depth=self.prefetch_queue_depth)
        def image_pipeline():
            # Create file reader
            if shuffle:
...
class DataLoader:
    ...
    def __init__(
        ...
        # Determine threading for fallback mode and DALI
    ...
    def _init_dali_pipeline(self):
        ...
        # Update dataset's DALI settings from the DataLoader's unified parameters
        self.dataset.device = self.device
        self.dataset.num_threads = self.num_workers
        self.dataset.prefetch_queue_depth = self.prefetch_batches
    ...
    def __repr__(self):
        ...
        return (f"<DataLoader: {len(self)} batches, "
                ...
                f"prefetch_depth={self.prefetch_batches}>")
    ...
    def apply_transforms(images):
        ...
        if rotation_angle != 0.0:
            images = fn.rotate(
                ...
                keep_size=True,  # ensures shape stays same
                fill_value=0
            )
...
# DataLoader is now the single point of configuration for performance
dataloader = DataLoader(
    dataset,
    batch_size=128,
    shuffle=True,
    device="cuda" if DALI_AVAILABLE else "cpu",  # Automatically adapts
    num_workers=8,                               # Used for DALI threads or fallback threads
    prefetch_batches=3                           # Used for DALI queue depth or fallback prefetch
)

...
class DataLoader:
    ...
    def __iter__(self):
        ...
        # Use process pool for parallel loading only if dataset supports it
        if self.num_workers > 0 and hasattr(self.dataset, '_supports_multiprocessing') and self.dataset._supports_multiprocessing():
            try:
                with ProcessPoolExecutor(max_workers=self.num_workers) as executor:
                    # Submit all batch loading tasks with batch index for ordering
                    future_to_idx = {
                    ...
                    # Yield results in original batch order
                    for i in range(len(batches)):
                        yield results[i]
            except Exception as e:
                print(f"Warning: Multiprocessing failed ({e}), falling back to sequential loading")
    ...
    def _load_batch(self, indices):
        ...
        if hasattr(Xs[0], 'data'):  # Tensor objects
            X = Tensor(xp.stack([x.data for x in Xs], axis=0), dtype=Xs[0].dtype)
        else:  # NumPy arrays
            X = Tensor(xp.stack(Xs, axis=0), dtype=float32)
            
        if hasattr(ys[0], 'data'):  # Tensor objects
            y = Tensor(xp.stack([y.data for y in ys], axis=0), dtype=ys[0].dtype)
        else:  # NumPy arrays
            y = Tensor(xp.stack(ys, axis=0), dtype=xp.int64)
...
class ImageFolder:
    ...
    def _supports_multiprocessing(self):
        """Check if this dataset can be safely used with multiprocessing"""
        # Disable multiprocessing if we have complex transforms that may not be picklable
        if self.img_transform is not None:
            # Check if it's an albumentations transform (which often has issues with pickling)
            if hasattr(self.img_transform, '__module__') and 'albumentations' in str(self.img_transform.__module__):
                return False
        if self.target_transform is not None:
            # Check for lambda functions or other non-picklable transforms
            if hasattr(self.target_transform, '__name__') and self.target_transform.__name__ == '<lambda>':
                return False
        return False  # Disabled for safety - ImageFolder has complex state
    ...
    def _preload_images(self):
        ...
        for i, img_path in enumerate(self.images):
            ...
            # Handle both NumPy and CuPy arrays for target
            if hasattr(target, 'get'):  # CuPy array
                target = target.get()  # Convert to NumPy
    ...
    def __getitem__(self, idx: int):
        ...
        return image, target.astype(self.target_dtype)  # Return numpy arrays, not Tensors

...
class EinSum(Function, Module):
    ...
    def forward(self, *operands: xp.ndarray) -> xp.ndarray:
        ...
        self.parent_tensors = operands 
        if opt_einsum is not None and self.optimize:
            if self.forward_path is None:
                dummy_arrays = [xp.zeros(shape) for shape in self.operand_shapes]
                self.forward_path = opt_einsum.contract_path(self.subscripts, *dummy_arrays, optimize='auto')
            return opt_einsum.contract(self.subscripts, *operands, 
    ...
    def backward(self, grad_output: xp.ndarray):
        if not hasattr(self, 'parent_tensors'):
            return tuple()
        grads = []
        if '->' in self.subscripts:
            ...
            output_str = ''.join([idx for idx in set(all_indices) if all_indices.count(idx) == 1])
        for i, tensor in enumerate(self.parent_tensors):
            ...
            for j, operand in enumerate(self.parent_tensors):
                if j != i:
                    grad_operands.append(getattr(operand, 'data', operand))
                    grad_specs.append(input_specs[j])
            grad_equation = ','.join(grad_specs) + '->' + input_specs[i]
            if opt_einsum is not None and self.optimize:
                if grad_equation not in self.backward_paths:
                    dummy_shapes = [grad_output.shape] + [getattr(op, 'data', op).shape 
                                                         for j, op in enumerate(self.parent_tensors) if j != i]
                    dummy_arrays = [xp.zeros(shape) for shape in dummy_shapes]
                    self.backward_paths[grad_equation] = opt_einsum.contract_path(grad_equation, *dummy_arrays, optimize='auto')
                grad = opt_einsum.contract(grad_equation, *grad_operands,
                                         backend='cupy' if xp.__name__ == 'cupy' else 'numpy',
                                         optimize=self.backward_paths[grad_equation])
            else:
                grad = xp.einsum(grad_equation, *grad_operands, optimize=self.optimize)   
            grads.append(self._handle_broadcasting(grad, self.operand_shapes[i]))
        return tuple(grads)

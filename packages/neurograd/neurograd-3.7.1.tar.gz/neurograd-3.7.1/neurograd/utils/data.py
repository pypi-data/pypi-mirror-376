...
class DataLoader:
    ...
    def _collate_batch(self, batch_data):
        """Efficiently collate batch data into tensors."""
        if not batch_data:
            return None, None
        ...
        if hasattr(samples[0], 'data'):
            # If samples are already Tensors, extract data
        ...
        # Pin memory if requested and using CUDA
        if self.pin_memory and hasattr(X, 'pin_memory'):
            X = X.pin_memory()
            y = y.pin_memory()
    ...
    def _start_workers(self):
        ...
        print(f"[DataLoader] Started {len(self._workers)} worker processes")

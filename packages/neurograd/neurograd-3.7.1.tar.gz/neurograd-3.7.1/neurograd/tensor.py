...
class Tensor:
    ...
    def pin_memory(self):
        """Pin tensor memory for faster GPU transfers (CUDA only)."""
        if xp is real_np:
            # CPU: pin memory using CuPy's pinned memory (if available)
            try:
                import cupy as cp
                if hasattr(cp, 'cuda') and hasattr(cp.cuda, 'pinned_memory'):
                    # Pin the NumPy array using CuPy's pinned memory
                    pinned_data = cp.cuda.to_gpu(self.data, stream=None)
                    pinned_data = cp.asnumpy(pinned_data)  # Convert back but keep pinned
                    return Tensor(pinned_data, requires_grad=self.requires_grad, 
                                grad_fn=self.grad_fn, name=f"{self.name}_pinned")
            except ImportError:
                pass
            # Fallback: return self (no pinning available)
            return self
        else:
            # GPU: memory is already managed by CUDA
            return self

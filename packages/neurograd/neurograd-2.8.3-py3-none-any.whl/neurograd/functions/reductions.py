...
class Var(Function, Module):
    ...
    def __init__(self, axis=None, keepdims=False, ddof=0, eps=1e-8):
        ...
        self.eps = eps
    def forward(self, x: xp.ndarray) -> xp.ndarray:
        # Compute mean and sum of squares in parallel using einops for efficiency
        self.mean = _reduce(x, self.axis, xp.mean, keepdims=True, dtype=xp.float32) 
        # Use single-pass variance calculation with improved numerical stability
        centered = x - self.mean
        sum_sq = _reduce(centered**2, self.axis, xp.sum, keepdims=self.keepdims, dtype=xp.float32)
        # Calculate number of elements and apply Bessel's correction
        if self.axis is None:
            n = x.size
        else:
            axes = (self.axis,) if isinstance(self.axis, int) else self.axis

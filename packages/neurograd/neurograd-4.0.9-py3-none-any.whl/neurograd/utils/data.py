...
class ImageFolder(Dataset):
    ...
    def __init__(
        ...
        # Accept DALI when a Pipeline is provided, when img_transform is None,
        # or when a DALI-compatible callable is provided (callable that uses DALI fn ops).
        self.use_dali = DALI_AVAILABLE and (
            isinstance(img_transform, Pipeline) or img_transform is None or callable(img_transform)
        )
    ...
    def get_dali_pipeline(self, batch_size: int, shuffle: bool = True):
        ...
        # Integrating two pipelines is complex; prefer using the provided pipeline as-is.
        if isinstance(self.img_transform, Pipeline):
            try:
                # Ensure pipeline is built; calling build() twice is safe in many cases,
                # but if it fails, we proceed and return the pipeline anyway.
                self.img_transform.build()
            except Exception:
                pass
        ...
        def image_pipeline():
            ...
            if shuffle:
                images, labels = fn.readers.file(
                    ...
                    random_shuffle=True,
                    initial_fill=batch_size * 4,  # Buffer size
                    name="Reader"
                )
            else:
                images, labels = fn.readers.file(
            ...
            # Decode images
            if self.device == "gpu":
                # Use mixed precision decoding (CPU decode, GPU processing)
            ...
            # Apply custom transforms if provided. Support two cases:
            #  - callable transforms that operate on DALI "images" (e.g., returned by create_dali_transforms)
            #  - Pipeline instances are already handled above and returned directly
            if self.img_transform is not None:
                # If it's a callable, assume it's a DALI-compatible transform function and apply it
                if callable(self.img_transform):
                    try:
                        images = self.img_transform(images)
                    except Exception:
                        # If transform failed inside pipeline definition, warn and continue without it
                        print("WARNING: Failed to apply DALI callable transform inside pipeline. Skipping transform.")
                else:
                    # Non-callable, non-Pipeline objects are unsupported inside DALI pipeline
                    print("WARNING: img_transform provided is not a DALI Pipeline or callable; skipping inside DALI pipeline.")

            # Normalize to [0, 1] if requested
            if self.img_normalize:
                images = fn.cast(images, device=self.device, dtype=types.FLOAT)
                images = images / 255.0
            else:
                images = fn.cast(images, device=self.device, dtype=types.FLOAT)

            # Convert to CHW format if requested
            if not self.chw:
                # DALI outputs CHW by default after transpose, so we need to handle HWC
                if len(images.shape) == 4:  # Batch dimension
                    images = fn.transpose(images, device=self.device, perm=[0, 2, 3, 1])
                else:
                    images = fn.transpose(images, device=self.device, perm=[1, 2, 0])
    ...
    def _apply_img_transform(self, arr: np.ndarray) -> np.ndarray:
        ...
        # Skip DALI pipelines in fallback mode
        if DALI_AVAILABLE and isinstance(self.img_transform, Pipeline):
            print("WARNING: DALI Pipeline transform provided but using fallback mode. Transform skipped.")
            return arr
...
class DataLoader:
    ...
    def _dali_iterator(self):
        ...
        class DALIGenericIterator:
            def __init__(self, pipeline, output_map, size, last_batch_policy):
            ...
            def __next__(self):
                ...
                try:
                    ...
                    images = outputs[0].as_array() if hasattr(outputs[0], 'as_array') else outputs[0]
                    labels = outputs[1].as_array() if hasattr(outputs[1], 'as_array') else outputs[1]
                    
                    # Convert to CPU if on GPU
                    if self.pipeline.device == "gpu":
                        if hasattr(images, 'as_cpu'):
                            images = images.as_cpu().as_array()
                        if hasattr(labels, 'as_cpu'):
                            labels = labels.as_cpu().as_array()
                    
                    # Convert to Tensors
                    X = Tensor(images, dtype=self.dataset.img_dtype)
                    y = Tensor(labels, dtype=self.dataset.target_dtype)

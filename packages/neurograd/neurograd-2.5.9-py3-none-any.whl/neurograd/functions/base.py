...
class Function(ABC):
    ...
    def __call__(self, *inputs) -> Tensor:
        ...
        # AUTOCAST if enabled - optimize to avoid unnecessary casting
        op_name = getattr(self, 'name', None) or self.__class__.__name__
        if is_autocast_enabled():
            if op_name != 'Cast':  # Avoid recursion with Cast operations
                # Optimized: only cast tensors that actually need casting
                new_processed_inputs = []
                for inp in processed_inputs:
                    cast_tensor = maybe_cast_tensor(inp, op_name=op_name)
                    new_processed_inputs.append(cast_tensor)
                processed_inputs = new_processed_inputs
        # Computations

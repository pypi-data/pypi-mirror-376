...
# Cache for autocast decisions to avoid repeated string operations
_AUTOCAST_CACHE = {}
...
def should_cast_to_fp16(op_name: str) -> bool:
    ...
    if op_name in _AUTOCAST_CACHE:
        return _AUTOCAST_CACHE[op_name]
    ...
    elif op_name_lower in _FP16_SAFE_OPS:
        result = True
    else:
    ...
    _AUTOCAST_CACHE[op_name] = result
    return result
...
def maybe_cast_tensor(tensor, target_dtype=None, op_name: str = "unknown") -> 'Tensor':
    ...
    # Early return if no casting needed - this prevents unnecessary Cast operations

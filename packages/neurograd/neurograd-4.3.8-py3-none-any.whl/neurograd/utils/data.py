...
# ============================================================================
def log_path(message, level="INFO"):
    """Enhanced logging with clear visual markers"""
    markers = {
        "INFO": "ℹ️ ",
        "SUCCESS": "✅",
        "WARNING": "⚠️ ",
    ...
    print(f"{marker} PATH_DEBUG: {message}")

# Try to import DALI first - this determines our capabilities
try:
    log_path("Attempting DALI import...", "INFO")
    ...
    DALI_AVAILABLE = True
    log_path(f"DALI import successful! Version: {dali.__version__}", "SUCCESS")
    ...
    class DALIGenericIterator:
        ...
        def __init__(self,
            ...
            log_path("Initializing DALIGenericIterator", "GPU")
            ...
            self._counter = 0
            log_path(f"DALI Iterator configured: {self._num_batches} batches, batch_size={self._batch_size}", "GPU")
        ...
        def __next__(self):
            ...
            try:
                log_path(f"DALI pipeline.run() - batch {self._counter + 1}/{self._num_batches}", "GPU")
                ...
                log_path(f"DALI batch ready: {[type(v).__name__ for v in outputs]}", "GPU")
                return [batch_dict]
        ...
        def reset(self):
            log_path("DALI pipeline reset", "GPU")
            self._pipeline.reset()
            self._counter = 0

except ImportError as e:
    DALI_AVAILABLE = False
    log_path(f"DALI import failed: {e}", "FALLBACK")
...
class ImageFolder(Dataset):
    ...
    def __init__(
        ...
        log_path(f"ImageFolder init: root={root}", "INFO")
        self.root = root
    ...
    def get_dali_pipeline(self, batch_size: int, shuffle: bool = True, 
        ...
        log_path(f"get_dali_pipeline called: device={device}, batch_size={batch_size}", "INFO")
        
        if not DALI_AVAILABLE:
            log_path("DALI not available, returning None", "FALLBACK")
            return None
            
        if isinstance(self.img_transform, Pipeline):
            log_path("Using pre-built DALI Pipeline", "GPU")
            return self.img_transform

        is_gpu = device == "gpu"
        log_path(f"Creating DALI pipeline: GPU={is_gpu}", "GPU" if is_gpu else "CPU")
        
        try:
            # Check GPU support first
            if is_gpu:
                from nvidia.dali.backend import CheckSupportedDevice
                gpu_supported = CheckSupportedDevice(0)
                log_path(f"DALI GPU device 0 support check: {gpu_supported}", "GPU")
                if not gpu_supported:
                    log_path("GPU not supported by DALI, forcing CPU", "WARNING")
                    is_gpu = False
                    device = "cpu"
            ...
            def image_pipeline():
                log_path(f"DALI @pipeline_def executing: GPU={is_gpu}, threads={num_threads}", "GPU" if is_gpu else "CPU")
                ...
                if self.img_transform and callable(self.img_transform):
                    log_path("Applying DALI img_transform", "GPU" if is_gpu else "CPU")
            ...
            log_path("Building DALI pipeline...", "GPU" if is_gpu else "CPU")
            pipeline = image_pipeline()
            pipeline.build()
            log_path("DALI pipeline built successfully!", "SUCCESS")
            return pipeline
            
        except Exception as e:
            log_path(f"DALI pipeline creation failed: {e}", "ERROR")
            import traceback
            traceback.print_exc()
            return None
    ...
    def _load_image_opencv(self, path: str) -> np.ndarray:
        ...
        log_path(f"OpenCV loading: {os.path.basename(path)}", "CPU")
    ...
    def __getitem__(self, idx: int):
        ...
        log_path(f"ImageFolder.__getitem__({idx}) - OpenCV path", "CPU")
...
class DataLoader:
    ...
    def __init__(
        ...
        log_path("=" * 60, "INFO")
        log_path("DataLoader.__init__ - DECISION TREE ANALYSIS", "INFO")
        log_path("=" * 60, "INFO")
        ...
        log_path(f"Initial config: device={self.device}, batch_size={batch_size}, num_workers={self.num_workers}", "INFO")
        ...
        # ============================================================================
        # 🔥 PATH DECISION LOGIC WITH DETAILED LOGGING
        # ============================================================================
        
        log_path("Step 1: Check DALI availability", "INFO")
        if not DALI_AVAILABLE:
            log_path("DALI not available - will use OpenCV multithreading path", "FALLBACK")
            self.use_dali = False
        else:
            log_path("DALI is available, proceeding to next check", "SUCCESS")
            
            log_path("Step 2: Check dataset type", "INFO")
            if not isinstance(dataset, ImageFolder):
                log_path(f"Dataset type {type(dataset).__name__} is not ImageFolder - will use regular iteration", "FALLBACK")
                self.use_dali = False
            else:
                log_path("Dataset is ImageFolder, proceeding to next check", "SUCCESS")
                
                log_path("Step 3: Attempt DALI memory preallocation", "INFO")
                try:
                    if self.device == "gpu": 
                        log_path("Preallocating GPU memory for DALI...", "GPU")
                        PreallocateDeviceMemory(int(0.5 * 1024**3), 0)
                    log_path("Preallocating pinned memory for DALI...", "GPU")
                    PreallocatePinnedMemory(int(0.25 * 1024**3))
                    log_path("Memory preallocation successful", "SUCCESS")
                except Exception as e: 
                    log_path(f"DALI memory preallocation failed: {e}", "WARNING")
                
                log_path("Step 4: Attempt DALI pipeline initialization", "INFO")
                self.use_dali = True  # Assume success until proven otherwise
                self._init_dali_pipeline()
        
        # Final decision logging
        if self.use_dali:
            log_path("🎉 FINAL DECISION: Using DALI GPU/CPU accelerated path", "SUCCESS")
        else:
            log_path("🔄 FINAL DECISION: Using OpenCV multithreading fallback path", "FALLBACK")
            if self.device == "gpu":
                log_path("GPU device requested but DALI not available. Using CPU fallback.", "WARNING")
                self.device = "cpu"
        
        log_path("=" * 60, "INFO")

    def _init_dali_pipeline(self):
        log_path("_init_dali_pipeline() called", "INFO")
        
        if not self.use_dali: 
            log_path("use_dali=False, skipping pipeline init", "FALLBACK")
            return
            
        try:
            log_path(f"Calling dataset.get_dali_pipeline(batch_size={self.batch_size}, device={self.device})", "INFO")
            self._pipeline = self.dataset.get_dali_pipeline(
                batch_size=self.batch_size, shuffle=self.shuffle, device=self.device,
                num_threads=self.num_workers, seed=self.seed)
            
            if self._pipeline is None:
                log_path("dataset.get_dali_pipeline() returned None - falling back to OpenCV", "ERROR")
                self.use_dali = False
                return
            
            log_path(f"DALI pipeline created: {type(self._pipeline)}", "SUCCESS")
            
            log_path("Creating DALI iterator...", "INFO")
            ...
            log_path(f"DALI iterator created successfully", "SUCCESS")
            log_path(f"DALI pipeline configured: batch_size={self.batch_size}, device={self.device}, num_threads={self.num_workers}", "GPU" if self.device == "gpu" else "CPU")
            
        except Exception as e:
            log_path(f"DALI pipeline initialization failed: {e}", "ERROR")
            import traceback
            traceback.print_exc()
            self.use_dali = False
            log_path("Falling back to OpenCV multithreading path", "FALLBACK")
    ...
    def __iter__(self):
        if self.use_dali and self._dali_iter:
            log_path("Using DALI iterator path", "GPU" if self.device == "gpu" else "CPU")
            self._dali_iter.reset()
            return self._dali_iterator()
        else:
            log_path(f"Using regular iterator path with {self.num_workers} workers", "FALLBACK")
    ...
    def reset(self):
        ...
        """
        if self.use_dali and self._dali_iter:
            log_path("Resetting DALI iterator", "GPU" if self.device == "gpu" else "CPU")
            self._dali_iter.reset()
        else:
            log_path("Reset called on regular iterator (no action needed)", "FALLBACK")

    def _dali_iterator(self):
        log_path("_dali_iterator() started", "GPU" if self.device == "gpu" else "CPU")
        batch_count = 0
        
        for data in self._dali_iter:
            batch_count += 1
            log_path(f"DALI batch {batch_count}: processing tensors", "GPU" if self.device == "gpu" else "CPU")
            ...
            log_path(f"DALI tensors: images={type(images_tensor).__name__}, labels={type(labels_tensor).__name__}", "GPU" if self.device == "gpu" else "CPU")
            
            # Convert DALI TensorList to proper arrays
            if self.device == 'gpu':
                log_path("Converting DALI tensors to CuPy arrays (GPU path)", "GPU")
                try:
                    import cupy as cp
                    ...
                    log_path(f"GPU tensor conversion successful: X.shape={X.shape}, y.shape={y.shape}", "GPU")
                except Exception as e:
                    log_path(f"GPU tensor conversion failed: {e} - falling back to CPU", "ERROR")
                ...
                log_path("Converting DALI tensors to NumPy arrays (CPU path)", "CPU")
                # For CPU TensorList, convert to numpy array
                X = Tensor(images_tensor.as_array(), dtype=self.dataset.img_dtype)
                y = Tensor(labels_tensor.as_array(), dtype=self.dataset.target_dtype if not self.dataset.one_hot_targets else float32)
                log_path(f"CPU tensor conversion successful: X.shape={X.shape}, y.shape={y.shape}", "CPU")
    
            yield X, y
            
    def _regular_iterator(self):
        """Fallback iterator with threading and prefetching."""
        log_path("_regular_iterator() started - OpenCV multithreading path", "FALLBACK")
        ...
        log_path(f"Regular iterator: {total} batches, {self.num_workers} workers, {self.prefetch_batches} prefetch", "FALLBACK")

        # Initialize prefetch window
        for _ in range(min(self.prefetch_batches, total)):
            log_path(f"Prefetching batch {next_to_submit + 1}/{total}", "FALLBACK")
            futs = self._schedule_batch(batches[next_to_submit])
            window.append(futs)
            next_to_submit += 1

        for batch_idx in range(total):
            # Get next batch from window
            if not window:
                futs = self._schedule_batch(batches[next_to_submit])
                window.append(futs)
                next_to_submit += 1
            ...
            if next_to_submit < total:
                log_path(f"Scheduling batch {next_to_submit + 1}/{total} (current: {batch_idx + 1})", "FALLBACK")
                next_futs = self._schedule_batch(batches[next_to_submit])
                window.append(next_futs)
                next_to_submit += 1

            log_path(f"Gathering batch {batch_idx + 1}/{total}", "FALLBACK")
    ...
    def _batch_indices(self):
        ...
        order = list(range(n))
        if self.shuffle:
            log_path(f"Shuffling {n} samples", "FALLBACK")
    ...
    def _ensure_executor(self):
        if self.num_workers > 0 and self._executor is None:
            log_path(f"Creating ThreadPoolExecutor with {self.num_workers} workers", "FALLBACK")
            self._executor = ThreadPoolExecutor(max_workers=self.num_workers)

    def _schedule_batch(self, idxs):
        if self.num_workers > 0:
            log_path(f"Scheduling {len(idxs)} samples across {self.num_workers} threads", "FALLBACK")
            self._ensure_executor()
            return [self._executor.submit(self.dataset.__getitem__, i) for i in idxs]
        else:
            log_path(f"Loading {len(idxs)} samples synchronously (no threading)", "FALLBACK")
            return [self.dataset[i] for i in idxs]

    def _gather_batch(self, futures_or_results):
        if self.num_workers > 0:
            log_path("Gathering results from threaded execution", "FALLBACK")
            batch = [f.result() for f in futures_or_results]
        else:
            log_path("Gathering results from synchronous execution", "FALLBACK")
            batch = futures_or_results
        
        Xs, ys = zip(*batch)
        log_path(f"Stacking batch: {len(Xs)} samples", "FALLBACK")
        return Tensor(xp.stack([x.data for x in Xs])), Tensor(xp.stack([y.data for y in ys]))

    def close(self):
        if self._executor is not None:
            log_path("Shutting down ThreadPoolExecutor", "FALLBACK")
...
def create_dali_transforms(
    ...
    """
    log_path(f"create_dali_transforms called: device={device}", "INFO")
    
    if not DALI_AVAILABLE:
        log_path("DALI not available. Transform creation skipped.", "WARNING")
        return None
    
    log_path("Creating DALI transform function", "SUCCESS")
        
    def apply_transforms(images):
        log_path("Applying DALI transforms to batch", "GPU" if device == "gpu" else "CPU")
        
        if brightness or contrast or saturation or hue:
            log_path("Applying color_twist transform", "GPU" if device == "gpu" else "CPU")
        ...
        if horizontal_flip_prob > 0.0:
            log_path("Applying horizontal flip transform", "GPU" if device == "gpu" else "CPU")
        ...
        if vertical_flip_prob > 0.0:
            log_path("Applying vertical flip transform", "GPU" if device == "gpu" else "CPU")
            images = fn.flip(images, device=device, vertical=fn.random.coin_flip(probability=vertical_flip_prob))
        if rotation_angle != 0.0:
            log_path("Applying rotation transform", "GPU" if device == "gpu" else "CPU")
            images = fn.rotate(

...
class Convolver(Function):
    ...
    def forward(self, inputs: xp.ndarray, filters: xp.ndarray) -> xp.ndarray:
        ...
        self.conv_desc = self._create_convolution_descriptor(
            self._get_cudnn_dtype(inputs.dtype)
        )
        ...
        # FIXED: Use correct dtype for alpha and beta to match input tensor dtype
        alpha = np.array(1.0, dtype=inputs.dtype)
        beta = np.array(0.0, dtype=inputs.dtype)
    ...
    def backward_input(self, grad_output: xp.ndarray) -> xp.ndarray:
        ...
        grad_input = xp.empty_like(inputs)        
        ...
        # Allocate workspace
        workspace = xp.empty(workspace_size, dtype=xp.uint8) if workspace_size > 0 else None
        # Alpha and beta - FIXED: Use correct dtype to match input tensor
        alpha = np.array(1.0, dtype=inputs.dtype)
        beta = np.array(0.0, dtype=inputs.dtype)
    ...
    def backward_filters(self, grad_output: xp.ndarray) -> xp.ndarray:
        inputs, filters = self.parent_tensors[0].data, self.parent_tensors[1].data
        grad_filters = xp.empty_like(filters)

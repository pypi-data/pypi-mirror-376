...
def conv2d(input: Union["Tensor", xp.ndarray], filters: Union["Tensor", xp.ndarray],
    ...
    if not depthwise:
        ...
        output = ng.einsum("ncpqhw,fchw->nfpq", slides, filters, optimize=True) # (N, F_N, out_H, out_W) 
    else:
        slides = slider(input)  # (N, C, out_H, out_W, F_H, F_W)
        filters = filters # (C, F_H, F_W)
        output = ng.einsum('ncpqhw,chw->ncpq', slides, filters, optimize=True) # (N, C, out_H, out_W)

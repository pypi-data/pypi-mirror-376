# ===================================================================
#
#  Part 1: Data Loading Library Code (Fixed)
#
# ===================================================================
...
class ImageFolder(Dataset):
    ...
    def __init__(
        ...
        self.dont_use_mmap = is_on_network_drive(self.root) and self.cache_dir is None
        self._collect_paths()

        if not self.images:
            raise ValueError(f"No images found in {root} with supported extensions: {IMG_EXTS}")

        # Initialize cached paths and ensure they're available
        self.cached_image_paths: List[str] = []
        if self.cache_available:
            self.cached_image_paths = [self._get_cache_destination(p) for p in self.images]
    ...
    def _prepare_cache_structure(self):
        ...
        cache_subdirs = set()
        for cached_path in self.cached_image_paths:
    ...
    def _get_cache_destination(self, original_path: str) -> str:
        ...
        # Create subdirectories in cache to maintain some organization
        path_parts = relative_path.split(os.sep)
        if len(path_parts) > 1:
            # Use class name as subdirectory
            class_dir = path_parts[0]
        ...
        return os.path.join(self.cache_dir, class_dir, f"{hashed_name}{extension}")

    def _ensure_file_is_cached(self, original_path: str, cached_path: str) -> str:
        """Thread-safe file caching with proper validation"""
        if not self.cache_available:
        ...
        # Need to cache the file (slow path)
        with self.cache_lock:
    ...
    def _add_to_cache(self, original_path: str, cached_path: str):
        """Add file to cache with proper error handling"""
    ...
    def _manage_cache_size(self):
        """Manage cache size with proper error handling"""
        ...
        if self.cache_strategy == "lru":
            ...
            items_to_remove = []
            for path in self.cache_access_times:
                try:
                    mtime = os.path.getmtime(path) if os.path.exists(path) else 0
                    items_to_remove.append((path, mtime))
                except OSError:
                    items_to_remove.append((path, 0))
    ...
    def get_active_file_paths(self) -> List[str]:
        """Get the actual file paths to use (cached or original)"""
        if not self.cache_available:
            return self.images
            
        # Return paths that exist, preferring cached versions
        active_paths = []
        for i, original_path in enumerate(self.images):
            cached_path = self.cached_image_paths[i]
            if os.path.exists(cached_path) and os.path.getsize(cached_path) > 0:
    ...
    def get_dali_pipeline(self, batch_size: int, shuffle: bool = True, device: str = "cpu", num_threads: int = 4, prefetch: int = 2, seed: int = 42):
        ...
        # CRITICAL FIX: Use active file paths that exist
        ...
        def image_pipeline():
            images, labels = fn.readers.file(
                ...
                name="Reader", 
                initial_fill=min(4096, len(image_source_paths)),  # Don't exceed available files
                read_ahead=True, 
...
class DataLoader:
    ...
    def _warmup_cache(self, num_files_to_warmup: int):
        ...
        with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix='CacheWarmer') as executor:
            future_to_idx = {
                executor.submit(ds._ensure_file_is_cached, ds.images[i], ds.cached_image_paths[i]): i
            ...
            for future in future_to_idx:
                try:
                    ...
                    if result_path == ds.cached_image_paths[future_to_idx[future]]:
                        cached_count += 1
                except Exception as e:

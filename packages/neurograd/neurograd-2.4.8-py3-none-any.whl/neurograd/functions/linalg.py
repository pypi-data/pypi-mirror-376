...
class EinSum(nn.Module):
    ...
    def forward(self, *operands):
        ...
        if xp is np.ndarray:
            ...
            # Assume cupy.ndarray
            return cuq.contract(self.subscripts, *operands, optimize=self.optimize)
    ...
    def backward(self, grad_output):
        ...
        if '->' in self.subscripts:
            ...
            output_str = ''.join(sorted(set(all_indices) - set([idx for idx in all_indices
                                                                if all_indices.count(idx) > 1])))

        for i, tensor in enumerate(self.input_operands):
            if not getattr(tensor, 'requires_grad', True):
                grads.append(None)
                continue

            grad_operands = [grad_output]
            grad_specs = [output_str]
            for j, operand in enumerate(self.input_operands):
                if j != i:
                    grad_operands.append(operand)
                    grad_specs.append(input_specs[j])

            grad_equation = ','.join(grad_specs) + '->' + input_specs[i]
            try:
                xp = type(grad_operands[0])
                if xp is np.ndarray:
                    grad = np.einsum(grad_equation, *grad_operands, optimize=self.optimize)
                else:
                    grad = cuq.contract(grad_equation, *grad_operands, optimize=self.optimize)

                grad = self._handle_broadcasting(grad, self.operand_shapes[i])
                grads.append(grad)
            except Exception:
                grads.append(cp.zeros_like(self.input_operands[i]))

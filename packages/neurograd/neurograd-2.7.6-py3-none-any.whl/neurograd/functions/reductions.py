...
#     """
#     Helper to perform reductions over multiple axes. Uses a
#     transpose-and-reshape strategy to convert multi-axis reduction
#     into an efficient single-axis reduction, avoiding CuPy slowdown
#     and iterative reduction overhead.
#     """
#     if axis is None:
#         # Reduce over all axes
#         result = reduction_func(arr.ravel(), axis=0, **kwargs)
#         return result.reshape((1,) * arr.ndim) if keepdims else result
#     if isinstance(axis, int):
#         # Single-axis reduction
#         return reduction_func(arr, axis=axis, keepdims=keepdims, **kwargs)
#     # --- Multi-axis reduction ("Reshape and Conquer") ---
#     reduce_axes = tuple(sorted(ax if ax >= 0 else arr.ndim + ax for ax in axis))
#     keep_axes = tuple(i for i in range(arr.ndim) if i not in reduce_axes)
#     # Transpose and reshape
#     permuted = arr.transpose(keep_axes + reduce_axes)
#     reshaped = permuted.reshape(-1, reduce(operator.mul, (arr.shape[i] for i in reduce_axes), 1))
#     # Reduce over combined axis
#     result = reduction_func(reshaped, axis=1, **kwargs)
#     # Reshape back
#     if keepdims:
#         shape = list(arr.shape)
#         for ax in reduce_axes: shape[ax] = 1
#         return result.reshape(shape)
#     return result.reshape([arr.shape[i] for i in keep_axes] if keep_axes else ())
...
def _reduce(arr, axis, reduction_func, keepdims=False, **kwargs):
    ...
    axes = tuple(sorted(axis, reverse=True))  # Sort in reverse to maintain axis indices
    result = arr
    for ax in axes:
        result = reduction_func(result, axis=ax, keepdims=True, **kwargs)
    if not keepdims:
        # Remove the reduced dimensions
        axes_to_squeeze = tuple(sorted(axis))
        for i, ax in enumerate(axes_to_squeeze):
            # Adjust axis index as we squeeze
            result = xp.squeeze(result, axis=ax - i)
    return result
...
class Max(Function, Module):
    ...
    def backward(self, grad_output: xp.ndarray) -> xp.ndarray:
        ...
        if self.axis is None:
            mask /= _reduce(mask, None, xp.sum)  # In-place division
        else:
...
class Std(Function, Module):
    ...
    def forward(self, x: xp.ndarray) -> xp.ndarray:
        ...
        self.std_vals = _reduce(x, self.axis, xp.std, keepdims=True, dtype=xp.float32)
        if self.keepdims:
...
class Var(Function, Module):
    ...
    def forward(self, x: xp.ndarray) -> xp.ndarray:
        self.mean = _reduce(x, self.axis, xp.mean, keepdims=True, dtype=xp.float32)
        var_vals = _reduce(x, self.axis, xp.var, keepdims=self.keepdims, ddof=self.ddof, dtype=xp.float32)
        return var_vals
    def backward(self, grad_output: xp.ndarray) -> xp.ndarray:
        x = self.parent_tensors[0]
        if not x.requires_grad:

    ...
    def get_dali_pipeline(self, batch_size: int, shuffle: bool = True, 
        ...
        def image_pipeline():
            ...
            if device == "gpu":
                ...
                # GPU-based random crop and resize
                ...
                # CPU-based random crop and resize
                images = fn.random_resized_crop(
                    images,
                    device="cpu",
                    random_area=[0.08, 1.0],
...
class DataLoader:
    ...
    def __init__(
        ...
        if DALI_AVAILABLE:
            try:
                if self.device == "gpu":
                    PreallocateDeviceMemory(0.5 * 1024**3)  # 0.5GB GPU memory
                PreallocatePinnedMemory(0.25 * 1024**3)  # 0.25GB pinned memory
            except Exception as e:

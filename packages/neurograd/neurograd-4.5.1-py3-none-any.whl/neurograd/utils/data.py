...
class ImageFolder(Dataset):
    ...
    def _load_image_opencv(self, path: str) -> np.ndarray:
        ...
        if arr.ndim == 3 and arr.shape[2] == 3: arr = cv2.cvtColor(arr, cv2.COLOR_BGR2RGB)
        elif arr.ndim == 3 and arr.shape[2] == 4: arr = cv2.cvtColor(arr, cv2.COLOR_BGRA2RGBA)
...
class DataLoader:
    ...
    def __iter__(self):
        if self.shuffle: self.dataset.shuffle(seed=self.seed)
    ...
    def _batch_indices(self):
        n = len(self.dataset)
        order = list(range(n))
        # Shuffling is now handled once in __iter__ for the whole dataset
        # if self.shuffle: random.Random(self.seed).shuffle(order)
    ...
    def _gather_batch(self, futures_or_results):
        batch = [f.result() for f in futures_or_results] if self.num_workers > 0 else futures_or_results
        Xs, ys = zip(*batch)

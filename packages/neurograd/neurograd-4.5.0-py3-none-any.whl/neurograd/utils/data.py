...
class ImageFolder(Dataset):
    ...
    def __init__(
        ...
        chw: bool = True,
        cache_dir: Optional[str] = None,  # New: Local cache directory
        cache_size_limit: int = 20 * 1024**3,  # New: 20GB cache limit
        cache_strategy: str = "lru",  # New: "fifo" or "lru"
    ):
        self.root = root
        ...
        if self.cache_dir:
            ...
            print(f"Cache initialized: {self.cache_current_size/(1024**3):.2f}GB used of {self.cache_size_limit/(1024**3):.2f}GB limit")
    ...
    def _get_cached_path(self, original_path):
        """Get the cached path for an image, copying to cache if needed"""
        if not self.cache_available:
            return original_path
            
        # Create a unique cache key based on the file path and parameters
        cache_key = hashlib.md5(
            f"{original_path}_{self.img_shape}_{self.img_mode}_{self.img_normalize}".encode()
        ).hexdigest()
        
        cached_path = os.path.join(self.cache_dir, cache_key + ".npy")
        ...
        # Not cached yet - need to copy and process
        return self._add_to_cache(original_path, cached_path)
    
    def _add_to_cache(self, original_path, cached_path):
        """Add an image to cache with size management"""
        # Load and process the image
        image = self._load_image_opencv(original_path)
        ...
        with self.cache_lock:
            # Update cache size and manage if over limit
            self.cache_current_size += file_size
            self.cache_access_times[cached_path] = time.time()
            
            # Manage cache size if over limit
    ...
    def _manage_cache_size(self):
        ...
        if self.cache_strategy == "lru":
            ...
            # Just use the keys in arbitrary order (we don't have creation time)
            items_to_remove = list(self.cache_access_times.items())
            
        # Remove files until we're under the limit
        for cached_path, _ in items_to_remove:
            if self.cache_current_size <= self.cache_size_limit * 0.9:  # Leave 10% buffer
                break
            ...
            if os.path.exists(cached_path):
                ...
                self.cache_current_size -= file_size
                del self.cache_access_times[cached_path]
        ...
        if os.path.exists(self.cache_dir):
            for file in os.listdir(self.cache_dir):
                if file.endswith('.npy'):
                    ...
                    self.cache_access_times[file_path] = os.path.getctime(file_path)
    ...
    def get_dali_pipeline(self, batch_size: int, shuffle: bool = True, 
        ...
        if self.cache_available:
            # For DALI, we can't use the cache directly, but we can use the original paths
            # since the cache is for the OpenCV fallback
            pass
            
        @pipeline_def(batch_size=batch_size, num_threads=num_threads, device_id=0 if is_gpu else None, seed=seed,
                      prefetch_queue_depth=prefetch)
        def image_pipeline():
            images, labels = fn.readers.file(files=image_paths, labels=self.numeric_targets, 
    ...
    def __getitem__(self, idx: int):
        """Get single item with caching support"""
        img_path = self.images[idx]
        target = self.numeric_targets[idx]
        
        # Use cached version if available
        if self.cache_available:
            cached_path = self._get_cached_path(img_path)
            image = np.load(cached_path)
        else:
...
class DataLoader:
    """
    Enhanced DataLoader with better network storage optimizations
    """
    def __init__(
        self,
        ...
        prefetch_batches: int = 4,  # Increased default prefetch for network storage
        drop_last: bool = False,
        seed: Optional[int] = None,
        cache_prefetch: bool = True,  # New: Pre-cache batches in background
    ):
        ...
        # Adjust workers based on storage type
        if num_workers is None:
            if hasattr(dataset, 'dont_use_mmap') and dataset.dont_use_mmap:
                # More workers for network storage
        ...
        self.seed = seed
        self.cache_prefetch = cache_prefetch and hasattr(dataset, 'cache_available') and dataset.cache_available
        
        # Pre-cache management
    ...
    def _start_prefetching(self):
        """Start pre-caching batches in the background"""
        if not self.cache_prefetch:
            return
            
        self.prefetch_executor = ThreadPoolExecutor(max_workers=2)  # Dedicated prefetch workers
        
        # Pre-cache the first few batches
        batches = list(self._batch_indices())
        for i in range(min(10, len(batches))):  # Pre-cache first 10 batches
            batch_indices = batches[i]
            self.prefetch_futures[i] = self.prefetch_executor.submit(
                self._precache_batch, batch_indices
            )
    
    def _precache_batch(self, indices):
        """Pre-cache a batch of images"""
        for idx in indices:
            try:
                # This will trigger caching if not already cached
                self.dataset._get_cached_path(self.dataset.images[idx])
            except:
                pass  # Don't fail entire batch on single image error
    ...
    def _regular_iterator(self):
        """Enhanced iterator with pre-caching support"""
        batches = list(self._batch_indices())
        
        # Pre-cache next batches in background
        if self.cache_prefetch:
            for i in range(len(batches)):
                if i not in self.prefetch_futures:
                    self.prefetch_futures[i] = self.prefetch_executor.submit(
                        self._precache_batch, batches[i]
                    )

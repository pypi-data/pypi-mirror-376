...
class ImageFolder(Dataset):
    ...
    def get_dali_pipeline(self, batch_size: int, shuffle: bool = True, 
        ...
        def image_pipeline():
            ...
            if is_gpu:
                # Use mixed precision for decode + resize in one operation
                images = fn.decoders.image_resize(
                    images, 
                    device="mixed",
                    output_type=types.RGB,
                    resize_x=w,
                    resize_y=h,
                    interp_type=types.INTERP_LINEAR  # Faster than default cubic
                )
            else:
                # CPU-only processing with optimized resize
                images = fn.decoders.image_resize(
                    images,
                    device="cpu", 
                    output_type=types.RGB,

...
class DC_and_CE_with_MSE_loss(nn.Module):
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor, target_mse: torch.Tensor):
        """
        ...
        mse_loss = self.mse(net_output[:,1,...], target_mse) if self.weight_mse != 0 else 0

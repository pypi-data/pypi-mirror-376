...
class BoolHybridArray(MutableSequence,Exception):
    class _CompactBoolArray(Sequence):
    ...
    def __init__(self, split_index: int, size=None, is_sparse=False) -> None:
        ...
        for existing_array, existing_hash in hybrid_array_cache:
            if all(self[i] == existing_array[i] for i in range(len(self))):
                self._cached_hash = existing_hash
                return
        new_hash = len(hybrid_array_cache)
        self._cached_hash = new_hash
        hybrid_array_cache.append((self, new_hash))
    def __hash__(self):

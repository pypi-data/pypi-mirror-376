...
class LegalCorpus:
    ...
    def read_corpus(self, dir_of_text_files=None):
        """
        Builds a structured dictionary from COCA text files.
        
        Parameters:
        - dir_of_text_files: Directory containing COCA text folders
                           (defaults to self.data_dir if None)
        
        Returns:
        - Dictionary structured as: {genre -> year/file_num -> id -> text}
        """
        ...
        # Loop through genre folders with progress bar
        for genre_folder in tqdm(genre_folders, desc="Genres", unit="genre"):
            genre = genre_folder.split('_')[1]  # Extract genre from folder name like 'text_acad_isi'
            print(f'Processing genre: {genre}')  # retain per-genre message for transparency

...
def test_mqtt_pubsub(host, ca_certs, cert_file, key_file, resource_id):
    ...
    def on_publish(client, userdata, mid, reason_code, properties):
        print(f"on_publish", userdata, mid, reason_code, properties)
    ...
    client.tls_set(ca_certs=ca_certs, certfile=cert_file, keyfile=key_file, cert_reqs=ssl.CERT_NONE)
...
def test_mqtt(host: str, cert_config: dict, use_legacy_cert: bool = False):
    cert_file_name = "client_cert_chain_legacy_file" if use_legacy_cert else "client_cert_chain_file"
    test_mqtt_pubsub(
        host,
        ca_certs=cert_config["root_ca_file"],
        cert_file=cert_config[cert_file_name],

...
class RawComponentsClient:
    ...
    def create_weather_map(
    ...
    ) -> HttpResponse[WeatherMapComponent]:
        """
        ...
        """
        _response = self._client_wrapper.httpx_client.request(
            "api/v1/dashboard/components/weather-map",
            method="POST",
            params={
                "name": name,
            },
            json={
                "config": convert_and_respect_annotation_metadata(
                    object_=config, annotation=WeatherMapConfig, direction="write"
                ),
            },
            headers={
                "content-type": "application/json",
        ...
        try:
            if 200 <= _response.status_code < 300:
                _data = typing.cast(
                    WeatherMapComponent,
                    parse_obj_as(
                        type_=WeatherMapComponent,  # type: ignore
                        object_=_response.json(),
                    ),
                )
                return HttpResponse(response=_response, data=_data)
    ...
    def update_weather_map_component(
        ...
        *,
        ...
        """
        _response = self._client_wrapper.httpx_client.request(
            f"api/v1/dashboard/components/weather-map/{jsonable_encoder(component_id)}",
            method="PUT",
            json={
                "id": id,
                "userId": user_id,
...
class AsyncRawComponentsClient:
    ...
    async def create_weather_map(
        self, *, name: str, config: WeatherMapConfig, request_options: typing.Optional[RequestOptions] = None
    ) -> AsyncHttpResponse[WeatherMapComponent]:
        """
        Create a new weather-map component.

...
async def add_heading(filename: str, text: str, level: int = 1) -> str:
    ...
    try:
        ...
        # Try to add heading with built-in style; if missing, create style and retry
        try:
            heading = doc.add_heading(text, level=level)
            doc.save(filename)
            return f"Heading '{text}' (level {level}) added to {filename}"
        except Exception as add_err:
            # Attempt to ensure a heading style exists, then apply it explicitly
            try:
                style_name = 'Heading ' + str(level)
                try:
                    _ = doc.styles[style_name]
                except KeyError:
                    # Create a basic heading style if it does not exist
                    new_style = doc.styles.add_style(style_name, WD_STYLE_TYPE.PARAGRAPH)
                    try:
                        new_style.base_style = doc.styles['Normal']
                    except Exception:
                        pass
                    # Basic font settings resembling a heading
                ...
                doc.save(filename)
                return f"Heading '{text}' (level {level}) added to {filename} using created style '{style_name}'"
            except Exception:
                # Fall back to direct formatting as a last resort
                paragraph = doc.add_paragraph(text)
                try:

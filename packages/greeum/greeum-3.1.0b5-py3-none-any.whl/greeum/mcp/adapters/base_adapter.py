...
**Generated**: Unified MCP v2.2.7"""
    ...
    def _auto_select_or_initialize_slot(self, stm_manager) -> str:
        """
        P1: 자동으로 STM 슬롯을 선택하거나 초기화
        ...
        if not stm_manager:
            return "A"  # Fallback to A if no STM manager
        ...
        # 모든 슬롯이 비활성이면 A 슬롯 초기화
        if not active_slots:
            logger.info("P1: No active slots found, initializing slot A")
            # 첫 메모리를 슬롯 A의 헤드로 설정될 예정
        ...
        logger.debug(f"P1: Selected slot {most_recent_slot} (active: {active_slots})")
    ...
    def _add_memory_via_core(self, content: str, importance: float = 0.5) -> Dict[str, Any]:
        ...
        # Let smart routing handle slot selection
        # Only use manual slot selection as fallback
        import os
        smart_routing_enabled = os.environ.get('GREEUM_SMART_ROUTING', '1').lower() in ('1', 'true', 'yes')

        if smart_routing_enabled:
            # Let BlockManager's smart routing choose the slot
            slot = None
        else:
            # P1: 슬롯 자동 선택/초기화 로직 (fallback)
        ...
        try:
            ...
            block_result = block_manager.add_block(
                ...
                metadata={'source': 'mcp'},
                slot=slot  # None if smart routing, else auto-selected
            )

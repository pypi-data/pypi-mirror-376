    ...
    def add_block(self, context: str, keywords: List[str], tags: List[str], 
        ...
        try:
            ...
            if slot:
                ...
                # Smart routing with DFS search
                import os
                # For testing, default to True. In production, use environment variable
                smart_routing = os.environ.get('GREEUM_SMART_ROUTING', '1').lower() in ('1', 'true', 'yes')

                if smart_routing:
                    # Use DFS to find best matching slot
                    logger.info(f"Smart routing: Finding optimal slot for '{context[:50]}...'")

                    # Lightweight DFS search from STM heads
                    from .dfs_search import DFSSearchEngine
                    dfs_engine = DFSSearchEngine(self.db_manager)

                    # Search with current context and embedding
                    search_results, search_meta = dfs_engine.search_with_dfs(
                        query=context,
                        query_embedding=embedding,
                        slot=None,  # Check all slots
                        entry="head",  # Start from heads
                        depth=2,  # Shallow search for speed
                        limit=5,  # Few results for quick decision
                        fallback=False  # No global fallback needed
                    )

                    # Find best slot based on search results
                    best_slot = None
                    best_similarity = 0.0

                    if search_results:
                        # Check which slot has the most similar content
                        for result in search_results:
                            if 'similarity' in result:
                                sim = result['similarity']
                                # Find which slot this result belongs to
                                result_hash = result.get('hash')
                                if result_hash:
                                    for slot_key, head_hash in stm.branch_heads.items():
                                        if head_hash == result_hash and sim > best_similarity:
                                            best_slot = slot_key
                                            best_similarity = sim
                                            break

                    # Decision logic based on similarity
                    if best_slot and best_similarity > 0.7:
                        # High similarity: use the same branch
                        slot = best_slot
                        logger.info(f"Smart routing: Using slot {slot} (similarity: {best_similarity:.3f})")
                    elif best_slot and best_similarity > 0.4:
                        # Medium similarity: might branch from here
                        # Find empty slot for branching
                        empty_slots = [s for s in ['A', 'B', 'C'] if not stm.branch_heads.get(s)]
                        if empty_slots:
                            slot = empty_slots[0]
                            # Connect to similar block as parent
                            if search_results:
                                before_id = search_results[0].get('hash', before_id)
                            logger.info(f"Smart routing: Branching to new slot {slot} from similar content")
                        else:
                            # All slots full, use LRU
                            slot = min(
                                stm.slot_hysteresis.keys(),
                                key=lambda k: stm.slot_hysteresis[k]["last_seen_at"]
                            )
                            logger.info(f"Smart routing: Using LRU slot {slot} (all slots occupied)")
                    else:
                        # Low similarity or no results: new independent branch
                        empty_slots = [s for s in ['A', 'B', 'C'] if not stm.branch_heads.get(s)]
                        if empty_slots:
                            slot = empty_slots[0]
                            logger.info(f"Smart routing: Starting new branch in slot {slot}")
                        else:
                            # Use least recently used slot
                            slot = min(
                                stm.slot_hysteresis.keys(),
                                key=lambda k: stm.slot_hysteresis[k]["last_seen_at"]
                            )
                            logger.info(f"Smart routing: Replacing LRU slot {slot} with new branch")

                    # Update the chosen slot
                    stm.update_head(slot, current_hash)
                    logger.info(f"Smart routing: Updated slot {slot} head to {current_hash[:8]}...")

                    # Store smart routing info in metadata
                    placement_type = 'existing_branch' if best_similarity > 0.7 else ('divergence' if best_similarity > 0.4 else 'new_branch')
                    enhanced_metadata['smart_routing'] = {
                        'enabled': True,
                        'slot_updated': slot,
                        'similarity_score': best_similarity,
                        'placement': placement_type,
                        'parent': parent_hash[:8] if parent_hash else 'root'
                    }

                    # Update metrics
                    self.metrics['smart_routing_used'] = self.metrics.get('smart_routing_used', 0) + 1
                    self.metrics['avg_routing_similarity'] = (
                        self.metrics.get('avg_routing_similarity', 0) * 0.9 + best_similarity * 0.1
                    )

                else:
                    # Original logic when smart routing is disabled
                    # P1: Auto-initialize first slot if all are empty
                    if not any(stm.branch_heads.values()):
                        slot = "A"
                        stm.update_head(slot, current_hash)
                        logger.info(f"P1: Initialized first slot A with head {current_hash[:8]}...")
                    else:
                        ...
                        logger.info(f"P1: Updated most recent slot {most_recent_slot} head to {current_hash[:8]}...")

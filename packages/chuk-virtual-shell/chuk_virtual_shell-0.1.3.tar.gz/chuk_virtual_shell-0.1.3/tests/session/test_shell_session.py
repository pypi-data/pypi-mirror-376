...
class TestShellSessionManager:
    ...
    async def test_session_persistence(self, session_manager):
        ...
        async for _ in session_manager.run_command(
            session_id=session_id,
            command="mkdir /persistdir"
        ):
            pass
        
        async for _ in session_manager.run_command(
            session_id=session_id,
            command="cd /persistdir"
        ):
            pass
        
        async for _ in session_manager.run_command(
            session_id=session_id,
            command="export PERSIST_VAR=test"
        ):
            pass
    ...
    async def test_multiple_sessions(self, session_manager):
        ...
        async for _ in session_manager.run_command(
            session_id=session1,
            command="export VAR1=session1"
        ):
            pass
        
        async for _ in session_manager.run_command(
            session_id=session2,
            command="export VAR2=session2"
        ):
            pass
        
        async for _ in session_manager.run_command(
            session_id=session3,
            command="export VAR3=session3"
        ):
            pass

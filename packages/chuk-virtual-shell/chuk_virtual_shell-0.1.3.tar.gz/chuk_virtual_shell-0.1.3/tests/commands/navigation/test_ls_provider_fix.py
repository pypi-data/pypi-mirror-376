...
class TestFileSystemCompatProviderFix:
    ...
    def test_ls_command_pattern_with_filesystem_compat(self):
        ...
        def get_node_info(path):
            if "file1" in path:
                return Mock(is_dir=False, name="file1.txt", size=100, permissions="rw-r--r--", modified="2024-01-01")
            elif "file2" in path:
                return Mock(is_dir=False, name="file2.txt", size=200, permissions="rw-r--r--", modified="2024-01-02")
            else:
                return Mock(is_dir=True, name="tmp", size=4096, permissions="rwxr-xr-x", modified="2024-01-01")

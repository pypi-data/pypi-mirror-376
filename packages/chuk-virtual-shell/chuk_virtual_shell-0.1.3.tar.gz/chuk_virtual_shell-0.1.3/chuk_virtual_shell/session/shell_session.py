...
class ShellSession:
    ...
    def _setup_pty(self):
        ...
        if sys.platform == "win32":
            ...
            logger.warning("PTY mode not supported on Windows, falling back to PIPE mode")
    ...
    async def run(
        ...
        try:
            ...
            if self.mode == SessionMode.PTY and self.pty_master:
                ...
                async for chunk in self._run_pty(command_id, command, timeout_ms, soft_timeout_ms, stream):
                    yield chunk
            else:
                # Standard pipe mode
                async for chunk in self._run_pipe(command_id, command, timeout_ms, soft_timeout_ms, stream):
                    yield chunk

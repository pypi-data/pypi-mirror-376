...
class ControlFlowExecutor:
    ...
    def _parse_if_structure(self, tokens: List[Token]) -> Optional[dict]:
        ...
        """
        structure: Dict[str, Any] = {"branches": []}
        current_branch: Optional[Dict[str, Any]] = None
        current_section: Optional[str] = None  # 'condition' or 'commands'
        ...
        while i < len(tokens):
            ...
            if token.type == TokenType.KEYWORD:
                if token.value == "if":
                    ...
                    elif current_branch and current_section == "commands":
                ...
                if current_branch and current_section == "commands":
    ...
    def _execute_commands(self, command_tokens: List[Token]) -> str:
        ...
        for cmd_tokens in commands:
            ...
            if (
                cmd_tokens
                and len(cmd_tokens) == 1
                and cmd_tokens[0].type == TokenType.KEYWORD
            ):
            ...
            if token.type == TokenType.KEYWORD:
                if token.value == "for":
                    if depth == 0 and section is None:
                        ...
                        if section == "commands" and "commands" in structure:
                ...
                elif section == "items" and "items" in structure:
                    if token.type != TokenType.SEPARATOR:
                        structure["items"].append(token)
                elif section == "commands" and "commands" in structure:
                    structure["commands"].append(token)
    ...
    def _parse_while_structure(
        self, tokens: List[Token], loop_type: str
    ) -> Optional[dict]:
        """

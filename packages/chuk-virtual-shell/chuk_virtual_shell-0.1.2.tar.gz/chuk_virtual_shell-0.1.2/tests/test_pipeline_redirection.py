...
def test_complex_pipeline(shell):
    ...
    # Create CSV data
    shell.fs.write_file("/tmp/data.csv", "Alice,30,Engineer\nBob,25,Designer\nCharlie,35,Manager\nDiana,28,Developer")
    
    # Complex pipeline - simpler awk syntax
    result = shell.execute('cat /tmp/data.csv | awk -F, \'{print $1 "," $2}\' | sort | head -n 2')
...
def test_pipeline_with_grep_and_wc(shell):
    ...
    # Create log file
    shell.fs.write_file("/tmp/app.log", "INFO message\nERROR failed\nINFO ok\nERROR timeout\nWARN slow")

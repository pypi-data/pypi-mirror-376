...
def dummy_fs_structure():
    ...
    return {
        "/": {"home": {}, "tmp": {}, "var": {}},
        "/home": {"user": {}},
        "/home/user": {"doc.txt": "Document content", "notes.md": "Notes content", "projects": {}},
        "/home/user/projects": {"project1": {}, "project2": {}},
        "/home/user/projects/project1": {"README.md": "Project 1 readme", "main.py": "print('Hello')"},
        "/home/user/projects/project2": {"README.md": "Project 2 readme", "config.json": '{"key": "value"}'},
            ...
            for key in dummy_fs_structure.keys():
                if key.startswith(path):
                    if key != path:  # Don't include the path itself
                        if recursive or '/' not in key[len(path)+1:]:  # Check depth if not recursive
                            results.append(key)
            return results
...
def test_find_non_existent(find_command):
    ...
    assert "no such file or directory" in output.lower() or "cannot access" in output.lower()

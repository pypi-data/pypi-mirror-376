...
async def test_mcp_command_async_execution_import_success():
    ...
    mock_send_tools_call = AsyncMock(return_value={"content": [{"type": "text", "text": "Success!"}]})
    
    # Set up module mocks
    mock_modules['chuk_mcp.mcp_client.transport.stdio.stdio_client'].stdio_client = mock_stdio_client
    mock_modules['chuk_mcp.mcp_client.messages.initialize.send_messages'].send_initialize = mock_send_initialize
    mock_modules['chuk_mcp.mcp_client.messages.ping.send_messages'].send_ping = mock_send_ping
    
    with patch.dict('sys.modules', mock_modules), \
         patch('chuk_virtual_shell.commands.mcp.mcp_command_loader.send_tools_call', mock_send_tools_call):
...
async def test_mcp_command_async_execution_error_response():
    ...
    mock_send_initialize = AsyncMock(return_value=True)
    mock_send_ping = AsyncMock(return_value=True)
    mock_send_tools_call = AsyncMock(return_value={"error": {"message": "Tool execution failed"}})

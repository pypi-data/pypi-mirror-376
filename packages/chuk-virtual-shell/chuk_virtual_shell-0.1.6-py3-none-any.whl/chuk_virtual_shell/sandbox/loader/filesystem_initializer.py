...
def create_filesystem(config: Dict[str, Any]) -> VirtualFileSystem:
    ...
    if provider_name == "s3":
        ...
        if (
            "TIGRIS_ACCESS_KEY_ID" in os.environ
            and "TIGRIS_SECRET_ACCESS_KEY" in os.environ
        ):
        ...
        if any(
            key in os.environ
            for key in ["TIGRIS_ACCESS_KEY_ID", "TIGRIS_SECRET_ACCESS_KEY"]
        ):
            if (
                "endpoint_url" not in provider_args
                and "AWS_ENDPOINT_URL_S3" not in os.environ
            ):
        ...
        is_tigris = (
            "t3.storage.dev" in provider_args.get("endpoint_url", "").lower()
            or "tigris" in provider_args.get("endpoint_url", "").lower()
        )
    ...
    logger.info(
        f"  S3 Addressing: {provider_args.get('s3_addressing_style', 'default')}"
    )
    ...
    if "aws_access_key_id" in provider_args:
        ...
        logger.info(
            f"  Access key ID: {key_id[:8]}...{key_id[-4:] if len(key_id) > 12 else '***'}"
        )
        ...
        if provider_name == "s3":
            ...
            if is_tigris:
                ...
                if not has_tigris_creds and not has_aws_creds:
                    ...
                    logger.error(
                        "Using AWS credentials for Tigris - this might work if configured correctly."
                    )
            else:

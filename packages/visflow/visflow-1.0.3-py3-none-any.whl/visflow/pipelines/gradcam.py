...
class GradCAMPipeline(BasePipeline):
    SUPPORTED_EXTENSIONS: t.ClassVar[t.Set[str]] = {'.jpg',
                                                    '.jpeg',
                                                    '.png',
                                                    '.bmp',
                                                    '.tiff',
    ...
    def __init__(
        ...
        *,
        ...
        alpha: float = 0.5,
        colormap: t.Literal[
            'jet', 'turbo', 'viridis', 'inferno', 'plasma'] = 'jet',
    ...
    def _step(
        ...
        if self.heatmap_only:
            ...
            ori_image = cv2.resize(cv2.imread(str(image_file)), (224, 224))
    ...
    def __call__(self) -> None:
        ...
        for idx, image_file in enumerate(self.image_files, 1):
            try:
                self._step(
                    image_file, gradcam, transform, idx, total_images
                )
                spinner.info(
                    f'Processed {image_file} ({idx}/{total_images})'
                )
            except Exception as e:
                spinner.warn(
                    f'Failed to process {image_file}: {e}'
                )
                continue

        if total_images == 1:
            output_type = ("heatmap"
                           if self.heatmap_only else "Grad-CAM image")
            spinner.succeed(f'{output_type} saved to {self.output_dir}')
        else:
            output_type = ("heatmaps"
                           if self.heatmap_only else "Grad-CAM images")
            spinner.succeed(
                f'{total_images} {output_type} saved to {self.output_dir}'
            )

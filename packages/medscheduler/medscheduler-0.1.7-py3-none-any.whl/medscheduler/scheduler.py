...
class AppointmentScheduler:
    ...
    date_ranges : list[tuple[date-like, date-like]], optional
        ...
        If not provided, defaults to a single range spanning the last 12 months
        up to `ref_date` (or today if `ref_date` is None).
    ref_date : date-like, optional
        ...
        If not provided, defaults to today at midnight, and is also used as the upper bound
        of the default `date_ranges` (with `booking_horizon` days added internally
        to extend availability into the future).
    ...
    working_hours : list[tuple[int, int]] | tuple[int, int] | list[tuple[str, str]] | tuple[str, str], default [(8, 18)]
        ...
        Accepts integer hours (e.g., (8, 16)) or strings "HH" / "HH:MM" with minutes == 00
    ...
    booking_horizon : int, default 30
        ...
        the end of the latest range in `date_ranges`.
    ...
    def __init__(
        ...
        # ============================
        # VALIDATION: date_ranges & ref_date defaults
        # ============================
        if ref_date is None:
            # Default reference is "today" (start of day, naive)
            ref_dt_default = datetime.today().replace(hour=0, minute=0, second=0, microsecond=0)
        else:
            ref_dt_default = self._to_datetime(ref_date).replace(hour=0, minute=0, second=0, microsecond=0)

        if date_ranges is None:
            # Base historical window: [ref_date - 365d, ref_date @ 23:59]
            base_start = (ref_dt_default - timedelta(days=365)).replace(hour=0, minute=0, second=0, microsecond=0)
            base_end   = ref_dt_default.replace(hour=23, minute=59, second=0, microsecond=0)
            self.date_ranges: list[tuple[datetime, datetime]] = [(base_start, base_end)]
        else:
        ...
        # If user supplied ref_date, validate it's inside at least one range.
        # If not supplied, stick with the computed ref_dt_default.
        ...
        # Effective ranges extend ONLY the latest end by booking_horizon (future bookable window)

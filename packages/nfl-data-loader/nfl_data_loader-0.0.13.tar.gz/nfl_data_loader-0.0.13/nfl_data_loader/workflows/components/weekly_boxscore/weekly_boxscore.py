...
from nfl_data_loader.workflows.components.players.state import WeeklyPlayer
...
STATE_NUM_COLS = [
    'activated', 'declared_inactive', 'starter', 'played',
    'offensive_snaps_played', 'defense_snaps_played', 'st_snaps_played',
    'total_offensive_snaps', 'total_defensive_snaps', 'total_st_snaps',
'depth_chart_rank',
    #'offensive_snap_share', 'defensive_snap_share', 'st_snap_share'
]
...
class WeeklyBoxscoreComponent:
    ...
    def extract(self) -> Dict[str, pd.DataFrame]:
        ...
        wp = WeeklyPlayer(self.load_seasons, self.season_type)
        player_state = wp.run_pipeline()
    ...
    def _prepare_state_features(self, s: pd.DataFrame) -> pd.DataFrame:
        s = s.copy()

        # Ensure needed columns exist (robust to partial data)
        for c in ['offensive_snaps_played', 'defense_snaps_played', 'st_snaps_played',
                  'total_offensive_snaps', 'total_defensive_snaps', 'total_st_snaps',
                  'activated', 'declared_inactive', 'starter', 'played','depth_chart_rank',]:
            if c not in s.columns:
                s[c] = pd.NA

        # cast booleans to 0/1 for aggregation
        for b in ['activated', 'declared_inactive', 'starter', 'played']:
            s[b] = s[b].fillna(False).astype(int)

        return s[['season', 'week', 'team', 'player_id', 'position', 'position_group','depth_chart_position'] +
                 ['activated', 'declared_inactive', 'starter', 'played',
                  'offensive_snaps_played', 'defense_snaps_played', 'st_snaps_played',
                  'total_offensive_snaps', 'total_defensive_snaps', 'total_st_snaps','depth_chart_rank',]]
    ...
    def make_stat_collection(self, stat_mode: str) -> pd.DataFrame:
        """
        ...
        numeric_cols_present = [c for c in ALL_NUM_COLS if c in df.columns]

        # ----- MERGE PLAYER STATE WHEN IN PLAYER MODE -----
        if stat_mode == "player":
            # bring state on the same keys you already use
            state = self._prepare_state_features(self.db['player_state']).copy()
            df = df.merge(
                state,
                on=['season', 'week', 'team', 'player_id', 'position', 'position_group','depth_chart_position'],
                how='left'
            )

            # actual, per-game state columns are numeric so we can aggregate later
            numeric_cols_present += [c for c in STATE_NUM_COLS if c in df.columns]
        ...
        if stat_mode in ["team", "opponent"]:
            ...
            # keep schema consistent: add state columns as NA
            for c in ['activated', 'declared_inactive', 'starter', 'played',
                      'offensive_snaps_played', 'defense_snaps_played', 'st_snaps_played',
                      'total_offensive_snaps', 'total_defensive_snaps', 'total_st_snaps',
                      'offensive_snap_share', 'defensive_snap_share', 'st_snap_share']:
    ...
    def make_dynamic_aggregations(self,
        ...
        if attrs is None:
            ...
            attrs = [c for c in ALL_NUM_COLS+ENGINEERED_COLS+STATE_NUM_COLS  + ([] if mode == 'Player' else ['n_players']) if c in base_df.columns]
    ...
    def make_view(self, mode, filters: dict):
        ...
        for type_key in ["Actual", "Average"]:
...
if __name__ == '__main__':
    pdc = WeeklyBoxscoreComponent(list(range(2024, 2026)))

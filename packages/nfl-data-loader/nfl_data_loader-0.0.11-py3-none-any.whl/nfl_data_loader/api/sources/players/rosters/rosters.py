...
def collect_roster(year):
    try:
        ...
        player_nfld_df = collect_players()[['player_id', 'espn_id', 'pfr_id','birth_date','position', 'latest_team','status_abbr', 'status','years_of_experience','jersey_number']]
    ...
    if player_nfld_df['week'].isnull().sum() > 0:
        player_nfld_df['week'] = 1
        player_nfld_df['week'] = player_nfld_df['week'].astype(int)
...
def collect_depth_chart(season):
    try:
        ...
        if 'week' not in data.columns:
            ...
            data = data.drop_duplicates(['player_id','formation'],keep='first')
        ...
        data = data.rename(columns={'depth_team':'depth_chart_rank','formation':'depth_formation'})
        return data
    except Exception as e:
        return pd.DataFrame()
...
def get_snap_counts(season):
    try:
        data = pd.read_parquet(f'https://github.com/nflverse/nflverse-data/releases/download/snap_counts/snap_counts_{season}.parquet')
        data = team_id_repl(data)
        return data
    except Exception as e:
        return pd.DataFrame()

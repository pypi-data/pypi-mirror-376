...
def calculate_ranks(df, group_by_col, rank_cols_methods):
    ...
    for col, method in rank_cols_methods.items():
        ...
        rank_col_series = df.groupby(group_by_col, as_index=False)[col].rank(
            method=method, ascending=(method == 'min')
        )
        # Use pandas nullable integer dtype so NA values are preserved instead of raising
        # when attempting to cast floats with NaN to a numpy int dtype.
        rank_col_df = rank_col_series.astype("Int64")

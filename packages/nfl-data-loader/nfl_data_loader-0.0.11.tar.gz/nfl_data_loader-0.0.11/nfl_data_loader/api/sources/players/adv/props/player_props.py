...
from nfl_data_loader.schemas.players.action_network import ACTION_NETWORK_ID_MAPPER
...
def _get_games(season, week, season_type, access_token=None):
    ...
    games_client = GamesClient(default_headers=default_headers)
...
def hunt_player_merge_ids(players_df, season, week):
    players_df = players_df[['player_id', 'abbr', 'position', 'team']].rename(columns={'player_id': 'action_network_player_id', 'abbr': 'join_name'}).copy()
    ...
    roster_df = roster_df[((roster_df['season'] == season) & (roster_df['week'] == week))].copy()
    ...
    players_df = pd.merge(roster_df[['season', 'week', 'team', 'player_id', 'position_group', 'join_name']], players_df, how='left', on=['season', 'week', 'team', 'position_group', 'join_name'], )

    players_df = pd.concat([players_df, team_df], ignore_index=True)

    new_matches = players_df[players_df.action_network_player_id.notnull()].copy()
    new_matches['action_network_player_id'] = new_matches['action_network_player_id'].astype(int).astype(str)
    new_matches = new_matches[~new_matches.action_network_player_id.isin(ACTION_NETWORK_ID_MAPPER.keys())]

    cant_match = players_df[players_df.action_network_player_id.isnull()].copy()

    res = dict(zip(list(new_matches['action_network_player_id']), list(new_matches['player_id'])))
    print(f"{len(res)} New Players found on hunt")
    print(res)

def get_player_props(season, week, season_type, access_token=None, debug=False):
    if access_token:
        default_headers = {
            "access_token": access_token
        }
    ...
    if debug:
        hunt_player_merge_ids(players_df, season, week)

    player_props_df = pd.merge(player_props_df[PROP_COLS], players_df[['team','player_id']], on=['player_id'], how='left')

    player_props_df = pd.concat([player_props_df, game_props_df], ignore_index=True)
    player_props_df = player_props_df.rename(columns={'player_id':'action_network_player_id'})
    player_props_df['action_network_player_id'] = player_props_df['action_network_player_id'].fillna(-1).astype(int).astype(str)
    player_props_df['player_id'] = player_props_df['action_network_player_id'].map(ACTION_NETWORK_ID_MAPPER)
    ...
    player_props_df['book_id'] = player_props_df['book_id'].map(MY_LINES)

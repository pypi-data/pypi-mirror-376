...
from nfl_data_loader.api.sources.players.boxscores.boxscores import collect_weekly_espn_player_stats, collect_weekly_player_stats
...
def stat_collection(year, season_type="REG", mode='team'):
    ...
    player_stats_df = collect_weekly_player_stats(year, season_type=season_type)
    player_stats_df = player_stats_df[player_stats_df.season==year].copy()
    player_stats_df = player_stats_df.rename(columns={'sacks_suffered':'sacks','sack_yards_lost':'sack_yards','team':'recent_team','passing_interceptions':'interceptions'})
    if 'dakota' not in player_stats_df.columns:
        player_stats_df['dakota'] = None
...
if __name__ == '__main__':
    df = stat_collection(2025, season_type='REG', mode='team')
    df
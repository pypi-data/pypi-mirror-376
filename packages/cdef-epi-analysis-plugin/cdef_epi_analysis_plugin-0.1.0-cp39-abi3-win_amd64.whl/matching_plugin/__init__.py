...
def extract_parent_socio13(
    ...
    """
    Extract parental socio-economic status (SOCIO13) from AKM and categorize per SOCIO13.md.

    Parameters
    ----------
    df : pl.DataFrame
        Input dataframe with child `identifier_col`, `index_date_col`, and parent columns `CPR_MODER`, `CPR_FADER`.
    identifier_col : str
        Column for child identifier (PNR/CPR).
    index_date_col : str
        Column for index date.
    akm_registry_pattern : str
        Glob path to AKM registry files containing `PNR`, `ARET`, `SOCIO13`.
    year_offset : int, default -1
        Year offset relative to index year to measure status (e.g., -1 to avoid reverse causality).

    Returns
    -------
    pl.DataFrame
        Columns: identifier_col, index_date_col, ARET, RELATIVE_YEAR, MOR_SOCIO13_CAT, FAR_SOCIO13_CAT
    """
...
def extract_cohabitation_status(
    ...
    """
    Extract cohabitation status from BEF (FAMILIE_TYPE/CIVST) per cohabitation.md.

    Parameters
    ----------
    df : pl.DataFrame
        Input dataframe with `identifier_col` and `index_date_col`.
    identifier_col : str
        Child identifier column.
    index_date_col : str
        Index date column.
    bef_registry_pattern : str
        Glob path to BEF files with `PNR`, `ARET`, `FAMILIE_TYPE`, `CIVST`.
    year_offset : int, default -1
        Year offset relative to index year (use -1 to avoid reverse causality).

    Returns
    -------
    pl.DataFrame
        Columns: identifier_col, index_date_col, ARET, RELATIVE_YEAR, cohabitation_status
    """
...
def extract_parent_income_timeseries(
    ...
    """
    Extract income time series for both parents across a wide temporal range (for outcomes).
    ...
    index_date_col : str
        Index date column.
    ind_registry_pattern : str
        Glob to IND registry with `PNR`, `ARET` and requested `income_vars`.
    income_vars : list[str]
        Income variables to extract (e.g., ["PERINDKIALT_13", "LOENMV_13"]).
    temporal_range : tuple[int, int], default (-10, 10)
        Years relative to index year to extract (inclusive).

    Returns
    -------
    pl.DataFrame
        Columns: identifier_col, index_date_col, ARET, RELATIVE_YEAR, MOR_<var>..., FAR_<var>...
    """

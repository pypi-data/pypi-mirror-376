...
def _assert_unique_keys(lf: pl.LazyFrame, keys: list[str], context: str, sample: int = 10) -> None:
    ...
    """
    offenders = (
        lf.group_by(keys).agg(pl.len().alias("n"))
        .filter(pl.col("n") > 1)
        .limit(sample)
...
def build_parent_did_panel(
    ...
    *,
    ...
    def _normalize_columns(df: pl.DataFrame, mapping: dict[str, list[str]]) -> pl.DataFrame:
    ...
    # If still not temporal, try casting to date via any known temporal dtype
    if not pl.datatypes.is_temporal(match_df.get_column("INDEX_DATE").dtype):
        match_df = match_df.with_columns(

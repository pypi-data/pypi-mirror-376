...
class AccountContext:
    ...
    def execute_trade(self, security: str, amount: int, price: float, 
        ...
        if amount > 0:  # 买入
            ...
            if target_portfolio.available_cash < total_cost:
                return False  # 资金不足
            
            # 执行买入
            target_portfolio.reduce_cash(total_cost)
            ...
            target_portfolio.add_cash(total_value)
                ...
                for security, position in sub.positions.items():
                    if position.total_amount > 0:
                        if security in aggregated_positions:
                            ...
                            aggregated_positions[security].closeable_amount = total_amount

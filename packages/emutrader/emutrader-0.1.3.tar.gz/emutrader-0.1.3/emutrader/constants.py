...
# 账户类型常量
class AccountTypes:
    """账户类型常量 - 区分账户类型与交易标的类型"""
    # 真正的账户类型
    STOCK = "stock"                    # 股票账户（可交易：股票、场内基金、货币基金、分级基金等）
    FUTURES = "futures"                # 期货账户（可交易：商品期货、金融期货等）
    
    # 未来可能支持的账户类型
    CRYPTO = "crypto"                  # 加密货币账户（未实现）
    
    # 所有支持的账户类型
    ALL = [STOCK, FUTURES]
    
    # 已实现的账户类型
    IMPLEMENTED = [STOCK, FUTURES]
    ...
    LEGACY_MAPPING = {
        ...
        "INDEX_FUTURE": FUTURES,  # 金融期货归类为期货账户
...
class InstrumentTypes:
    ...
    @classmethod
    def get_instruments_for_account(cls, account_type: str) -> list:
        """获取指定账户类型可交易的标的类型"""
        if account_type == AccountTypes.STOCK:
            return cls.STOCK_INSTRUMENTS.copy()
        elif account_type == AccountTypes.FUTURES:
            return cls.FUTURES_INSTRUMENTS.copy()
        else:
            return []
...
class ErrorCodes:
    ...
    INSUFFICIENT_POSITION = 6002
    INVALID_SECURITY = 6003
    ORDER_ERROR = 6004
    ORDER_REJECTED = 6005
    ORDER_TIMEOUT = 6006
    ...
    # 策略上下文错误 (7000-7999)
    CONTEXT_ERROR = 7000
    CONTEXT_NOT_SET = 7001
    CONTEXT_INVALID = 7002
    
    # 投资组合错误 (8000-8999)
    PORTFOLIO_ERROR = 8000
    PORTFOLIO_DATA_ERROR = 8001
    PORTFOLIO_CALCULATION_ERROR = 8002
    
    # 子账户错误 (9000-9999)
    SUBPORTFOLIO_ERROR = 9000
    SUBPORTFOLIO_NOT_FOUND = 9001
    SUBPORTFOLIO_CONFIG_ERROR = 9002
    TRANSFER_ERROR = 9003
    
    # 配置错误 (10000-10999)
    CONFIGURATION_ERROR = 10000
    CONFIG_FILE_ERROR = 10001
    CONFIG_PARSE_ERROR = 10002

...
class Position:
    ...
    def value(self) -> float:
        ...
        if self.last_price <= 0:
            return 0.0
        return abs(self.total_amount) * self.last_price
    ...
    def pnl(self) -> float:
        ...
        if self.side == 'long':
            return self.total_amount * (self.last_price - self.avg_cost)
        else:  # short
            return self.total_amount * (self.avg_cost - self.last_price)
    ...
    def add_position(self, amount: int, cost: float) -> bool:
        ...
        if total_amount > 0:
            self.avg_cost = (old_value + new_value) / total_amount

...
class SnapshotMergeJobRemoteForensic(MergeRemoteJob):
    ...
    @staticmethod
    def _createBatchForRemoteId(job: IngestMergeJob, connection: Connection, key: str, remoteBatchId: int) -> bool:
    ...
    def _ingestSeedBatch(
            job: IngestMergeJob, sourceConn: Connection, mergeEngine: Engine, sourceTableName: str, stagingTableName: str,
        ...
        if rows:
            recordsInserted = SnapshotMergeJobRemoteForensic._insertRowsToStaging(
                job,
                mergeEngine, stagingTableName, list(rows), allColumns, batchId, sp, list(column_names))
    ...
    @staticmethod
    def genericMergeStagingToMergeAndCommitForensic(
            job: IngestMergeJob,
            mergeEngine: Engine, batchId: int, key: str, chunkSize: int = 10000) -> tuple[int, int, int]:
        ...
        with mergeEngine.begin() as connection:
            ...
            for datasetToMergeName in state.all_datasets:
                ...
                rc: Optional[Row[Any]] = count_result.fetchone()
                if rc is None:
    ...
    def _processForensicSeedBatch(
            job: IngestMergeJob, connection: Connection, stagingTableName: str, mergeTableName: str,
    ...
    @staticmethod
    def _updateBatchCounter(job: IngestMergeJob, connection: Connection, key: str, batchId: int) -> None:
        """Update the batch counter to reflect the current remote batch ID.

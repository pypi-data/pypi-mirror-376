...
class AirflowAWSComponent(Component):
    ...
    def generateBootstrapArtifacts(self, eco: Ecosystem, context: dict[str, Any], jinjaEnv: Environment, ringLevel: int) -> dict[str, str]:
        ...
        aws_secret_manager_template: Template = jinjaEnv.get_template('airflowAWS/awsSecrets.py.j2')
        aws_secret_manager_rendered: str = aws_secret_manager_template.render(context)

        comp_context: dict[str, Any] = context.copy()
        comp_context.update({
            "airflow_k8s_name": K8sUtils.to_k8s_name(self.name),
            "airflow_iam_role_arn": self.airflow_iam_role_arn,
            "platform_secret_manager": aws_secret_manager_rendered,
        })

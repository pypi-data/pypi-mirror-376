...
class DataPlatformChooser(UserDSLObject):
    ...
    through intermediate Workspaces. This is metadata that can be used to provide a hint to the system (manual or automatic) to
    choose a DataPlatform for a Workspace. If it's not provided, the system will choose a DataPlatform based on other means such a policy,
    a phone call etc."""
    def __init__(self):
        UserDSLObject.__init__(self)
...
class DSGDataPlatformAssignment(UserDSLObject):
    ...
    def __str__(self) -> str:
        return f"DSGDataPlatformAssignment({self.workspace}:{self.dsgName}:{self.dataPlatform})"
...
class DatasetGroupDataPlatformAssignments(UserDSLObject):
    ...
    def __str__(self) -> str:
        return f"DatasetGroupDataPlatformAssignments({self.workspace}:{self.dsgName})"
    ...
    def lint(self, eco: Ecosystem, tree: ValidationTree):
        ...
        if w is None:
            ...
            if dsg is None:
                ...
                # The DSG should have a platformMD but it's not a hard constraint
                if dsg.platformMD is None:
                    tree.addRaw(ConstraintViolation(f"DSG {self.workspace}:{self.dsgName} should have a platformMD",
                                                    ProblemSeverity.WARNING))

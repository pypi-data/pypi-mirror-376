...
class Test_RepoCache(unittest.TestCase):
    ...
    def test_cloneToCache_success(self, mock_load, mock_run):  # pyright: ignore[reportMissingParameterType]
        """Test successful clone to cache"""
        # Mock successful git clone
        mock_run.return_value = MagicMock(returncode=0)

        # Mock successful ecosystem loading
        mock_eco = MagicMock()
        mock_tree = MagicMock()
        ...
        with patch('time.time', return_value=mock_time):
            ...
            with patch('os.path.exists') as mock_exists:
                ...
                eco, tree = cloneToCache(self.cred_store, self.test_repo, self.cache_dir)
    ...
    @patch('datasurface.cmd.platform.loadEcosystemFromEcoModule')
    def test_getCachedOrCloneModel_cache_hit(self, mock_load):  # pyright: ignore[reportMissingParameterType]
        ...
        with patch('datasurface.cmd.platform.isModelCacheStale', return_value=False):
            eco, tree = getCachedOrCloneModel(self.cred_store, self.test_repo, self.cache_dir, 5)
            self.assertEqual(eco, mock_eco)
            self.assertEqual(tree, mock_tree)
    ...
    def test_getCachedOrCloneModel_cache_miss(self, mock_clone):  # pyright: ignore[reportMissingParameterType]
        ...
        # Mock cloneToCache to return loaded ecosystem and tree
        mock_eco = MagicMock()
        mock_tree = MagicMock()
        mock_tree.hasErrors.return_value = False
        mock_clone.return_value = (mock_eco, mock_tree)
    ...
    def test_getLatestModelAtTimestampedFolder_with_cache(self, mock_get_cached):  # pyright: ignore[reportMissingParameterType]
        """Test getLatestModelAtTimestampedFolder using cache"""
        # Mock getCachedOrCloneModel returning loaded ecosystem and tree
        mock_eco = MagicMock()
        mock_tree = MagicMock()
        mock_tree.hasErrors.return_value = False
        mock_get_cached.return_value = (mock_eco, mock_tree)

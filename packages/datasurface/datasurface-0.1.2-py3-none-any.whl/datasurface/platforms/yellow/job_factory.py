...
def calculateCorrectJob(eco: Ecosystem, dp: YellowDataPlatform, store: Datastore, datasetName: Optional[str] = None) -> Optional[IngestMergeJob]:
    ...
    if dp.milestoneStrategy == YellowMilestoneStrategy.SCD1:
        if isinstance(store.cmd, SQLMergeIngestion):
            job = SnapshotMergeJobRemoteLive(eco, dp.getCredentialStore(), dp, store, datasetName)
        elif isinstance(store.cmd, SQLSnapshotIngestion) or isinstance(store.cmd, DataTransformerOutput):
            job = SnapshotMergeJobLiveOnly(eco, dp.getCredentialStore(), dp, store, datasetName)
        else:
        ...
        if isinstance(store.cmd, SQLMergeIngestion):
            ...
            job = SnapshotMergeJobRemoteForensic(eco, dp.getCredentialStore(), dp, store, datasetName)
        elif isinstance(store.cmd, SQLSnapshotIngestion) or isinstance(store.cmd, DataTransformerOutput):
            job = SnapshotMergeJobForensic(eco, dp.getCredentialStore(), dp, store, datasetName)
        else:
            logger.error("Unknown ingestion type", ingestion_type=type(store.cmd))
            return None
    else:

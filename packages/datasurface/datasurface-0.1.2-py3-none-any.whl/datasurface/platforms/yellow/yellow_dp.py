...
from datasurface.md.governance import DatasurfaceTransformerCodeArtifact
...
class YellowGraphHandler(DataPlatformGraphHandler):
    ...
    def createTerraformForAllIngestedNodes(self, eco: Ecosystem, tree: ValidationTree) -> str:
        ...
        for dsgRoot in self.graph.roots:
            ...
            if workspace.dataTransformer is not None:
                if not isinstance(workspace.dataTransformer.code, PythonRepoCodeArtifact) and \
                   not isinstance(workspace.dataTransformer.code, DatasurfaceTransformerCodeArtifact):
                    tree.addRaw(ObjectNotSupportedByDataPlatform(
                                    workspace.dataTransformer.code, [PythonRepoCodeArtifact, DatasurfaceTransformerCodeArtifact],
    ...
    def lintGraph(self, eco: Ecosystem, credStore: 'CredentialStore', tree: ValidationTree) -> None:
        ...
        for dsgRoot in self.graph.roots:
            ...
            if workspace.dataTransformer:
                ...
                if not isinstance(dt.code, PythonRepoCodeArtifact) and not isinstance(dt.code, DatasurfaceTransformerCodeArtifact):
                    dtTree.addRaw(ObjectNotSupportedByDataPlatform(dt, [PythonRepoCodeArtifact, DatasurfaceTransformerCodeArtifact], ProblemSeverity.ERROR))
        ...
        for node in self.graph.nodes.values():
            ...
            if workspace.dataTransformer is not None:
                try:
                    ...
                    if isinstance(workspace.dataTransformer.code, PythonRepoCodeArtifact) and \
                       workspace.dataTransformer.code.repo.credential is not None:
    ...
    def generateSecretsDocumentation(self, eco: Ecosystem, issueTree: ValidationTree) -> str:
        ...
        for dsgRoot in self.graph.roots:
            ...
            if workspace.dataTransformer is not None:
                # If the DataTransformer code is a PythonRepoCodeArtifact, add the credential to the secrets info
                if isinstance(workspace.dataTransformer.code, PythonRepoCodeArtifact):
                    if workspace.dataTransformer.code.repo.credential is not None:
                        ...
                        if k8s_name not in secrets_info:
                            ...
                            secrets_info[k8s_name]['purpose'] = \
                                f"Git repository access for {', '.join(secrets_info[k8s_name]['workspaces'])} DataTransformer code"

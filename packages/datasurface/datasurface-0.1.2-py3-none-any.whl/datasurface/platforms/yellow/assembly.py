...
class Airflow2XComponent(Component):
    ...
    def generateBootstrapArtifacts(self, eco: Ecosystem, context: dict[str, Any], jinjaEnv: Environment, ringLevel: int) -> dict[str, str]:
        dag_template: Template = jinjaEnv.get_template('airflow2XCommon/infrastructure_dag.py.j2')
        k8s_secret_manager_template: Template = jinjaEnv.get_template('airflow2X/k8sSecrets.py.j2')

        k8s_secret_manager_rendered: str = k8s_secret_manager_template.render(context)

        context_secrets: dict[str, Any] = context.copy()
        context_secrets.update({
            "platform_secret_manager": k8s_secret_manager_rendered
        })
        rendered_infrastructure_dag: str = dag_template.render(context_secrets)

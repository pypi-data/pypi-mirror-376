...
class AceFlowTools:
    ...
    def _get_dynamic_working_directory(self, provided_directory: Optional[str] = None) -> str:
        ...
        # 1. Current working directory (most reliable)
        current_cwd = os.getcwd()
        candidates.append(("current_cwd", current_cwd))
        
        # 2. IDE-specific environment variables (cross-platform)
        ide_env_vars = [
            # VS Code
            ...
            # System environment variables (Unix-like)
        ...
        # Select the best candidate (prioritize current working directory)
        ...
        if self.fallback_working_directory:
            ...
            if self._is_valid_working_directory(fallback_path):
                print(f"[DEBUG] Using fallback working directory: {fallback_path}", file=sys.stderr)
                return fallback_path
        
        # If all fails, require user input
        error_msg = (
            "无法确定当前工作目录。请在调用工具时明确指定 'directory' 参数。\n"
            f"检测到的候选目录: {[path for _, path in candidates]}\n"

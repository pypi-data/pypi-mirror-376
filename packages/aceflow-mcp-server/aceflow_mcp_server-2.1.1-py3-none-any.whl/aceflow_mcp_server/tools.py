...
class AceFlowTools:
    ...
    def _detect_client_working_directory(self, provided_directory: Optional[str] = None) -> str:
        ...
        Prioritizes current working directory, but avoids IDE installation paths.
        """
        if provided_directory:
            return provided_directory
            
        # Get current working directory first (highest priority)
        current_cwd = os.getcwd()
        
        # Check if current directory is safe to use
        if not self._is_ide_installation_path(current_cwd):
            # Current directory is safe, use it directly
            print(f"[DEBUG] Using current working directory: {current_cwd}", file=sys.stderr)
            return current_cwd
        
        # Current directory is IDE installation path, try alternatives
        print(f"[DEBUG] Current directory is IDE installation path: {current_cwd}", file=sys.stderr)
        
        # Priority order for alternatives when current dir is IDE path
        candidates = []
        
        # 1. Check for MCP/Cline specific environment variables
        mcp_cwd = os.environ.get('MCP_CWD')
        if mcp_cwd and os.path.exists(mcp_cwd) and not self._is_ide_installation_path(mcp_cwd):
            candidates.append(mcp_cwd)
            
        client_cwd = os.environ.get('CLIENT_CWD')  
        if client_cwd and os.path.exists(client_cwd) and not self._is_ide_installation_path(client_cwd):
            candidates.append(client_cwd)
            
        # 2. Check PWD (Unix-style current working directory)
        pwd = os.environ.get('PWD')
        if pwd and os.path.exists(pwd) and not self._is_ide_installation_path(pwd):
            candidates.append(pwd)
            
        # 3. Look for project indicators in parent directories
        search_dir = Path(current_cwd)
        for _ in range(5):  # Search up to 5 levels up
            if self._is_project_directory(search_dir) and not self._is_ide_installation_path(str(search_dir)):
        ...
        # 4. Safe defaults as last resort
        if not candidates:
            safe_defaults = [
                str(Path.home()),
            ...
            for safe_dir in safe_defaults:
                if os.path.exists(safe_dir):
                    candidates.append(safe_dir)
                    break
        
        # Choose the best alternative
        selected = candidates[0] if candidates else current_cwd
        
        # Debug logging
        print(f"[DEBUG] Working directory alternatives: {candidates}", file=sys.stderr)
    ...
    def _is_ide_installation_path(self, path: str) -> bool:
        ...
        # Common IDE installation path patterns
        ide_patterns = [
            # Windows patterns
            'microsoft vs code',
            'code.exe',

...
- 队列相关接口直接接收 `SortedTask`（字段：`id: str`, `payload: str`, `meta: dict`）
- 流相关接口直接接收 `StreamMsg`（字段：`stream: str`, `payload: str`, `meta: dict`）
...
```json
{
  "count": 3,
  "tasks": [
    {"id": "urgent-task", "payload": "...", "score": 1.0, "processed_at": 1703123456.789},
    {"id": "delayed-task", "payload": "...", "score": 1703127056, "processed_at": 1703123457.100}
  ]
}
```
...
curl -X POST "http://localhost:8081/queue/push?score=3.0" \
  -H "Content-Type: application/json" \
  -d '{"id": "task-1", "payload": "{\"data\":\"test1\"}", "meta": {}}'

curl -X POST "http://localhost:8081/queue/push?score=1.0" \
  -H "Content-Type: application/json" \
  -d '{"id": "task-2", "payload": "{\"data\":\"test2\"}", "meta": {}}'

curl -X POST "http://localhost:8081/queue/push?score=2.0" \
  -H "Content-Type: application/json" \
  -d '{"id": "task-3", "payload": "{\"data\":\"test3\"}", "meta": {}}'
...
curl -X POST "http://localhost:8081/stream/push" \
  -H "Content-Type: application/json" \
  -d '{"stream": "test-stream", "payload": "{\"event\": \"test\", \"data\": \"hello world\"}", "meta": {"source": "tests/TEST.md", "seq": 1}}'

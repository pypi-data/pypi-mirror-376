...
async def queue_push(task: SortedTask, score: float | None = None):
    ...
    try:
        ...
        sorted_queue.push("default-queue", task, s)
        return {
            "ok": True,
            "message": f"Task {task.id} pushed to queue",
            "queue": "default-queue",
            "score": s,
            "task": {"id": task.id, "payload": task.payload, "meta": task.meta},
        }
    except Exception as e:
...
async def queue_pop(count: int = 1):
    ...
    try:
        ...
        return {
            ...
            "tasks": [{"id": t.id, "payload": t.payload, "meta": t.meta} for t in popped_tasks]
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

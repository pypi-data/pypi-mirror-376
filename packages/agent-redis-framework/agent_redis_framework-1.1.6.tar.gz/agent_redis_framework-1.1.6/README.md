...
# 推送任务（分数越小优先级越高）
import json
queue.push(queue_key, SortedTask(id="urgent_task", payload=json.dumps({"priority": "high"})), score=1)
queue.push(queue_key, SortedTask(id="normal_task", payload=json.dumps({"priority": "normal"})), score=5)
queue.push(queue_key, SortedTask(id="delayed_task", payload=json.dumps({"priority": "low"})), score=10)

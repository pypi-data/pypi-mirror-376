...
```python
from agent_redis_framework import StreamClient, StreamMsg, get_redis
import threading
import time
import json
...
msg_key = stream_client.push(stream_name, msg)
...
for i in range(3):
    ...
    stream_client.push(stream_name, msg)
    time.sleep(1)
```
...
stream_client = StreamClient(redis_client=get_redis())
...
for msg in messages:
    ...
    stream_client.push("order_stream", stream_msg, maxlen=1000)  # 限制流长度
...
- `consume(group, consumer, callback, block_ms=5000, count=1)` - 消费消息；`callback` 签名为 `(msg_key: str, msg: StreamMsg) -> bool`

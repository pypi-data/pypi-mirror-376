...
class SortedSetQueue:
    ...
    def pop_and_handle(
        self,
        key: str,
        callback: Callable[[float, Task], bool],
        *,
        on_failure: Callable[[float, Task], None] | None = None,
        ascending: bool = True,
        count: int = 1,
    ) -> list[Task]:
        """原子性地弹出并处理任务
        ...
        Args:
            key: Redis中有序集合的键名
            callback: 任务处理函数，接收 (score: float, task: Task) 作为参数，返回bool值表示处理结果
            count: 要弹出的任务数量，默认为1
            on_failure: 可选的失败处理回调函数，接收 (score: float, task: Task)

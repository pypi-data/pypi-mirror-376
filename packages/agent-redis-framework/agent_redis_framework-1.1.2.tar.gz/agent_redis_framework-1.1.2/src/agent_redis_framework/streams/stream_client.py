...
class RedisStreamsClient:
    ...
    def push(self, msg: StreamMsg, maxlen: int | None = None) -> str:
        ...
        if maxlen is not None:
            msg_key = self.redis.xadd(msg.stream, fields, maxlen=maxlen, approximate=True)
        else:
            msg_key = self.redis.xadd(msg.stream, fields)
        ...
        while True:
            ...
            for stream_name, messages in entries:
                for msg_key, fields in messages:
                    ...
                    msg = StreamMsg(stream=s_name, payload=payload_str, meta=meta)
                    try:

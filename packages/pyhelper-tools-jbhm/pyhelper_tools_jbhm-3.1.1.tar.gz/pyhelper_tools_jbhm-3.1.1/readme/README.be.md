...
Pyhelper прапануе ўбудаваныя пераклады для ** 131 мовы ** , уключаючы:
...
** Pyhelper ** - гэта універсальны інструментар Python, прызначаны для спрашчэння ** аналізу дадзеных, візуалізацыі, статыстычных аперацый і працоўных працэсаў утыліты ** .  
...
- 🚀 аптымізаваны для хуткага прататыпа ** і ** Адукацыя **
...
- Штрыхавыя дыяграмы: Гарызантальны і вертыкальны (`hbar`, `vbar`)  
- Сюжэты распаўсюджвання: Гістаграмы (`histo`), сюжэтныя ўчасткі (`boxplot`), участкі KDE (`kdeplot`)  
- Параўнальныя сюжэты: скрыпка, рой, паласавыя ўчасткі  
- Аналіз карэляцыі: цеплавыя карты (`heatmap`), участкі рассейвання (`scatter`)  
- Пашыраныя візуалізацыі: пары сюжэты, сумесныя ўчасткі, рэгрэсійныя ўчасткі  
- Табліцы дадзеных: адфарматаваная табліца адлюстроўвае (`table`)  

### 📈 Статыстычны аналіз
- ** Цэнтральная тэндэнцыя ** : Сярэдняя (`get_media`), медыяна (`get_median`), рэжым (`get_moda`)  
- ** Дысперсія ** : дыяпазон (`get_rank`), дысперсія (`get_var`), стандартнае адхіленне (`get_desv`)  
- ** Аналіз ** : Справаздачы аб дысперсіі (`disp`), разлікі IQR, нармалізацыя, умоўныя пераўтварэнні  
- ** Выяўленне Outlier ** : метады IQR і Z-Score  

### 🗂 Кіраванне файламі
- Разумнае адкрыццё з `call()` (auto-detect CSV, JSON, XML, etc.)  
- Падтрымка з некалькімі фарматамі (CSV, JSON, XML, PDF, spatial data)  
- Кіраванне базамі дадзеных SQL з класам `DataBase`  
- Утыліты пераўтварэння (`convert_file`)  

### 🛠 Інструменты распрацоўшчыка
- Сістэма пераключэння (`Switch`, `AsyncSwitch`)  
- Сінтаксічная праверка (`check_syntax`, `PythonFileChecker`)  
- Насычаная справаздача аб памылках (multi-language)  
- Убудаваная сістэма даведкі (`help`)  

### 🌍 Інтэрнацыяналізацыя
- 100+ убудаваных перакладаў  
- Загрузіце карыстацкія пераклады  
- Пераключэнне мовы выканання (`set_language() `)  
...
Гэты праект ліцэнзуецца пад ** MIT π_4 _ ** .  
Падрабязнасці глядзіце файл [LICENSE](LICENSE).

---

## 🔮 Дарожная карта

- Дадатковыя тыпы візуалізацыі

- Пашыраная падтрымка базы дадзеных (NoSQL, graph databases)

- Інтэграцыя машыннага навучання
...
⚡ гатовы да перазагрузкі вашых працоўных працэсаў Python з ** pyhelper ** ? Пачніце вывучаць сёння!
...
Pyhelper joffri traduzzjonijiet inkorporati għal ** 131 lingwa ** inklużi:
...
** Pyhelper ** huwa għodda versatili Python iddisinjata biex tissimplifika ** analiżi tad-dejta, viżwalizzazzjoni, operazzjonijiet statistiċi, u flussi tax-xogħol ta 'utilità ** .  
Jintegra bla xkiel fi proġetti akkademiċi, ta 'riċerka u professjonali, li jippermettilek tiffoka fuq għarfien aktar milli kodiċi boilerplate.

Vantaġġi ewlenin:
- 🧮 built-in ** Statistika u Utilitajiet tal-Matematika ** 
- 📊 Faċli biex tużah ** Tgeżwir tal-Viżwalizzazzjoni tad-Dejta ** 
- 🗂 Handy ** Fajls Immaniġġjar u Tiftix ** 
- 🔍 ** Validazzjoni tas-Sintassi ** għal fajls Python
- 🌍 ** Appoġġ b'ħafna lingwi ** bi traduzzjonijiet lesti għall-użu
- 🚀 Ottimizzat għal ** prototipi veloċi ** u ** edukazzjoni ** 

---

## ✨ Karatteristiċi ewlenin

### 📊 Viżwalizzazzjoni tad-dejta
- Charts Bar: Orizzontali & Vertikali (`hbar`, `vbar`)  
- Plots tad-distribuzzjoni: istogrammi (`histo`), plots tal-kaxxa (`boxplot`), plots kde (`kdeplot`)  
- Plots komparattivi: vjolin, swarm, plots strip  
- Analiżi tal-Korrelazzjoni: Heatmaps (`heatmap`), Scatter Plots (`scatter`)  
- Viżwalizzazzjonijiet Avvanzati: Plots Par, Plots Konġunti, Plots ta 'Regressjoni  
- Tabelli tad-Dejta: Tabella Formattjata turi (`table`)  

### 📈 Analiżi statistika
- ** Tendenza Ċentrali ** : Medja (`get_media`), medjan (`get_median`), modalità (`get_moda`)  
- ** dispersjoni ** : firxa (`get_rank`), varjanza (`get_var`), devjazzjoni standard (`get_desv`)  
- ** Analiżi ** : Rapporti ta 'Dispersjoni (`disp`), Kalkoli IQR, Normalizzazzjoni, Trasformazzjonijiet Kundizzjonali  
- ** Sejbien Outlier ** : Metodi IQR u Z-Score  

### 🗂️ Ġestjoni tal-fajl
- skoperta intelliġenti ma '`call()` (auto-detect CSV, JSON, XML, etc.)  
- Appoġġ b'ħafna format (CSV, JSON, XML, PDF, spatial data)  
- Ġestjoni tad-database SQL bi klassi `DataBase`  
- Utilitajiet ta 'konverżjoni (`convert_file`)  

### 🛠️ Għodda tal-iżviluppatur
- Switch System (`Switch`, `AsyncSwitch`)  
- Sintassi li tiċċekkja (`check_syntax`, `PythonFileChecker`)  
- Żball rikk li jirrapporta (multi-language)  
- Sistema ta 'Għajnuna Integrata (`help`)  

### 🌍 Internazzjonalizzazzjoni
- 100+ traduzzjonijiet inkorporati  
- Tagħbija traduzzjonijiet tad-dwana  
- Swiċċjar tal-lingwa ta 'runtime (`set_language() `)  
...
Dan il-proġett huwa liċenzjat taħt il- ** MIT π_4 _ ** .  
Ara l-fajl [LICENSE](LICENSE) għad-dettalji.

---

## 🔮 Pjan Direzzjonali

- Tipi ta 'viżwalizzazzjoni addizzjonali

- Appoġġ ta 'database estiż (NoSQL, graph databases)

- Integrazzjoni tat-Tagħlim tal-Magni

- Interfaċċa bbażata fuq il-web

- Sistema tal-plugin

---

⚡ Lest biex tiċċarġja l-flussi tax-xogħol Python tiegħek bi ** pyhelper ** ? Ibda tesplora llum!
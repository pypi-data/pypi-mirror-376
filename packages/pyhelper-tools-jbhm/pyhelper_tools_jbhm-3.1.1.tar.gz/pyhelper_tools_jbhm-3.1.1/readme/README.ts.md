...
## 🌍 Tindzimi leti kumekaka .

Pyhelper yi nyika vuhundzuluxi lebyi akiweke endzeni ku fikela eka **131 tindzimi** ku katsa na:
...
**Pyhelper** i xitirhisiwa xa Python lexi tirhisiwaka hi tindlela to hambana-hambana lexi endleriweke ku olovisa** nxopaxopo wa datha, ku vonakala, matirhelo ya tinhlayo, na maendlelo ya ntirho ya vukorhokeri** .  
Yi hlanganisa hi ndlela leyi nga na swiphiqo eka tiphurojeke ta swa tidyondzo, ndzavisiso, na xiphurofexinali, leswi ku pfumelelaka ku kongomisa eka vutivi ku tlula khodi ya boilerplate.

Swipfuno swa nkoka:
- 🧮 Yi akiwile **Tistatistics na Utilities ta Tinhlayo** 
- 📊 Ku olova ku tirhisa **Data Visualization Wrappers** 
- 🗂 Handy **Ku khoma fayili na ku lavisisa** 
- 🔍 **Ku tiyisisiwa ka Syntax** ya tifayela ta Python .
- 🌍 **Nseketelo wa tindzimi to tala** na vuhundzuluxi lebyi lunghekeleke ku tirhisiwa
- .
...
### 📊 Ku vonakala ka data .
- Tichati ta tibara: Horizontal & vertical (`hbar`, `vbar`) .  
- Tiploti ta ku hangalasiwa: Tihistogiramu (`histo`), Tiploti ta Bokisi (`boxplot`), Tiploti ta KDE (`kdeplot`)  
- Switandi swo pimanisa: violin, ntlhambi, switandi swa strip .  
- Nxopaxopo wa vuhlanganisi: tiheatmap (`heatmap`), tiphloto to hangalaka (`scatter`) .  
- Swifaniso swa le henhla: Tiploti ta mpatswa, tiphloto ta nhlangano, tiphloto ta ku tlhelela endzhaku .  
- Matafula ya datha: tafula leri fomatiweke ri kombisa (`table`) .  

### 📈 Nxopaxopo wa tinhlayo .
- **Ntshikelelo wa le xikarhi** : xiringaniso xa (`get_media`), xa le xikarhi (`get_median`), movha (`get_moda`) .  
- **Dispersion** : Nhlayo ya (`get_rank`), Ku hambana (`get_var`), Ku hambuka ka ntolovelo (`get_desv`)  
- **Nxopaxopo** : Swiviko swa ku hangalaka (`disp`), Swibalo swa IQR, Ku toloveta, Ku hundzuka ka xiyimo  
- **Ku kumiwa ka Outlier** : Maendlelo ya IQR na Z-score .  

### 🗂️ Vulawuri bya fayili .
- Ku tshuburiwa hi vutlhari na `call()` (auto-detect CSV, JSON, XML, etc.) .  
- Nseketelo wa tifomati to tala (CSV, JSON, XML, PDF, spatial data) .  
- Vulawuri bya database ya SQL na tlilasi ya `DataBase` .  
- Switirhisiwa swa ku hundzuka (`convert_file`) .  

### 🛠️ Switirhisiwa swa vatumbuluxi
- Sisiteme ya swiwitsi (`Switch`, `AsyncSwitch`) .  
- Ku kamberiwa ka xivumbeko xa marito (`check_syntax`, `PythonFileChecker`) .  
- Ku vika xihoxo lexi fuweke (multi-language) .  
- Sisiteme ya mpfuneto leyi hlanganisiweke (`help`) .  

### 🌍 Ku hlanganisiwa ka matiko ya misava .
- 100+ wa vuhundzuluxi lebyi akiweke endzeni .  
- load custom tradunces .  
- Ku cinca ka ririmi ra nkarhi wo famba (`set_language() `)  
...
### Ku vona hi mahlo ya mianakanyo .
```python
import helper as hp
...
### Ku tiyisisiwa ka xivumbeko xa marito .
```python
...
Phurojeke leyi yi nyikiwile layisense ehansi ka **MIT License** .  
Vona fayili ya [LICENSE](LICENSE) ku kuma vuxokoxoko.

---

## 🔮 Mepe wa ndlela .

- Tinxaka to engetela ta ku vonakala .

- Nseketelo wa database lowu andlariweke (NoSQL, graph databases) .

- Ku hlanganisiwa ka dyondzo ya muchini .

- Xihlanganisi lexi simekiweke eka webu .

- Sisiteme ya Plugin .

---

⚡ U lunghekele ku supercharge maendlelo ya wena ya ntirho ya Python hi **pyhelper** ? Sungula ku hlota namuntlha!
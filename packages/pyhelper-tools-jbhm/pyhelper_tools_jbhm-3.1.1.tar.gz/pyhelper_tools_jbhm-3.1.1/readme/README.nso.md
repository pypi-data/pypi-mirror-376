...
## 🌍 Maleme ao a lego gona .

Pyhelper e fana ka diphetolelo tše di agetšwego ka gare bakeng sa go fihla go **131 maleme** go akaretša le:
...
**Pyhelper** ke sedirišwa sa Python se se nago le mahlakore a mantši seo se hlamilwego go nolofatša**tshekatsheko ya data, go bona dilo ka leihlo la kgopolo, ditiro tša dipalopalo, le ditshepedišo tša mošomo tša utility** .  
E kopanya ka ntle le mathata ka gare ga diprotšeke tša thuto, tša nyakišišo le tša profešenale, e go dumelela go tsepamiša kgopolo go ditemogo go e na le khoutu ya boilerplate.

Melemo ya Bohlokwa: .
- 🧮 Go ageleditšwe **Dipalopalo le didirišwa tša dipalo** 
- 📊 Go bonolo go šomišwa **diphuthelwana tša go bona ka leihlo la kgopolo tša data** 
- 🗂 Seatleng **Go swara faele le go nyaka** 
- 🔍 **Syntax netefatso** bakeng sa Python difaele .
- 🌍 **Thekgo ya leleme le legolo** ka diphetolelo tše di loketšego go šomišwa
- 🚀 E lokišitšwe bakeng sa **Pheto ya ka pela** le**Thuto** 

--- .

## ✨ Dikarolo tša bohlokwa .

### 📊 Pono ya data .
- Ditšhate tša dibara: tše di rapaletšego & tše di emego (`hbar`, `vbar`) .  
- Diploto tša kabo: di-histogram (`histo`), diploto tša lepokisi (`boxplot`), diploto tša kde (`kdeplot`) .  
- Diploto tša go bapetša: violin, sehlopha, diploto tša go hlobola .  
- Tshekatsheko ya kgokagano: Dimmapa tša phišo (`heatmap`), diploto tša go phatlalatša (`scatter`) .  
- Dipono tše di tšwetšego pele: diploto tša para, diploto tša mohlakanelwa, diploto tša go boela morago .  
- Data litafole: tafole fomate bontša (`table`) .  

### 📈 Tshekatsheko ya dipalopalo .
- **Temelo ya bogareng** : bolela (`get_media`), magareng (`get_median`), mokgwa wa (`get_moda`) .  
- **phatlalatšo ** : Range (`get_rank`), phapano (`get_var`), kheloha maemo a (`get_desv`)  
- **tshekatsheko** : Dipegelo tša go phatlalatšwa (`disp`), dipalo tša IQR, go tlwaetša, diphetogo tša maemo  
- **Tlhahlobo ya ka ntle** : Mekgwa ya IQR le Z-Score .  

### 🗂️ Taolo ya faele .
- Kutollo ya bohlale ka `call()` (auto-detect CSV, JSON, XML, etc.) .  
- Thekgo ya sebopego sa go fapana (CSV, JSON, XML, PDF, spatial data) .  
- SQL polokelongtshedimosetso taolo le `DataBase` sehlopha .  
- Didirišwa tša Phetošo (`convert_file`) .  

### 🛠️ Didirišwa tša motšweletši .
- Switch tsamaiso (`Switch`, `AsyncSwitch`) .  
- Popegopolelo ya go hlahloba (`check_syntax`, `PythonFileChecker`) .  
- Go bega phošo ye e humilego (multi-language) .  
- Kopantswe thuso tsamaiso (`help`) .  

### 🌍 Go dira gore dilo e be tša boditšhabatšhaba .
- 100+ Diphetolelo tše di agetšwego ka gare .  
- Load Diphetolelo tša Tlwaelo .  
- Go fetola polelo ya nako ya go šoma (`set_language() `)  
...
### Pono ya go bona ka leihlo la kgopolo .
```python
import helper as hp
...
Protšeke ye e na le laesense ka fase ga **MIT License** .  
Bona faele ya [LICENSE](LICENSE) bakeng sa dintlha.

--- .

## 🔮 Mmapa wa tsela .

- Mehuta ya tlaleletšo ya go bona dilo ka leihlo la kgopolo .

- Thekgo ya polokelo ya tshedimošo ye e atološitšwego (NoSQL, graph databases) .

- Kopanyo ya go ithuta ka motšhene .

- Segokaganyi se se theilwego go wepe .

- Tsamaiso ya plugin .

--- .

⚡ Loketse go supercharge Python workflows ya gago ka **Pyhelper** ? Thoma go hlahloba lehono!
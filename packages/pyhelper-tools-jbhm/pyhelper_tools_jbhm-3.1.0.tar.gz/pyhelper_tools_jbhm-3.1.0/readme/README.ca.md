...
Pyhelper ofereix traduccions integrades fins a ** 131 idiomes ** inclosos:
...
** Pyhelper ** és un versàtil Python Toolkit dissenyat per simplificar ** Anàlisi de dades, visualització, operacions estadístiques i fluxos de treball d’utilitat ** .  
...
- 🔍 ** Validació de sintaxi ** per als fitxers Python
-🌍 ** Suport multi-llenguatge ** amb traduccions llestes per utilitzar
- 🚀 Optimitzat per a ** Prototipat ràpid ** i ** Educació ** 
...
- Gràfics de barres: horitzontal i vertical (`hbar`, `vbar`)  
- Parcel·les de distribució: histogrames (`histo`), trames de caixa (`boxplot`), trames KDE (`kdeplot`)  
- Parcel·les comparatives: violí, eixam, trames de tira  
- Anàlisi de correlació: mapes de calor (`heatmap`), trames de dispersió (`scatter`)  
- Visualitzacions avançades: parcel·les de parells, parcel·les articulars, trames de regressió  
- Taules de dades: La taula formatat mostra (`table`)  

### 📈 Anàlisi estadística
- ** Tendència central ** : mitjana (`get_media`), mediana (`get_median`), mode (`get_moda`)  
- ** Dispersió ** : Range (`get_rank`), variància (`get_var`), desviació estàndard (`get_desv`)  
- ** Anàlisi ** : Informes de dispersió (`disp`), càlculs d'IQR, normalització, transformacions condicionals  
- ** Detecció més destacada ** : Mètodes IQR i Z-Score  

### 🗂️ Gestió de fitxers
- Descobriment intel·ligent amb `call()` (auto-detect CSV, JSON, XML, etc.)  
- Suport multi-format (CSV, JSON, XML, PDF, spatial data)  
- Gestió de bases de dades SQL amb classe `DataBase`  
- Utilitats de conversió (`convert_file`)  

### 🛠️ Eines de desenvolupador
- Switch System (`Switch`, `AsyncSwitch`)  
- Comprovació de la sintaxi (`check_syntax`, `PythonFileChecker`)  
- Informes d'error rics (multi-language)  
- Sistema d'ajuda integrada (`help`)  

### 🌍 Internacionalització
- 100 traduccions integrades  
- Carregueu traduccions personalitzades  
- Commutador de llenguatge d'execució (`set_language() `)  
...
Aquest projecte té llicència sota el ** MIT π_4 _ ** .  
Consulteu el fitxer [LICENSE](LICENSE) per obtenir més informació.

---

## 🔮 Full de ruta

- Tipus de visualització addicionals

- Suport de base de dades ampliat (NoSQL, graph databases)

- Integració de l'aprenentatge automàtic

- Interfície basada en la web

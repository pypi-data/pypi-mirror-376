...
ПИХЕЛПЕР нуди уграђене преводе за до ** 131 језика ** Укључујући:
...
** ПИХЕЛПЕР ** је свестрани Python Тоолкит дизајниран да поједностави ** Анализа података, визуелизацију, статистичке операције и комуналне токове ** .  
Бежалично се интегрише у академске, истраживачке и професионалне пројекте, омогућавајући вам да се фокусирате на увид, а не код кода за плочицу.

Кључне предности:
- 🧮 Уграђена ** Статистика и математичке комуналне услуге ** 
- 📊 Једноставна употреба ** Визуелизација података ** 
- 🗂 Ханди ** руковање датотеком и претрага ** 
- 🔍 ** СИНТАКС Валидатион ** за Python датотеке
- 🌍 ** Подршка за више језика ** са преводима спремним за употребу
- 🚀 Оптимизовано за ** брзу прототипирање ** и ** образовање ** 
...
- Бар графикони: Хоризонтално и вертикално (`hbar`, `vbar`)  
- Дистрибутивне плоче: Хистограми (`histo`), кутије за плоче (`boxplot`), КДЕ Плотс (`kdeplot`)  
- Упоредне парцеле: виолина, рој, стриптизете  
- Анализа корелације: топлотна капуте (`heatmap`), Сцаттер Парцес (`scatter`)  
- Напредне визуализације: упариве парцеле, заједничке парцеле, регресијске парцеле  
- Табеле података: Форматирана табела Дисплаи (`table`)  

### 📈 Статистичка анализа
- ** Централна тенденција ** : значи (`get_media`), медиан (`get_median`), мод (`get_moda`)  
- ** Дисперзија ** : Распон (`get_rank`), варијанца (`get_var`), стандардна девијација (`get_desv`)  
- ** Анализа ** : Извештаји о дисперзији (`disp`), ИКР прорачуни, нормализација, условна трансформација  
- ** Оутлиер Детецтион ** : Методе ИКР и З-Сцоре  

### 🗂 Управљање датотеком
- Смарт Дисцовери са `call()` (auto-detect CSV, JSON, XML, etc.)  
- Подршка за више формата (CSV, JSON, XML, PDF, spatial data)  
- управљање СКЛ база података са `DataBase` класе  
- Услужни програми за конверзију Π_21_  

### 🛠 Алати за програмере
- Свитцх систем (`Switch`, `AsyncSwitch`)  
- Синтакса Провера (`check_syntax`, `PythonFileChecker`)  
- Рицх Еррор Пријављивање (multi-language)  
- Интегрисани систем помоћи (`help`)  

### 🌍 Интернационализација
- 100+ уграђени преводи  
- Учитајте прилагођене преводе  
- Пребацивање језика рунтиме-а (`set_language() `)  
...
Овај пројекат је лиценциран под ** MIT π_4 _ ** .  
Погледајте детаље о датотеци [LICENSE](LICENSE).

----

## 🔮 мапе пута

- Додатне врсте визуелизације

- Подршка за проширену базу података (NoSQL, graph databases)

- Интеграција учења машина
...
Спремни сте да надмашите своје Python радне токове са ** пихелпер ** ? Почните да истражите данас!
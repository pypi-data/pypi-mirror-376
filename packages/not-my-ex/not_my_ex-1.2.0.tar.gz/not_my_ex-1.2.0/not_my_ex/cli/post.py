...
async def send(key: str, http: AsyncClient, auth: Auth, post: Post) -> Sent | None:
    try:
        cls = CLIENTS[key]
    except KeyError:
...
async def media_from(
    auth: Auth, path: str, alt_text: str | None, ask_for_alt_text: bool
) -> Media:
    media = await Media.from_img(path, alt_text, auth.image_size_limit)
    if ask_for_alt_text:
...
async def main(
    ...
    if len(images) > 4:
        error("You can only post up to 4 images")

    if len(alt_texts) > len(images):
        error(f"Got {len(alt_texts)} alt texts, but only {len(images)}")
    ...
    load = tuple(
        media_from(auth, path, alt_text, not yes_to_all)
        for path, alt_text in zip_longest(images, alt_texts)
    )
    try:
    ...
    if post.is_empty():
        error("No text (or image) to post")
...
def post(
    ...
    alt_texts: Annotated[
        list[str],
        Option("--alt-texts", "-a", help="Alt text for --images (same order)"),
    ] = [],
    lang: Annotated[
        str | None,
        Option(
    ...
    loop = get_event_loop()
    try:
        loop.run_until_complete(main(auth, text, images, alt_texts, lang, yes_to_all))

...
class TestFunChangeUnitWithArrayCustomArray(parameterized.TestCase):
    ...
    def test_fun_change_unit_power_with_array(self, value, power_exponents, unit):
        ...
        for bm_fun, jnp_fun in zip(bm_fun_list, jnp_fun_list):
            ...
            array_input = Array(q)
            result = bm_fun(array_input.value, power_exponents)
            array_result = Array(result)
            assert isinstance(array_result, u.CustomArray)
            assert_quantity(array_result.value, expected, unit=result_unit)
    ...
    def test_fun_change_unit_binary_divmod_with_array(self, value, unit1, unit2):
        ...
        for bm_fun, jnp_fun in zip(bm_fun_list, jnp_fun_list):
            ...
            for i, r in enumerate(result):
                ...
                assert isinstance(array_result, u.CustomArray)
                assert_quantity(array_result.value, expected[i])
            ...
            assert_quantity(array_result1.value, expected[0], unit=unit1 / unit2)
            assert_quantity(array_result2.value, expected[1], unit=unit1)
    ...
    def test_array_with_unit_change_functions(self):
        ...
        assert_quantity(test_array.value, jnp.array([4.0, 9.0, 16.0]), unit=meter ** 2)
        
        sqrt_result = um.sqrt(test_array.value)
        sqrt_array = Array(sqrt_result)
        assert isinstance(sqrt_array, u.CustomArray)
        assert_quantity(sqrt_array.value, jnp.array([2.0, 3.0, 4.0]), unit=meter)
        
        square_result = um.square(test_array.value)
        square_array = Array(square_result)
        assert isinstance(square_array, u.CustomArray)
        assert_quantity(square_array.value, jnp.array([16.0, 81.0, 256.0]), unit=meter ** 4)
    ...
    def test_array_with_custom_array_binary_operations(self):
        ...
        multiply_result = um.multiply(array1.value, array2.value)
        multiply_array = Array(multiply_result)
        assert isinstance(multiply_array, u.CustomArray)
        assert_quantity(multiply_array.value, jnp.array([2.0, 8.0, 18.0]), unit=meter * second)
        
        divide_result = um.divide(array1.value, array2.value)
        divide_array = Array(divide_result)
        assert isinstance(divide_array, u.CustomArray)
        assert_quantity(divide_array.value, jnp.array([2.0, 2.0, 2.0]), unit=meter / second)
    ...
    def test_array_with_power_operations(self):
        ...
        power_result = um.power(test_array.value, 3)
        power_array = Array(power_result)
        assert isinstance(power_array, u.CustomArray)
        assert_quantity(power_array.value, jnp.array([8.0, 27.0, 64.0]), unit=meter ** 3)
        
        float_power_result = um.float_power(test_array.value, 2)
        float_power_array = Array(float_power_result)
        assert isinstance(float_power_array, u.CustomArray)
        assert_quantity(float_power_array.value, jnp.array([4.0, 9.0, 16.0]), unit=meter ** 2)

...
def GetSecrets(vault_addr: str, role_id: str, secret_id: str, mount_point: str, secret_path: str, keys_to_extract: List[Pair]) -> dict:
    ...
    try:
        ...
        for key in keys_to_extract:
            if key['VAULT_VALUE'] in secret_data:
                d[key['ENV_NAME']] = secret_data.get(key['VAULT_VALUE'])
...
def LoadSecretsIntoEnv(
    ...
    d = GetSecrets(vault_addr, role_id, secret_id, mount_point, secret_path, [ Pair(var1['ENV_NAME'],var1['VAULT_VALUE']) for var1 in keys_to_extract])
    for key in keys_to_extract:
        if key['ENV_NAME'] in d:

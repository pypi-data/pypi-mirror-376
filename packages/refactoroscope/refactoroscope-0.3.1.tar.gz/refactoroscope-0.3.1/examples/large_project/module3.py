...
class CodeAnalyzer:
    ...
    def _calculate_cyclomatic_complexity(self, code: str) -> float:
        ...
        for line in lines:
            stripped = line.strip()
            if any(keyword in stripped for keyword in ["if ", "elif ", "for ", "while ", "except ", "case "]):
                decision_points += 1
    ...
    def _estimate_maintainability(self, code: str, loc: int, complexity: float) -> float:
        """Estimate maintainability index"""
        ...
        mi = 171 - 5.2 * math.log(max(1, complexity)) - 0.23 * loc - 16.2 * math.log(max(1, 1))

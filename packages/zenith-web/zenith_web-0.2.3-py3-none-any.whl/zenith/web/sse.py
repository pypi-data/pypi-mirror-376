...
class ServerSentEvents:
    ...
    async def _stream_events_with_backpressure(
        ...
        try:
            ...
            async with asyncio.TaskGroup() as tg:
                ...
                async for formatted_event in self._generate_sse_stream(connection, stream_task):
            ...
            try:
                async for event in event_generator:
                    ...
                    try:
                        ...
                        logger.warning(f"Event queue full for connection {connection.connection_id}")
    ...
    def _update_client_buffer_estimate(self, connection: SSEConnection) -> None:
        ...
        time_since_last_update = current_time - getattr(connection, '_last_buffer_update', current_time)
    ...
    async def unsubscribe_from_channel(self, connection_id: str, channel: str) -> bool:
        ...
        logger.info(f"SSE connection {connection_id} unsubscribed from channel {channel}")
        return True
        ...
        return {
            **self._stats,
            ...
            "performance_improvement_percent": (
                (self._stats["concurrent_processing_time_saved"] / max(self._stats["events_sent"] * 0.001, 0.001)) * 100
                if self._stats["events_sent"] > 0 else 0
            ),
...
class SSEEventManager:
    ...
    async def create_event_stream(self, event_generator: AsyncGenerator[Dict[str, Any], None]):
...
def create_sse_response(event_generator: AsyncGenerator[Dict[str, Any], None]) -> StreamingResponse:
    """

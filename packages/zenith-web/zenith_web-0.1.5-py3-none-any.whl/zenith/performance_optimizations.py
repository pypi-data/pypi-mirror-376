...
def create_performance_app_factory():
    ...
    def create_optimized_app(profile: str = "api", **kwargs):
        ...
        app.add_middleware(RateLimitMiddleware, default_limits=config["rate_limits"])
...
if __name__ == "__main__":
    ...
    print(
        f"Optimized:     {results['optimized']:.0f} req/s ({results['optimized'] / results['bare'] * 100:.1f}% retention)"
    )
    print(
        f"Default:       {results['default']:.0f} req/s ({results['default'] / results['bare'] * 100:.1f}% retention)"
    )

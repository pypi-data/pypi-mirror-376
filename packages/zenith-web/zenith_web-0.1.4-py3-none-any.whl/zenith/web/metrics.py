...
class MetricsCollector:
    ...
    def histogram(self, name: str, value: float, labels: dict[str, str] = None) -> None:
        ...
        with self._lock:
            ...
            if len(self._histograms[metric_key]) > self.max_points:
                self._histograms[metric_key] = self._histograms[metric_key][-self.max_points:]
    ...
    def timing(self, name: str, duration_seconds: float, labels: dict[str, str] = None) -> None:
        """
        ...
        for metric_key, values in current_values["histograms"].items():
            if values:
                ...
                for q in [0.5, 0.9, 0.95, 0.99]:
                    ...
                    quantile_labels_str = self._format_prometheus_labels(quantile_labels)
...
def record_request_metrics(
    ...
    # Error rate
    if status_code >= 400:
        error_labels = {**labels, "error_type": "client" if status_code < 500 else "server"}

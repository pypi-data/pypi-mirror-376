...
class AudioChatUI:
    ...
    def create_layout(self) -> Layout:
        """Create an intuitive, well-spaced UI layout focused on user action"""
        layout = Layout()

        # === MAIN ACTION STATUS (Hero Element) ===
        # This is what the user needs to know RIGHT NOW
        status_content = Text()
        
        # Show notification if active (highest priority)
        if self.notification_timeout > 0:
            status_content.append(f"\n   ✅ {self.notification_message}\n", style="bold green")
        elif self.state == AppState.READY:
            status_content.append("\n   🎤 Press SPACE to start recording\n", style="bold cyan")
        elif self.state == AppState.RECORDING:
            status_content.append("\n   ● RECORDING - Press SPACE to send\n", style="bold red")
        elif self.state == AppState.PROCESSING:
        ...
        live_indicator = "● LIVE" if self.state != AppState.PROCESSING else "⟳ BUSY"
        live_style = "bold green" if self.state != AppState.PROCESSING else "bold yellow"
        
        # Create well-spaced audio content
        from rich.console import Group
        audio_group = Group(
            "",  # Top padding
            Columns([
                Text(f"🎤 {device_name}", style="white"),
                Align.right(Text(live_indicator, style=live_style))
            ]),
            "",  # Spacing
            "",  # More spacing
            Align.center(Text("Audio Waveform", style="dim white")),
            "",  # Small gap
            Align.center(self._create_waveform()),
            "",  # Good spacing between waveform and volume
            "",  # More spacing
            Align.center(self._create_volume_bar()),
            "",  # Bottom padding
        )
        ...
        # === CONTROLS (Reference) ===
        controls_content = Group(
            "",  # Padding
            Columns([
                Text("[SPACE] Record/Send", style="bold cyan"),
                Text("[Q] Quit", style="bold red"),
                Text("[C] Copy", style="bold blue"),
                Text(f"[A] Auto-Copy: {'ON' if self.auto_copy else 'OFF'}", 
                     style="bold green" if self.auto_copy else "bold yellow")
            ], align="center"),
            "",  # Padding
        )
        ...
        # Prioritized by importance to user workflow
        layout.split_column(
            Layout(status_panel, size=5),      # What should I do? (Hero)
            Layout(audio_panel, size=10),      # Is my audio working?
            Layout(response_panel, size=10),   # What was transcribed?
            Layout(controls_panel, size=4)     # How do I control it? (Reference)
        )

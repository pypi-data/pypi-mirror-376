...
def get_command(view, window):
    ...
    try:
        matching_class_str, scene_line_no = next(
            filter(lambda cl: cl[1] <= row, reversed(class_lines))
        )
...
def send_terminus_command(
    ...
    # Amend command with various keyboard shortcuts
    full_command = "".join(
        [
...
class ManimRunScene(sublime_plugin.TextCommand):
    def run(self, edit):
        ...
        sublime.set_timeout(
            lambda: send_terminus_command(command, enter=enter), timeout
        )

        if enter:
            # Keep cursor where it started
            sublime.set_timeout(
                lambda: threading.Thread(target=self.focus_sublime).start(), 1000
            )
        else:
            ...
            for line in lines:
                ...
                if line.lstrip().startswith("#"):
                    ...
                    if (
                        ...
                        and not last_line_was_comment
                    ):
                        if last_comment_line_end:
                            regions_to_fold.append(
                                sublime.Region(

...
def build_parent_did_panel(
    ...
    *,
    ...
    if "parent_pnr" not in panel.columns:
        panel = (
            panel.join(relations_map, on="PNR", how="left")
            .with_columns(
                [
                    pl.when(pl.col("gender") == "F")
                    .then(pl.col("CPR_MODER"))
                    .otherwise(pl.col("CPR_FADER"))
                    .alias("parent_pnr")
                ]
            )
        )
    ...
    if bef_registry_pattern is not None:
        ...
        base_unique = (
            base.select(["PNR", "INDEX_DATE", "CPR_MODER", "CPR_FADER"]).unique(
                maintain_order=True
            )
        )
    ...
    if uddf_file_path is not None:
        ...
        if parents_map.height > 0:
            edu_input = parents_map.select(
                [pl.col("INDEX_DATE"), pl.col("parent_pnr").alias("PNR")]
    ...
    if cat_cols:
        panel = panel.with_columns([pl.col(c).cast(pl.Categorical) for c in cat_cols])

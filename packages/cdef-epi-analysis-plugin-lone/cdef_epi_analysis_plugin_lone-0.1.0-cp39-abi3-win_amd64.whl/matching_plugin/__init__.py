...
def build_parent_did_panel(
    ...
    *,
    ...
    if missing:
        raise ValueError(f"match_df missing required columns: {sorted(missing)}")
    required_rel_cols = {"PNR", "CPR_MODER", "CPR_FADER"}
    missing_rel = required_rel_cols - set(relations_df.columns)
    if missing_rel:
        raise ValueError(
            f"relations_df missing required columns: {sorted(missing_rel)}"
        )

    # Ensure a single parent mapping per child to prevent join fanouts
    relations_map = relations_df.select(["PNR", "CPR_MODER", "CPR_FADER"]).unique(
        subset=["PNR"], keep="first"
    )
    # Base child info with parents
    base = match_df.join(relations_map, on="PNR", how="inner")
    ...
    income_wide = income_wide.join(relations_map, on="PNR", how="left").unique(
        subset=["PNR", "INDEX_DATE", "ARET", "RELATIVE_YEAR"], keep="first"
    )
    ...
    panel = pl.concat(long_parts, how="vertical").unique(
    ...
    panel = panel.join(
        match_df.select(["PNR", "INDEX_DATE", "ROLE"]),
        on=["PNR", "INDEX_DATE"],
        how="left",
    )
    ...
    if akm_registry_pattern is not None:
        ...
        socio_long = socio_long.unique(
            subset=["PNR", "INDEX_DATE", "parent_pnr", "gender"], keep="first"
        )
        panel = panel.join(
            socio_long.select(
    ...
    if bef_registry_pattern is not None:
        # Reuse de-duplicated child-date pairs if available
        base_unique = base.select(["PNR", "INDEX_DATE"]).unique(maintain_order=True)
        ...
        eth = eth.filter(pl.col("RELATIVE_YEAR") == -1).select(
            [
                "PNR",
                "INDEX_DATE",
    ...
    if uddf_file_path is not None:
        ...
        mothers = (
            ...
            .filter(pl.col("parent_pnr").is_not_null())
            .unique(subset=["parent_pnr", "INDEX_DATE", "child_pnr"], keep="first")
        )
        edu_m = None
        if mothers.height > 0:
            edu_m_input = mothers.select([
                pl.col("INDEX_DATE"),
                pl.col("parent_pnr").alias("PNR"),
            ])
            edu_m_df = extract_highest_education_level(
                df=edu_m_input,
                identifier_col="PNR",
                index_date_col="INDEX_DATE",
        ...
        if fathers.height > 0:
            ...
            edu_f_df = extract_highest_education_level(
                df=edu_f_input,
                identifier_col="PNR",
                index_date_col="INDEX_DATE",
                uddf_file_path=uddf_file_path,
        ...
        edu_long = None
        if edu_m is not None and edu_f is not None:
            edu_long = pl.concat([edu_m, edu_f], how="vertical")
        elif edu_m is not None:
            edu_long = edu_m
        elif edu_f is not None:
            edu_long = edu_f

        if edu_long is not None:
            edu_long = edu_long.unique(
                subset=["PNR", "INDEX_DATE", "parent_pnr", "gender"], keep="first"
            )
            panel = panel.join(
                edu_long,
    ...
    panel = panel.unique(
        subset=[
            c
            for c in ["PNR", "INDEX_DATE", "parent_pnr", "gender", "year", "event_time"]

...
class PlanBasedOrchestrator(Component[PlanBasedOrchestratorConfig], BaseOrchestrator):
    ...
    def __init__(
        ...
        self.step_attempts: Dict[int, List[AgentResponse]] = {}  # step_index -> attempts
    ...
    async def prepare_context_for_agent(self, agent: BaseAgent) -> Union[str, UserMessage, List[Message]]:
        """Prepare context including step-specific instructions and retry context."""
        if not self.execution_plan or self.current_step_index >= len(self.execution_plan.steps):
    ...
    async def update_shared_state(self, result: AgentResponse) -> None:
        ...
        new_messages = [msg for msg in result.messages if not isinstance(msg, UserMessage)]
        ...
        if progress_eval.step_completed:
            ...
            if self.current_step_retry_count <= self.max_step_retries:
                # Prepare for retry
                self.retry_instructions[self.current_step_index] = self._create_retry_instructions(
                    current_step, progress_eval
                )
            else:
...
"""
        ...
        try:
            ...
            if result.structured_output and isinstance(result.structured_output, ExecutionPlan):
    ...
    def extract_relevant_context(self, _step: PlanStep) -> List[Message]:
        ...
        return self.shared_messages[-5:] if len(self.shared_messages) > 5 else self.shared_messages.copy()
    
    async def evaluate_step_progress(self, step: PlanStep, result: AgentResponse) -> StepProgressEvaluation:
...
Consider the step successful if the agent made meaningful progress toward the stated goal, even if not perfect."""
        ...
        try:
            ...
            if eval_result.structured_output and isinstance(eval_result.structured_output, StepProgressEvaluation):
    ...
    def _fallback_step_evaluation(self, agent_output: str) -> StepProgressEvaluation:
        ...
        has_error_indicators = any(word in output_lower for word in ["error", "failed", "cannot", "unable", "sorry"])
        has_positive_indicators = any(word in output_lower for word in ["completed", "found", "created", "analyzed", "wrote"])
    ...
    def _find_agent_by_name(self, name: str) -> BaseAgent:
        ...
        print(f"Warning: Agent '{name}' not found, using fallback: {self.agents[0].name}")
        return self.agents[0]
    ...
    def _format_step_task(self, step: PlanStep) -> str:
        ...
        if self.current_step_retry_count > 0 and self.current_step_index in self.retry_instructions:
            retry_info = self.retry_instructions[self.current_step_index]
            base_task += f"\n\nRETRY INSTRUCTIONS (Attempt {self.current_step_retry_count + 1}):\n{retry_info}"
        
        return base_task
    
    def _create_retry_instructions(self, _step: PlanStep, progress_eval: StepProgressEvaluation) -> str:
    ...
    def _get_pattern_metadata(self) -> Dict[str, Any]:
        ...
        if self.execution_plan:
            ...
            plan_metadata = {
                ...
                "total_retries": sum(len(attempts) - 1 for attempts in self.step_attempts.values()),
    ...
    @classmethod
    def _from_config(cls, config: PlanBasedOrchestratorConfig) -> "PlanBasedOrchestrator":

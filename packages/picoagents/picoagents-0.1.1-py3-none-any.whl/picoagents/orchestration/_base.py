...
from typing import List, Optional, Union, Dict, Any, Sequence, cast
...
from ..types import (
    Usage, OrchestrationResponse, StopMessage, OrchestrationEvent,
    OrchestrationStartEvent, OrchestrationCompleteEvent, 
    AgentSelectionEvent, AgentExecutionStartEvent, AgentExecutionCompleteEvent,
    AgentResponse
)
...
class BaseOrchestrator(ComponentBase[BaseModel], ABC):
    ...
    async def run(self, task: Union[str, UserMessage, List[Message]], cancellation_token: Optional[CancellationToken] = None) -> OrchestrationResponse:
        """
    ...
    async def run_stream(self, task: Union[str, UserMessage, List[Message]], cancellation_token: Optional[CancellationToken] = None, verbose: bool = False) -> AsyncGenerator[Union[Message, OrchestrationEvent, OrchestrationResponse], None]:
        """
        ...
        try:
            ...
            while self.iteration_count < self.max_iterations:
                ...
                try:
                    async for item in next_agent.run_stream(context, cancellation_token=cancellation_token, verbose=verbose):
                ...
                assert hasattr(result, 'messages'), "Result must be AgentResponse with messages"
                ...
                new_streamed_messages = streamed_messages[self._last_termination_check_count:]
                self._last_termination_check_count = len(streamed_messages)
    ...
    def _normalize_task_to_messages(self, task: Union[str, UserMessage, List[Message]]) -> List[Message]:
        ...
        if isinstance(task, str):
            ...
            return [task] if hasattr(task, 'content') else [UserMessage(content=str(task), source="user")]
    
    def _extract_new_messages(self, agent_messages: List[Message], sent_context: Union[str, UserMessage, List[Message]]) -> List[Message]:
        ...
        if isinstance(sent_context, list):
            ...
            return agent_messages[context_len:] if len(agent_messages) > context_len else []
        else:

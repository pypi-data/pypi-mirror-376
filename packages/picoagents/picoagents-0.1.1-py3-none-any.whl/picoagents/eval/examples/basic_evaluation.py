...
from picoagents.eval import EvalRunner, AgentEvalTarget, ModelEvalTarget, LLMEvalJudge
...
async def main():
    ...
    for target_name, scores in targets:
        ...
        for i, (task, score) in enumerate(zip(tasks, scores)):
            ...
            print(f"   Dimensions: {', '.join(f'{k}={v:.1f}' for k, v in score.dimensions.items())}")
            
            # Show one reasoning example
            first_reasoning = list(score.reasoning.values())[0] if score.reasoning else "No reasoning"

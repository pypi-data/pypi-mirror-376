...
from ..types import EvalTask, EvalScore
        ...
        if self.parallel:
            ...
            for task in tasks:
                if cancellation_token and cancellation_token.is_cancelled():
                    break
                score = await self._evaluate_single_task(target, task, criteria, cancellation_token)
                scores.append(score)
            return scores
    ...
    async def _evaluate_single_task(
        ...
        try:
            ...
            return EvalScore(
                ...
                reasoning={dim: f"Execution failed: {str(e)}" for dim in (criteria or ["accuracy"])},

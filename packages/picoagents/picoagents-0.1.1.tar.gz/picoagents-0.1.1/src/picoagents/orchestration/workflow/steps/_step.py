...
class BaseStep(ComponentBase[BaseStepConfig], Generic[InputType, OutputType]):
    ...
    def _deserialize_types(cls, config: BaseStepConfig) -> Tuple[Type[InputType], Type[OutputType]]:
        ...
        """
        from typing import List, Dict, Any as AnyType, Optional, Union
                ...
                for type_option in any_of_types:
                    if isinstance(type_option, dict):
                        ...
                        if option_type == 'null':
                            ...
                            python_type = _json_type_to_python_type(option_type, type_option)
        ...
        def _json_type_to_python_type(json_type: str, schema_details: Optional[Dict[str, Any]] = None) -> Any:
            ...
            if json_type == 'string':
                ...
                if items_schema:
                    if isinstance(items_schema, dict) and 'type' in items_schema:
                        item_type = _json_type_to_python_type(items_schema['type'], items_schema)
                ...
                if additional_props and isinstance(additional_props, dict):
                    if 'type' in additional_props:
                        value_type = _json_type_to_python_type(additional_props['type'], additional_props)
                        return Dict[str, value_type]
                    elif 'items' in additional_props:
                        # Dict[str, List[...]]
                        items_type = _json_type_to_python_type(additional_props['items'].get('type', 'string'), additional_props['items'])

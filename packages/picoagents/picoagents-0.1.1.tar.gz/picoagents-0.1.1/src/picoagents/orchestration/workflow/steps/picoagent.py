...
class PicoAgentInput(BaseModel):
    ...
    additional_context: Optional[Dict[str, Any]] = Field(default=None, description="Additional context or parameters")
...
class PicoAgentOutput(BaseModel):
    ...
    messages: list = Field(default_factory=list, description="Complete conversation messages")
...
class PicoAgentStepConfig(BaseStepConfig):
    ...
    agent_config: Dict[str, Any] = Field(default_factory=dict, description="Additional agent configuration")


class PicoAgentStep(Component[PicoAgentStepConfig], BaseStep[PicoAgentInput, PicoAgentOutput]):
    ...
    component_provider_override = "picoagents.orchestration.workflow.steps.PicoAgentStep"
    ...
    async def execute(self, input_data: PicoAgentInput, context: Context) -> PicoAgentOutput:
        ...
        """
        logger.info(f"Executing PicoAgent step '{self.step_id}' with task: {input_data.task[:50]}...")
        ...
        try:
            ...
            context.set(f'{self.step_id}_output', {

...
from picoagents.messages import UserMessage, AssistantMessage, ToolMessage
...
from picoagents.types import AgentResponse, Usage, OrchestrationResponse
...
def test_text_mention_termination():
    ...
    messages2 = [AssistantMessage(content="I think we should TERMINATE this conversation", source="assistant")]
    result = termination.check(messages2)
    assert result is not None
    assert termination.is_met()
...
def test_text_mention_termination_case_sensitive():
    ...
    messages1 = [AssistantMessage(content="We should terminate this", source="assistant")]
    result = termination.check(messages1)
    assert result is None
    assert not termination.is_met()
    
    # Uppercase should trigger
    messages2 = [AssistantMessage(content="We should TERMINATE this", source="assistant")]
    result = termination.check(messages2)
    assert result is not None
    assert termination.is_met()
...
def test_composite_termination_all():
    ...
    messages2 = [AssistantMessage(content="Second message, we are DONE", source="assistant")]
...
def test_termination_reset_functionality():
    ...
    for term in terminations:
        if isinstance(term, TimeoutTermination):
            ...
            if isinstance(term, MaxMessageTermination):
                ...
                messages = [UserMessage(content="Very long message" * 100, source="user")]
    ...
    for term in terminations:
        ...
        assert term.get_reason() == "", f"{type(term).__name__} reason should be cleared"
...
async def test_max_message_termination_orchestrator_integration():
    ...
    assert len(assistant_messages) == 4, f"Expected 4 assistant messages, got {len(assistant_messages)}"
    ...
    expected_responses = ["First response", "Second response", "First response", "Second response"]
...
async def test_max_message_termination_streaming_consistency():
    ...
    assert result1.stop_message.source == result2.stop_message.source == "MaxMessageTermination"

...
import os
import tempfile
from .testcase_model import TestCaseList,TestCase,TestCaseStep
# Create an MCP server
mcp = FastMCP("lsy-testcase-to-xmind")

def create_xmind_from_test_cases(test_case_list: TestCaseList, output_path: str):
    """
    根据测试用例列表创建XMind文件，根据path字段创建层级结构
    ...
    # 设置画布标题
    sheet.setTitle(test_case_list.feature)

    # 获取根主题
    root_topic = sheet.getRootTopic()
    root_topic.setTitle(test_case_list.feature)
    ...
    for test_case in test_case_list.test_cases:
        ...
        case_title = f"tc-p{test_case.priority.replace('p', '')}：{test_case.name}"
...
@mcp.tool(name="safe_create_xmind",description="安全创建XMind文件，处理各种异常情况")
def safe_create_xmind(test_case_list: TestCaseList, output_path: str):
    """
    ...
    """
    try:
        create_xmind_from_test_cases(test_case_list, output_path)
        return True
    except Exception as e:

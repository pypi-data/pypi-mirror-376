...
class CiscoIOSCLIRenderer(RendererBase):
    ...
    def physical_interface_names(self, configuration, asset) -> None:
        ...
        for intf in configuration.get("interfaces", {}).get("physical", []):
            intf_prefix = self.get_port_prefix(asset.os, intf.get("speed"))
            intf_suffix = self.get_port_suffix(asset.hardware_model, intf.get("index"))
            intf["name"] = intf_prefix + intf_suffix
    ...
    def get_port_prefix(self, os:str, speed:int) -> Optional[str]:
        ...
        return PREFIX_MAP.get(os, {}).get(speed)

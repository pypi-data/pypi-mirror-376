...
class TagBasedDiscovery:
    ...
    def search_by_text(self, query: str, search_fields: Optional[List[str]] = None) -> List[Tuple[str, float]]:
        """
        ...
        try:
            ...
            for pipeline_id in all_pipelines:
                ...
                if "all" in search_fields:
                    ...
                    responsibility = atomic_props.get("single_responsibility", "").lower()
                    for term in query_terms:
        ...
        try:
            ...
            for pipeline_id in all_pipelines:
                ...
                if len(similar_pipelines) > 1:
                    ...
                    cluster_name = "_".join(sorted(list(common_tags))[:3]) if common_tags else f"cluster_{len(clusters)}"
        ...
        try:
            ...
            for category, category_tags in tag_index.items():
                category_stats = {
                    ...
                    "total_pipelines": sum(len(pipelines) for pipelines in category_tags.values()),
                ...
                if category_tags:
                    # Calculate average
                    category_stats["average_pipelines_per_tag"] = category_stats["total_pipelines"] / len(category_tags)
                    
                    # Find most common tags
                    tag_counts = [(tag, len(pipelines)) for tag, pipelines in category_tags.items()]
    ...
    def suggest_tags_for_pipeline(self, pipeline_id: str) -> Dict[str, List[str]]:
        ...
        try:
            # Find similar pipelines
            similar_pipelines = self.suggest_similar_pipelines(pipeline_id, similarity_threshold=0.2)

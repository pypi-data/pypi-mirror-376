...
def create_dag() -> PipelineDAG:
    ...
    logger.info(
        f"Created MODS XGBoost training with evaluation DAG with {len(dag.nodes)} nodes and {len(dag.edges)} edges"
    )
    return dag
...
def create_pipeline(
    ...
    dag_compiler.pipeline_name = (
        pipeline_name or "MODS-XGBoost-Training-With-Evaluation"
    )
    dag_compiler.pipeline_description = (
        pipeline_description
        or "MODS-enhanced XGBoost training pipeline with model evaluation"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create a MODS-enhanced XGBoost training with evaluation pipeline"
    )
    parser.add_argument(
    ...
    if args.sync_registry:
        ...
        print(
            "Successfully synchronized pipeline metadata to registry"
            if success
            else "Failed to synchronize pipeline metadata to registry"
        )
        exit(0)

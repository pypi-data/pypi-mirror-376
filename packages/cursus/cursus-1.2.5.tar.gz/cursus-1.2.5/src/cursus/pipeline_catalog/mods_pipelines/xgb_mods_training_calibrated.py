"""
MODS XGBoost Training with Calibration Pipeline

This pipeline implements a MODS-enhanced version of the XGBoost training workflow with calibration:
...
Example:
    ```python
    from cursus.pipeline_catalog.mods_pipelines.xgb_mods_training_calibrated import create_pipeline
...
def create_dag() -> PipelineDAG:
    """
    Create an XGBoost training with calibration pipeline DAG.
    ...
    logger.info(
        f"Created MODS XGBoost training with calibration DAG with {len(dag.nodes)} nodes and {len(dag.edges)} edges"
    )
    return dag
...
def get_enhanced_dag_metadata() -> EnhancedDAGMetadata:
    """
    Get enhanced DAG metadata with Zettelkasten integration for xgb_mods_training_calibrated.

    Returns:
        EnhancedDAGMetadata: Enhanced metadata with Zettelkasten properties
    """
    zettelkasten_metadata = ZettelkastenMetadata(
        atomic_id="xgb_mods_training_calibrated",
        title="MODS XGBoost Training with Calibration Pipeline",
        single_responsibility="MODS-enhanced XGBoost model training with probability calibration",
        input_interface=[
        ...
        output_interface=[
            ...
            "calibration metrics",
            "MODS template registration",
        ],
        side_effects="Creates model and calibration artifacts in S3 and registers template in MODS global registry",
        independence_level="high",
        node_count=6,
        edge_count=5,
        framework="xgboost",
        complexity="standard",
        use_case="MODS-enhanced XGBoost training with probability calibration",
        features=[
            "training",
            "xgboost",
            "calibration",
        ...
        source_file="mods_pipelines/xgb_mods_training_calibrated.py",
        migration_source="mods_frameworks/xgboost/training/with_calibration_mods.py",
        created_date="2025-08-20",
        priority="medium",
        framework_tags=["xgboost"],
        task_tags=["training", "calibration", "mods", "template_registration"],
        ...
        quality_tags=["production_ready", "tested", "calibrated", "mods_validated"],
        data_tags=["tabular", "structured"],
        creation_context="MODS-enhanced XGBoost training with probability calibration for reliable predictions",
        ...
        use_cases=[
            "MODS-enhanced binary classification requiring calibrated probabilities",
            "Risk assessment models with probability outputs and template registration",
            "Models where prediction confidence is critical with operational integration",
        ],
        skill_level="intermediate",
    )

    return EnhancedDAGMetadata(
        pipeline_name="xgb_mods_training_calibrated",
        description="MODS-enhanced XGBoost training pipeline with probability calibration and template registration",
        framework="xgboost",
        task_type="training",
        complexity_level="standard",
        estimated_duration_minutes=30,
        resource_requirements=["ml.m5.large"],
        dependencies=["xgboost", "sagemaker", "sklearn", "mods"],
...
def create_pipeline(
    ...
    """
    Create a SageMaker Pipeline from the DAG for XGBoost training with calibration and MODS features.
    ...
    if enable_mods:
        try:
            ...
            logger.warning(
                "MODS compiler not available, falling back to standard compiler"
            )
    ...
    if pipeline_name:
        ...
        dag_compiler.pipeline_name = "MODS-XGBoost-Training-With-Calibration"

    if pipeline_description:
        dag_compiler.pipeline_description = pipeline_description
    else:
        dag_compiler.pipeline_description = (
            "MODS-enhanced XGBoost training pipeline with calibration support"
        )
    ...
    if validate:
        try:
            ...
            logger.warning(f"DAG validation failed: {e}")
    ...
    if template_instance is None:
        ...
        if enable_mods and hasattr(template_instance, "mods_metadata"):
            logger.info(
                "MODS-decorated template instance retrieved for global registry"
            )
        else:
    ...
    if enable_mods:
        logger.info(
            "MODS features enabled: template registration, enhanced metadata, operational integration"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create a MODS-enhanced XGBoost training with calibration pipeline"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--disable-mods", action="store_true", help="Disable MODS features"
    )
    ...
    try:
        ...
        pipeline, report, dag_compiler, template_instance = create_pipeline(
            ...
            session=pipeline_session,
            role=role,
            pipeline_name="MODS-XGBoost-Training-With-Calibration",
            pipeline_description="MODS-enhanced XGBoost training pipeline with calibration support",
        ...
        if not args.disable_mods:
            logger.info(
                "MODS features: Template registration, enhanced metadata, operational integration"
            )
        ...
        if "mods" in str(e).lower():
            logger.info(
                "Consider using --disable-mods flag or install MODS dependencies"
            )

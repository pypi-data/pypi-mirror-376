            ...
            if pattern in pipeline_configs:
                ...
                if registration_configs:
                    for step_name, config in registration_configs.items():
                        ...
                        self.logger.info(
                            f"Updated execution config for registration step: {pattern}"
                        )
                    ...
                    try:
                        ...
                        if all(
                            ...
                            try:
                                ...
                                self.logger.warning(
                                    f"Could not retrieve image URI with framework info: {e}"
                                )

                        # Create execution document config
                        if image_uri and self._has_required_registration_fields(
        ...
        if model_name:
            for pattern in search_patterns:
                if pattern in pipeline_configs:
                    ...
                    self.logger.info(
                        f"Updated model name in registration config: {model_name}"
                    )
                    break

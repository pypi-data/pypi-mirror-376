...
from .enhanced_metadata import EnhancedDAGMetadata, ZettelkastenMetadata, ComplexityLevel, PipelineFramework
...
class DAGMetadataRegistrySync:
    ...
    def load_registry(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"Registry file not found at {self.registry_path}, creating new one")
            self._create_empty_registry()
    ...
    def _validate_registry_structure(self, registry: Dict[str, Any]) -> None:
        ...
        required_metadata_fields = ["total_pipelines", "frameworks", "complexity_levels", "last_updated"]
        for field in required_metadata_fields:
            if field not in metadata:
                raise RegistryValidationError(f"Missing required metadata field: {field}")
    ...
    def _validate_node_structure(self, node_id: str, node_data: Dict[str, Any]) -> None:
        ...
        """
        required_node_fields = ["id", "title", "description", "atomic_properties", "zettelkasten_metadata"]
    ...
    def sync_metadata_to_registry(
        ...
        try:
            ...
            # Update tag index
            self._update_tag_index(registry, atomic_id, dag_metadata.zettelkasten_metadata)
    ...
    def _extract_zettelkasten_metadata_from_node(self, node: Dict[str, Any]) -> ZettelkastenMetadata:
        """
        ...
        try:
            ...
            for tag_category in ["framework_tags", "task_tags", "complexity_tags"]:
                ...
                if dag_tags != reg_tags:
                    errors.append(f"{tag_category} mismatch: DAG={dag_tags} vs Registry={reg_tags}")
    ...
    def _update_registry_metadata(self, registry: Dict[str, Any]) -> None:
        ...
        if len(nodes) > 1:
            ...
            graph_meta["connection_density"] = total_connections / max_possible_connections if max_possible_connections > 0 else 0.0
        else:
    ...
    def _remove_from_tag_index(self, registry: Dict[str, Any], pipeline_id: str) -> None:
        """
        ...
        for category, tag_dict in tag_index.items():
            ...
            empty_tags = [tag for tag, pipeline_list in tag_dict.items() if not pipeline_list]
    ...
    def get_registry_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "complexity_levels": registry.get("metadata", {}).get("complexity_levels", []),
                "total_connections": registry.get("connection_graph_metadata", {}).get("total_connections", 0),
                "connection_density": registry.get("connection_graph_metadata", {}).get("connection_density", 0.0),
                "isolated_nodes": registry.get("connection_graph_metadata", {}).get("isolated_nodes", []),
            ...
            for tag, pipeline_list in tag_dict.items():
                for pipeline_id in pipeline_list:
                    if pipeline_id not in nodes:
                        errors.append(f"Tag index references non-existent pipeline: {pipeline_id}")

...
def create_pytorch_training_dag() -> PipelineDAG:
    ...
    logger.debug(f"Created PyTorch training DAG with {len(dag.nodes)} nodes and {len(dag.edges)} edges")
    return dag
...
def validate_dag() -> bool:
    ...
    if set(dag.nodes) != expected_nodes:
        raise ValueError(f"DAG nodes mismatch. Expected: {expected_nodes}, Got: {set(dag.nodes)}")
    ...
    if actual_edges != expected_edges:
        raise ValueError(f"DAG edges mismatch. Expected: {expected_edges}, Got: {actual_edges}")

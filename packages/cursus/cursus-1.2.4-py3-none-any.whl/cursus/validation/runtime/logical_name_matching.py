...
class PathMatch(BaseModel):
    ...
    semantic_details: Optional[Dict[str, Any]] = Field(None, description="Detailed semantic matching information")
...
class EnhancedScriptExecutionSpec(ScriptExecutionSpec):
    ...
    def from_script_execution_spec(cls, original_spec, input_aliases: Optional[Dict[str, List[str]]] = None, 
                                 output_aliases: Optional[Dict[str, List[str]]] = None) -> 'EnhancedScriptExecutionSpec':
    ...
    def find_path_matches(self, source_spec: EnhancedScriptExecutionSpec, 
        ...
        for src_logical_name, src_path_spec in source_spec.output_path_specs.items():
            for dest_logical_name, dest_path_spec in dest_spec.input_path_specs.items():
                ...
                best_alias_match = self._find_best_alias_match(src_path_spec, dest_path_spec, 
                                                             src_logical_name, dest_logical_name)
                if best_alias_match:
        ...
        logger.debug(f"Found {len(matches)} path matches between {source_spec.script_name} and {dest_spec.script_name}")
        for match in matches[:3]:  # Log top 3 matches
            logger.debug(f"  {match.match_type.value}: {match.matched_source_name} -> {match.matched_dest_name} "
                        f"(confidence: {match.confidence:.3f})")
        
        return matches
    
    def _find_best_alias_match(self, src_path_spec: PathSpec, dest_path_spec: PathSpec,
                              src_logical_name: str, dest_logical_name: str) -> Optional[PathMatch]:
        """
    ...
    def generate_matching_report(self, matches: List[PathMatch]) -> Dict[str, Any]:
        ...
        if not matches:
            return {
                ...
                "recommendations": ["Check logical names and aliases for compatibility"]
            }
        ...
        if len(high_confidence_matches) == 0:
            recommendations.append("No high-confidence matches found - review logical names")
        if MatchType.SEMANTIC.value in matches_by_type:
            recommendations.append("Semantic matches found - consider standardizing naming conventions")
...
class EnhancedDataCompatibilityResult(BaseModel):
    ...
    execution_time: float = Field(default=0.0, description="Total execution time for compatibility test")
    ...
    def validate_dag_structure(self, dag, script_specs: Dict[str, Any]) -> List[str]:
        ...
        for spec_name in script_specs.keys():
            if spec_name not in dag.nodes:
                errors.append(f"ScriptExecutionSpec '{spec_name}' not found in DAG nodes")
...
class LogicalNameMatchingTester:
        ...
        try:
            ...
            if not file_mapping:
                return EnhancedDataCompatibilityResult(
                    ...
                    script_b=spec_b.script_name,
                    compatible=False,
                    compatibility_issues=["No output files could be mapped to destination inputs"],
            ...
            return EnhancedDataCompatibilityResult(
                ...
                data_format_b=self._detect_primary_format(output_files),  # Same format for successful transfer
                execution_time=time.time() - start_time
            )
    ...
    def _create_file_mapping(self, path_matches: List[PathMatch], output_files: List[Path]) -> Dict[str, str]:
        """
    ...
    def _find_best_file_for_logical_name(self, logical_name: str, output_files: List[Path]) -> Optional[Path]:
        """
    ...
    def test_pipeline_with_topological_execution(
        ...
        try:
            ...
            validation_errors = self.topological_executor.validate_dag_structure(dag, script_specs)
            ...
            for current_node in execution_order:
                ...
                for src_node, dst_node in outgoing_edges:
                    ...
                    if not path_matches:
                        results["pipeline_success"] = False
                        results["errors"].append(f"No logical name matches found for edge: {edge_key}")
            ...
            if missing_edges:
                results["pipeline_success"] = False
                results["errors"].append(f"Unprocessed edges: {', '.join(missing_edges)}")
            
            return results
            
        except Exception as e:
            results["pipeline_success"] = False
            results["errors"].append(f"Pipeline topological execution test failed: {str(e)}")
            return results

...
def run_script(repo_name: str, run_path: str = "compile.py") -> None:
    ...
    env.update(
        {
            "MODAIC_CACHE": "../../temp/test_repos/modaic_cache",
        }
    )
...
@pytest.mark.parametrize("repo_name", ["multi_module_repo", "nested_repo", "nested_repo_2"])
def test_repos_retrievers(repo_name):
    prepare_repo(repo_name)
    if repo_name == "nested_repo":
        run_script(repo_name, run_path="agent/compile.py")
    else:

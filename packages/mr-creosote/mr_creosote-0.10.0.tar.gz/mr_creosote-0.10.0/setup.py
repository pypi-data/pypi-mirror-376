...
def test_package_install(package_name, timeout=30):
    """
    Test if a package can be downloaded from PyPI.
    Returns True if the package can be downloaded, False otherwise.
    """
    try:
        # Create a temporary directory for the test download
        with tempfile.TemporaryDirectory() as temp_dir:
            # Try to download the package (pip is now available via build requirements)
            result = subprocess.run([
                sys.executable, '-m', 'pip', 'download',
                '--no-deps', '--dest', temp_dir, package_name
            ], capture_output=True, timeout=timeout, text=True)

            # If download succeeds, the package is installable

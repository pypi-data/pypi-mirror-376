...
def Fourier( geo:                           Union[Geometry2D,Geometry3D],\
        ...
        if mode =='nonlinear':
         ...
         if N_flux_sides > 0:
          ...
          H = jnp.concatenate([H, jnp.zeros((NT_save,batch_size,N_flux_sides))], axis=2)
         

         if linear_solver == 'iterative':
          X,info = jax.scipy.sparse.linalg.cg(L,B+H[0],tol=tol,x0=T0,maxiter=maxiter)
          
         elif linear_solver == 'direct': 
          
          if collapse_direct: 
                     
           X = sparse.sparse_solve(data[0],rows,cols,(B+H[0]).T).T

          else:
           #a = sparse.sparse_solve(data[0],rows,cols,(B+H[0])[0])
          
           X = jnp.array([sparse.sparse_solve(d,rows,cols,(B+H[0])[i])  for i,d in enumerate(data)])
        ...
        #Prepare the output
        output = Fields(geo,batch_size,jnp.array([NT_save]))
        output.add_field('Temperature',jax.device_get(T) ,units='K')
        #output = {'Temperature':T.shape}
        #output.add_field('Heat_source',H,units='W/m^3')
        #output.add_field('Heat_flux',J,units='W/m^2')
        #output.add_field('Thermal_conductivity',kappa,units='W/m/K')
        #output.add_field('Boundary_flux',P_boundary,units='W/m^2',surface=True)
        #output.add_field('Internal_flux',P_internal,units='W/m^2',surface=True)
        #output.add_field('Boundary_temperature',T_boundary,units='K',surface=True)

...
def WriteVTR(geo,
    ...
    if regions is not None:
        arr = ET.SubElement(celldata, "DataArray",
                            type="Int32", Name="RegionsID", format="ascii")
        
        
        flat = np.asarray(regions.flatten(order="F"),dtype=np.int32)
        arr.text = '\n'.join(
            ' '.join(map(str, flat[i:i+20]))
    ...
    for field in fields:
        ...
        for b in range(batch_size):
            name = f"{field.name}[{b}]_{field.units}" if batch_size > 1 else f"{field.name}_{field.units}"
        
            arr = np.array(field.data)[0,b] #Zero is because we consider only the first time step for now
            ...
            if arr.ndim == 1:
                ...
                ncomp = arr.shape[1] * arr.shape[2]
                da = ET.SubElement(celldata, "DataArray",
                                   type="Float32", Name=name,
                                   NumberOfComponents=str(ncomp), format="ascii")
                lines = [' '.join(map(str, row.flatten())) for row in arr]
                da.text = '\n'.join(lines)

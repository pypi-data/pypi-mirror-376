...
class x_cls_make_pypi_x(BaseMake):
    ...
    def ensure_type_metadata(
        ...
        try:
            ...
            for a in ancillary_files or []:
                try:
                    # Treat ancillary entries as paths inside the package dir
                    rel = a.replace("\\", "/").lstrip("/")
                    p_in_pkg = os.path.join(base_dir, rel.replace("/", os.sep))
                    if os.path.isfile(p_in_pkg):
                        # Normalize to forward slashes for MANIFEST portability
                        manifest_lines.append(f"include {rel}")
                except Exception:
    ...
    def create_files(self, main_file: str, ancillary_files: list[str]) -> None:
        ...
        for ancillary_path in ancillary_files or []:
            # Normalize entry to a clean, relative OS path
            rel_norm = ancillary_path.replace("\\", "/").lstrip("/")
            src_path = rel_norm.replace("/", os.sep)

            if os.path.isdir(src_path):
                # Preserve relative directory structure under the package dir
                dest_root = os.path.join(package_dir, src_path)
                for root, _dirs, files in os.walk(src_path):
                    rel = os.path.relpath(root, src_path)
                    target_root = (
                        os.path.join(dest_root, rel)
                        if rel != "."
                        else dest_root
                    )
                ...
                if _is_allowed(src_path):
                    # Preserve relative file path under the package dir

...
def send_sms(email: str, api_key: str, phone: int, message: str) -> Optional[Dict]:
    ...
    try:
        ...
        logger.error("Unexpected error while sending SMS: %s", str(e))
        raise
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Your email registered with SMS Aero"
    )
    parser.add_argument(
        "--api_key",
        type=str,
        required=True,
        help="Your SMS Aero API key"
    )
    parser.add_argument(
        "--phone",
        type=int,
        required=True,
        help="Phone number to send SMS to (international format without +)"
    )
    parser.add_argument(
        "--message",
        type=str,
        required=True,
        help="Message to send (2-640 characters)"
    )
    parser.add_argument(
        "--debug",
        action="store_true",

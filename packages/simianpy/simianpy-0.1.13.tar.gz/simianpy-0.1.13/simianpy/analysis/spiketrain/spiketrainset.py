...
class SpikeTrainSet:
    ...
    def from_arrays(
        ...
        return cls(
            event_labels_rep, spike_labels_aligned, spike_times_aligned, window, 
            unit_metadata=unit_metadata, trial_metadata=trial_metadata, 
            epochids=epochids_rep, epoch_names=epoch_names
        )
    ...
    def to_psth(
        self, time_bins: Optional[Sequence] = None, time_step: Optional[float] = None, epochs: Optional[Sequence[str]] = None
    ) -> xr.DataArray:
        ...
        if epochs is not None:
            if self.epoch_names is None:
                raise ValueError("epoch_names must be provided if epochs is specified")
            epoch_indices = [self.epoch_names.index(e) for e in epochs if e in self.epoch_names]
            if not epoch_indices:
                raise ValueError("None of the specified epochs are in epoch_names")
            if self.epochids is None:
                raise ValueError("epochids is None, cannot filter by epochs")
            mask = np.isin(self.epochids, epoch_indices)
            if not np.any(mask):
                raise ValueError("No spikes found for the specified epochs")
            trialids = self.trialids[mask]
            unitids = self.unitids[mask]
        ...
        trialids_unique, trialids_digitized = np.unique(
            trialids, return_inverse=True
        )
        unitids_unique, unitids_digitized = np.unique(unitids, return_inverse=True)
        ...
        if epochids is not None:
            ...
            coords["epoch"] = epoch_names if epoch_names is not None else epochids_unique
            dims = ["trialid", "unitid", "epoch", "time"]
        else:
            psth, _ = np.histogramdd(
                np.array([trialids_digitized, unitids_digitized, spike_times]).T,
                bins=[
                    np.arange(trialids_unique.size + 1),
                    np.arange(unitids_unique.size + 1),
                    time_bin_array,
        ...
        with zipfile.ZipFile(path, "w") as zf:
            ...
            if self.trial_metadata is not None:
                with zf.open("trial_metadata.csv", "w") as f:
                    ...
                    f.write(csv_bytes)
            if self.unit_metadata is not None:
                with zf.open("unit_metadata.csv", "w") as f:
                    csv_bytes = self.unit_metadata.to_csv().encode("utf-8")
                    f.write(csv_bytes)
            if self.epochids is not None:
    ...
    def load(cls, path: str) -> "SpikeTrainSet":
        with zipfile.ZipFile(path, "r") as zf:
            ...
            try:
                with zf.open("unit_metadata.csv") as f:
                    unit_metadata = pd.read_csv(io.StringIO(f.read().decode("utf-8")), index_col=0)
            except KeyError:
                unit_metadata = None
        ...
        return cls(trialids, unitids, spike_times, window, 
            unit_metadata=unit_metadata, trial_metadata=trial_metadata, 
            epochids=epochids, epoch_names=epoch_names
        )

...
def get_tapers(n_samples, NW=2.5, k=None):
    if k is None:
        k = int(NW*2)
    tapers = dpss(n_samples, NW, k)
    assert isinstance(tapers, np.ndarray)
    return tapers

def xr_get_multitaper_lfp_power(lfp, sampling_rate, NFFT=None, NW=2.5, k=None, dim='time'):
    dt = 1/sampling_rate
    time_dim = dim
    n_samples = lfp.sizes[time_dim]
    if NFFT=='auto':
        NFFT = max(256, int(2**np.ceil(np.log2(n_samples))))
    elif NFFT==None:
        NFFT = n_samples
    tapers = get_tapers(n_samples, NW, k)
    tapers = xr.DataArray(tapers, dims=['tapers', time_dim])
    tapers, lfp = xr.broadcast(tapers, lfp)
    tapered = tapers * lfp
    Sk_complex = xr.apply_ufunc(
        np.fft.fft, 
        tapered,
    ...
    Sk_complex = Sk_complex.assign_coords(freq=freq)
    psd = 2 * (np.abs(Sk_complex) ** 2).mean('tapers')
    return psd
...
def get_multitaper_lfp_power(lfp, depths, sampling_rate, NFFT=None, NW=2.5, k=None, normalize=True):
    ...
    if NFFT=='auto':
        ...
        NFFT = n_samples
    tapers = get_tapers(n_samples, NW, k).T

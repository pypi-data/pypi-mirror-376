...
def get_object(value, field_type: str, field, time_zone: Optional[str] = None):
    """
    Convert value based on field_type, similar to CZArrowResultSet#getObject in Java.
    
    Args:
        value: Raw value from Arrow table
        field_type: The field type string (from DataTypeCategory enum)
        field: The Field object with additional metadata
    ...
    try:
        ...
        # Handle string types
        elif field_type in ['STRING', 'CHAR', 'VARCHAR', 'JSON']:
        ...
        # Handle bitmap type
            ...
            # For bitmap, we typically return the raw bytes or a special bitmap object
            if isinstance(value, bytes):
                return value
            else:
                return bytes(value) if value is not None else None
        ...
        elif field_type in ['TIMESTAMP_LTZ', 'TIMESTAMP_NTZ']:
            return as_timezone(value, time_zone, field_type == 'TIMESTAMP_NTZ')
            
        # Handle interval types
        elif field_type in ['INTERVAL_DAY_TIME', 'INTERVAL_YEAR_MONTH']:
            # Return as-is for intervals, they are typically handled by PyArrow
            return value
            
        # Handle complex types
        elif field_type == 'ARRAY':
            # For arrays, return as Python list
            ...
            # For maps, return as Python dict
            ...
            # For structs, return as Python dict
            if isinstance(value, dict):
                return value
            else:
                return value
                
        elif field_type == 'VECTOR_TYPE':
            # For vector types, return as-is (typically bytes or array)
            return value
            
        elif field_type == 'VOID':
            return None
            
        else:
            # For unknown types, return the value as-is
            _log.warning(f"Unknown field type: {field_type}, returning value as-is")
            return value
            
    except Exception as e:
        _log.warning(f"Error converting value {value} for field type {field_type}: {e}, returning as-is")
        return value

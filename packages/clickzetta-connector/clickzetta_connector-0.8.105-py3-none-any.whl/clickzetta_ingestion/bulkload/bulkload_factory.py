...
class BulkLoadFactory(ABC, Generic[InputT, CommT]):
    """Factory interface for creating writers and committers."""

    @abstractmethod
    def create_writer(self, context: BulkLoadContext, conf: BulkLoadConf) -> BulkLoadWriter[InputT, CommT]:
        """Create a BulkLoadWriter instance."""
        pass

    @abstractmethod
    def create_committer(self, context: BulkLoadContext, conf: BulkLoadConf) -> BulkLoadCommitter[CommT]:
        """Create a BulkLoadCommitter instance."""
        pass


class BulkLoadFactoryImpl(BulkLoadFactory[InputT, CommT]):
    """Default implementation of BulkLoadFactory."""

    def __init__(self, bulk_load_handler: AbstractBulkLoadHandler[CommT]):

...
class BulkLoadHandler(AbstractBulkLoadHandler[Committable]):
    ...
    def _generate_merge_into_sql(self, table: BulkLoadTable, format: str,
        ...
        select_column_from_volume = "*"
        table_volume_source_fields = ""

        # Check if we need special handling for partial update columns
        if table.get_partial_update_column_indices():
            special_convert = BulkLoadHandler._get_special_type_convert(table)
            select_column_from_volume = special_convert[0]
            table_volume_source_fields = special_convert[1]

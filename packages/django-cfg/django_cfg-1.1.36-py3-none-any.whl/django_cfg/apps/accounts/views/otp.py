...
class OTPViewSet(viewsets.GenericViewSet):
    ...
    def request_otp(self, request):
        ...
        logger.debug(f"Starting OTP request for email: {email}, source: {source_url}")

        try:
            success, error_type = OTPService.request_otp(email, source_url)
        ...
        if success:
            ...
            if error_type == "invalid_email":
                ...
                logger.error(f"Unknown error type: {error_type} for email: {email}")
                return Response(
    ...
    def verify_otp(self, request):
        ...
        user = OTPService.verify_otp(email, otp, source_url)

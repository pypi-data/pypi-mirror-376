...
class TwilioConfig(BaseModel):
    ...
    def validate_service_configuration(self) -> "TwilioConfig":
        ...
        if not self.verify and not self.sendgrid:
            raise TwilioConfigurationError(
                "At least one Twilio service must be configured (verify or sendgrid)",
                missing_fields=["verify", "sendgrid"],
                suggestions=[
                    "Configure Twilio Verify for OTP services",
                    "Configure SendGrid for email services",
                    "Both services can be configured simultaneously"
                ]
            )
    ...
    @model_validator(mode="after")
    def validate_email_channel_requirements(self) -> "TwilioConfig":

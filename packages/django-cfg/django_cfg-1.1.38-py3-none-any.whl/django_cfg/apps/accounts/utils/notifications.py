...
class AccountNotifications:
    ...
    @staticmethod
    def send_failed_otp_attempt(email, ip_address=None, reason="Invalid OTP", send_telegram=True):
        ...
        if send_telegram:
            try:
                ...
                logger.info(f"Failed OTP attempt telegram notification sent for {email}")
            except Exception as e:

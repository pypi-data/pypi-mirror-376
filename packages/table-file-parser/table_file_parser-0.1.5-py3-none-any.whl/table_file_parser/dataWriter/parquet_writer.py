    ...
    def __init__(self, output_path, schema):
        ...
        if os.path.exists(output_path):
            ...
            if existing_schema != self.schema:
                print(
                    f"现有文件的schema与最终schema不匹配: {existing_schema} vs {self.schema}")
                # 如果文件已存在但schema不匹配，删除文件重新开始
...
#                         # 读取现有文件的 Schema 用于校验
#                         existing_schema = pq.read_schema(output_path)
#                         if existing_schema != final_schema:
#                             print(
#                                 f"现有文件的schema与最终schema不匹配: {existing_schema} vs {final_schema}")
#                             # 如果文件已存在但schema不匹配，删除文件重新开始
#                             os.remove(output_path)
#
#                     # 初始化 ParquetWriter

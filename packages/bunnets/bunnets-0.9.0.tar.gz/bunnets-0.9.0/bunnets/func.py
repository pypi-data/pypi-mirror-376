...
def hasattrs(
    obj, *attrs: str, all_of: tuple[str, ...] | None = None, any_of: tuple[str, ...] | None = None
):
    """Checks if the given list of attributes exist in the specified object.

    :param obj: Input object to check the attributes of.
    :param attrs: A collection of attributes to check that the object has.
                If any of the attributes are missing, returns False.
    :param all_of: An optional keyword-only tuple of attributes to check. Same as specifying attrs.
    :param any_of: An optional keyword-only tuple of attributes to check.
                Returns False if none of the attributes exist.
    """

    result: bool = True
    if len(attrs) > 0:
        result = False not in [hasattr(obj, attr) for attr in attrs]
    if result and all_of and isinstance(all_of, (tuple, list, set)):
        result = False not in [hasattr(obj, attr) for attr in all_of]
    if result and any_of and isinstance(any_of, (tuple, list, set)):
        result = True in [hasattr(obj, attr) for attr in any_of]
    return result

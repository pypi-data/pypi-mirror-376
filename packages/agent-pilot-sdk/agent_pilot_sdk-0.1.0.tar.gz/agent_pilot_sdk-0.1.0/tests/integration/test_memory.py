...
def test_memory_operations(get_config):
    ...
    print(
        f"Updating the memory with memory_id={memory.memory_id} and \
            new memory_content={new_memory_content}, \
            new_session_id={new_session_id}, \
            new_agent_id={new_agent_id}"
    )
    ...
    print(f"memory {memory.memory_id} is_updated = {is_updated}")

    print(f"Querying again with memory_id={memory.memory_id}")
    queried_memory = apx.get_agent_memory(
        memory_id=memory.memory_id,
        api_key=api_key,
        api_url=api_url,
    ...
    assert queried_memory.session_id == new_session_id
    assert queried_memory.agent_id == new_agent_id
    # Skip timestamp check because there is a small diff when creating the record.
    # assert int(queried_memory.create_time.timestamp()) == int(memory.create_time.timestamp())
    assert queried_memory.update_time > memory.create_time

    print(f"Deleting the memory with memory_id={memory.memory_id}")
    ...
    print(f"memory {memory.memory_id} is_deleted = {is_deleted}")

    print(f"Querying the deleted memory with memory_id={memory.memory_id}")
    queried_memory = apx.get_agent_memory(
        memory.memory_id,
        api_key=api_key,
        api_url=api_url,

...
class Gate(nn.Module):
    def __init__(
        ...
        super().__init__()
        assert mode in ['batch', 'channel'], "mode must be 'batch' or 'channel'"
        self.mode = mode
        gate_dim = n_experts if mode == 'batch' else in_channels * n_experts
        ...
        self._current_counts = torch.zeros(n_experts)
    ...
    def forward(self, x):
        ...
        if self.mode == 'channel':
            scores = scores.view(self._current_batch_size, -1, self.num_experts)

...
def show_static(plot_buffer):
    ...
    tmp_path = Path(tempfile.gettempdir()) / f"plotly_plot_{time.time_ns()}.html"
    
    # Write the plot to the file
    pyo.plot(fig, filename=str(tmp_path), auto_open=False)

    # Get the directory and filename using pathlib attributes
...
# Example usage with a dummy plot_buffer and is_termux function
if __name__ == "__main__":
    class DummyPlotBuffer:
        def get_all(self):
            return {
                "Series A": {"x": [1, 2, 3], "y": [4, 1, 2]},
                "Series B": {"x": [1, 2, 3], "y": [2, 4, 5]}
            }

    # is_termux is not a standard Python function, so we mock it for this example
    def is_termux():
        return "termux" in os.environ.get("PATH", "").lower()

    show_static(DummyPlotBuffer())
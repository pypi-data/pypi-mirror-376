    ...
    for group_key, group_data in data['groups'].items():
        ...
        if 'basic_stats' in group_data:
            ...
            if 'ax' in basic['columns']:
                ...
                print(f"  ax values: mean={ax_stats['mean']:.2f}, std={ax_stats['std']:.2f}")
        ...
        if 'categorical_profile' in group_data and 'activity' in group_data['categorical_profile']:
            cat = group_data['categorical_profile']['activity']
            if 'error' not in cat:
                print(f"  Activities: {cat['cardinality']} unique, {cat['total_count']} total")

...
ExceptionHandler = Union[
    Callable[[ROContext[R], BaseException], bool],
    Callable[[ROContext[R], BaseException], Awaitable[bool]]
    ]

ExceptioHandlerAsync = Callable[[ROContext[R], BaseException], Awaitable[bool]]

def _log_exception(ctx: ROContext[R], exc: BaseException):
    ctx.logger.exception(
        f"Frame[{ctx.frame_name}]: "
        f"{type(exc).__name__}"
    )

async def _default_handler(ctx: ROContext[R], exc: BaseException) -> bool:
...
class ExceptionHandlerWrapper:
    ...
    async def __call__(self, ctx: ROContext[R], exc: BaseException) -> bool:
        try:
    ...
    def set_handler(self, handler: ExceptionHandler):
        if inspect.iscoroutinefunction(handler):
            ...
            async def sync_caller(message: ROContext, exc: BaseException) -> bool:
